	...
	public class StatesInstance : GameStateMachine<States, StatesInstance, PeeChore, object>.GameInstance
	{
		...
		public void SpawnDirtyWater(float dt)
		{
			...
			if (num > 0f)
			{
				...
				if (equippable != null)
				{
					equippable.GetComponent<Storage>().AddLiquid(SimHashes.DirtyWater, mass, bodyTemperature.value, index, Mathf.CeilToInt((float)DUPLICANTSTATS.STANDARD.Secretions.DISEASE_PER_PEE * num));
				}
				else
				{
					SimMessages.AddRemoveSubstance(gameCell, SimHashes.DirtyWater, CellEventLogger.Instance.Vomit, mass, bodyTemperature.value, index, Mathf.CeilToInt((float)DUPLICANTSTATS.STANDARD.Secretions.DISEASE_PER_PEE * num));
				}
			}
		}
	}
