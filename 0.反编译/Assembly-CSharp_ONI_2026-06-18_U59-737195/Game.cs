...
public class Game : KMonoBehaviour
{
	...
	protected override void OnPrefabInit()
	{
		...
		changelistsPlayedOn.Add(737790u);
		...
		foreach (WorldContainer worldContainer in ClusterManager.Instance.WorldContainers)
		{
			...
			for (int i = worldContainer.WorldOffset.X; i < worldContainer.WorldOffset.X + worldContainer.WorldSize.X; i++)
			{
				for (int j = 0; j < Grid.TopBorderHeight; j++)
				{
					...
					{
						SimMessages.Dig(num, -1, skipEvent: true);
					}
				}
			}
		}
	...
	private unsafe Sim.GameDataUpdate* StepTheSim(float dt)
	{
		...
		{
			ElementConsumer.AddMass(ptr->removedMassEntries[num5]);
		}
		...
		{
			SimTemperatureTransfer.DoOreMeltTransition(ptr->elementChunkMeltedInfos[num11].handle);
		}
		int numBuildingOverheatInfos = ptr->numBuildingOverheatInfos;
		for (int num12 = 0; num12 < numBuildingOverheatInfos; num12++)
		{
			StructureTemperatureComponents.DoOverheat(ptr->buildingOverheatInfos[num12].handle);
		}
		...
		{
			StructureTemperatureComponents.DoNoLongerOverheated(ptr->buildingNoLongerOverheatedInfos[num13].handle);
		}
		int numBuildingMeltedInfos = ptr->numBuildingMeltedInfos;
		for (int num14 = 0; num14 < numBuildingMeltedInfos; num14++)
		{
			StructureTemperatureComponents.DoStateTransition(ptr->buildingMeltedInfos[num14].handle);
		}
		int numCellMeltedInfos = ptr->numCellMeltedInfos;
		for (int num15 = 0; num15 < numCellMeltedInfos; num15++)
		{
			int gameCell = ptr->cellMeltedInfos[num15].gameCell;
	...
	private void InitializeFXSpawners()
	{
		for (int i = 0; i < fxSpawnData.Length; i++)
		{
			...
			{
				GameObject obj = GameUtil.KInstantiate(fxSpawnData[fx_idx].fxPrefab, Grid.SceneLayer.Front);
	...
	public void Save(BinaryWriter writer)
	{
		...
		if (!changelistsPlayedOn.Contains(737790u))
		{
			changelistsPlayedOn.Add(737790u);
		}
	...
	public static bool IsCorrectDlcActiveForCurrentSave(IHasDlcRestrictions restrictions)
	{
		...
		if ((restrictions.GetAnyRequiredDlcIds() == null || IsAnyDlcActiveForCurrentSave(restrictions.GetAnyRequiredDlcIds())) && IsAllDlcActiveForCurrentSave(restrictions.GetRequiredDlcIds()))
		{
			return !IsAnyDlcActiveForCurrentSave(restrictions.GetForbiddenDlcIds());
		}
		return false;
	}
