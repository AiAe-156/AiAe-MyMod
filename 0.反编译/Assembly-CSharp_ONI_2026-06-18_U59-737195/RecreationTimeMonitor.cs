...
public class RecreationTimeMonitor : GameStateMachine<RecreationTimeMonitor, RecreationTimeMonitor.Instance, IStateMachineTarget, RecreationTimeMonitor.Def>
{
	...
	public new class Instance : GameInstance
	{
			...
			: base(master, def)
		{
			...
			moraleModifier = new AttributeModifier(Db.Get().Attributes.QualityOfLife.Id, 0f, () => (Mathf.Clamp(moraleAddedTimes.Count - 1, 0, 5) == 5) ? ((string)DUPLICANTS.MODIFIERS.BREAK_BONUS.MAX_NAME) : ((string)DUPLICANTS.MODIFIERS.BREAK_BONUS.NAME));
		...
		{
			if (ScheduleManager.Instance.GetSchedule(schedulable).GetPreviousScheduleBlock().GroupId == Db.Get().ScheduleGroups.Recreation.Id)
			{
