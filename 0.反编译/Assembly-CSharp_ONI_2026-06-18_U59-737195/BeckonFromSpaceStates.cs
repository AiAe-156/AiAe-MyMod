...
internal class BeckonFromSpaceStates : GameStateMachine<BeckonFromSpaceStates, BeckonFromSpaceStates.Instance, IStateMachineTarget, BeckonFromSpaceStates.Def>
{
	...
	private static void DoBeckon(Instance smi)
	{
		...
		float num2 = (num - position.y) * Mathf.Tan(MathF.PI / 12f);
		...
		GameObject obj = Util.KInstantiate(position: new Vector3(num6, num, layerZ), original: Assets.GetPrefab(smi.ChosenSong.meteorID), rotation: Quaternion.identity);
