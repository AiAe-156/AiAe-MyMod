...
public class SandboxSpawnerTool : InterfaceTool
{
	...
	private void SpawnMinion(string prefabID)
	{
		...
		TransformExtensions.SetLocalPosition(position: Grid.CellToPosCBC(currentCell, Grid.SceneLayer.Move), transform: gameObject.transform);
		gameObject.SetActive(value: true);
		new MinionStartingStats(model, is_starter_minion: false).Apply(gameObject);
