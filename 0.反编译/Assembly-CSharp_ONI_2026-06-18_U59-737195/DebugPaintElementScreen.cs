...
public class DebugPaintElementScreen : KScreen
{
	...
	protected override void OnPrefabInit()
	{
		...
		massInput.onEndEdit.AddListener(delegate
		{
			OnChangeMass();
		});
	...
	private void ConfigureElements()
	{
		...
		if (string.IsNullOrEmpty(filter))
		{
			...
			{
				Element element = ElementLoader.FindElementByHash(array[num]);
		...
		{
			if (list[num2].id == this.element)
			{
				elementPopup.SelectOption(list2[num2], num2);
			}
		}
	...
	public void OnChangeMass()
	{
		...
		try
		{
			num = Convert.ToSingle(massInput.text);
		}
		catch
		{
			num = -1f;
	...
	{
		massInput.text = Grid.Mass[cell].ToString();
