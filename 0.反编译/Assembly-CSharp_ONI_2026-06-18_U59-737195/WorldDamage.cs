...
public class WorldDamage : KMonoBehaviour
{
	...
	public float ApplyDamage(int cell, float amount, int src_cell, DamageType damageType, string source_name = null, string pop_text = null)
	{
		...
		{
			float num4 = Grid.Damage[cell];
			result = Mathf.Min(amount, 1f - num4);
			num4 += amount;
			Grid.Damage[cell] = Mathf.Min(1f, num4);
			if (Grid.Damage[cell] >= 1f)
			{
	...
	public void OnDigComplete(int cell, float mass, float temperature, ushort element_idx, byte disease_idx, int disease_count)
	{
		...
		if (!(num <= 0f))
		{
			...
			{
				PopFXManager.Instance.SpawnFX(Def.GetUISprite(element).first, PopFXManager.Instance.sprite_Plus, Mathf.RoundToInt(num) + " " + element.name, gameObject.transform, Vector3.zero);
			}
		}
	}
