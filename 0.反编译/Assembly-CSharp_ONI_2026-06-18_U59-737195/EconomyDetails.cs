...
public class EconomyDetails
{
	...
	public void DumpTransformations(Scenario scenario, StreamWriter o)
	{
		...
		for (int num5 = 0; num5 < transformations.Count; num5++)
		{
			...
			if (flag)
			{
				WriteProduct(o, a, (num6 / transformation2.timeInSeconds).ToString("0.00000"), text2);
			}
		...
		{
			if (num8 >= num7 && num8 < num7 + used_resources.Count)
			{
				string text3 = ((char)(65 + num8 % 26)).ToString();
				int num9 = Mathf.FloorToInt((float)num8 / 26f);
				if (num9 > 0)
				{
					text3 = (char)(65 + num9 - 1) + text3;
				}
	...
	public Transformation CreateTransformation(KPrefabID prefab_id, Tag tag)
	{
		...
		if (transformation != null)
		{
			if (component2 != null && component2.consumedElements != null)
			{
				...
				for (int num = 0; num < consumedElements.Length; num++)
				{
					ElementConverter.ConsumedElement consumedElement = consumedElements[num];
				...
				if (component2.outputElements != null)
				{
					...
					for (int num = 0; num < outputElements.Length; num++)
					{
						ElementConverter.OutputElement outputElement = outputElements[num];
			...
			if (component7 != null)
			{
				if (component7.formula.inputs != null)
				{
					...
					{
						EnergyGenerator.InputItem inputItem = inputs[num];
				...
				if (component7.formula.outputs != null)
				{
					...
					for (int num = 0; num < outputs.Length; num++)
					{
						EnergyGenerator.OutputItem outputItem = outputs[num];
			...
			if (list2 != null && list2.Count > 0)
			{
				foreach (IrrigationMonitor.Def item2 in list2)
				{
					...
					for (int num = 0; num < consumedElements2.Length; num++)
					{
						PlantElementAbsorber.ConsumeInfo consumeInfo2 = consumedElements2[num];
