...
public static class BaseSquidConfig
{
	...
	private static void InkAttack(GameObject attacker, GameObject target)
	{
		...
		if (Grid.IsValidCell(num2) && !Grid.Solid[num2])
		{
			...
			SimMessages.AddRemoveSubstance(num2, SimHashes.Ink, CellEventLogger.Instance.ElementEmitted, 5f, component.Temperature, byte.MaxValue, 0);
			...
			GameUtil.KInstantiate(Assets.GetPrefab(EffectConfigs.SquidAttackId), vector, Grid.SceneLayer.FXFront).SetActive(value: true);
			PopFX popFX = PopFXManager.Instance.SpawnFX(Def.GetUISprite(element).first, null, STRINGS.CREATURES.SPECIES.SQUID.INK_PUNCH, null, vector);
			if (popFX != null)
			{
