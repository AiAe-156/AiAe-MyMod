...
public class ClusterGrid
{
	...
	public StarmapHexCellInventory AddOrGetHexCellInventory(AxialI cell)
	{
		...
		{
			StarmapHexCellInventoryVisuals component = Util.KInstantiate(Assets.GetPrefab("StarmapHexCellInventory")).GetComponent<StarmapHexCellInventoryVisuals>();
	...
	public bool GetVisibleUnidentifiedMeteorShowerWithinRadius(AxialI center, int radius, out ClusterMapMeteorShower.Instance result)
	{
		for (int i = 0; i <= radius; i++)
		{
			foreach (AxialI item in AxialUtil.GetRing(center, i))
			{
				...
				foreach (ClusterGridEntity item2 in GetEntitiesOfLayerAtCell(item, EntityLayer.Meteor))
				{
	...
	public List<AxialI> GetPath(AxialI start, AxialI end, ClusterDestinationSelector destination_selector, out string fail_reason, bool dodgeHiddenAsteroids = false)
	{
		...
		if (destination_selector.requiredEntityLayer != EntityLayer.None && GetVisibleEntityOfLayerAtCell(end, destination_selector.requiredEntityLayer) == null)
		{
