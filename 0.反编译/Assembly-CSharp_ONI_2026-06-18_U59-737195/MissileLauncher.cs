...
public class MissileLauncher : GameStateMachine<MissileLauncher, MissileLauncher.Instance, IStateMachineTarget, MissileLauncher.Def>
{
	...
	public new class Instance : GameInstance, IMissileSelectionInterface
	{
			...
			: base(master, def)
		{
			...
			foreach (ManualDeliveryKG manualDeliveryKG in components2)
			{
		...
		public override void StartSM()
		{
			base.StartSM();
			OnStorage(null);
			base.smi.master.GetComponent<FlatTagFilterable>().currentlyUserAssignable = AmmunitionIsAllowed("MissileBasic");
		...
		public void LaunchLongRangeMissile()
		{
			...
			if (gameObject != null)
			{
				...
				if (!gameObject2.IsNullOrDestroyed())
				{
					pickupable.GetSMI<MissileLongRangeProjectile.StatesInstance>().PrepareLaunch(gameObject2, base.def.launchSpeed, launchPosition, base.smi.cannonRotation);
		...
		{
			if (!base.smi.sm.longRangeTarget.Get(base.smi).IsNullOrDestroyed() && LongRangeStorage.items.Count > 0)
			{
				return IsPathClear(launchPosition, launchPosition + new Vector3(0f, 50f, 0f));
			}
			return false;
		}
