...
public class BionicBatteryMonitor : GameStateMachine<BionicBatteryMonitor, BionicBatteryMonitor.Instance, IStateMachineTarget, BionicBatteryMonitor.Def>
{
	...
	public new class Instance : GameInstance
	{
		...
		public void DebugAddCharge(float joules)
		{
			...
			if (num > 0f && pooledList.Count < ElectrobankCountCapacity)
			{
				int num4 = storage.items.Count - 1;
				while (num > 0f && num4 >= 0)
				{
					GameObject gameObject = storage.items[num4];
					if (!(gameObject == null))
					{
			...
			if (num > 0f && storage.items.Count < ElectrobankCountCapacity)
			{
				do
				{
				...
				while (num > 0f && storage.items.Count < ElectrobankCountCapacity && num > 0f);
			}
			RefreshCharge();
			pooledList.Recycle();
		}
	...
	{
		if (!IsCriticallyLow(smi))
		{
			if (smi.InUpkeepTime)
			{
				return smi.ChargedElectrobankCount < smi.ElectrobankCountCapacity;
			}
			return false;
		}
		return true;
