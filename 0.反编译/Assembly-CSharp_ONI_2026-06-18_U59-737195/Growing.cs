...
public class Growing : StateMachineComponent<Growing.StatesInstance>, IGameObjectEffectDescriptor, IManageGrowingStates
{
	public class StatesInstance : GameStateMachine<States, StatesInstance, Growing, object>.GameInstance
	{
		...
		public void ModifyOldAgeGrowthRate(float newValue)
		{
			...
			attributes.Remove(getOldRate);
			getOldRate = new AttributeModifier(base.master.oldAge.deltaAttribute.Id, newValue);
			if (base.smi.IsInsideState(base.sm.grown))
			{
				attributes.Add(getOldRate);
			}
		}
		...
		public bool CanExitStalled()
		{
			if (!IsWilting())
			{
				if (base.master.CustomGrowStallCondition_IsStalled != null)
				{
					return !base.master.CustomGrowStallCondition_IsStalled(base.master.gameObject);
				}
				return true;
			}
			return false;
