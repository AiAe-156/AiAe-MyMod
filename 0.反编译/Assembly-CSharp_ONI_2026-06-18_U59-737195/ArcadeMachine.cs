...
public class ArcadeMachine : StateMachineComponent<ArcadeMachine.StatesInstance>, IGameObjectEffectDescriptor
{
	public class States : GameStateMachine<States, StatesInstance, ArcadeMachine>
	{
		...
		public override void InitializeStates(out BaseState default_state)
		{
			...
			operational.playing.PlayAnim((Func<StatesInstance, string>)GetPlayingAnim, KAnim.PlayMode.Loop).ParamTransition(playerCount, operational.post, (StatesInstance smi, int p) => p == 0).ParamTransition(playerCount, operational.playing_coop, (StatesInstance smi, int p) => p > 1);
			operational.playing_coop.PlayAnim((Func<StatesInstance, string>)GetPlayingAnim, KAnim.PlayMode.Loop).ParamTransition(playerCount, operational.post, (StatesInstance smi, int p) => p == 0).ParamTransition(playerCount, operational.playing, (StatesInstance smi, int p) => p == 1);
	...
	protected override void OnSpawn()
	{
		...
		{
			Vector3 pos = Grid.CellToPosCBC(Grid.OffsetCell(Grid.PosToCell(this), choreOffsets[num]), Grid.SceneLayer.Move);
			...
			arcadeMachineWorkable.overrideAnims = overrideAnims[num];
			arcadeMachineWorkable.workAnims = workAnims[num];
			workables[num] = arcadeMachineWorkable;
			workables[num].owner = this;
		}
		base.smi.StartSM();
	}
