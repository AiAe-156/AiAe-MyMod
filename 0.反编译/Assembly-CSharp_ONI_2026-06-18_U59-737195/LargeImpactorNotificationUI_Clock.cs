...
public class LargeImpactorNotificationUI_Clock : KMonoBehaviour, ISim4000ms
{
	...
	private IEnumerator AnimationCoroutineLogic()
	{
		if (!hasPlayedEntryAnimation)
		{
			...
			_ = entryAnimationDuration / 600f;
			yield return this.Interpolate(delegate(float n)
			{
				TimerOutCircleFill.fillAmount = n * lastLargeImpactorTime;
		...
		while (true)
		{
			...
			{
				float t = Mathf.Abs(Mathf.Sin(reminderAnimationTimer / reminderAnimationDuration * MathF.PI * 16f));
				TimerOutCircleBG.color = Color.Lerp(color_timerOutCircleBGOriginalColor, Color.red, t);
				NeedleTrailBg.color = Color.Lerp(color_needleTrailBgOriginalColor, softRed, t);
