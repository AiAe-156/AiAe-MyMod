...
public class CodexScreen : KScreen
{
	...
	private void SetupPrefabs()
	{
		...
		ContentPrefabs[typeof(CodexElementCategoryList)] = prefabElementCategoryList;
	}
	...
	private HashSet<CodexEntry> FilterSearch(string input)
	{
		...
		if (string.IsNullOrEmpty(input))
		{
			...
			{
				if (Game.IsCorrectDlcActiveForCurrentSave(entry.Value) && !entry.Value.searchOnly)
				{
					searchResults.Add(entry.Value);
				}
			}
			FilterEntries(allowOpenCategories: false);
		}
		else
		{
			foreach (KeyValuePair<string, CodexEntry> entry2 in CodexCache.entries)
			{
				if (!Game.IsCorrectDlcActiveForCurrentSave(entry2.Value))
				{
					continue;
				}
				try
				{
					if (SearchUtil.IsPassingScore(FuzzySearch.CanonicalizeAndScore(input, entry2.Value.name).score))
					{
						searchResults.Add(entry2.Value);
					}
					bool flag = false;
					if (!flag && entry2.Value.title != null && SearchUtil.IsPassingScore(FuzzySearch.CanonicalizeAndScore(input, Strings.Get(entry2.Value.title)).score))
					{
						subEntrySearchResults.UnionWith(entry2.Value.subEntries);
						flag = true;
					}
					if (!flag && entry2.Value.category != null && SearchUtil.IsPassingScore(FuzzySearch.CanonicalizeAndScore(input, entry2.Value.category).score))
					{
						subEntrySearchResults.UnionWith(entry2.Value.subEntries);
						flag = true;
					}
	...
	private void PopulatePools()
	{
		foreach (KeyValuePair<Type, GameObject> contentPrefab in ContentPrefabs)
		{
			...
			if (!contentPoolsByName.TryAdd(contentPrefab.Value.name, uIGameObjectPool))
			{
				DebugUtil.DevLogError("Duplicate Codex content prefab name '" + contentPrefab.Value.name + "'; recycle-by-name will mis-route widgets.");
			}
		}
	}
	...
	private void CategorizeEntries()
	{
		...
		{
			Tuple<string, CodexEntry> tuple = list[num];
		...
		{
			list2[num2].Value.transform.parent.SetSiblingIndex(num2);
		}
	...
	public void ChangeArticle(string id, bool playClickSound = false, Vector3 targetPosition = default(Vector3), HistoryDirection historyMovement = HistoryDirection.NewArticle)
	{
		...
		if (!CodexCache.entries.ContainsKey(id))
		{
			...
			{
				Debug.LogWarning("Codex SubEntry not found for target id: " + id);
				id = "PAGENOTFOUND";
			}
		}
		if (CodexCache.entries[id].disabled)
		{
			Debug.LogWarning("Codex Entry disabled for target id: " + id);
			id = "PAGENOTFOUND";
		}
		...
		while (contentContainers.transform.childCount > 0)
		{
			...
			while (gameObject.transform.childCount > 0)
			{
				...
				if (contentPoolsByName.TryGetValue(gameObject2.name, out var value))
				{
					value.ClearElement(gameObject2);
					continue;
				}
				...
				gameObject2.transform.SetParent(widgetPool);
			}
			contentContainerPool.ClearElement(gameObject);
		}
		...
		for (int i = 0; i < CodexCache.entries[id].contentContainers.Count; i++)
		{
			...
			if (!Game.IsCorrectDlcActiveForCurrentSave(contentContainer))
			{
				continue;
			}
			...
			if (!string.IsNullOrEmpty(contentContainer.lockID) && !Game.Instance.unlocks.IsUnlocked(contentContainer.lockID))
			{
				if (text2 != contentContainer.lockID)
				{
			...
			{
				if (Game.IsCorrectDlcActiveForCurrentSave(item as IHasDlcRestrictions))
				{
		...
		else
		{
			...
			case HistoryDirection.NewArticle:
			{
				...
				for (int num2 = history.Count - 1; num2 > currentHistoryIdx; num2--)
				{
