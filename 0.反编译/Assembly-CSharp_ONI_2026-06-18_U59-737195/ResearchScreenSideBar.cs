...
public class ResearchScreenSideBar : KScreen
{
	...
	private void RefreshProjectsActive()
	{
		...
		for (int i = 0; i != techs.Count; i++)
		{
			...
			{
				bool flag = SearchUtil.IsPassingScore(GetTechItemMatchScore(techCache, item.Key));
		...
		foreach (Tuple<int, int> item2 in pooledList)
		{
			...
			bool flag2 = SearchUtil.IsPassingScore(GetTechMatchScore(tech3.Id));
	...
	{
		if (completionFilter == CompletionState.All)
		{
	...
	private int GetTechMatchScore(string techID)
	{
		...
		return techCaches[techID].Score;
	}
