...
public class SteamTurbineConfig2 : IBuildingConfig
{
	...
	{
		string[] array = new string[2] { "RefinedMetal", "BuildingGasket" };
		BuildingDef obj = BuildingTemplates.CreateBuildingDef(construction_mass: new float[2]
		{
			...
			4f
		}, construction_materials: array, melting_point: 1600f, build_location_rule: BuildLocationRule.OnFloor, noise: NOISE_POLLUTION.NONE, id: "SteamTurbine2", width: 5, height: 3, anim: "steamturbine2_kanim", hitpoints: 30, construction_time: 60f, decor: TUNING.BUILDINGS.DECOR.NONE, temperature_modification_mass_scale: 1f);
		...
		obj.AddSearchTerms(SEARCH_TERMS.STEAM);
		return obj;
	}
	...
	private static void AddVisualizer(GameObject go)
	{
		...
		go.GetComponent<KPrefabID>().instantiateFn += delegate(GameObject gameObject)
		{
			gameObject.GetComponent<RangeVisualizer>().BlockingCb = SteamTurbineBlockingCB;
		};
	}
