...
public class SimCellOccupier : KMonoBehaviour, IGameObjectEffectDescriptor
{
	...
	protected override void OnSpawn()
	{
		...
		building.RunOnArea(delegate(int offset_cell)
		{
			...
			else
			{
				...
				runSimCallback = true;
				Sim.Cell.Properties simCellProperties2 = GetSimCellProperties();
				SimMessages.SetCellProperties(offset_cell, (byte)simCellProperties2, Game.Instance.callbackManager.Add(new Game.CallbackInfo(OnCellPropertiesChanged)).index);
			}
	...
	{
		if (!(this == null) && !(base.gameObject == null) && runSimCallback)
		{
			building.RunOnArea(delegate(int offset_cell)
			{
