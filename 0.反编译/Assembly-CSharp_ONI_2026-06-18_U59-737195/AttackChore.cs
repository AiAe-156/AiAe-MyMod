...
public class AttackChore : Chore<AttackChore.StatesInstance>
{
	...
	public class States : GameStateMachine<States, StatesInstance, AttackChore>
	{
		...
		public override void InitializeStates(out BaseState default_state)
		{
			...
			approachtarget.InitializeStates(attacker, attackTarget, attack, null, BaseMinionConfig.ATTACK_OFFSETS, NavigationTactics.Range_3_ProhibitOverlap).Enter(delegate(StatesInstance smi)
			{
				smi.master.CleanUpMultitool();
				smi.master.Trigger(1039067354, attackTarget.Get(smi));
				Health component = attackTarget.Get(smi).GetComponent<Health>();
			...
			attack.Target(attacker).Enter(delegate(StatesInstance smi)
			{
				...
				{
					smi.master.multiTool = new MultitoolController.Instance(attackTarget.Get(smi).GetComponent<Workable>(), smi.master.GetComponent<WorkerBase>(), "attack", Assets.GetPrefab(EffectConfigs.AttackSplashId));
					smi.master.multiTool.StartSM();
				}
				attackTarget.Get(smi).Subscribe(1969584890, smi.master.OnTargetDestroyed);
				smi.ScheduleGoTo(1f / DUPLICANTSTATS.STANDARD.Combat.BasicWeapon.ATTACKS_PER_SECOND, success);
	...
	public string GetHitAnim()
	{
		...
		if ((bool)component)
		{
			return MultitoolController.GetAnimationStrings(component, gameObject.GetComponent<WorkerBase>(), "hit")[1].Replace("_loop", "");
		}
		return "hit";
	}
