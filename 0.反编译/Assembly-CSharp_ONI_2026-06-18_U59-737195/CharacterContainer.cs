...
public class CharacterContainer : KScreen, ITelepadDeliverableContainer
{
	...
	protected override void OnSpawn()
	{
		...
		foreach (SkillGroup item2 in new List<SkillGroup>(Db.Get().SkillGroups.resources))
		{
	...
	private void SetInfoText()
	{
		...
		for (int num = 1; num < stats.Traits.Count; num++)
		{
			...
			for (int num2 = 0; num2 < trait.SelfModifiers.Count; num2++)
			{
				...
				_ = trait.SelfModifiers[num2].AttributeId == "GermResistance";
	...
	private IEnumerator SetAttributes()
	{
		...
		for (int num = 0; num < list.Count; num++)
		{
			...
			LocText componentInChildren = gameObject.GetComponentInChildren<LocText>();
			gameObject.SetActive(value: true);
			float totalValue = list[num].GetTotalValue();
			if (totalValue > 0f)
			{
			...
			componentInChildren.text = string.Format(UI.CHARACTERCONTAINER_SKILL_VALUE, GameUtil.AddPositiveSign(totalValue.ToString(), totalValue > 0f), list[num].Name);
			AttributeInstance attributeInstance = list[num];
