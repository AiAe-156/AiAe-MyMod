...
public class Door : Workable, ISaveLoadable, ISim200ms, INavDoor
{
		...
		public override void InitializeStates(out BaseState default_state)
		{
			...
			}).Enter(SetWaterproof).Enter("SetWorldStateClosed", delegate(Instance smi)
			{
				...
				.Exit(delegate(Instance smi)
				{
					...
					smi.master.requestedState = ControlState.Opened;
					smi.master.RefreshControlState();
	...
	protected override void OnSpawn()
	{
		...
		ApplyControlState();
		bool num = SaveLoader.Instance.GameInfo.IsVersionOlderThan(7, 38);
		bool flag = doorType == DoorType.Sealed && hasBeenUnsealed && controlState == ControlState.Opened;
		if (num && flag)
		{
		...
		int num2 = ((rotatable.GetOrientation() == Orientation.Neutral) ? (building.Def.WidthInCells * (building.Def.HeightInCells - 1)) : 0);
		int num3 = ((rotatable.GetOrientation() == Orientation.Neutral) ? building.Def.WidthInCells : building.Def.HeightInCells);
		for (int i = 0; i != num3; i++)
		{
			int num4 = building.PlacementCells[num2 + i];
