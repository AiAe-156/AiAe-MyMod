...
public class SuitFabricatorConfig : IBuildingConfig
{
	...
	public override BuildingDef CreateBuildingDef()
	{
		...
		obj.AddSearchTerms(SEARCH_TERMS.ATMOSUIT);
		return obj;
	}
	...
	private void ConfigureRecipes()
	{
		...
		{
			new ComplexRecipe.RecipeElement(GameTags.BasicRefinedMetals, 300f, ComplexRecipe.RecipeElement.TemperatureOperation.AverageTemperature, "", storeElement: false, inheritElement: true),
		...
		new ComplexRecipe(ComplexRecipeManager.MakeRecipeID("SuitFabricator", array3, array4), array3, array4)
		{
			...
			requiredTech = Db.Get().TechItems.atmoSuit.parentTechId,
			sortOrder = 2,
			customName = STRINGS.EQUIPMENT.PREFABS.ATMO_SUIT.REPAIR_WORN_RECIPE_NAME,
		...
		new ComplexRecipe(ComplexRecipeManager.MakeRecipeID("SuitFabricator", array7, array8), array7, array8)
		{
			time = TUNING.EQUIPMENT.SUITS.ATMOSUIT_FABTIME,
			description = STRINGS.EQUIPMENT.PREFABS.JET_SUIT.REPAIR_WORN_DESC,
			nameDisplay = ComplexRecipe.RecipeNameDisplay.Custom,
			fabricators = new List<Tag> { "SuitFabricator" },
			requiredTech = Db.Get().TechItems.jetSuit.parentTechId,
			sortOrder = 4,
			customName = STRINGS.EQUIPMENT.PREFABS.JET_SUIT.REPAIR_WORN_RECIPE_NAME
		};
		...
		if (DlcManager.FeatureRadiationEnabled())
		{
			...
			new ComplexRecipe(ComplexRecipeManager.MakeRecipeID("SuitFabricator", array11, array12), array11, array12)
			{
				time = TUNING.EQUIPMENT.SUITS.ATMOSUIT_FABTIME,
				description = STRINGS.EQUIPMENT.PREFABS.LEAD_SUIT.REPAIR_WORN_DESC,
				nameDisplay = ComplexRecipe.RecipeNameDisplay.Custom,
				fabricators = new List<Tag> { "SuitFabricator" },
				requiredTech = Db.Get().TechItems.leadSuit.parentTechId,
				sortOrder = 6,
				customName = STRINGS.EQUIPMENT.PREFABS.LEAD_SUIT.REPAIR_WORN_RECIPE_NAME
			};
		}
	}
