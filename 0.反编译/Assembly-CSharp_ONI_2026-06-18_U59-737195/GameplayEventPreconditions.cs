...
public class GameplayEventPreconditions
{
	...
	public GameplayEventPrecondition DifficultySetting(SettingConfig config, string levelId)
	{
		...
		gameplayEventPrecondition.condition = () => CustomGameSettings.Instance.GetCurrentQualitySetting(config).id == levelId;
		gameplayEventPrecondition.description = "The config " + config.id + " is level " + levelId + ".";
	...
	public GameplayEventPrecondition ClusterHasTag(string tag)
	{
		return new GameplayEventPrecondition
		{
			condition = () => CustomGameSettings.Instance.GetCurrentClusterLayout()?.clusterTags.Contains(tag) ?? false,
