...
public class AsteroidGridEntity : ClusterGridEntity
{
	...
	{
		if (((Boxed<int>)_worldID).value == m_worldContainer.id)
		{
			RefreshMeteorShowerEffect();
		}
	}
	...
	{
		if (data == null || ((Boxed<AxialI>)data).value != m_location || !ClusterGrid.Instance.IsCellVisible(base.Location) || !DlcManager.FeatureClusterSpaceEnabled())
		{
			return;
		}
