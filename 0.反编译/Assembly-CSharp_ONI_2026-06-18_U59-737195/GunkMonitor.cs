...
public class GunkMonitor : GameStateMachine<GunkMonitor, GunkMonitor.Instance, IStateMachineTarget, GunkMonitor.Def>
{
	...
	public new class Instance : GameInstance
	{
		...
		public bool DoesCurrentScheduleAllowsGunkToilet
		{
			get
			{
				if (!schedulable.IsAllowed(Db.Get().ScheduleBlockTypes.Eat))
				{
					return schedulable.IsAllowed(Db.Get().ScheduleBlockTypes.Hygiene);
				}
				return true;
			}
		}
		...
		public void ExpellGunk(float mass, Storage targetStorage = null)
		{
			...
			if (Sim.IsRadiationEnabled())
			{
				...
				float num2 = Math.Min(val2: 300f * (DUPLICANTSTATS.STANDARD.BaseStats.BLADDER_INCREASE_PER_SECOND / DUPLICANTSTATS.BIONICS.BaseStats.BLADDER_INCREASE_PER_SECOND) * sMI.difficultySettingMod * num, val1: amountInstance.value);
				if (num2 >= 1f)
				{
					PopFXManager.Instance.SpawnFX(PopFXManager.Instance.sprite_Negative, Math.Floor(num2).ToString() + UI.UNITSUFFIXES.RADIATION.RADS, component.transform, Vector3.up * 2f);
				}
				amountInstance.ApplyDelta(0f - num2);
			}
