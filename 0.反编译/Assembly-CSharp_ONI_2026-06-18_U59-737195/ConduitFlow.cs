...
public class ConduitFlow : IConduitFlow
{
	...
	private bool UpdateConduit(Conduit conduit)
	{
		...
		if (num <= 0f)
		{
			for (int i = 0; i != 4; i++)
			{
				...
				if ((permittedFlowDirections & flowDirections) != FlowDirections.None)
				{
					...
					Debug.Assert(conduitFromDirection.idx != -1);
					if ((soaInfo.GetSrcFlowDirection(conduitFromDirection.idx) & Opposite(flowDirections)) != FlowDirections.None)
					{
		...
		if (srcFlowDirection2 == FlowDirections.None || (Opposite(srcFlowDirection2) & pullDirection) != FlowDirections.None)
		{
			...
			for (int k = 0; k != 2; k++)
			{
				...
				for (int l = 0; l != 4; l++)
				{
					...
					if (conduitFromDirection3.idx != -1 && (soaInfo.GetPermittedFlowDirections(conduitFromDirection3.idx) & Opposite(flowDirections2)) != FlowDirections.None)
					{
