...
public class WarpPortal : Workable
{
	...
	public IEnumerator DelayedWarp(WarpReceiver receiver)
	{
		...
		GameUtil.FocusCameraOnWorld(myWorldId2, Grid.CellToPos(Grid.PosToCell(receiver)));
		WorkerBase workerBase = base.worker;
		workerBase.StopWork();
		receiver.ReceiveWarpedDuplicant(workerBase);
		ClusterManager.Instance.MigrateMinion(workerBase.GetComponent<MinionIdentity>(), myWorldId2, myWorldId);
