...
public class SandboxToolParameterMenu : KScreen
{
	...
	public SelectorValue targetLayerSelector;
	...
	private void ConfigureSettings()
	{
		...
		sandboxSettings2.OnChangeTargetLayer = (System.Action)Delegate.Combine(sandboxSettings2.OnChangeTargetLayer, (System.Action)delegate
		{
			string text = ((settings.GetIntSetting("SandboxTools.SelectedTargetLayer") == 1) ? UI.SANDBOXTOOLS.SETTINGS.TARGET_LAYER.BACKGROUND : UI.SANDBOXTOOLS.SETTINGS.TARGET_LAYER.FOREGROUND);
			targetLayerSelector.button.GetComponentInChildren<LocText>().text = text;
		});
		SandboxSettings sandboxSettings3 = settings;
		sandboxSettings3.OnChangeMass = (System.Action)Delegate.Combine(sandboxSettings3.OnChangeMass, (System.Action)delegate
		{
		...
		sandboxSettings4.OnChangeDisease = (System.Action)Delegate.Combine(sandboxSettings4.OnChangeDisease, (System.Action)delegate
		{
		...
		sandboxSettings5.OnChangeDiseaseCount = (System.Action)Delegate.Combine(sandboxSettings5.OnChangeDiseaseCount, (System.Action)delegate
		{
		...
		sandboxSettings6.OnChangeStory = (System.Action)Delegate.Combine(sandboxSettings6.OnChangeStory, (System.Action)delegate
		{
			string stringSetting = instance.settings.GetStringSetting("SandboxTools.SelectedStory");
		...
		sandboxSettings7.OnChangeEntity = (System.Action)Delegate.Combine(sandboxSettings7.OnChangeEntity, (System.Action)delegate
		{
			...
			else
			{
				...
				Tuple<Sprite, Color> tuple = ((!gameObject.HasTag(GameTags.BaseMinion)) ? Def.GetUISprite(stringSetting) : new Tuple<Sprite, Color>(BaseMinionConfig.GetSpriteForMinionModel(gameObject.PrefabID()), Color.white));
				if (tuple != null)
				{
		...
		sandboxSettings8.OnChangeBrushSize = (System.Action)Delegate.Combine(sandboxSettings8.OnChangeBrushSize, (System.Action)delegate
		{
		...
		sandboxSettings9.OnChangeNoiseScale = (System.Action)Delegate.Combine(sandboxSettings9.OnChangeNoiseScale, (System.Action)delegate
		{
		...
		sandboxSettings10.OnChangeNoiseDensity = (System.Action)Delegate.Combine(sandboxSettings10.OnChangeNoiseDensity, (System.Action)delegate
		{
			if (PlayerController.Instance.ActiveTool is SandboxSprinkleTool)
			{
				(PlayerController.Instance.ActiveTool as SandboxSprinkleTool).SetBrushSize(settings.GetIntSetting("SandboxTools.BrushSize"));
		...
		sandboxSettings11.OnChangeTemperature = (System.Action)Delegate.Combine(sandboxSettings11.OnChangeTemperature, (System.Action)delegate
		{
		...
		sandboxSettings12.OnChangeAdditiveTemperature = (System.Action)Delegate.Combine(sandboxSettings12.OnChangeAdditiveTemperature, (System.Action)delegate
		{
		...
		sandboxSettings13.OnChangeAdditiveStress = (System.Action)Delegate.Combine(sandboxSettings13.OnChangeAdditiveStress, (System.Action)delegate
		{
			stressAdditiveSlider.SetValue(settings.GetFloatSetting("SandbosTools.StressAdditive"), runOnValueChanged: false);
		});
		SandboxSettings sandboxSettings14 = settings;
		sandboxSettings14.OnChangeMoraleAdjustment = (System.Action)Delegate.Combine(sandboxSettings14.OnChangeMoraleAdjustment, (System.Action)delegate
		{
	...
	public void DisableParameters()
	{
		...
		targetLayerSelector.row.SetActive(value: false);
	...
	protected override void OnSpawn()
	{
		...
		SpawnSelector(targetLayerSelector);
	...
	private void ConfigureElementSelector()
	{
		...
		foreach (Element element in ElementLoader.elements)
		{
			...
			for (int num = 0; num < oreTags.Length; num++)
			{
				if (oreTags[num] == GameTags.HideFromSpawnTool)
				{
					flag = true;
					break;
				}
	...
	private void ConfigureEntitySelector()
	{
		...
		SelectorValue.SearchFilter item = new SelectorValue.SearchFilter(UI.SANDBOXTOOLS.FILTERS.ENTITIES.FOOD, delegate(object entity)
		{
			...
			return (!kPrefabID.HasTag(GameTags.Egg) && EdiblesManager.GetAllFoodTypes().Find((EdiblesManager.FoodInfo match) => match.Id == idString) != null) || kPrefabID.HasTag(GameTags.Dehydrated);
		...
		{
			KPrefabID kPrefabID = entity as KPrefabID;
			return kPrefabID.HasTag(GameTags.Comet) && Game.IsCorrectDlcActiveForCurrentSave(kPrefabID);
		...
		}, null, new Tuple<Sprite, Color>(BaseMinionConfig.GetSpriteForMinionModel(GameTags.Minions.Models.Standard), Color.white));
		list.Add(item3);
		if (Game.IsDlcActiveForCurrentSave("DLC3_ID"))
		{
			SelectorValue.SearchFilter item4 = new SelectorValue.SearchFilter(UI.SANDBOXTOOLS.FILTERS.ENTITIES.BIONICUPGRADES, delegate(object entity)
			{
				KPrefabID kPrefabID = entity as KPrefabID;
				return kPrefabID.HasTag(GameTags.BionicUpgrade) && Game.IsCorrectDlcActiveForCurrentSave(kPrefabID);
			}, null, new Tuple<Sprite, Color>(Def.GetUISpriteFromMultiObjectAnim(Assets.GetAnim("upgrade_disc_kanim")), Color.white));
			list.Add(item4);
		}
		...
		foreach (GameObject item12 in Assets.GetPrefabsWithTag(GameTags.CreatureBrain))
		{
			CreatureBrain brain = item12.GetComponent<CreatureBrain>();
			...
			if (!component.HasTag(GameTags.HideFromSpawnTool) && !list2.Contains(brain.species) && Game.IsCorrectDlcActiveForCurrentSave(component))
			{
				...
				if (CodexCache.entries.TryGetValue(brain.species.ToString().ToUpper(), out var value))
				{
					icon = new Tuple<Sprite, Color>(value.icon, value.iconColor);
				}
				list2.Add(brain.species);
				SelectorValue.SearchFilter item5 = new SelectorValue.SearchFilter(Strings.Get("STRINGS.CREATURES.FAMILY_PLURAL." + brain.species.ToString().ToUpper()), delegate(object entity)
				{
		...
		SelectorValue.SearchFilter item6 = new SelectorValue.SearchFilter(UI.SANDBOXTOOLS.FILTERS.ENTITIES.EQUIPMENT, delegate(object entity)
		{
			...
			return gameObject != null && gameObject.GetComponent<Equippable>() != null;
		...
		SelectorValue.SearchFilter searchFilter3 = new SelectorValue.SearchFilter(UI.SANDBOXTOOLS.FILTERS.ENTITIES.PLANTS, delegate(object entity)
		{
			...
			return kPrefabID != null && Game.IsCorrectDlcActiveForCurrentSave(kPrefabID) && (kPrefabID.GetComponent<Harvestable>() != null || kPrefabID.GetComponent<WiltCondition>() != null);
		}, null, Def.GetUISprite(Assets.GetPrefab("PrickleFlower")));
		list.Add(searchFilter3);
		SelectorValue.SearchFilter item7 = new SelectorValue.SearchFilter(UI.SANDBOXTOOLS.FILTERS.ENTITIES.SEEDS, delegate(object entity)
		{
			if ((entity as KPrefabID).gameObject == null)
			{
				return false;
			...
			GameObject gameObject = (entity as KPrefabID).gameObject;
			return gameObject != null && gameObject.GetComponent<PlantableSeed>() != null;
		}, searchFilter3, Def.GetUISprite(Assets.GetPrefab("PrickleFlowerSeed")));
		list.Add(item7);
		SelectorValue.SearchFilter item8 = new SelectorValue.SearchFilter(UI.SANDBOXTOOLS.FILTERS.ENTITIES.INDUSTRIAL_PRODUCTS, delegate(object entity)
		{
			KPrefabID kPrefabID = entity as KPrefabID;
			if (kPrefabID == null || kPrefabID.gameObject == null)
			{
			...
			return kPrefabID.HasTag(GameTags.IndustrialIngredient) || kPrefabID.HasTag(GameTags.TechComponents) || kPrefabID.HasTag(GameTags.IndustrialProduct) || kPrefabID.HasTag(GameTags.Medicine) || kPrefabID.HasTag(GameTags.MedicalSupplies) || kPrefabID.HasTag(GameTags.ChargedPortableBattery) || kPrefabID.HasTag(TableSaltConfig.TAG) || kPrefabID.HasTag(GameTags.MoltShell);
		}, null, Def.GetUISprite(Assets.GetPrefab("BasicCure")));
		list.Add(item8);
		SelectorValue.SearchFilter item9 = new SelectorValue.SearchFilter(UI.SANDBOXTOOLS.FILTERS.ENTITIES.ORE_CHUNKS, (object entity) => ElementLoader.FindElementByTag((entity as KPrefabID).PrefabTag)?.IsSolid ?? false, null, new Tuple<Sprite, Color>(Assets.GetSprite("ui_elements-solid"), Color.white));
		list.Add(item9);
		SelectorValue.SearchFilter item10 = new SelectorValue.SearchFilter(UI.SANDBOXTOOLS.FILTERS.ENTITIES.BOTTLES, (object entity) => ElementLoader.FindElementByTag((entity as KPrefabID).PrefabTag)?.IsLiquid ?? false, null, new Tuple<Sprite, Color>(Assets.GetSprite("ui_elements-liquids"), Color.white));
		list.Add(item10);
		SelectorValue.SearchFilter item11 = new SelectorValue.SearchFilter(UI.SANDBOXTOOLS.FILTERS.ENTITIES.CANISTERS, (object entity) => ElementLoader.FindElementByTag((entity as KPrefabID).PrefabTag)?.IsGas ?? false, null, new Tuple<Sprite, Color>(Assets.GetSprite("ui_elements-gases"), Color.white));
		list.Add(item11);
		List<KPrefabID> list3 = new List<KPrefabID>();
		foreach (KPrefabID prefab2 in Assets.Prefabs)
		{
			if (!Game.IsCorrectDlcActiveForCurrentSave(prefab2) || prefab2.HasTag(GameTags.HideFromSpawnTool))
			{
				continue;
			}
			foreach (SelectorValue.SearchFilter item13 in list)
			{
				if (item13.condition(prefab2))
				{
					list3.Add(prefab2);
					break;
				}
	...
	{
		object[] options = new object[2] { 0, 1 };
		targetLayerSelector = new SelectorValue(options, delegate(object layer)
		{
			settings.SetIntSetting("SandboxTools.SelectedTargetLayer", (int)layer);
		}, (object layer) => ((int)layer == 1) ? UI.SANDBOXTOOLS.SETTINGS.TARGET_LAYER.BACKGROUND : UI.SANDBOXTOOLS.SETTINGS.TARGET_LAYER.FOREGROUND, null, (object layer) => new Tuple<Sprite, Color>(Assets.GetSprite("dash"), Color.white), UI.SANDBOXTOOLS.SETTINGS.TARGET_LAYER.NAME);
	}
