...
public class LaunchButtonSideScreen : SideScreenContent
{
	...
	private void Refresh()
	{
		...
		bool flag4 = rocketModule.CraftInterface.HasTag(GameTags.RocketNotOnGround);
		if (num)
		{
		...
		if (interiorWorld != null)
		{
			...
			bool flag5 = passengerModule.CheckPassengersBoarded(robotPilotModule == null);
			if (!flag5 && robotPilotModule != null)
			{
				flag5 |= !passengerModule.HasCrewAssigned();
			}
			bool flag6 = !passengerModule.CheckExtraPassengers();
			bool flag7 = robotPilotModule != null || rocketControlStationLaunchWorkable.worker != null;
			if (!flag3)
			{
				statusText.text = UI.UISIDESCREENS.LAUNCHPADSIDESCREEN.STATUS.STILL_PREPPING;
			}
