...
public class CreatureDeliveryPoint : StateMachineComponent<CreatureDeliveryPoint.SMInstance>
{
	...
	public class States : GameStateMachine<States, SMInstance, CreatureDeliveryPoint>
	{
		...
		{
			if (IsLogicEnabled(smi) && IsOperational(smi) && !IsStrawBlocked(smi))
			{
		...
		public static void DropAllCreatures(SMInstance smi)
		{
			...
			if (component.IsEmpty() || smi.isDroppingAllCreatures)
			{
				return;
			}
	...
	private void RebalanceFetches()
	{
		...
		if (num7 == 0 && fetches.Count < num)
		{
			...
			FetchOrder2 fetchOrder = new FetchOrder2(creatureFetch, tags, FetchChore.MatchCriteria.MatchID, GameTags.Creatures.Deliverable, null, component, minimumFetchAmount, Operational.State.Operational);
