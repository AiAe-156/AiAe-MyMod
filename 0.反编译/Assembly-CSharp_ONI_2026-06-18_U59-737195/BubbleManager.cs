	...
	public void Sim33ms(float dt)
	{
		...
		foreach (var (item, instanceData2) in bubbles)
		{
			...
			foreach (InstanceData.Subscript item2 in instanceData2)
			{
				...
				{
					if (Grid.Solid[cell] && Grid.Element[cell].IsSolid)
					{
						cell = Grid.PosToCell(current.Position);
					}
					else if (Grid.Element[cell].IsLiquid && Grid.Element[cell].id != value.element)
					{
						int num = Grid.CellAbove(cell);
						if (Grid.IsValidCell(num) && (Grid.IsGas(num) || Grid.Element[num].IsVacuum))
						{
							cell = num;
						}
					}
					SimMessages.AddRemoveSubstance(cell, value.element, CellEventLogger.Instance.FallingWaterAddToSim, current.Mass, current.Temperature, current.Disease.Idx, current.Disease.Count);
