...
public class ElementConverter : StateMachineComponent<ElementConverter.StatesInstance>, IGameObjectEffectDescriptor
{
	...
	public class StatesInstance : GameStateMachine<States, StatesInstance, ElementConverter, object>.GameInstance
	{
		...
		public void UpdateStatusItems()
		{
			...
			for (int i = 0; i < base.master.consumedElements.Length; i++)
			{
				ConsumedElement element = base.master.consumedElements[i];
				if (element.IsActive)
				{
					bool flag = element.Rate > 0f;
					bool flag2 = base.master.consumedElementStatusHandles.ContainsKey(element.Tag);
					if (flag && !flag2)
					{
					...
					{
						RemoveStatusItem(element.Tag, base.master.consumedElementStatusHandles);
						base.master.consumedElementStatusHandles.Remove(element.Tag);
					}
				}
			}
			for (int j = 0; j < base.master.outputElements.Length; j++)
			{
				OutputElement element2 = base.master.outputElements[j];
				if (element2.IsActive)
				{
					bool flag3 = element2.Rate > 0f;
					bool flag4 = base.master.outputElementStatusHandles.ContainsKey(element2.elementHash);
					if (flag3 && !flag4)
					{
					...
					{
						RemoveStatusItem(element2.elementHash, base.master.outputElementStatusHandles);
						base.master.outputElementStatusHandles.Remove(element2.elementHash);
					}
				}
			}
		}
