	...
	private void SpawnRow(ColonyDiagnostic diagnostic, GameObject container)
	{
		...
		reference.onClick = (System.Action)Delegate.Combine(reference.onClick, (System.Action)delegate
		{
			...
			else
			{
				...
				int num2 = (int)(ColonyDiagnosticUtility.Instance.diagnosticDisplaySettings[activeWorldId][id2] - 1);
				if (num2 < 0)
				{
					num2 = 2;
				}
				ColonyDiagnosticUtility.Instance.diagnosticDisplaySettings[activeWorldId][id2] = (ColonyDiagnosticUtility.DisplaySetting)num2;
			}
			RefreshRows();
		...
		diagnosticRows.Add(id, gameObject);
		criteriaRows.Add(id, new Dictionary<string, GameObject>());
		currentlyDisplayedRows.Add(id, value: true);
		component.GetReference<Image>("Icon").sprite = Assets.GetSprite(diagnostic.icon);
		RefreshPinnedState(id);
		...
		foreach (KeyValuePair<string, GameObject> diagnosticRow in diagnosticRows)
		{
			...
			if (diagnostic != null)
			{
				...
				if (tracker != null)
				{
					...
					reference.graph.axis_x.max_value = array[^1].first;
