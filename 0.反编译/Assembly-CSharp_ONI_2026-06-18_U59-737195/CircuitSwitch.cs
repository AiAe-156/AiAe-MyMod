...
public class CircuitSwitch : Switch, IPlayerControlledToggle, ISim33ms
{
	...
	{
		objectDestroyedHandle = wire.Subscribe(1969584890, OnWireDestroyed);
		buildingFullyRepairedHandle = wire.Subscribe(-1735440190, OnWireStateChanged);
		buildingBrokenHandle = wire.Subscribe(774203113, OnWireStateChanged);
	}

	private void UnsubscribeFromWire(Wire wire)
	{
		wire.Unsubscribe(ref objectDestroyedHandle);
		wire.Unsubscribe(ref buildingFullyRepairedHandle);
		wire.Unsubscribe(ref buildingBrokenHandle);
	}
