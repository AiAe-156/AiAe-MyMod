...
public class MultitoolController : GameStateMachine<MultitoolController, MultitoolController.Instance, WorkerBase>
{
	public new class Instance : GameInstance
	{
			...
			: base(worker)
		{
			...
			{
				anims = GetDroneAnimationStrings(context);
			}
			else
			{
		...
		public void UpdateHitEffectTarget()
		{
			if (!(hitEffect == null))
			{
				...
				workerBase.GetComponent<Facing>().Face(workable.transform.GetPosition());
				anims = GetAnimationStrings(workable, workerBase);
				PlayLoop();
		...
		{
			WorkerBase workerBase = base.sm.worker.Get<WorkerBase>(base.smi);
			if (workerBase == null || workable == null)
			{
				return;
			}
			if (Grid.PosToCell(workable) != Grid.PosToCell(workerBase))
			{
				workerBase.Trigger(-673283254);
			}
			Diggable diggable = workable as Diggable;
			if ((bool)diggable)
			{
				Element targetElement = diggable.GetTargetElement();
				workerBase.Trigger(-1762453998, (object)targetElement);
			}
		...
		public void DestroyHitEffect()
		{
			...
			if (workerBase != null)
			{
				workerBase.Trigger(-1559999068);
				workerBase.Trigger(939543986);
			}
	...
	private static readonly string[][][] ANIM_BASE = new string[5][][]
	{
		new string[5][]
		{
			...
			new string[3] { "swim_{verb}_dn_pre", "swim_{verb}_dn_loop", "swim_{verb}_dn_pst" }
		},
		new string[5][]
		{
			...
			new string[3] { "swim_{verb}_diag_dn_pre", "swim_{verb}_diag_dn_loop", "swim_{verb}_diag_dn_pst" }
		},
		new string[5][]
		{
			...
			new string[3] { "swim_{verb}_fwd_pre", "swim_{verb}_fwd_loop", "swim_{verb}_fwd_pst" }
		},
		new string[5][]
		{
			...
			new string[3] { "swim_{verb}_diag_up_pre", "swim_{verb}_diag_up_loop", "swim_{verb}_diag_up_pst" }
		},
		new string[5][]
		{
			...
			new string[3] { "swim_{verb}_up_pre", "swim_{verb}_up_loop", "swim_{verb}_up_pst" }
		}
	};
	...
	public override void InitializeStates(out BaseState default_state)
	{
		...
		}).Exit("DestroyHitEffect", delegate(Instance smi)
		{
			...
			.EventTransition(GameHashes.WorkerPlayPostAnim, pst, (Instance smi) => smi.GetComponent<WorkerBase>().GetState() == WorkerBase.State.PendingCompletion);
	...
	{
		if (drone_anims.TryGetValue(context, out var value))
		{
			return value;
		}
		Debug.LogError($"Missing drone multitool anims for context {context}");
		return new string[3] { "", "", "" };
	}
