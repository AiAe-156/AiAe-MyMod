...
public class HotTubWorkerStateMachine : GameStateMachine<HotTubWorkerStateMachine, HotTubWorkerStateMachine.StatesInstance, WorkerBase>
{
	...
	public override void InitializeStates(out BaseState default_state)
	{
		...
		loop.PlayAnim((StatesInstance smi) => workAnimLoopVariants[Random.Range(0, workAnimLoopVariants.Length)]).OnAnimQueueComplete(loop_reenter).EventTransition(GameHashes.WorkerPlayPostAnim, pst_back, (StatesInstance smi) => smi.GetComponent<WorkerBase>().GetState() == WorkerBase.State.PendingCompletion);
		loop_reenter.GoTo(loop).EventTransition(GameHashes.WorkerPlayPostAnim, pst_back, (StatesInstance smi) => smi.GetComponent<WorkerBase>().GetState() == WorkerBase.State.PendingCompletion);
