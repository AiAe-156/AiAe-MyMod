...
public class FishOvercrowingManager : KMonoBehaviour, ISim1000ms
{
	...
	public void Sim1000ms(float dt)
	{
		...
		foreach (KPrefabID allAquaticEntity in allAquaticEntities)
		{
			...
			if (num4)
			{
				FloodFill.DepthTraverse(num3, new FloodFill.PredicateCondition(isLiquidCell), new VisitTracker(grid, new Cell(num, num5)), default(FloodFill.NoMaxDepth), new Visitor(pond2));
			}
		}
		if (num2 != -1)
		{
			int num6 = ponds.Count - num2;
			if (num6 > 0)
			{
				ponds.RemoveRange(num2, num6);
			}
		}
	...
	public Pond GetPond(int cell)
	{
		...
		if (cell2.Generation != nextGeneration - 1)
		{
			return null;
		}
		return ponds[cell2.PondIndex];
	}
