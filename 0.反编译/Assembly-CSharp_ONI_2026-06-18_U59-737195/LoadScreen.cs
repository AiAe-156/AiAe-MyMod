...
public class LoadScreen : KModalScreen
{
	...
	private bool CheckSaveVersion(SaveGameFileDetails save, LocText display)
	{
		if (IsSaveFileFromUnsupportedFutureBuild(save.FileHeader, save.FileInfo))
		{
			if (display != null)
			{
				display.text = string.Format(UI.FRONTEND.LOADSCREEN.SAVE_TOO_NEW, save.FileName, save.FileHeader.buildVersion, save.FileInfo.saveMinorVersion, 737790u, 38);
			}
			return false;
		}
		if (save.FileInfo.saveMajorVersion < 7)
		{
			if (display != null)
			{
				display.text = string.Format(UI.FRONTEND.LOADSCREEN.UNSUPPORTED_SAVE_VERSION, save.FileName, save.FileInfo.saveMajorVersion, save.FileInfo.saveMinorVersion, 7, 38);
			}
			return false;
		}
		return true;
	...
	private void ShowColonySave(SaveGameFileDetails save)
	{
		...
		component2.gameObject.SetActive(!CheckSaveVersion(save, component2));
	...
	private void ShowColony(List<SaveGameFileDetails> saves, int selectIndex = -1)
	{
		...
		for (int num2 = 0; num2 < saves.Count; num2++)
		{
			...
			component3.GetReference<RectTransform>("NewestLabel").gameObject.SetActive(num2 == 0);
			...
			foreach (string dlcId in save.FileInfo.dlcIds)
			{
				...
				{
					GameObject obj = Util.KInstantiateUI(reference2.gameObject, reference2.transform.parent.gameObject, force_active: true);
					obj.GetComponent<Image>().sprite = Assets.GetSprite(DlcManager.GetDlcSmallLogo(dlcId));
					obj.GetComponent<ToolTip>().SetSimpleTooltip(string.Format(UI.FRONTEND.LOADSCREEN.TOOLTIP_SAVE_USES_DLC, UI.UNKNOWN_DLC.NAME));
				}
			}
			for (int num3 = DlcManager.RELEASED_VERSIONS.Count - 1; num3 >= 0; num3--)
			{
				...
				{
					GameObject obj2 = Util.KInstantiateUI(reference2.gameObject, reference2.transform.parent.gameObject, force_active: true);
					obj2.GetComponent<Image>().sprite = Assets.GetSprite(DlcManager.GetDlcSmallLogo(text));
					obj2.GetComponent<ToolTip>().SetSimpleTooltip(string.Format(UI.FRONTEND.LOADSCREEN.TOOLTIP_SAVE_USES_DLC, DlcManager.GetDlcTitle(text)));
				}
			}
			bool num4 = CheckSaveVersion(save, null) && CheckSaveDLCsCompatable(save);
			...
			if (!num4)
			{
				...
				component4.GetComponent<ToolTip>().SetSimpleTooltip(GetSaveDLCIncompatabilityTooltip(save));
			}
			else
			{
				component4.onClick += delegate
	...
	private void AddColonyToList(List<SaveGameFileDetails> saves)
	{
		...
		freeElement.GetReference<RectTransform>("HeaderDate").GetComponent<LocText>().text = string.Format("{0:H:mm:ss} - " + Localization.GetFileDateFormat(0), save.FileDate.ToLocalTime());
		...
		SetPreview(save.FileName, colonyName, component, fallbackToTimelapse: true);
		...
		foreach (var item3 in list)
		{
			...
			(component2.sprite, _) = item3;
			component3.SetSimpleTooltip(item3.Item2);
		}
		...
		GetSaveDLCIncompatabilityTooltip(save);
	...
	{
		if (gameInfo.saveMajorVersion > 7 || (gameInfo.saveMajorVersion == 7 && gameInfo.saveMinorVersion > 38))
		{
			return true;
		}
		return header.buildVersion > 737790;
	}
	...
	public static void DoLoad(string filename)
	{
		...
		string arg = null;
		string arg2 = null;
		if (header.buildVersion > 737790)
		{
			arg = header.buildVersion.ToString();
			arg2 = 737790u.ToString();
		}
