...
public class VitalsTableScreen : TableScreen
{
	protected override void OnActivate()
	{
		...
		AddLabelColumn("QOLExpectations", on_load_qualityoflife_expectations, get_value_qualityoflife_expectations_label, compare_rows_qualityoflife_expectations, on_tooltip_qualityoflife_expectations, on_tooltip_sort_qualityoflife_expectations, 64, should_refresh_columns: true);
		if (Game.IsDlcActiveForCurrentSave("DLC3_ID"))
		{
			AddLabelColumn("PowerBanks", on_load_power_banks, get_value_power_banks_label, compare_rows_power_banks, on_tooltip_power_banks, on_tooltip_sort_power_banks, 64, should_refresh_columns: true);
		}
	...
	private void on_load_health(IAssignableIdentity minion, GameObject widget_go)
	{
		...
		{
			string text = (componentInChildren.text = UI.VITALSSCREEN_HEALTH.ToString());
			text3 = text;
		}
		else
		{
	...
	private string get_value_sickness_label(IAssignableIdentity minion, GameObject widget_go)
	{
		...
		if (widgetRow.rowType == TableRow.RowType.Minion)
		{
			...
			if (minionIdentity != null)
			{
				...
				foreach (SicknessInstance modifier in minionIdentity.GetComponent<MinionModifiers>().sicknesses.ModifierList)
				{
					list.Add(new KeyValuePair<string, float>(modifier.modifier.Name, modifier.GetInfectedTimeRemaining()));
				}
				if (DlcManager.FeatureRadiationEnabled())
				{
					...
					if (sMI != null && sMI.sm.isSick.Get(sMI))
					{
						...
						string text = "";
						text = ((!component.HasEffect(RadiationMonitor.minorSicknessEffect) && !component.HasEffect(RadiationMonitor.bionic_minorSicknessEffect)) ? ((!component.HasEffect(RadiationMonitor.majorSicknessEffect) && !component.HasEffect(RadiationMonitor.bionic_majorSicknessEffect)) ? ((!component.HasEffect(RadiationMonitor.extremeSicknessEffect) && !component.HasEffect(RadiationMonitor.bionic_extremeSicknessEffect)) ? ((string)DUPLICANTS.MODIFIERS.RADIATIONEXPOSUREDEADLY.NAME) : Db.Get().effects.Get(RadiationMonitor.extremeSicknessEffect).Name) : Db.Get().effects.Get(RadiationMonitor.majorSicknessEffect).Name) : Db.Get().effects.Get(RadiationMonitor.minorSicknessEffect).Name);
		...
		case TableRow.RowType.Minion:
		{
			...
			if (sicknesses.IsInfected())
			{
				...
				foreach (SicknessInstance modifier in sicknesses.ModifierList)
				{
					...
					tooltip.AddMultiStringTooltip(modifier.modifier.Name, null);
					StatusItem statusItem = modifier.GetStatusItem();
					tooltip.AddMultiStringTooltip(statusItem.GetTooltip(modifier.ExposureInfo), null);
				}
			}
			if (!flag)
			{
		...
		case TableRow.RowType.Minion:
		{
			...
			if (minionIdentity != null)
			{
				...
				if (amountInstance != null)
				{
					...
					tooltip.AddMultiStringTooltip("\n" + string.Format(UI.VITALSSCREEN.EATEN_TODAY_TOOLTIP, GameUtil.GetFormattedCalories(RationsEatenToday(minionIdentity))), null);
				}
			}
			break;
		}
	...
	private string get_value_power_banks_label(IAssignableIdentity minion, GameObject widget_go)
	{
		...
		if (widgetRow.rowType == TableRow.RowType.Minion)
		{
			...
			{
				return GameUtil.GetFormattedJoules(minionIdentity.GetAmounts().Get(Db.Get().Amounts.BionicInternalBattery).value);
			}
			return UI.TABLESCREENS.NA;
		}
		if (widgetRow.rowType == TableRow.RowType.StoredMinon)
	...
	private int compare_rows_power_banks(IAssignableIdentity a, IAssignableIdentity b)
	{
		...
		float num = 0f;
		float num2 = 0f;
		num = ((!(minionIdentity != null) || !minionIdentity.HasTag(GameTags.Minions.Models.Bionic)) ? (-1f) : minionIdentity.GetAmounts().Get(Db.Get().Amounts.BionicInternalBattery).value);
		return ((!(minionIdentity2 != null) || !minionIdentity2.HasTag(GameTags.Minions.Models.Bionic)) ? (-1f) : minionIdentity2.GetAmounts().Get(Db.Get().Amounts.BionicInternalBattery).value).CompareTo(num);
	}
