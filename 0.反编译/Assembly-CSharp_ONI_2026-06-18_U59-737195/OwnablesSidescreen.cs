...
public class OwnablesSidescreen : SideScreenContent
{
	...
	{
		((OwnablesSecondSideScreen)DetailsScreen.Instance.SetSecondarySideScreen(selectedSlotScreenPrefab, slot.slot.Name)).SetSlot(slot);
	...
	public override void SetTarget(GameObject target)
	{
		...
		{
			Assignables owner = categories[i].getAssignablesFn(component);
	...
	public void RefreshSelectedStatusOnRows()
	{
		if (categoryRows != null)
		{
			for (int i = 0; i < categoryRows.Length; i++)
			{
				categoryRows[i].SetSelectedRow_VisualsOnly(lastSelectedSlot);
			}
		}
	}
