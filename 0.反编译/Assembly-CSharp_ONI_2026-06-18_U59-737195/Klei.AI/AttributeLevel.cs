...
public class AttributeLevel
{
	...
	public void Apply(AttributeLevels levels)
	{
		...
		if (modifier != null)
		{
			...
			modifier.Reconstruct(attribute.Id, GetLevel(), DUPLICANTS.MODIFIERS.SKILLLEVEL.NAME);
		}
		else
		{
	...
	{
		float num = Mathf.Pow((float)level / (float)maxGainedLevel, DUPLICANTSTATS.ATTRIBUTE_LEVELING.EXPERIENCE_LEVEL_POWER) * (float)DUPLICANTSTATS.ATTRIBUTE_LEVELING.TARGET_MAX_LEVEL_CYCLE * 600f;
		return Mathf.Pow(((float)level + 1f) / (float)maxGainedLevel, DUPLICANTSTATS.ATTRIBUTE_LEVELING.EXPERIENCE_LEVEL_POWER) * (float)DUPLICANTSTATS.ATTRIBUTE_LEVELING.TARGET_MAX_LEVEL_CYCLE * 600f - num;
	}
	...
	public void LevelUp(AttributeLevels levels)
	{
		...
		instance.StartSM();
		levels.Trigger(-110704193, (object)attribute.Id);
	}

	public bool AddExperience(AttributeLevels levels, float experience)
	{
		if (level >= maxGainedLevel)
		{
			return false;
		}
		this.experience += experience;
