...
public class DropDown : KMonoBehaviour
{
	...
	private void RefreshEntries()
	{
		...
		if (sortFunction != null)
		{
			...
			{
				rowLookup[entries[num]].transform.SetAsFirstSibling();
			}
			if (emptyRow != null)
			{
	...
	public void Open()
	{
		...
		float num = Mathf.Max(32f, rowEntryPrefab.GetComponent<LayoutElement>().preferredHeight);
		scrollRect.rectTransform().sizeDelta = new Vector2(scrollRect.rectTransform().sizeDelta.x, num * (float)Mathf.Min(contentContainer.childCount, 8));
