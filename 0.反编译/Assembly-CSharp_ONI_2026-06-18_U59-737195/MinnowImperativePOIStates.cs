...
public class MinnowImperativePOIStates : GameStateMachine<MinnowImperativePOIStates, MinnowImperativePOIStates.Instance, IStateMachineTarget, MinnowImperativePOIStates.Def>
{
	...
	public new class Instance : GameInstance, ISidescreenButtonControl
	{
		...
		public string SidescreenTitle => BUILDINGS.PREFABS.MINNOW_IMPERATIVE_POI_A.UI_HEADER;
		...
		public bool WasCompletedAndAcknowledged
		{
			get
			{
				if (base.smi.sm.isCompleted.Get(base.smi))
				{
					return base.smi.sm.hasShownCompletedPopup.Get(base.smi);
				}
				return false;
			}
		}
		...
		private string GetStartPopupDescription(MinnowPOIIdentity identity)
		{
			...
			case MinnowPOIIdentity.POI_B:
				...
				result = result.Replace("{AMOUNT1}", GameUtil.GetFormattedCaloriesForItem(MinnowImperativePOIBConfig.RequiredDeliveryTag, 10f)).Replace("{AMOUNT2}", "1");
				break;
			case MinnowPOIIdentity.POI_C:
		...
		private IEnumerator CompletionCameraSequence(bool wasLastPOI)
		{
			...
			CameraController.Instance.SetTargetPos(position, 8f, playSound: false);
		...
		{
			MinionStartingStats minionStartingStats = new MinionStartingStats(Db.Get().Personalities.Get("MINNOW"));
			...
			TransformExtensions.SetLocalPosition(position: Grid.CellToPosCBC(Grid.PosToCell(base.gameObject), Grid.SceneLayer.Move), transform: gameObject.transform);
			gameObject.SetActive(value: true);
			minionStartingStats.Apply(gameObject);
			gameObject.GetComponent<MinionIdentity>().arrivalTime = -1 * Random.Range(2050, 2180);
			gameObject.GetMyWorld().SetDupeVisited();
		}

		private void ShowQuestPopup()
		{
			EventInfoScreen.ShowPopup(EventInfoDataHelper.GenerateStoryTraitData(GetStartPopupTitle(base.smi.def.minnowPOIIdentity), GetStartPopupDescription(base.smi.def.minnowPOIIdentity), UI.TOOLTIPS.CLOSETOOLTIP, GetStartPopupImage(base.smi.def.minnowPOIIdentity), EventInfoDataHelper.PopupType.BEGIN, null, null, delegate
			{
		...
		{
			if (IsInPopupEligibleState())
			{
				return base.smi.sm.hasShownQuestPopup.Get(base.smi);
			}
			return false;
		}
	...
	public override void InitializeStates(out BaseState default_state)
	{
		...
		default_state = off;
		off.PlayAnim("empty_water").ParamTransition(hasShownCompletedPopup, off_poi_completed, GameStateMachine<MinnowImperativePOIStates, Instance, IStateMachineTarget, Def>.IsTrue).ParamTransition(isCompleted, poi_completed_pending, GameStateMachine<MinnowImperativePOIStates, Instance, IStateMachineTarget, Def>.IsTrue)
			...
			.ToggleStatusItem(Db.Get().MiscStatusItems.MinnowPOIDehydratedStatus);
		...
		poi_completed_acknowledged.PlayAnim((Instance smi) => (!Instance.AllPOIsCompleted()) ? "exit" : "victory").Enter(delegate(Instance smi)
		{
			smi.ClearUserPriority();
		...
		off_poi_completed.PlayAnim("off").Toggle("Toggle selectable", MakeItUnselectable, MakeItSelectable).Enter(delegate(Instance smi)
		{
			smi.ClearUserPriority();
		});
	}
	...
	private static bool IsVisibleOnCamera(Instance smi)
	{
		...
		{
			return CameraController.Instance.IsVisiblePos(smi.transform.GetPosition());
		}
		return false;
	}
