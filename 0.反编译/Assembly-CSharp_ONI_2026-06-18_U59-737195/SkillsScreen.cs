...
public class SkillsScreen : KModalScreen
{
	...
	private void PopulateBoosters()
	{
		foreach (GameObject item in Assets.GetPrefabsWithTag(GameTags.BionicUpgrade))
		{
			...
			GameObject obj = Util.KInstantiate(boosterPrefab, boosterContentGrid, item.name);
			...
			obj.GetComponentInChildren<LocText>().SetText(item.GetProperName());
			...
			{
				GameObject gameObject = Util.KInstantiateUI(boosterSlotIconPrefab, boosterSlotIconPrefab.transform.parent.gameObject);
				boosterSlotIcons.Add(gameObject);
				int slotIdx = num;
				gameObject.transform.GetChild(0).GetComponent<MultiToggle>().onClick = delegate
				{
					GetMinionIdentity(currentlySelectedMinion, out var minionIdentity, out var _);
					if (!(minionIdentity == null))
					{
	...
	private void RefreshBoosters()
	{
		...
		if (flag)
		{
			...
			{
				_ = instance.upgradeComponentSlots[7 - i];
			...
			}
			{
				foreach (KeyValuePair<Tag, HierarchyReferences> widget in boosterWidgets)
				{
					...
					if (instance != null && instance.upgradeComponentSlots != null)
					{
						...
						{
							if (upgradeComponentSlot.assignedUpgradeComponent != null && upgradeComponentSlot.assignedUpgradeComponent.PrefabID() == widget.Key)
							{
								num3++;
							}
						}
