...
public class SaveLoader : KMonoBehaviour
{
	...
	public static SaveGame.GameInfo LoadHeader(string filename, out SaveGame.Header header)
	{
		...
		return SaveGame.GetHeader(new FastReader(array2), out header, filename);
	}
	...
	public bool LoadFromWorldGen()
	{
		...
		SimMessages.DefineWorldOffsets(m_cluster.worlds.Select((WorldGen world) => new SimMessages.WorldOffsetData
		{
	...
	private List<MinionMetricsData> GetMinionMetrics()
	{
		...
		foreach (MinionIdentity item in Components.MinionIdentities.Items)
		{
			...
			int boosterSlots = 0;
			BionicUpgradesMonitor.Instance sMI = item.gameObject.GetSMI<BionicUpgradesMonitor.Instance>();
			if (sMI != null && sMI.upgradeComponentSlots != null)
			{
				...
				boosterSlots = sMI.UnlockedSlotCount;
			}
	...
	{
		if (IsAllDlcActiveForCurrentSave(required))
		{
			return !IsAnyDlcActiveForCurrentSave(forbidden);
		}
		return false;
	}
