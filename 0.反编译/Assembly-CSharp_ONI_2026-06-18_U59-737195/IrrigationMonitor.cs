...
public class IrrigationMonitor : GameStateMachine<IrrigationMonitor, IrrigationMonitor.Instance, IStateMachineTarget, IrrigationMonitor.Def>
{
	public class Def : BaseDef, IGameObjectEffectDescriptor
	{
		...
		public List<Descriptor> GetDescriptors(GameObject obj)
		{
			if (consumedElements.Length != 0)
			{
				...
				if (consumedElements.Length > 1)
				{
					string[] array = new string[consumedElements.Length];
					for (int i = 0; i < consumedElements.Length; i++)
					{
						array[i] = consumedElements[i].tag.ProperName();
					}
					string arg = string.Join(UI.GAMEOBJECTEFFECTS.REQUIREMETS_OR, array);
					float num = consumedElements[0].massConsumptionRate * preModifiedAttributeValue;
					list.Add(new Descriptor(string.Format(UI.GAMEOBJECTEFFECTS.IDEAL_FERTILIZER, arg, GameUtil.GetFormattedMass(0f - num, GameUtil.TimeSlice.PerCycle)), string.Format(UI.GAMEOBJECTEFFECTS.TOOLTIPS.IDEAL_FERTILIZER, arg, GameUtil.GetFormattedMass(num, GameUtil.TimeSlice.PerCycle)), Descriptor.DescriptorType.Requirement));
				}
				else
				{
					PlantElementAbsorber.ConsumeInfo[] array2 = consumedElements;
					for (int j = 0; j < array2.Length; j++)
					{
						PlantElementAbsorber.ConsumeInfo consumeInfo = array2[j];
						float num2 = consumeInfo.massConsumptionRate * preModifiedAttributeValue;
						list.Add(new Descriptor(string.Format(UI.GAMEOBJECTEFFECTS.IDEAL_FERTILIZER, consumeInfo.tag.ProperName(), GameUtil.GetFormattedMass(0f - num2, GameUtil.TimeSlice.PerCycle)), string.Format(UI.GAMEOBJECTEFFECTS.TOOLTIPS.IDEAL_FERTILIZER, consumeInfo.tag.ProperName(), GameUtil.GetFormattedMass(num2, GameUtil.TimeSlice.PerCycle)), Descriptor.DescriptorType.Requirement));
					}
				}
				return list;
			}
	...
	public new class Instance : GameInstance, IWiltCause
	{
		...
		{
			if (storage == null || consumed_infos == null)
			{
				return;
			}
			for (int num = storage.items.Count - 1; num >= 0; num--)
			{
				...
				if (gameObject == null || !gameObject.TryGetComponent<PrimaryElement>(out var component) || !gameObject.TryGetComponent<ElementChunk>(out var _))
				{
					continue;
				}
				if (validate_solids)
				...
				for (int i = 0; i < consumed_infos.Length; i++)
				{
					...
					if (component3.HasTag(consumeInfo.tag))
					{
						flag = true;
						break;
					}
		...
		public void UpdateIrrigation(float dt)
		{
			...
			if (storage != null)
			{
				...
				for (int i = 0; i < base.def.consumedElements.Length; i++)
				{
					...
					{
						flag = true;
						flag2 = num >= consumeInfo.massConsumptionRate * totalValue * (dt * 30f);
						break;
					}
				}
				for (int k = 0; k < items.Count; k++)
				{
					GameObject go = items[k];
					if (!go.HasTag(base.def.wrongIrrigationTestTag))
					{
						continue;
					}
					bool flag3 = false;
					...
					{
						if (go.HasTag(base.def.consumedElements[l].tag))
						{
							flag3 = true;
							break;
						}
