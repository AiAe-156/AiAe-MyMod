...
public class ArtifactSelector : KMonoBehaviour
{
	...
	public string GetUniqueArtifactID(ArtifactType artifactType = ArtifactType.Any)
	{
		...
		{
			if (!placedArtifacts[artifactType].Contains(item) && Game.IsCorrectDlcActiveForCurrentSave(Assets.GetPrefab(item.ToTag()).GetComponent<KPrefabID>()))
			{
				list.Add(item);
			}
		}
		...
		if (list.Count == 0 && artifactType != ArtifactType.Any)
		{
			...
			{
				if (!placedArtifacts[ArtifactType.Any].Contains(item2) && Game.IsCorrectDlcActiveForCurrentSave(Assets.GetPrefab(item2.ToTag()).GetComponent<KPrefabID>()))
				{
					list.Add(item2);
