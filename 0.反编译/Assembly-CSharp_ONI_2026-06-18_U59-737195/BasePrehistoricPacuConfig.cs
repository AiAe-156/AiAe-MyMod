...
public static class BasePrehistoricPacuConfig
{
	...
	public static Action<object, object> OnCaloriesConsumed = delegate(object context, object data)
	{
		...
		{
			((Effects)context).Add("AteWellPreparedJawboFood", should_save: true);
		}
	};
	...
	public static GameObject CreatePrefab(string id, string base_trait_id, string name, string description, string anim_file, bool is_baby, string symbol_prefix, float warnLowTemp, float warnHighTemp, float lethalLowTemp, float lethalHighTemp)
	{
		...
		{
			new Diet.Info(hashSet, PrehistoricPacuTuning.POOP_ELEMENT, CALORIES_PER_KG_OF_PACU, 60f / PacuTuning.MASS, null, 0f, produce_solid_tile: false, Diet.Info.FoodType.EatPrey),
			new Diet.Info(hashSet2, PrehistoricPacuTuning.POOP_ELEMENT, CALORIES_PER_KG_OF_PACU_MEAT, 60f),
			new Diet.Info(hashSet3, PrehistoricPacuTuning.POOP_ELEMENT, CALORIES_PER_KG_OF_PACU_MEAT, 60f)
		}.ToArray());
		CreatureCalorieMonitor.Def def2 = gameObject.AddOrGetDef<CreatureCalorieMonitor.Def>();
		def2.diet = diet;
		def2.minConsumedCaloriesBeforePooping = CALORIES_PER_KG_OF_PACU * 60f;
		...
		component.prefabSpawnFn += SubscribeCookedSeafoodEffect;
		return gameObject;
	}
	...
	public static void SubscribeCookedSeafoodEffect(GameObject jawboGameObject)
	{
		...
		jawboGameObject.Subscribe(-2038961714, OnCaloriesConsumed, component);
	}
