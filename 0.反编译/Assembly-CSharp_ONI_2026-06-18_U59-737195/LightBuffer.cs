...
public class LightBuffer : MonoBehaviour
{
	...
	private void LateUpdate()
	{
		...
		GridArea visibleAreaExtended = GridVisibleArea.GetVisibleAreaExtended((int)largestLightRange + 4);
		...
		{
			if (item == null || !visibleAreaExtended.Contains(item.cachedCell))
			{
				continue;
			}
			Vector3 position = item.transform.GetPosition();
			int key = Grid.PosToCell(position);
			pooledDictionary.TryGetValue(key, out var value);
			if (value >= maxLights)
			{
				continue;
			}
			pooledDictionary[key] = value + 1;
			...
			largestLightRange = Mathf.Max(largestLightRange, item.Range);
		}
