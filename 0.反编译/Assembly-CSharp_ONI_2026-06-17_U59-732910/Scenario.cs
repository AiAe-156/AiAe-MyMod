...
public class Scenario : KMonoBehaviour
{
	...
	public class Builder
	{
		...
		public void Placer(string prefab_id, Element element)
		{
			...
			Scenario.RunAfterNextUpdate(delegate
			{
				...
				buildingDef2.TryPlace(null, pos, Orientation.Neutral, buildingDef2.DefaultElementsWithPrimary(element.tag));
			});
		}
	...
	public static GameObject PlaceBuilding(int root_cell, int x, int y, string prefab_id, SimHashes element = SimHashes.Cuprite)
	{
		...
		GameObject obj = buildingDef.Build(buildingDef.GetBuildingCell(cell), Orientation.Neutral, null, buildingDef.DefaultElementsWithPrimary(element2.tag), 293.15f, playsound: false);
	...
	public void SetupElementTest()
	{
		...
		int bot = builder.Bot;
		foreach (Element item in (from element3 in ElementLoader.elements
			where element3.IsSolid
			orderby element3.highTempTransitionTarget
			select element3).ToList())
		{
			if (!item.IsSolid)
			{
				continue;
		...
		foreach (Element item2 in (from element3 in ElementLoader.elements
			where element3.IsLiquid && !elements.Contains(element3)
			orderby element3.highTempTransitionTarget
			select element3).ToList())
		{
			...
			do
			{
				...
				flag2 = element2.HasTransitionUp;
				if (flag2)
				{
		...
		foreach (Element item3 in ElementLoader.elements.Where((Element element3) => element3.state == Element.State.Gas && !elements.Contains(element3)).ToList())
		{
