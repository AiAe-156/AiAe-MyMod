...
public class CropTendingStates : GameStateMachine<CropTendingStates, CropTendingStates.Instance, IStateMachineTarget, CropTendingStates.Def>
{
	...
	public override void InitializeStates(out BaseState default_state)
	{
		...
		state.ToggleStatusItem(text, tooltip, "", StatusItem.IconType.Info, NotificationType.Neutral, allow_multiples: false, default(HashedString), 129022, null, null, main).MoveTo((Instance smi) => smi.moveCell, tendCrop, behaviourcomplete).ParamTransition(targetCrop, behaviourcomplete, (Instance smi, GameObject p) => targetCrop.Get(smi) == null);
		State state2 = tendCrop.DefaultState(tendCrop.pre);
		string text2 = CREATURES.STATUSITEMS.DIVERGENT_TENDING.NAME;
		string tooltip2 = CREATURES.STATUSITEMS.DIVERGENT_TENDING.TOOLTIP;
		main = Db.Get().StatusItemCategories.Main;
		state2.ToggleStatusItem(text2, tooltip2, "", StatusItem.IconType.Info, NotificationType.Neutral, allow_multiples: false, default(HashedString), 129022, null, null, main).ParamTransition(targetCrop, behaviourcomplete, (Instance smi, GameObject p) => targetCrop.Get(smi) == null).Enter(delegate(Instance smi)
		{
	...
	private void FindCrop(Instance smi)
	{
		...
		{
			if (Vector2.SqrMagnitude((Vector2)(worldItem.transform.position - smi.transform.position)) > 625f)
			{
				continue;
			}
			if (smi.effect != null)
