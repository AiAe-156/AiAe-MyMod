...
public class LogicBitSelectorSideScreen : SideScreenContent, IRenderEveryTick
{
	...
	{
		for (int i = 0; i < target.GetBitDepth(); i++)
		{
			int n = i;
			if (!toggles_by_int.ContainsKey(i))
			{
				GameObject obj = Util.KInstantiateUI(rowPrefab, rowPrefab.transform.parent.gameObject, force_active: true);
				obj.GetComponent<HierarchyReferences>().GetReference<LocText>("bitName").SetText(string.Format(UI.UISIDESCREENS.LOGICBITSELECTORSIDESCREEN.BIT, i + 1));
				obj.GetComponent<HierarchyReferences>().GetReference<KImage>("stateIcon").color = (target.IsBitActive(i) ? activeColor : inactiveColor);
				obj.GetComponent<HierarchyReferences>().GetReference<LocText>("stateText").SetText(target.IsBitActive(i) ? UI.UISIDESCREENS.LOGICBITSELECTORSIDESCREEN.STATE_ACTIVE : UI.UISIDESCREENS.LOGICBITSELECTORSIDESCREEN.STATE_INACTIVE);
				MultiToggle component = obj.GetComponent<MultiToggle>();
				toggles_by_int.Add(i, component);
			}
			toggles_by_int[i].onClick = delegate
			{
				SelectToggle(n);
			};
		}
