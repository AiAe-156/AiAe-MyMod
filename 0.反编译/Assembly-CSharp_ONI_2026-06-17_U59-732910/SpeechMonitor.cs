...
public class SpeechMonitor : GameStateMachine<SpeechMonitor, SpeechMonitor.Instance, IStateMachineTarget, SpeechMonitor.Def>
{
	...
	public new class Instance : GameInstance
	{
		...
		public void DrawMouth()
		{
			...
			bool num = firstFrameElement.symbol != HashedString.Invalid;
			DebugUtil.DevAssert(num, "Mouth frame element invalid");
			if (num)
			{
				...
				KAnim.Build.Symbol symbol2 = KAnimBatchManager.Instance().GetBatchGroupData(AnimController.batchGroupID).GetSymbol(HASH_SNAPTO_MOUTH);
				DebugUtil.DevAssert(build == symbol.build, "Mouth build mismatch");
				KAnim.Build.SymbolFrameInstance symbol_frame_instance = KAnimBatchManager.Instance().GetBatchGroupData(build.batchTag).symbolFrameInstances[symbol.firstFrameIdx + firstFrameElement.frame];
	...
	public static bool IsAllowedToPlaySpeech(KPrefabID prefabID, KBatchedAnimController controller)
	{
		...
		{
			return CanOverrideHead(controller);
		}
		return false;
	}
	...
	private static KAnim.Anim.FrameElement GetFirstFrameElement(KBatchedAnimController controller)
	{
		...
		for (int i = 0; i < frame.numElements; i++)
		{
			...
			bool num2 = num < frameElements.Count;
			DebugUtil.DevAssert(num2, "Frame element index out of range");
			if (num2)
			{
