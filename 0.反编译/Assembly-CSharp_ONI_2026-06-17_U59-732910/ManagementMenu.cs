...
public class ManagementMenu : KIconToggleMenu
{
	...
	public bool HasActiveResearch => researchScreen.HasActiveResearch;
	...
	protected override void OnPrefabInit()
	{
		...
		AddToggleTooltipForResearch(researchInfo, UI.TOOLTIPS.MANAGEMENTMENU_REQUIRES_RESEARCH);
	...
	private ToolTip.ComplexTooltipDelegate CreateToggleTooltip(ManagementMenuToggleInfo toggleInfo, string disabledTooltip = null)
	{
		return delegate
		{
	...
	{
		toggleInfo.getTooltipText = CreateToggleTooltip(toggleInfo, disabledTooltip);
	}

	private void AddToggleTooltipForResearch(ManagementMenuToggleInfo toggleInfo, string disabledTooltip = null)
	{
		toggleInfo.getTooltipText = delegate
		{
			List<Tuple<string, TextStyleSetting>> list = new List<Tuple<string, TextStyleSetting>>();
			TechInstance activeResearch = Research.Instance.GetActiveResearch();
			string a = ((activeResearch == null) ? ((string)UI.TOOLTIPS.MANAGEMENTMENU_RESEARCH_NO_RESEARCH) : string.Format(UI.TOOLTIPS.MANAGEMENTMENU_RESEARCH_CARD_NAME, activeResearch.tech.Name));
			string text = null;
			list.Add(new Tuple<string, TextStyleSetting>(a, ToolTipScreen.Instance.defaultTooltipHeaderStyle));
			if (activeResearch != null)
			{
				...
				list.Add(new Tuple<string, TextStyleSetting>(text, ToolTipScreen.Instance.defaultTooltipBodyStyle));
			}
			if (disabledTooltip != null && !toggleInfo.toggle.interactable)
			{
				list.Add(new Tuple<string, TextStyleSetting>(disabledTooltip, ToolTipScreen.Instance.defaultTooltipBodyStyle));
	...
	public bool CheckHasResearchCenter()
	{
		...
		{
			return Game.Instance.SandboxModeActive;
		}
		return true;
	}
	...
	public void CheckResearch(object o)
	{
		...
		{
			bool flag = !CheckHasResearchCenter();
	...
	{
		if (Components.Telescopes.Count <= 0 && !DebugHandler.InstantBuildMode)
		{
			return Game.Instance.SandboxModeActive;
		}
		return true;
	...
	public void OpenResearch(string zoomToTech = null)
	{
		...
		if (zoomToTech != null)
		{
			UIScheduler.Instance.Schedule("ResearchCameraFocus", 0.25f, delegate
			{
				researchScreen.ZoomToTech(zoomToTech, highlight: true);
			});
		}
	}
