...
public class TraitUtil
{
	...
	public static System.Action CreateSkillGrantingTrait(string id, string name, string desc, string[] skillIds)
	{
		return delegate
		{
			Trait trait = Db.Get().CreateTrait(id, name, desc, null, should_save: true, null, positive_trait: true, is_valid_starter_trait: true);
			trait.TooltipCB = delegate
			{
				...
				foreach (string id2 in skillIds)
				{
					text += "\n\n";
					text = text + GameUtil.ApplyBoldString(Db.Get().Skills.Get(id2).Name) + ":\n";
					text += SkillWidget.SkillPerksString(Db.Get().Skills.Get(id2));
				}
				return text;
			};
			trait.OnAddTrait = delegate(GameObject go)
			{
				...
				if (component != null)
				{
					...
					foreach (string skillId in array)
					{
						component.GrantSkill(skillId);
					}
				}
