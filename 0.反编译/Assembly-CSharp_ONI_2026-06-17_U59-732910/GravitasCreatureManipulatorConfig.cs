...
public class GravitasCreatureManipulatorConfig : IBuildingConfig
{
	...
	{
		if (!Strings.TryGet("STRINGS.CREATURES.FAMILY_PLURAL." + species.ToString().ToUpper(), out var result))
		{
			return Option.None;
		}
	...
	{
		if (!Strings.TryGet("STRINGS.CODEX.STORY_TRAITS.CRITTER_MANIPULATOR.SPECIES_ENTRIES." + species.ToString().ToUpper().Replace("SPECIES", ""), out var result))
		{
			return Option.None;
		}
		return Option.Some((string)result);
