...
public class Building : KMonoBehaviour, IGameObjectEffectDescriptor, IUniformGridObject, IApproachable
{
	...
	public string DescFlavour
	{
		get
		{
			if (descriptionFlavour != null)
			{
				return descriptionFlavour;
			}
			return Def.Effect;
		}
	}
	...
	{
		RegisterBlockTileRenderer(isBlueprint: false);
	}
	...
	protected void RegisterBlockTileRenderer(bool isBlueprint)
	{
		if (Def.BlockTileAtlas != null)
		{
			...
			if (component != null)
			{
				...
				World.Instance.blockTileRenderer.AddBlock(base.gameObject.layer, Def, isReplacement, visualizationElementID, cell, isBlueprint);
			}
		}
	}
	...
	public List<Descriptor> RequirementDescriptors(BuildingDef def)
	{
		...
		{
			if (!TryGetComponent<IEnergyConsumer>(out var component2))
			{
				component.TryGetComponent<IEnergyConsumer>(out component2);
			}
			float num = component2?.WattsNeededWhenActive ?? 0f;
			if (num > 0f)
			{
				string formattedWattage = GameUtil.GetFormattedWattage(num);
		...
		{
			Constructable component3 = def.BuildingUnderConstruction.GetComponent<Constructable>();
			if (component3 != null && component3.requiredSkillPerk != HashedString.Invalid)
			{
				StringBuilder stringBuilder = new StringBuilder();
				List<Skill> skillsWithPerk = Db.Get().Skills.GetSkillsWithPerk(component3.requiredSkillPerk);
				...
				list.Add(new Descriptor(UI.BUILD_REQUIRES_SKILL.Replace("{Skill}", replacement2), UI.BUILD_REQUIRES_SKILL_TOOLTIP.Replace("{Skill}", replacement2), Descriptor.DescriptorType.Requirement));
			}
		}
		return list;
	}
