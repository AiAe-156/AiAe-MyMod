...
public class NavGridUpdater
{
	...
	{
		foreach (int dirty_cell in dirty_cells)
		{
			for (int i = 0; i < validators.Length; i++)
			{
				validators[i].UpdateCell(dirty_cell, nav_table, bounding_offsets);
			}
		}
	}
	...
	private static void CreateLinks(int cell, NavTable nav_table, int max_links_per_cell, NavGrid.Link[] links, NavGrid.Transition[][] transitions_by_nav_type, Dictionary<int, int> teleport_transitions)
	{
		...
		for (int i = 0; i < 11; i++)
		{
			...
			for (int j = 0; j < array2.Length; j++)
			{
				...
				NavGrid.Transition transition = (transition2 = array2[j]);
				...
				int num3 = transition2.IsValid(cell, nav_table);
				if (num3 != Grid.InvalidCell)
				{
					links[num] = new NavGrid.Link(num3, transition2.start, transition2.end, transition2.id, transition2.cost);
					num++;
					num2++;
				}
			}
		...
		{
			Debug.LogError($"Out of nav links on grid '{nav_table.NavGridId}' at cell {cell}. Needed {num2} links but maxLinksPerCell is {max_links_per_cell}. Increase maxLinksPerCell to at least {num2 + 1}.");
		}
