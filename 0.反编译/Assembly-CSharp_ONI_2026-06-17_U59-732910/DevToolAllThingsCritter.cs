	...
	private void OccupyAreaContents(GameObject go)
	{
		...
		if (component != null && ImGui.CollapsingHeader("Occupy Area", ImGuiTreeNodeFlags.DefaultOpen))
		{
			...
			if (drawOccupyArea)
			{
				...
				DevToolEntity.DrawScreenRect((cornerA: screenPosition2, cornerB: screenPosition), go.name, Color.cyan, new Color(0f, 1f, 1f, 0.33f));
			}
	...
	private void ColliderContents(GameObject go)
	{
		...
		if (component != null && ImGui.CollapsingHeader("Collider", ImGuiTreeNodeFlags.DefaultOpen))
		{
			...
			if (drawCollider)
			{
				...
				DevToolEntity.DrawScreenRect((cornerA: screenPosition, cornerB: screenPosition2), go.name, Color.green, new Color(0f, 1f, 0f, 0.33f));
			}
