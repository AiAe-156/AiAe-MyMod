...
public class JetSuitLocker : StateMachineComponent<JetSuitLocker.StatesInstance>, ISecondaryInput
{
	public class States : GameStateMachine<States, StatesInstance, JetSuitLocker>
	{
		...
		public override void InitializeStates(out BaseState default_state)
		{
			...
			charged.Transition(charging, (StatesInstance smi) => !smi.master.IsSuitFullyCharged()).EventTransition(GameHashes.OnStorageChange, empty, (StatesInstance smi) => smi.master.GetStoredOutfit() == null);
		}
	}
	...
	public float FuelAvailable => Math.Min((0f + storage.GetMassAvailable(fuel_tag)) / 100f, 1f);

	protected override void OnSpawn()
	{
		base.OnSpawn();
		fuel_tag = GameTags.CombustibleLiquid;
		...
		fuel_consumer.capacityTag = GameTags.CombustibleLiquid;
	...
	private void FuelSuit(float dt)
	{
		...
		if (!(storedOutfit == null) && HasFuel())
		{
			JetSuitTank component = storedOutfit.GetComponent<JetSuitTank>();
			float a = dt * 10f;
			a = Mathf.Min(a, 100f - component.amount);
			while (a > 0f && HasFuel())
			{
				float massAvailable = storage.GetMassAvailable(fuel_tag);
				massAvailable = Mathf.Min(massAvailable, a);
				component.amount += massAvailable;
				a -= massAvailable;
				SimHashes mostRelevantItemElement = SimHashes.Petroleum;
				storage.ConsumeAndGetDisease(fuel_tag, massAvailable, out var _, out var _, out var _, out mostRelevantItemElement);
	...
	public bool HasFuel()
	{
		if (storage.Has(fuel_tag) && storage.GetMassAvailable(fuel_tag) > 0f)
		{
			return true;
		}
		return false;
