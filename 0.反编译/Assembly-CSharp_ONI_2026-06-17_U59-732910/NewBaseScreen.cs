...
public class NewBaseScreen : KScreen
{
	...
	private void SpawnMinions(GameObject start_pad)
	{
		...
		{
			MinionStartingStats obj = (MinionStartingStats)m_minionStartingStats[i];
			int x2 = x + i % (baseRight - baseLeft) + 1;
			int y2 = y;
			int cell = Grid.XYToCell(x2, y2);
			GameObject prefab = Assets.GetPrefab(BaseMinionConfig.GetMinionIDForModel(obj.personality.model));
			...
			bool extra_power_banks = obj.personality.model == GameTags.Minions.Models.Bionic;
			if (component != null)
			{
				component.AddNewBaseMinion(gameObject, extra_power_banks);
			}
		}
