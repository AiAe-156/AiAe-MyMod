...
public class Sublimates : KMonoBehaviour, ISim200ms
{
	...
	private bool SimMightOffcellOverpressure(int cell, SimHashes offgass)
	{
		...
		ReadOnlySpan<int> readOnlySpan = stackalloc int[3]
		{
			Grid.CellLeft(cell),
			Grid.CellRight(cell),
			Grid.CellAbove(cell)
		...
		for (int i = 0; i < readOnlySpan2.Length; i++)
		{
			...
			{
				result = true;
				if (Grid.Mass[num] < info.maxDestinationMass)
				{
					return false;
				}
			}
	...
	public void Sim200ms(float dt)
	{
		...
		if (num2 < info.maxDestinationMass)
		{
			...
			if (mass > 0f)
			{
				...
				if (num8 > 0f && !SimMightOffcellOverpressure(num, element.id))
				{
					...
					{
						storage.BoxingTrigger(-794517298, new BuildingHP.DamageSourceInfo
						{
							damage = 1,
			...
			else if (sublimatedMass > 0f)
			{
				...
				if (num9 > 0f && !SimMightOffcellOverpressure(num, element.id))
				{
	...
	private void UpdateStorage()
	{
		...
		{
			component.storage.Trigger(-1697596308, (object)base.gameObject);
		}
	}
