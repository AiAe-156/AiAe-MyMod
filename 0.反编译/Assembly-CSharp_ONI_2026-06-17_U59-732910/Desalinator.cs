...
public class Desalinator : StateMachineComponent<Desalinator.StatesInstance>
{
	public class StatesInstance : GameStateMachine<States, StatesInstance, Desalinator, object>.GameInstance
	{
		...
		public void CreateEmptyChore()
		{
			...
			base.smi.master.remoteChore.SetChore(emptyChore);
		}
	...
	private void UpdateConverterStatusVisibility()
	{
		...
		{
			bool flag = converters[i].CanConvertAtAll();
			if (converters[i].ShowInUI != flag)
			{
				converters[i].smi.RemoveStatusItems();
				converters[i].ShowInUI = flag;
				converters[i].smi.AddStatusItems();
			}
		}
	}
