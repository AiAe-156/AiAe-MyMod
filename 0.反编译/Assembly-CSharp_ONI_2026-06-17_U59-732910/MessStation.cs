...
public class MessStation : Workable, IDiningSeat
{
	public class MessStationSM : GameStateMachine<MessStationSM, MessStationSM.Instance, MessStation>
	{
		...
		public new class Instance : GameInstance
		{
			...
			public bool HasGarnish => Garnish.HasAny(garnishStorage);
			...
			public void UpdateGarnishOverride()
			{
				...
				{
					KAnim.Build.Symbol symbol = Garnish.GetActive(garnishStorage)?.GetOverrideSymbol();
					if (symbol != null)
					{
						symbolOverrideController.AddSymbolOverride(SALT_SYMBOL, symbol);
					}
					else
					{
						symbolOverrideController.RemoveSymbolOverride(SALT_SYMBOL);
					}
				}
			}
	...
	public static bool CanBeAssignedTo(IAssignableIdentity assignee)
	{
		...
		if (Db.Get().Amounts.Calories.Lookup(minionIdentity) != null)
		{
			return true;
		}
