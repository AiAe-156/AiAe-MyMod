...
public class RocketClusterDestinationSelector : ClusterDestinationSelector
{
	...
	protected override void OnClusterLocationChanged(object data)
	{
		...
		if (m_repeat)
		{
			if ((ClusterGrid.Instance.GetVisibleEntityOfLayerAtCell(clusterLocationChangedEvent.newLocation, EntityLayer.POI) != null && CanRocketDrill()) || CanCollectFromHexCellInventory())
			{
				WaitForPOIHarvest();
			}
			else
	...
	{
		bool flag = false;
		foreach (RocketModuleHexCellCollector.Instance allHexCellCollectorModule in GetComponent<Clustercraft>().GetAllHexCellCollectorModules())
		{
			flag = flag || (allHexCellCollectorModule != null && RocketModuleHexCellCollector.CanCollect(allHexCellCollectorModule));
		}
		return flag;
	}
