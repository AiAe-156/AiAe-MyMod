...
public class FallMonitor : GameStateMachine<FallMonitor, FallMonitor.Instance>
{
	...
	public new class Instance : GameInstance
	{
		...
		{
			if (shouldPlayEmotes && UnityEngine.Random.Range(0, 9) == 8)
			{
		...
		public bool CanRecoverToSwim()
		{
			...
			if (sMI != null)
			{
				if (sMI.CanSwim() && !IsIncapacitated())
				{
					return navigator.NavGrid.NavTable.IsValid(cell, NavType.Swim);
				}
				return false;
			}
			return false;
		...
		private bool IsIncapacitated()
		{
			...
			{
				return base.gameObject.HasTag(GameTags.SuffocatingIncapacitated);
			}
			return true;
		}
