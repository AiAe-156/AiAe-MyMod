...
public static class RoomConstraints
{
	public static class ConstraintTags
	{
		...
		public static Tag KitchenRefrigerator = AllTags.AddAndReturn("KitchenRefrigerator".ToTag());
		...
		public static Tag Submergible = AllTags.AddAndReturn("Submergible".ToTag());
	...
	public static Constraint ORNAMENTDISPLAYED = new Constraint(null, null, delegate(Room room)
	{
		...
		{
			if (CheckOrnament(room.buildings[i]))
			{
				return true;
			}
		}
		...
		{
			if (CheckOrnament(room.otherEntities[j]))
			{
				return true;
			}
		}
	...
	public static Constraint KITCHENREFRIGERATOR = new Constraint((KPrefabID bc) => bc.HasTag(ConstraintTags.KitchenRefrigerator), null, 1, ROOMS.CRITERIA.KITCHENREFRIGERATOR.NAME, ROOMS.CRITERIA.KITCHENREFRIGERATOR.DESCRIPTION);
