...
public class WorldGen
{
	...
	public bool GenerateLayout(OfflineCallbackFunction updateProgressFn)
	{
		try
		{
			...
			for (int i = 0; i < data.voronoiTree.ChildCount(); i++)
			{
				...
				TerrainCellLogged terrainCellLogged = new TerrainCellLogged(data.worldLayout.overworldGraph.FindNodeByID(tree.site.id), tree.site, tree.minDistanceToTag);
				new Vector2((int)Mathf.Floor(terrainCellLogged.poly.bounds.xMax - terrainCellLogged.poly.bounds.xMin), (int)Mathf.Floor(terrainCellLogged.poly.bounds.yMax - terrainCellLogged.poly.bounds.yMin));
	...
	public void IdentifyAllBiomes()
	{
		...
		for (int i = 0; i < data.terrainCells.Count; i++)
		{
			...
			Action<TerrainCell, List<TerrainCell>> action = delegate(TerrainCell cell, List<TerrainCell> biomeCells)
			{
				...
				{
					TerrainCell terrainCell2 = data.terrainCells[neighbourTerrainCell];
					SubWorld.ZoneType zoneType = Settings.GetSubWorld(terrainCell2.node.GetSubworld()).zoneType;
					if ((!biomeCells.Contains(terrainCell2) & (terrainCell2.biomeIdx == byte.MaxValue)) && zoneType == biomeType)
					{
			...
			{
				list[num2].biomeIdx = (byte)data.biomes.Count;
			}
	...
	{
		int num = pos.x + data.world.size.x * pos.y;
		return GetElementFromBiomeElementTable(chunk.backwallNoise[num], table);
	}
