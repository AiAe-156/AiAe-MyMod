...
public class GameSpawnData
{
	...
	public void AddTemplate(TemplateContainer template, Vector2I position, ref Dictionary<int, int> claimedCells)
	{
		...
		{
			foreach (Prefab backwallEntity in template.backwallEntities)
			{
				if (!claimedCells.ContainsKey(Grid.OffsetCell(cell, backwallEntity.location_x, backwallEntity.location_y)) && (flag || !IsWarpTeleporter(backwallEntity)))
				{
					backwallEntities.Add(backwallEntity.Clone(position));
				}
			}
		}
