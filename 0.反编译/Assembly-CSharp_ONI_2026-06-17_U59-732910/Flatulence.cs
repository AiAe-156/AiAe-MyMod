...
public class Flatulence : StateMachineComponent<Flatulence.StatesInstance>
{
	...
	private void Emit(object data)
	{
		...
		else
		{
			...
			int gameCell = Grid.PosToCell(position);
			if (Grid.IsVisiblyInLiquid(position))
			{
				BubbleManager.instance.SpawnBubble(SimHashes.Methane, position, 0.1f, value, BubbleManager.Disease.None);
			}
			else
			{
			...
			KBatchedAnimController kBatchedAnimController = FXHelpers.CreateEffect("odor_fx_kanim", position, gameObject.transform, update_looping_sounds_position: true);
			kBatchedAnimController.Play(WorkLoopAnims);
			kBatchedAnimController.destroyOnAnimComplete = true;
		}
		bool flag = SoundEvent.ObjectIsSelectedAndVisible(gameObject);
		Vector3 vector3 = gameObject.transform.GetPosition();
		vector3.z = 0f;
		float volume = 1f;
		if (flag)
		{
