...
public class Components
{
	...
	public class CmpsByWorld<T>
	{
		...
		public int CountWorldItems(int worldId, bool includeChildren = false)
		{
			...
			if (includeChildren)
			{
				foreach (int childWorldId in ClusterManager.Instance.GetWorld(worldId).GetChildWorldIds())
				{
					num += GetItems(childWorldId).Count;
				}
			}
			return num;
		}
		...
		public IEnumerable<T> WorldItemsEnumerate(int worldId, ICollection<int> otherWorldIds = null)
		{
			...
			foreach (int otherWorldId in otherWorldIds)
			{
				items = GetItems(otherWorldId);
				for (int index = 0; index < items.Count; index++)
				{
					yield return items[index];
				}
	...
	public static Cmps<ClamHarvestable> ClamHarvestables = new Cmps<ClamHarvestable>();
