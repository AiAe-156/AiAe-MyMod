...
public class AlarmSideScreen : SideScreenContent
{
	...
	private void InitializeToggles()
	{
		...
		foreach (NotificationType type in validTypes)
		{
			...
			for (int num = 0; num < toggles_by_type[type].states.Length; num++)
			{
				toggles_by_type[type].states[num].on_click_override_sound_path = NotificationScreen.Instance.GetNotificationSound(type);
			}
		}
	}
