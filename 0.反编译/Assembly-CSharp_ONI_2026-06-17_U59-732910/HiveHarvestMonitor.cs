...
public class HiveHarvestMonitor : GameStateMachine<HiveHarvestMonitor, HiveHarvestMonitor.Instance, IStateMachineTarget, HiveHarvestMonitor.Def>
{
	...
	public override void InitializeStates(out BaseState default_state)
	{
		...
		harvest.ready.ToggleChore((Instance smi) => smi.CreateHarvestChore(), SetRemoteChore, harvest.not_ready).EventTransition(GameHashes.OnStorageChange, harvest.not_ready, (Instance smi) => smi.storage.GetMassAvailable(smi.def.producedOre) < smi.def.harvestThreshold);
	}
