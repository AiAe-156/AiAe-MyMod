...
public class CellSelectionObject : KMonoBehaviour, ICellSelectionProxy
{
	...
	public KSelectable Selectable => mSelectable;
	...
	private void Update()
	{
		...
		if (SelectTool.Instance.selected != mSelectable)
		{
			...
			bool flag = alternateSelectionObject != null && SelectTool.Instance.selected == alternateSelectionObject.Selectable && mouseCell == alternateSelectionObject.SelectedCell;
	...
	public void UpdateValues()
	{
		...
		bool flag2 = IsExposedToSpace(selectedCell);
		mSelectable.ToggleStatusItem(Db.Get().MiscStatusItems.Space, flag2);
		UpdateBubbleStatusItem();
	}
	...
	private void UpdateBubbleStatusItem()
	{
		...
		{
			BubbleManager.instance.GetBubblesInCell(selectedCell, bubbleInfos);
		}
		if (bubbleInfos.Count == 0)
		{
			if (bubbleStatusHandle != Guid.Empty)
			{
				mSelectable.RemoveStatusItem(bubbleStatusHandle);
				bubbleStatusHandle = Guid.Empty;
			}
		}
		else if (bubbleStatusHandle != Guid.Empty)
		{
			mSelectable.RemoveStatusItem(bubbleStatusHandle, immediate: true);
			bubbleStatusHandle = mSelectable.AddStatusItem(Db.Get().MiscStatusItems.BubbleContents, this);
		}
		else
		{
			bubbleStatusHandle = mSelectable.AddStatusItem(Db.Get().MiscStatusItems.BubbleContents, this);
	...
	{
		if (Game.Instance.world.zoneRenderData.GetSubWorldZoneType(cell) == SubWorld.ZoneType.Space && !BackwallManager.HasBackwall(cell) && Grid.Objects[cell, 2] == null && Grid.Objects[cell, 9] == null)
		{
