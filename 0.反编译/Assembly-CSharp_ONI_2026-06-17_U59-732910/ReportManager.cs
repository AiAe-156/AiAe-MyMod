...
public class ReportManager : KMonoBehaviour
{
	...
	public class ReportEntry
	{
		...
		public ReportEntry(ReportType reportType, int note_storage_id, string context, int contextId, bool is_child = false)
		...
		public ReportEntry FindEntry(string dataContext)
		{
			for (int i = 0; i < contextEntries.Count; i++)
			{
				ReportEntry reportEntry = contextEntries[i];
		...
		public ReportEntry FindEntryFromPrefabInstance(KPrefabID prefabId)
		{
			DebugUtil.DevAssert(prefabId != null, "Don't call with invalid context id");
			if (prefabId == null)
			{
				return null;
			}
			...
			{
				return FindEntry(prefabId.gameObject.GetProperName());
			}
			ReportEntry reportEntry = null;
			for (int i = 0; i < contextEntries.Count; i++)
			{
				ReportEntry reportEntry2 = contextEntries[i];
			...
			if (reportEntry == null)
			{
				...
				for (int j = 0; j < contextEntries.Count; j++)
				{
					ReportEntry reportEntry3 = contextEntries[j];
		...
		public ReportEntry FindEntryFromGameObject(GameObject contextGameObject)
		{
			DebugUtil.DevAssert(contextGameObject != null, "Don't call with invalid context id");
			if (contextGameObject == null)
			{
				return null;
			}
			if (!contextGameObject.TryGetComponent<KPrefabID>(out var component))
			{
				return FindEntry(contextGameObject.GetProperName());
			}
			return FindEntryFromPrefabInstance(component);
		}

		private void AddDataToContext(NoteStorage note_storage, float value, string context, string note)
		{
			DebugUtil.DevAssert(context != null, "context must be non-null");
			if (context != null)
			{
				ReportEntry reportEntry = FindEntry(context);
				if (reportEntry == null)
				{
					reportEntry = new ReportEntry(reportType, note_storage.GetNewNoteId(), context, is_child: true);
		...
		public void AddData(NoteStorage note_storage, float value, string note = null, string dataContext = null)
		{
			...
			if (dataContext != null)
			{
				AddDataToContext(note_storage, value, dataContext, note);
			}
		}

		private void AddDataToProperNameContext(NoteStorage note_storage, float value, GameObject context, string note)
		{
			DebugUtil.DevAssert(context != null, "context must be non-null");
			if (context != null)
			{
				AddDataToContext(note_storage, value, context.GetProperName(), note);
			}
		}

		private void AddDataToPrefabInstanceContext(NoteStorage note_storage, float value, KPrefabID context, string note)
		{
			DebugUtil.DevAssert(context != null, "context must be non-null");
			if (context == null)
			{
			...
			{
				ReportEntry reportEntry = FindEntryFromPrefabInstance(context);
				if (reportEntry == null)
				{
					reportEntry = new ReportEntry(reportType, note_storage.GetNewNoteId(), context.gameObject.GetProperName(), context.InstanceID, is_child: true);
					contextEntries.Add(reportEntry);
				}
				reportEntry.AddActualData(note_storage, value, note);
			}
			...
			{
				AddDataToProperNameContext(note_storage, value, context.gameObject, note);
			}
		}

		public void AddDataWithPrefabInstanceContext(NoteStorage note_storage, float value, KPrefabID context, string note = null)
		{
			AddActualData(note_storage, value, note);
			DebugUtil.DevAssert(context != null, "Use the other overload of AddData for null contexts");
			AddDataToPrefabInstanceContext(note_storage, value, context, note);
		}

		public void AddDataWithGameObjectContext(NoteStorage note_storage, float value, GameObject context, string note = null)
		{
			AddActualData(note_storage, value, note);
			DebugUtil.DevAssert(context != null, "Use the other overload of AddData for null contexts");
			if (!(context == null))
			{
				if (context.TryGetComponent<KPrefabID>(out var component))
				{
					AddDataToPrefabInstanceContext(note_storage, value, component, note);
				}
				else
				{
					AddDataToProperNameContext(note_storage, value, context, note);
				}
			}
		}
	...
	public class DailyReport
	{
		...
		public void AddDataWithPrefabInstanceContext(ReportType reportType, float value, KPrefabID context, string note = null)
		{
			GetEntry(reportType).AddDataWithPrefabInstanceContext(noteStorage, value, context, note);
		}

		public void AddDataWithGameObjectContext(ReportType reportType, float value, GameObject context, string note = null)
		{
			GetEntry(reportType).AddDataWithGameObjectContext(noteStorage, value, context, note);
		}
	}
	...
	public class NoteStorage
	{
		...
		private class NoteEntries
		{
			...
			public void Add(int report_entry_id, float value, int note_id)
			{
				...
				NoteEntryKey key = new NoteEntryKey
				{
					noteHash = note_id,
					isPositive = (value > 0f)
				};
			...
			public void Deserialize(BinaryReader reader, int version)
			{
				...
				for (int j = 0; j < num; j++)
				{
					...
					{
						NoteEntryKey key2 = new NoteEntryKey
						{
	...
	public void ReportValueWithPrefabInstanceContext(ReportType reportType, float value, KPrefabID context, string note = null)
	{
		TodaysReport.AddDataWithPrefabInstanceContext(reportType, value, context, note);
	}

	public void ReportValueWithGameObjectContext(ReportType reportType, float value, GameObject context, string note = null)
	{
		TodaysReport.AddDataWithGameObjectContext(reportType, value, context, note);
	}
