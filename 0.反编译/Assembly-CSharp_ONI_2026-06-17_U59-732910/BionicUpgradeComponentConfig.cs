...
public class BionicUpgradeComponentConfig : IMultiEntityConfig
{
	...
	public class BionicUpgradeData
	{
		...
		public string uiAnimName
		{
			get
			{
				if (!(animStateName == "object"))
				{
					return "ui_" + animStateName;
				}
				return "ui";
			}
		}
	...
	public List<GameObject> CreatePrefabs()
	{
		...
		list.Add(CreateNewUpgradeComponent(text18, null, null, 0f, (StateMachine.Instance smi) => new BionicUpgrade_SkilledWorker.Instance(smi.GetMaster(), skill_worker_def18), skill_worker_def18.GetDescription() + "\n\n" + string.Format(STRINGS.ITEMS.BIONIC_BOOSTERS.FABRICATION_SOURCE, STRINGS.BUILDINGS.PREFABS.ADVANCEDCRAFTINGTABLE.NAME), DlcManager.DLC3, "upgrade_disc_kanim", "machinery_1", SimHashes.Creature, null, BoosterType.Intermediate, isStartingBooster: false, isCarePackage: false, skillPerks15));
		if (DlcManager.IsContentSubscribed("DLC5_ID"))
		{
			string text19 = "Booster_Mod_Swim";
			AttributeModifier[] array18 = CreateBoosterModifiers(text19, new Dictionary<string, float>
			{
				{
					Db.Get().Attributes.Athletics.Id,
					5f
			...
			SkillPerk[] skillPerks16 = new SkillPerk[5]
			{
			...
			modifiers = array18;
			BionicUpgrade_SkilledWorker.Def skill_worker_def19 = new BionicUpgrade_SkilledWorker.Def(text19, Db.Get().Attributes.Athletics.Id, modifiers, skillPerks16, new string[2] { "hat_role_swimming", "hat_role_swimming2" });
			list.Add(CreateNewUpgradeComponent(text19, null, null, 0f, (StateMachine.Instance smi) => new BionicUpgrade_SkilledWorker.Instance(smi.GetMaster(), skill_worker_def19), skill_worker_def19.GetDescription() + "\n\n" + string.Format(STRINGS.ITEMS.BIONIC_BOOSTERS.FABRICATION_SOURCE, STRINGS.BUILDINGS.PREFABS.ADVANCEDCRAFTINGTABLE.NAME), new string[2] { "DLC3_ID", "DLC5_ID" }, "upgrade_disc_kanim", "mod_swim", SimHashes.Creature, "ArtificialFriends", BoosterType.Overclocked, isStartingBooster: false, isCarePackage: false, skillPerks16, new ComplexRecipe.RecipeElement[2]
			{
	...
	public static GameObject CreateNewUpgradeComponent(string id, string name = null, string desc = null, float wattageCost = 0f, Func<StateMachine.Instance, StateMachine.Instance> stateMachine = null, string sm_description = "", string[] dlcIDs = null, string animFile = "upgrade_disc_kanim", string animStateName = "object", SimHashes element = SimHashes.Creature, string craftTechUnlockID = null, BoosterType booster = BoosterType.Basic, bool isStartingBooster = false, bool isCarePackage = false, SkillPerk[] skillPerks = null, ComplexRecipe.RecipeElement[] recipeInputOverride = null, bool addTechItem = true)
	{
		...
		string ID = id;
		TechItem techItem = new TechItem(ID, Db.Get().TechItems, Strings.Get("STRINGS.RESEARCH.OTHER_TECH_ITEMS." + id.ToUpper() + ".NAME"), Strings.Get("STRINGS.RESEARCH.OTHER_TECH_ITEMS." + id.ToUpper() + ".DESC"), (string a, bool b) => Def.GetUISprite(Assets.GetPrefab(ID)).first, craftTechUnlockID, dlcIDs);
		...
		{
			ComplexRecipe.RecipeElement[] array = ((recipeInputOverride != null) ? recipeInputOverride : new ComplexRecipe.RecipeElement[1]
			{
				new ComplexRecipe.RecipeElement("PowerStationTools", 8f)
			});
			LocString locString = ((booster != BoosterType.Overclocked) ? STRINGS.BUILDINGS.PREFABS.ADVANCEDCRAFTINGTABLE.BIONIC_COMPONENT_RECIPE_DESC : STRINGS.BUILDINGS.PREFABS.ADVANCEDCRAFTINGTABLE.BIONIC_OVERCLOCK_RECIPE_DESC);
			...
			new ComplexRecipe(ComplexRecipeManager.MakeRecipeID(ID, array, array2), array, array2)
			{
				time = INDUSTRIAL.RECIPES.STANDARD_FABRICATION_TIME,
				description = string.Format(locString, STRINGS.ITEMS.INDUSTRIAL_PRODUCTS.POWER_STATION_TOOLS.NAME, name) + "\n\n" + UpgradesData[ID].stateMachineDescription,
				nameDisplay = ComplexRecipe.RecipeNameDisplay.Result,
				fabricators = new List<Tag> { "AdvancedCraftingTable" },
				requiredTech = craftTechUnlockID,
				sortOrder = 3,
				runTimeDescription = () => GetColonyBoosterAssignmentString(ID)
			};
		}
		else
		{
			ComplexRecipe.RecipeElement[] array3 = ((recipeInputOverride != null) ? recipeInputOverride : new ComplexRecipe.RecipeElement[1]
			{
			...
			new ComplexRecipe(ComplexRecipeManager.MakeRecipeID(ID, array3, array4), array3, array4, dlcIDs)
			{
