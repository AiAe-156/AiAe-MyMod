...
public class AssignableSlots : ResourceSet<AssignableSlot>
{
	...
	public AssignableSlots()
	{
		...
		BionicUpgrade = Add(new OwnableSlot("BionicUpgrade", MISC.TAGS.BIONICUPGRADE));
		...
		Outfit = Add(new EquipmentSlot(TUNING.EQUIPMENT.CLOTHING.SLOT, UI.StripLinkFormatting(MISC.TAGS.CLOTHES)));
		Shoes = Add(new EquipmentSlot(TUNING.EQUIPMENT.SHOES.SLOT, UI.StripLinkFormatting(MISC.TAGS.SHOES)));
	}
}
