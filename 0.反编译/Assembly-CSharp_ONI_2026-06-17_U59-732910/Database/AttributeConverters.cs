...
public class AttributeConverters : ResourceSet<AttributeConverter>
{
	...
	public AttributeConverter Create(string id, string name, string description, Attribute attribute, float multiplier, float base_value, IAttributeFormatter formatter, string[] requiredDlcIds = null, string[] forbiddenDlcIds = null)
	{
	...
	public AttributeConverters()
	{
		...
		PilotingSpeed = Create("PilotingSpeed", "Piloting Speed", DUPLICANTS.ATTRIBUTES.SPACENAVIGATION.SPEED_MODIFIER, Db.Get().Attributes.SpaceNavigation, 0.025f, 0f, formatter, DlcManager.EXPANSION1);
		...
		CapturableSpeed = Create("CapturableSpeed", "Capturable Speed", DUPLICANTS.ATTRIBUTES.RANCHING.CAPTURABLESPEED, Db.Get().Attributes.Ranching, 0.05f, 0f, formatter);
		GeotuningSpeed = Create("GeotuningSpeed", "Geotuning Speed", DUPLICANTS.ATTRIBUTES.LEARNING.GEOTUNER_SPEED_MODIFIER, Db.Get().Attributes.Learning, 0.05f, 0f, formatter);
