...
public class VomitChore : Chore<VomitChore.StatesInstance>
{
	public class StatesInstance : GameStateMachine<States, StatesInstance, VomitChore, object>.GameInstance
	{
			...
			: base(master)
		{
			...
			{
				elementToVomit = SimHashes.LiquidGunk;
			}
		}
		...
		{
			SpawnVomitLiquid(dt, SimHashes.DirtyWater);
		}
		...
		public void SpawnVomitLiquid(float dt, SimHashes element)
		{
			if (dt > 0f)
			{
				...
				for (int i = 0; i < sicknesses.Count && sicknesses[i].modifier.sicknessType != Sickness.SicknessType.Pathogen; i++)
				{
				}
				...
				if (equippable != null)
				{
					equippable.GetComponent<Storage>().AddLiquid(element, STRESS.VOMIT_AMOUNT * num, bodyTemperature.value, invalid.idx, invalid.count);
				}
				else
				{
					SimMessages.AddRemoveSubstance(num3, element, CellEventLogger.Instance.Vomit, STRESS.VOMIT_AMOUNT * num, bodyTemperature.value, invalid.idx, invalid.count);
				}
			}
		}
	...
	public class States : GameStateMachine<States, StatesInstance, VomitChore>
	{
		...
		public override void InitializeStates(out BaseState default_state)
		{
			...
			vomit.DefaultState(vomit.buildup).ToggleAnims((Func<StatesInstance, KAnimFile>)GetAnimFileName).ToggleStatusItem((StatesInstance smi) => smi.statusItem)
			...
			vomit.release.ToggleEffect("Vomiting").PlayAnim("vomit_loop", KAnim.PlayMode.Once).Update("SpawnVomitLiquid", delegate(StatesInstance smi, float dt)
			{
	...
	private static KAnimFile GetAnimFileName(StatesInstance smi)
	{
