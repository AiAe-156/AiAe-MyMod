public class RobotAi : GameStateMachine<RobotAi, RobotAi.Instance>
{
	...
	public new class Instance : GameInstance
	{
			...
			: base(master, (object)def)
		{
			...
			onBeginChoreHandlerID = Subscribe(-1988963660, OnBeginChore);
		}
		...
		{
			Unsubscribe(ref onBeginChoreHandlerID);
			base.OnCleanUp();
		}
	...
	public override void InitializeStates(out BaseState default_state)
	{
		...
		alive.normal.TagTransition(GameTags.Stored, alive.stored).Enter(delegate(Instance smi)
		{
			if (!smi.HasTag(GameTags.Robots.Models.FetchDrone))
			{
				smi.fallMonitor = new FallMonitor.Instance(smi.master, shouldPlayEmotes: false);
