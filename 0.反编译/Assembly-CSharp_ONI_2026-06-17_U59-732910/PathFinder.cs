...
public class PathFinder
{
	public struct Cell
	{
		...
		public NavType navType => (NavType)(navTypes & 0xF);
	...
	public static void Initialize()
	{
		...
		for (int j = 0; j < Grid.CellCount; j++)
		{
			...
			{
				FloodFill.DepthTraverse(j, new FloodFill.PredicateCondition(notSolid), default(PathTracker), default(FloodFill.NoMaxDepth), default(FloodFill.DoNothing));
			}
		}
	...
	public static bool ValidatePath(NavGrid nav_grid, PathFinderAbilities abilities, ref Path path, PotentialPath.Flags flags)
	{
		if (!path.IsValid())
		{
			return false;
		...
		for (int i = 0; i < path.nodes.Count; i++)
		{
			...
			while (link.link != InvalidHandle)
			{
				...
				{
					PotentialPath path2 = new PotentialPath(node2.cell, node.navType, flags);
	...
	{
		potentials.Clear();
		ushort num = (ushort)(PathGrid.SerialNo + 1);
		if (num == 0)
		{
			num++;
		}
		PathGrid.BeginUpdate(num, potential_path.cell);
		...
		while (potentials.Count > 0)
		{
			...
			{
				if (cell_data.navType != NavType.Tube && query.IsMatch(keyValuePair.Value.cell, cell_data.parent, cell_data.cost) && cell_data.cost < num2)
				{
	...
	public static void AddPotentials(PotentialScratchPad potential_scratch_pad, PotentialPath potential, int cost, ref PathFinderAbilities abilities, PathFinderQuery query, int max_links_per_cell, NavGrid.Link[] links, PotentialList potentials, PathGrid path_grid, int parent_cell, NavType parent_nav_type)
	{
		...
		{
			PotentialScratchPad.PathGridCellData pathGridCellData = linksInCellRange[k];
			NavGrid.Link link5 = pathGridCellData.link;
			int link6 = link5.link;
			Cell cell_data = pathGridCellData.pathGridCell;
			int num7 = cost + link5.cost;
			PotentialPath path = potential;
			path.cell = link6;
			path.navType = link5.endNavType;
			if (pathGridCellData.isSubmerged || path.navType == NavType.Swim)
			{
				int submergedPathCostPenalty2 = abilities.GetSubmergedPathCostPenalty(path, link5);
				num7 += submergedPathCostPenalty2;
			}
			bool num8 = cell_data.cost == -1;
			bool flag3 = num7 < cell_data.cost;
			if (num8 || flag3)
			{
				PotentialPath.Flags flags = path.flags;
				bool num9 = abilities.TraversePath(ref path, potential.cell, potential.navType, num7, link5.transitionId, pathGridCellData.isSubmerged);
				_ = path.flags;
				if (num9)
				{
					AddPotential(path, potential.cell, potential.navType, num7, link5.transitionId, potentials, path_grid, ref cell_data);
				}
			}
		}
	}
