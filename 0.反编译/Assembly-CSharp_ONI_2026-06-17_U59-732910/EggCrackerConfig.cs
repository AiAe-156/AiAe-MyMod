...
public class EggCrackerConfig : IBuildingConfig
{
	...
	public static void CategorizeEggs()
	{
		...
		{
			Tag spawnedCreature = Assets.GetPrefab(uncategorizedEggDatum.id).GetDef<IncubationMonitor.Def>().spawnedCreature;
	...
	{
		BuildingDef obj = BuildingTemplates.CreateBuildingDef("EggCracker", 2, 2, "egg_cracker_kanim", 30, 10f, TUNING.BUILDINGS.CONSTRUCTION_MASS_KG.TIER1, MATERIALS.RAW_METALS, 1600f, BuildLocationRule.OnFloor, noise: NOISE_POLLUTION.NONE, decor: TUNING.BUILDINGS.DECOR.BONUS.TIER0);
	...
	public void MakeRecipes()
	{
		...
		foreach (KeyValuePair<Tag, List<EggData>> eggsBySpecy in EggsBySpecies)
		{
			...
			new ComplexRecipe(text, array2, array3, eggData.requiredDlcIds, eggData.forbiddenDlcIds)
			{
