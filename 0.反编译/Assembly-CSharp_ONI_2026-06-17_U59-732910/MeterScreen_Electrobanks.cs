...
public class MeterScreen_Electrobanks : MeterScreen_ValueTrackerDisplayer
{
	...
	protected override string OnTooltip()
	{
		...
		string formattedJoules = GameUtil.GetFormattedJoules(WorldResourceAmountTracker<ElectrobankTracker>.Get().CountAmount(per_electrobankType_UnitCount_Dictionary, out totalUnitsFound, ClusterManager.Instance.activeWorld.worldInventory, excludeUnreachable: true));
		...
		foreach (KeyValuePair<string, float> item in per_electrobankType_UnitCount_Dictionary.OrderByDescending((KeyValuePair<string, float> x) => x.Value).ToDictionary((KeyValuePair<string, float> t) => t.Key, (KeyValuePair<string, float> t) => t.Value))
		{
