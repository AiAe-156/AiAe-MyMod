...
public class UglyCryChore : Chore<UglyCryChore.StatesInstance>
{
	public class StatesInstance : GameStateMachine<States, StatesInstance, UglyCryChore, object>.GameInstance
	{
			...
			: base(master)
		{
			...
			suitEquipper = crier.GetComponent<SuitEquipper>();
		}
		...
		public void ProduceTears(float dt)
		{
			...
			Equippable equippable = suitEquipper.IsWearingAirtightSuit();
			if (equippable != null)
			{
				equippable.GetComponent<Storage>().AddLiquid(SimHashes.Water, num, bodyTemperature.value, byte.MaxValue, 0);
				return;
			}
			pendingTearsMass += num;
			if (pendingTearsMass >= 0.0101f)
			{
				SimMessages.AddRemoveSubstance(Grid.PosToCell(base.smi.master.gameObject), SimHashes.Water, CellEventLogger.Instance.Tears, pendingTearsMass, bodyTemperature.value, byte.MaxValue, 0);
				pendingTearsMass = 0f;
			}
		}
	}
