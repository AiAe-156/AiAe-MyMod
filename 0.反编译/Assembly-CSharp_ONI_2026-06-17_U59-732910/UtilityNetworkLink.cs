...
public abstract class UtilityNetworkLink : KMonoBehaviour
{
	...
	private SpawnFlags spawnState = SpawnFlags.PendingSpawn;
	...
	protected override void OnSpawn()
	{
		base.OnSpawn();
		Connect();
		if ((spawnState & SpawnFlags.PendingBroken) != SpawnFlags.None)
		{
			Disconnect();
			spawnState &= ~SpawnFlags.PendingBroken;
		}
		if ((spawnState & SpawnFlags.PendingRepaired) != SpawnFlags.None)
		{
			Connect();
			spawnState &= ~SpawnFlags.PendingRepaired;
		}
		spawnState &= ~SpawnFlags.PendingSpawn;
		DebugUtil.DevAssert(spawnState == SpawnFlags.None, "Failed to resolve all pending spawn actions");
	}
	...
	{
		if ((spawnState & SpawnFlags.PendingSpawn) != SpawnFlags.None)
		{
			spawnState |= SpawnFlags.PendingBroken;
		}
		else
