...
public class BipedTransitionLayer : TransitionDriver.OverrideLayer
{
	...
	public override void BeginTransition(Navigator navigator, Navigator.ActiveTransition transition)
	{
		...
		bool num4 = (navigator.flags & PathFinder.PotentialPath.Flags.HasJetPack) != 0;
		bool flag6 = (navigator.flags & PathFinder.PotentialPath.Flags.HasLeadSuit) != 0;
		bool flag7 = num4 || flag5 || flag6;
		...
		if (transition.x == 0 && (transition.start == NavType.Ladder || transition.start == NavType.Pole) && transition.start == transition.end)
		{
			...
			else
			{
				...
				if (gameObject != null)
				{
					...
					if (component != null)
					{
						float num5 = component.upwardsMovementSpeedMultiplier;
						if (transition.y < 0)
						{
							num5 = component.downwardsMovementSpeedMultiplier;
						}
						transition.speed *= num5;
						transition.animSpeed *= num5;
					}
				}
			}
		}
