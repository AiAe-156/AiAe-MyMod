...
public class JettisonableCargoModule : GameStateMachine<JettisonableCargoModule, JettisonableCargoModule.StatesInstance, IStateMachineTarget, JettisonableCargoModule.Def>
{
	...
	public class StatesInstance : GameInstance, IEmptyableCargo
	{
		...
		public void FinalDeploy()
		{
			...
			base.master.GetComponent<RocketModuleCluster>().CraftInterface.GetComponent<Clustercraft>().gameObject.Trigger(1792516731, (object)component);
		...
		{
			if (base.sm.hasCargo.Get(base.smi) && IsValidDropLocation() && (!ChooseDuplicant || (ChosenDuplicant != null && !ChosenDuplicant.HasTag(GameTags.Dead))))
			{
				return !landerPlaced;
			}
			return false;
