...
public class DetailsScreen : KTabMenu
{
	...
	public class SidescreenTab
	{
		...
		public void SetTitle(string title)
		{
			...
			{
				bodyReferences.GetReference<LocText>("TitleLabel").SetText(title);
			}
		}
		...
		public void SetNoConfigMessageVisibility(bool visible)
		{
			...
			{
				bodyReferences.GetReference("NoConfigMessage").gameObject.SetActive(visible);
			}
		}
		...
		public void RepositionTitle()
		{
			...
			{
				bodyReferences.GetReference("Title").transform.SetSiblingIndex(0);
			}
		}
	...
	private SidescreenTabTypes selectedSidescreenTabID;
	...
	public void InitiateSidescreenTabs()
	{
		for (int i = 0; i < sidescreenTabs.Length; i++)
		{
			...
			case SidescreenTabTypes.Errands:
				sidescreenTab.ValidateTargetCallback = (GameObject target, SidescreenTab _tab) => target.GetComponent<MinionIdentity>() != null;
				break;
	...
	private void RefreshSideScreenTabs()
	{
		...
		{
		case SidescreenTabTypes.Material:
			GetTabOfType(SidescreenTabTypes.Material).bodyInstance.GetComponentInChildren<DetailsScreenMaterialPanel>().SetTarget(target);
			break;
		case SidescreenTabTypes.Blueprints:
		{
	...
	public void UpdatePortrait(GameObject target)
	{
		...
		{
			string animName2 = (component6.Element.IsSolid ? "ui" : component6.Element.substance.name);
			Sprite uISpriteFromMultiObjectAnim3 = Def.GetUISpriteFromMultiObjectAnim(component6.Element.substance.anim, animName2);
