...
public class BionicOxygenTankMonitor : GameStateMachine<BionicOxygenTankMonitor, BionicOxygenTankMonitor.Instance, IStateMachineTarget, BionicOxygenTankMonitor.Def>
{
	...
	public new class Instance : GameInstance, OxygenBreather.IGasProvider
	{
			...
			: base(master, def)
		{
			...
			float movementSpeedMultiplier = BipedTransitionLayer.GetMovementSpeedMultiplier(Db.Get().AttributeConverters.MovementSpeed.Lookup(navigator.gameObject));
		...
		private void CompareOxygenSources()
		{
			...
			{
				float num2 = num / movementRate * oxygenBreather.ConsumptionRate;
				if (oxygenBreather.GetAmounts().Get(Db.Get().Amounts.Breath).value < num2)
				{
					pickupable = null;
				}
			}
		...
		public void UpdatePotentialCellToAbsorbOxygen(int previouslyReservedCell)
		{
			...
			bool num2 = AbsorbOxygenCell != num;
	...
	{
		if (!IsAllowedToSeekOxygenBySchedule(smi))
		{
			return !FindOxygenSourceChoreIsRunning(smi);
		}
		return false;
	}
	...
	public static bool IsNotAllowedToSeekOxygenSourceItemsByScheduleAndAbsorbChoreHasNotBegun(Instance smi)
	{
		if (!IsAllowedToSeekOxygenBySchedule(smi))
		{
			return !AbsorbChoreIsRunning(smi);
		}
		return false;
	}
	...
	{
		if (AreOxygenLevelsCritical(smi))
		{
			return !IsConsumingOxygen(smi);
		}
		return false;
	}
	...
	public static bool NoOxygenSourceAvailableButAbsorbCellAvailable(Instance smi, SignalParameter param)
	{
		...
		{
			return AbsorbCellAvailable(smi);
		}
		return false;
	}
	...
	{
		if (IsThereAnOxygenSourceItemAvailable(smi))
		{
			return !AbsorbChoreIsRunning(smi);
		}
		return false;
	...
	{
		if (!ChoreIsRunning(smi, Db.Get().ChoreTypes.BionicAbsorbOxygen))
		{
			return ChoreIsRunning(smi, Db.Get().ChoreTypes.BionicAbsorbOxygen_Critical);
		}
		return true;
	}
	...
	{
		if (!ChoreIsRunning(smi, Db.Get().ChoreTypes.FindOxygenSourceItem))
		{
			return ChoreIsRunning(smi, Db.Get().ChoreTypes.FindOxygenSourceItem_Critical);
		}
		return true;
	}
