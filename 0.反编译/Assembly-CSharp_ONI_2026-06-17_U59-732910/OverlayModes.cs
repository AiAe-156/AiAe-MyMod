...
public abstract class OverlayModes
{
	...
	public class Disease : Mode
	{
		...
		protected override void OnSaveLoadRootRegistered(SaveLoadRoot item)
		{
			if (!(item == null) && !(item.GetComponent<KBatchedAnimController>() == null))
			{
		...
		private void UpdatePowerLabels()
		{
			foreach (UpdatePowerInfo item2 in updatePowerInfo)
			{
				...
				if (generator != null && consumer == null)
				{
					...
					Color color = (unitLabel.color = (powerLabel.color = ((component != null && !component.IsEnabled) ? GlobalAssets.Instance.colorSet.powerBuildingDisabled : GlobalAssets.Instance.colorSet.powerGenerator)));
