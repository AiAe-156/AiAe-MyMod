...
public class BionicBedTimeModeChore : Chore<BionicBedTimeModeChore.Instance>
{
	...
	public class Instance : GameStateMachine<States, Instance, BionicBedTimeModeChore, object>.GameInstance
	{
		...
		{
			return GetComponent<Sensors>().GetSensor<AssignableReachabilitySensor>().IsReachable(Db.Get().AssignableSlots.Bed);
		}
	...
	{
		if (defragmentationZone != null)
		{
			return smi.IsDefragmentationZoneReachable();
		}
		return false;
	}
	...
	{
		if (smi.sm.defragmentationZone.Get(smi) != null)
		{
			return smi.IsDefragmentationZoneReachable();
		}
		return false;
