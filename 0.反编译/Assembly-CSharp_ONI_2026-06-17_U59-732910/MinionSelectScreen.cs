...
public class MinionSelectScreen : CharacterSelectionController
{
	...
	private IEnumerator SetDefaultMinionsRoutine()
	{
		...
		bool aquaticStart = IsAquaticStartWorld(clusterData);
		if (clusterData.startingMinions != null)
		{
			...
			SetupMinion((CharacterContainer)containers[2], (clusterData.startingMinions.Length != 0) ? clusterData.startingMinions[0] : null);
			SetupMinion((CharacterContainer)containers[1], (clusterData.startingMinions.Length > 1) ? clusterData.startingMinions[1] : null);
			SetupMinion((CharacterContainer)containers[0], (clusterData.startingMinions.Length > 2) ? clusterData.startingMinions[2] : null);
		}
	...
	private static bool IsAquaticStartWorld(ClusterLayout cluster)
	{
		...
		if (worldData != null && worldData.worldTags != null)
		{
			return worldData.worldTags.Contains("Aquatic");
		}
		return false;
	}
	...
	private static void EnsureSwimmingSkill(CharacterContainer container)
	{
		...
		{
			if (trait2 != null && trait2.Id == "GrantSkill_Swimming")
			{
				return;
			}
		}
