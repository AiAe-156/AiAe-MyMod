...
public class CodexEntryGenerator_Elements
{
	...
	private static void CollectSharedConversionIO(GameObject prefab, HashSet<ElementUsage> inSet, HashSet<ElementUsage> outSet, List<ElementConverter> outputOnlyConverters)
	{
		...
		foreach (ElementConsumer item4 in components ?? Enumerable.Empty<ElementConsumer>())
		{
	...
	private static void AddIndependentConversionEntries(GameObject prefab, HashSet<ElementUsage> inSet, CodexElementMap usedMap, CodexElementMap madeMap, List<ElementConverter> categoryConverters)
	{
		...
		foreach (ElementConverter categoryConverter in categoryConverters)
		{
			...
			{
				foreach (Element item4 in ElementLoader.FindElements((Element e) => e.HasTag(c.Tag)))
				{
					ConversionEntry conversionEntry = new ConversionEntry();
					conversionEntry.title = prefab.GetProperName();
					conversionEntry.prefab = prefab;
			...
			{
				ElementUsage item = new ElementUsage(ElementLoader.FindElementByHash(item5.elementHash).tag, item5.massGenerationRate, continuous: true);
				foreach (ConversionEntry item6 in list)
				{
					item6.outSet.Add(item);
				}
			}
			if (TryGetByproductUsage(prefab.GetDefImplementingInterface<IConverterByproduct>(), categoryConverter, out var usage))
			{
		...
		if (component2 != null)
		{
			...
			foreach (KPrefabID item11 in prefabsOfPossiblePrey)
			{
				...
				Tag tag = ((component4 == null) ? item11.PrefabID() : component4.species);
				...
				if (!list2.Contains(text))
				{
					...
					conversionEntry3.inSet.Add(new ElementUsage(tag, (component == null) ? 1f : (1f / component.cropVal.cropDuration), component != null, (Tag t, float units, bool flag) => GameUtil.GetFormattedUnits(units, flag ? GameUtil.TimeSlice.PerCycle : GameUtil.TimeSlice.None)));
					if (component != null)
					{
		...
		if (def5 != null)
		{
			...
			{
				new ElementUsage(tag2, def5.dropMass, continuous: false)
			};
			madeMap.Add(tag2, conversionEntry6);
		}
		MilkProductionMonitor.Def def6 = prefab.GetDef<MilkProductionMonitor.Def>();
		if (def6 != null)
		{
			string obj = (prefab.GetComponent<KPrefabID>().HasTag(GameTags.SwimmingCreature) ? "UnderwaterMilkingStation" : "MilkingStation");
			ConversionEntry conversionEntry7 = new ConversionEntry();
			GameObject prefab6 = Assets.GetPrefab(obj);
		...
		if (def7 != null)
		{
			...
			float amount = num2 + def7.GetMaxModification();
			ElementUsage item2 = new ElementUsage(def7.lubricant.CreateTag(), amount, continuous: true)
			{
				customFormating = (Tag tag3, float mass, bool continous) => string.Format(CODEX.FORMAT_STRINGS.SECRETED, GameUtil.GetFormattedMass(mass, GameUtil.TimeSlice.PerCycle))
			};
		...
		if (def8 != null)
		{
			...
			{
				customFormating = (Tag tag3, float mass, bool continous) => CODEX.FORMAT_STRINGS.MOLTED.Replace("{Amount}", GameUtil.GetFormattedMass(mass, GameUtil.TimeSlice.PerCycle))
			};
		...
		{
			conversionEntry10.outSet.Add(new ElementUsage(text3, amount2, continuous: false));
			madeMap.Add(text3, conversionEntry10);
		}
	}
	...
	private static void AddDietConversions(GameObject prefab, CodexElementMap usedMap, CodexElementMap madeMap)
	{
		...
		foreach (Diet.Info info in infos)
		{
			foreach (Tag consumedTag in info.consumedTags)
			{
				...
				bool num3 = diet.IsConsumedTagAbleToBeEatenDirectly(consumedTag);
				...
				if (num3)
				{
					...
					{
						float num4 = diet.AvailableCaloriesInPrey(consumedTag);
						num2 = (0f - num) / num4;
						amount = num2 * info.producedConversionRate * num4 / info.caloriesPerKg;
