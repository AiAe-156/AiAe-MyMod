...
public class SafetyConditions
{
	...
	public SafetyConditions()
	{
		...
		IsNotCoveredInLiquid = new SafetyChecker.Condition("IsNotCoveredInLiquid", num *= 2, delegate(int cell, int cost, SafetyChecker.Context context)
		{
			...
			return !Grid.Element[cell].IsLiquid || !Grid.Element[num2].IsLiquid;
		});
		...
		HasSomeOxygen = new SafetyChecker.Condition("HasSomeOxygen", num *= 2, (int cell, int cost, SafetyChecker.Context context) => context.oxygenBreather == null || GasBreatherFromWorldProvider.GetBestBreathableCellAroundSpecificCell(cell, Grid.DefaultOffset, context.oxygenBreather).IsBreathable);
