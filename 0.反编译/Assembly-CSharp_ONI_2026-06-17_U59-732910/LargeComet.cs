...
public class LargeComet : KMonoBehaviour, ISim33ms
{
	...
	public void SetVelocity()
	{
		...
		GetComponent<KBatchedAnimController>().Rotation = (float)(-num) - 90f;
	}
	...
	public void Sim33ms(float dt)
	{
		...
		if (vector2I2.y != vector2I.y)
		{
			...
			foreach (int item in cellsCentrePassedThrough)
			{
				...
				{
					int cell = Grid.OffsetCell(Grid.OffsetCell(item, 0, Mathf.Abs(lowestTemplateYLocalPosition)), value.x, value.y);
					if (Grid.IsValidCellInWorld(cell, worldID) && DestroyCell(cell) && !flag)
					{
						Vector3 position4 = Grid.CellToPos(cell);
						if (IsPositionFarAwayFromOtherExplosions(position4))
						{
							flag = true;
							position3 = Grid.CellToPos(cell);
						}
					}
				}
			}
