...
public class AnimEventManager : Singleton<AnimEventManager>
{
	...
	{
		AnimData initial_data = new AnimData
		{
			frameRate = anim.frameRate,
			totalTime = anim.totalTime,
			numFrames = anim.numFrames,
			useUnscaledTime = use_unscaled_time
		};
	...
	{
		IndirectionData data = indirectionData.GetData(handle);
		KCompactedVector<AnimData> obj = (data.isUIData ? uiAnimData : animData);
		AnimData data2 = obj.GetData(data.eventDataHandle);
		data2.frameRate = anim.frameRate;
		data2.totalTime = anim.totalTime;
		data2.numFrames = anim.numFrames;
		obj.SetData(data.animDataHandle, data2);
		KCompactedVector<EventPlayerData> obj2 = (data.isUIData ? uiEventData : eventData);
		EventPlayerData data3 = obj2.GetData(data.eventDataHandle);
		data3.events = GameAudioSheets.Get().GetEvents(anim.id);
		if (data3.events == null)
		{
			data3.events = emptyEventList;
		}
		obj2.SetData(data.eventDataHandle, data3);
	}
