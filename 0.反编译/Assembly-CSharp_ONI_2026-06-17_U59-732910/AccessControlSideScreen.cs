...
public class AccessControlSideScreen : SideScreenContent
{
	...
	private void Refresh()
	{
		...
		if (component != null)
		{
			_ = component.IsRotated;
		}
		else
			_ = 0;
		ClearOldRows();
		PopulateRows();
		standardMinionSectionHeader.GetComponent<HierarchyReferences>().GetReference<RectTransform>("EmptyRow").gameObject.SetActive(standardMinionSectionContent.transform.childCount <= 1);
		...
		bionicMinionSectionHeader.GetComponent<HierarchyReferences>().GetReference<RectTransform>("EmptyRow").gameObject.SetActive(bionicMinionSectionContent.transform.childCount <= 1);
		...
		robotSectionHeader.GetComponent<HierarchyReferences>().GetReference<RectTransform>("EmptyRow").gameObject.SetActive(robotSectionContent.transform.childCount <= 1);
