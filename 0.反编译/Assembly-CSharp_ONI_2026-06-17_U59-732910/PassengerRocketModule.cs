...
public class PassengerRocketModule : KMonoBehaviour
{
	...
	public bool CheckPilotBoarded()
	{
		...
		if (dupePilot != null)
		{
			return dupePilot.GetMyWorldId() == Grid.WorldIdx[GetComponent<ClustercraftExteriorDoor>().TargetCell()];
		}
		return false;
	}
	...
	public GameObject GetDupePilot()
	{
		...
		{
			gameObject = item2.GetTargetGameObject();
	...
	{
		if (((ICollection<IAssignableIdentity>)GetComponent<AssignmentGroupController>().GetMembers()).Count > 0)
		{
			return true;
		}
		return false;
	...
	{
		return ((ICollection<IAssignableIdentity>)GetComponent<AssignmentGroupController>().GetMembers()).Count;
	}
