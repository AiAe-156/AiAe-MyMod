	...
	public new class Instance : GameInstance
	{
		private class StickerBombReactable : Reactable
		{
			...
			private Func<int, FloodFill.BoundaryCheckResult> canPlaceStickerCb = delegate(int cell)
			{
				...
				return Grid.IsCellOpenToSpace(cell) ? FloodFill.BoundaryCheckResult.Halt : FloodFill.BoundaryCheckResult.Continue;
			};
			...
			{
				int startCell = Grid.PosToCell(reactor.transform.GetPosition() + Vector3.up);
				ListPool<int, PathFinder>.PooledList pooledList = ListPool<int, PathFinder>.Allocate();
				FloodFill.BreadthCollect(startCell, canPlaceStickerCb, pooledList, 2);
				int result = ((pooledList.Count > 0) ? pooledList.GetRandom() : 0);
				pooledList.Recycle();
				return result;
			}
