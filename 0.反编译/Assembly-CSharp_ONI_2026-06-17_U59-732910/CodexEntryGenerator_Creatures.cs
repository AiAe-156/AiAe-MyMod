	...
	private static void GenerateCreatureDescriptionContainers(GameObject creature, List<ContentContainer> containers)
	{
		...
		foreach (Diet.Info info in array)
		{
			...
			foreach (Tag consumedTag in info.consumedTags)
			{
				...
				float num4 = (0f - num) / info.caloriesPerKg;
				float outputAmount = num4 * info.producedConversionRate;
				if (num3)
				{
					if (info.foodType == Diet.Info.FoodType.EatPlantDirectly)
					{
						list.Add(new CodexConversionPanel(consumedTag.ProperName(), consumedTag, num4, inputContinuous: true, GameUtil.GetFormattedDirectPlantConsumptionValuePerCycle, info.producedElement, outputAmount, outputContinuous: true, null, creature));
					}
					else if (info.foodType == Diet.Info.FoodType.EatPlantStorage)
					{
						list.Add(new CodexConversionPanel(consumedTag.ProperName(), consumedTag, num4, inputContinuous: true, GameUtil.GetFormattedPlantStorageConsumptionValuePerCycle, info.producedElement, outputAmount, outputContinuous: true, null, creature));
					}
					else if (info.foodType == Diet.Info.FoodType.EatPrey || info.foodType == Diet.Info.FoodType.EatButcheredPrey)
					{
						...
						list.Add(new CodexConversionPanel(consumedTag.ProperName(), consumedTag, num4, inputContinuous: true, GameUtil.GetFormattedPreyConsumptionValuePerCycle, info.producedElement, outputAmount, outputContinuous: true, null, creature));
					}
				}
				else
				{
				...
				if (component3 != null)
				{
					list.Add(new CodexConversionPanel(CODEX.HEADERS.CRITTER_EXTRA_DIET_PRODUCTION, consumedTag, num4, inputContinuous: true, component3.producedElement.CreateTag(), num4 * 1000f * component3.kgProducedPerKcalConsumed * 2f, outputContinuous: true, creature));
				}
			}
		}
