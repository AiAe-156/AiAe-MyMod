...
public static class Grid
{
	...
	public static bool HasUsableTubeEntrance(int cell, int minionInstanceID)
	{
		...
		return reservedInstanceIDs.Count < value.reservationCapacity || reservedInstanceIDs.Contains(minionInstanceID);
	}
	...
	public static bool ReserveEmptyLocker(int cell, int minionInstanceID, bool reserve)
	{
		...
		if (reserve)
		{
			...
			DebugUtil.Assert(test, "Reservation not made");
			return true;
		}
	...
	public static bool HasEmptyLocker(int cell, int minionInstanceID)
	{
		...
		return minionIDsWithEmptyLockerReservations.Count < value.emptyLockerCount || minionIDsWithEmptyLockerReservations.Contains(minionInstanceID);
	}
	...
	{
		return (cell % WidthInCells > 0) ? (cell - 1) : InvalidCell;
	}

	public static int CellRight(int cell)
	{
		return (cell % WidthInCells < WidthInCells - 1) ? (cell + 1) : InvalidCell;
	}
	...
	{
		return IsValidCell(cell) && WorldIdx[cell] != byte.MaxValue;
	}
	...
	{
		return cell >= 0 && cell < CellCount && WorldIdx[cell] == world;
	}
	...
	public static bool IsCellOpenToSpace(int cell)
	{
		...
		GameObject gameObject = Objects[cell, 2];
		if (gameObject != null)
		{
			return false;
		}
	...
	{
		return IsValidCell(cell) && IsSubstantialLiquidUnsafe(cell, threshold);
	}
	...
	{
		return IsValidCell(cell) && IsNavigatableLiquidUnsafe(cell);
	}
