	...
	public class EGG_CHANCE_MODIFIERS
	{
		...
		private static System.Action CreateElementCreatureModifier(string id, Tag eggTag, Tag element, float modifierPerSecond, bool alsoInvert, bool checkSubstantialLiquid, string tooltipOverride = null)
		{
			return delegate
			{
				...
				Db.Get().CreateFertilityModifier(id, eggTag, name, description, (string descStr) => (tooltipOverride != null) ? tooltipOverride : string.Format(descStr, ElementLoader.GetElement(element).name), delegate(FertilityMonitor.Instance inst, Tag eggType)
				{
					...
					instance.OnUpdateEggChances += delegate(float dt)
					{
						...
						if (Grid.IsValidCell(num))
						{
							...
							if (element2.HasTag(element) && (!checkSubstantialLiquid || Grid.IsSubstantialLiquid(num)))
							{
								inst.AddBreedingChance(eggType, dt * modifierPerSecond);
							}
							else if (alsoInvert)
