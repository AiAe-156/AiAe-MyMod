...
public class MotdDataFetchRequest : IDisposable
{
	...
	public static Texture2D ReadCachedMotdImage(string url)
	{
		...
		try
		{
			byte[] buffer = File.ReadAllBytes(cachedFilePath);
	...
	public static Texture2D ParseImage(byte[] buffer)
	{
		...
		{
			return array[0] == byte.MaxValue && array[1] == 216 && array[6] == 74 && array[7] == 70 && array[8] == 73 && array[9] == 70;
		}
		static bool IsKleiTex(byte[] array)
		{
			return array[0] == 75 && array[1] == 84 && array[2] == 69 && array[3] == 88;
		}
		static bool IsPng(byte[] array)
		{
			return array[0] == 137 && array[1] == 80 && array[2] == 78 && array[3] == 71 && array[4] == 13 && array[5] == 10 && array[6] == 26 && array[7] == 10;
		}
	}
	...
	public static void GetUrlParams(out string platformCode, out string languageCode)
	{
		...
		Localization.Language? language2 = language;
		if (language2.HasValue && language2.GetValueOrDefault() == Localization.Language.Chinese)
		{
