...
public class CreatureSimTemperatureTransfer : SimTemperatureTransfer, ISim200ms
{
	...
	private float lastTemperatureRecordTime = 0f;

	public bool LastTemperatureRecordIsReliable => Time.time - lastTemperatureRecordTime < 2f && average_kilowatts_exchanged.ValidRecordsInLastSeconds(4f) > 5;
	...
	protected unsafe void unsafeUpdateAverageKiloWattsExchanged(float dt)
	{
		if (!(Time.time < lastTemperatureRecordTime + 0.2f) && Sim.IsValidHandle(simHandle))
		{
			...
			float num = Game.Instance.simData.elementChunks[handleIndex].deltaKJ;
			if (num != 0f)
			{
	...
	public void RefreshRegistration()
	{
		SimUnregister();
		Attributes attributes = base.gameObject.GetAttributes();
		AttributeInstance attributeInstance = attributes.Get(Db.Get().Attributes.ThermalConductivityBarrier);
