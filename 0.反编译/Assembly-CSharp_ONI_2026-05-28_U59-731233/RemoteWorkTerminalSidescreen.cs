...
public class RemoteWorkTerminalSidescreen : SideScreenContent
{
	...
	private void RefreshOptions(object data = null)
	{
		...
		List<RemoteWorkerDock> items = Components.RemoteWorkerDocks.GetItems(targetTerminal.GetMyWorldId());
		foreach (RemoteWorkerDock item2 in items)
		{
			...
			SetRow(num++, UI.StripLinkFormatting(item2.GetProperName()), Def.GetUISprite(item2.gameObject)?.first, item2);
		}
