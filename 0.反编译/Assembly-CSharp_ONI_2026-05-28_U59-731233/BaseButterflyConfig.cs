...
public static class BaseButterflyConfig
{
	...
	public static GameObject BaseButterfly(string id, string name, string desc, string anim_file, string traitId, string symbolOverridePrefix = null)
	{
		...
		PollinateMonitor.Def def = gameObject.AddOrGetDef<PollinateMonitor.Def>();
		def.radius = 10;
		OvercrowdingMonitor.Def def2 = gameObject.AddOrGetDef<OvercrowdingMonitor.Def>();
		def2.spaceRequiredPerCreature = CREATURES.SPACE_REQUIREMENTS.TIER2;
		LureableMonitor.Def def3 = gameObject.AddOrGetDef<LureableMonitor.Def>();
		def3.lures = new Tag[2]
		{
			GameTags.Algae,
