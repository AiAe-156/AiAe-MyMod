...
public class HelmetController : KMonoBehaviour
{
	...
	{
		if (has_jets && !(jet_anim != null))
		{
			jet_anim = AddTrackedAnim("jet", Assets.GetAnim("jetsuit_thruster_fx_kanim"), "loop", Grid.SceneLayer.Creatures, "snapTo_neck", require_looping_sound: true);
			glow_anim = AddTrackedAnim("glow", Assets.GetAnim("jetsuit_thruster_glow_fx_kanim"), "loop", Grid.SceneLayer.Front, "snapTo_neck");
		}
	}
	...
	private void DisableJets()
	{
		if (has_jets)
		{
			if (jet_anim != null)
			{
				Object.Destroy(jet_anim.gameObject);
				jet_anim = null;
			}
			if (glow_anim != null)
			{
				Object.Destroy(glow_anim.gameObject);
				glow_anim = null;
			}
		}
	}
