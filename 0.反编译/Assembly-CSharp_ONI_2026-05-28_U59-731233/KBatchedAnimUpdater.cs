...
public class KBatchedAnimUpdater : Singleton<KBatchedAnimUpdater>
{
	...
	public event System.Action OnClear = null;
	...
	public void UpdateRegister(KBatchedAnimController controller)
	{
		...
		{
			List<KBatchedAnimController> list = ((controller.visibilityType == KAnimControllerBase.VisibilityType.Always) ? alwaysUpdateList : updateList);
			list.Add(controller);
			controller.updateRegistrationState = RegistrationState.Registered;
			break;
		}
	...
	private static void UpdateRegisteredAnims(List<KBatchedAnimController> list, float dt)
	{
		...
		{
			KBatchedAnimController kBatchedAnimController = list[num];
			if (kBatchedAnimController == null)
			{
				list[num] = list[list.Count - 1];
				list.RemoveAt(list.Count - 1);
			}
			else if (kBatchedAnimController.updateRegistrationState != RegistrationState.Registered)
			{
				kBatchedAnimController.updateRegistrationState = RegistrationState.Unregistered;
				list[num] = list[list.Count - 1];
				list.RemoveAt(list.Count - 1);
			}
		...
		for (int i = 0; i < queuedRegistrations.Count; i++)
		{
			...
			if (registrationInfo.register)
			{
				if (!(registrationInfo.controller == null))
				{
					...
					ControllerChunkInfo value = new ControllerChunkInfo
					{
					...
					if (Singleton<CellChangeMonitor>.Instance != null)
					{
						...
						registrationInfo.controller.movementChangedHandlerId = Singleton<CellChangeMonitor>.Instance.RegisterMovementStateChanged(registrationInfo.controller.transform, onMovementStateChangedAction, registrationInfo.controller);
					}
					Dictionary<int, KBatchedAnimController> controllerMap = GetControllerMap(value.chunkXY);
					if (controllerMap != null)
					{
	...
	public void OnMovementStateChanged(Transform transform, bool is_moving, KBatchedAnimController controller)
	{
		...
		if (is_moving)
		{
			if (movingControllerInfos.ContainsKey(instanceID))
			{
				DebugUtil.DevAssertArgs(!movingControllerInfos.ContainsKey(instanceID), "Readding controller which is already moving", controller.name, value.chunkXY, movingControllerInfos.ContainsKey(instanceID) ? movingControllerInfos[instanceID].chunkXY.ToString() : null);
			}
			movingControllerInfos[instanceID] = new MovingControllerInfo
			{
				controllerInstanceId = instanceID,
