...
public class CircuitManager
{
	...
	public void Sim200msLast(float dt)
	{
		...
		for (int i = 0; i < circuitInfo.Count; i++)
		{
			...
			if (!flag)
			{
				...
				{
					Generator generator2 = outputTransformers[num2];
			...
			else if (flag2)
			{
				...
				{
					IEnergyConsumer energyConsumer2 = list2[num10];
			...
			else
			{
				...
				{
					IEnergyConsumer energyConsumer3 = list2[num11];
		...
		for (int num12 = 0; num12 < circuitInfo.Count; num12++)
		{
			...
			{
				Battery battery3 = value2.batteries[num13];
			...
			{
				Battery battery4 = value2.inputTransformers[num14];
	...
	public float GetWattsNeededWhenActive(ushort originCircuitId)
	{
		...
		foreach (ushort item3 in hashSet4)
		{
	...
	public float GetMaxSafeWattageForCircuit(ushort circuitID)
	{
		...
		return (Game.Instance.electricalConduitSystem.GetNetworkByID(circuitID) is ElectricalUtilityNetwork electricalUtilityNetwork) ? electricalUtilityNetwork.GetMaxSafeWattage(circuitInfo[circuitID].bridgeGroups) : 0f;
	}
}
