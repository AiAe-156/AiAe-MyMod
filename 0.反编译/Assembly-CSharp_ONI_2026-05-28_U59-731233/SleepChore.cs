...
public class SleepChore : Chore<SleepChore.StatesInstance>
{
	public class StatesInstance : GameStateMachine<States, StatesInstance, SleepChore, object>.GameInstance
	{
		...
		{
			GameObject go = base.sm.sleeper.Get(base.smi);
			ExternalTemperatureMonitor.Instance sMI = go.GetSMI<ExternalTemperatureMonitor.Instance>();
			if (sMI.IsTooCold())
			{
		...
		{
			GameObject gameObject = base.sm.sleeper.Get(base.smi);
			if (gameObject.GetComponent<GlowStick>() != null)
			{
				return true;
			}
			return false;
		...
		public void SetAnim()
		{
			...
			if (sleepable.GetComponent<Building>() == null)
			{
				string text = base.sm.sleeper.Get<Navigator>(base.smi).CurrentNavType switch
				{
					...
					NavType.Swim => "anim_sleep_swim_kanim", 
	...
	public static bool IsDarkAtCell(int cell)
	{
		...
		return num < DUPLICANTSTATS.STANDARD.Light.LOW_LIGHT;
	}
}
