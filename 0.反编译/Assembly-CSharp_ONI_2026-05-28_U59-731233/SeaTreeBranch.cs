...
public class SeaTreeBranch : PlantBranchGrowerBase<SeaTreeBranch, SeaTreeBranch.Instance, IStateMachineTarget, SeaTreeBranch.Def>
{
	...
	public new class Instance : GameInstance, IManageGrowingStates, IWiltCause
	{
		...
		private bool IsRootWilting => RootSMI != null && RootSMI.IsWilting;

		public bool IsWilting => wiltCondition.IsWilting() || IsRootWilting;
		...
		public void SpawnCritter(bool wasHarvestedByDupe)
		{
			...
			GameObject gameObject = component.SpawnAndGetConfiguredFruit(null, wasHarvestedByDupe);
			bool symbolVisible;
			...
			if (gameObject != null)
			{
				...
				component2.Temperature = base.gameObject.GetComponent<PrimaryElement>().Temperature;
			}
			else
			{
		...
		private GameObject SpawnBranchOnCell(int cell)
		{
			...
			sMI.SetupFromPreviousBranchInformation(this);
			return gameObject;
		}
	...
	public static bool CanGrowOnCell(GameObject questionerObj, int cell)
	{
		...
		return cell != Grid.InvalidCell && Grid.WorldIdx[cell] == num2 && Grid.IsLiquid(cell) && Grid.Objects[cell, 1] == null && Grid.Objects[cell, 5] == null;
	}
}
