...
public class CarePackageContainer : KScreen, ITelepadDeliverableContainer
{
	...
	private void SetAnimator()
	{
		...
		int num = ((ElementLoader.FindElementByName(info.id) != null) ? 1 : ((foodInfo == null || !(foodInfo.CaloriesPerUnit > 0f)) ? ((int)info.quantity) : ((int)Mathf.Max(1f, info.quantity % foodInfo.CaloriesPerUnit))));
		if (prefab != null)
		{
			for (int i = 0; i < num; i++)
			{
	...
	private string GetCurrentQuantity(WorldInventory inventory)
	{
		...
		if (foodInfo != null && foodInfo.CaloriesPerUnit > 0f)
		{
			float calories = WorldResourceAmountTracker<RationTracker>.Get().CountAmountForItemWithID(info.id, inventory);
	...
	private string GetSpawnableQuantity()
	{
		...
		string format = UI.IMMIGRANTSCREEN.CARE_PACKAGE_ELEMENT_COUNT;
		string properName = Assets.GetPrefab(info.id).GetProperName();
		float num = info.quantity;
		return string.Format(format, properName, num.ToString());
	}
	...
	private string GetSpawnableEffects()
	{
		...
		if (components != null)
		{
			...
			foreach (IGameObjectEffectDescriptor gameObjectEffectDescriptor in array)
			{
				List<Descriptor> descriptors = gameObjectEffectDescriptor.GetDescriptors(prefab);
				if (descriptors == null)
				{
					continue;
				}
				foreach (Descriptor item in descriptors)
				{
					text = text + item.text + "\n";
				}
			}
	...
	private void SetInfoText()
	{
		...
		effects.SetText(GetSpawnableEffects());
