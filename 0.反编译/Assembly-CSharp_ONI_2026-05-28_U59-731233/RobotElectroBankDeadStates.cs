...
public class RobotElectroBankDeadStates : GameStateMachine<RobotElectroBankDeadStates, RobotElectroBankDeadStates.Instance, IStateMachineTarget, RobotElectroBankDeadStates.Def>
{
	...
	public override void InitializeStates(out BaseState default_state)
	{
		...
		powerdown.landed.PlayAnim("power_down_pst").Enter(delegate(Instance smi)
		{
			...
			component.PauseSound(GlobalAssets.GetSound("Flydo_flying_LP"), paused: true);
		...
		{
			LoopingSounds component = smi.GetComponent<LoopingSounds>();
			component.PauseSound(GlobalAssets.GetSound("Flydo_flying_LP"), paused: false);
			Brain brain = smi.Get<Brain>();
			brain.Resume("power up");
		});
