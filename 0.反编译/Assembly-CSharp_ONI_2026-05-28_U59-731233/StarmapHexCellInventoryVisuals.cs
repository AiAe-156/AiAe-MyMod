...
public class StarmapHexCellInventoryVisuals : ClusterGridEntity
{
	...
	protected override void OnSpawn()
	{
		...
		{
			StarmapHexCellInventory starmapHexCellInventory = StarmapHexCellInventory.AllInventories[base.Location];
			starmapHexCellInventory.TransferAllItemsFromExternalInventory(inventory);
	...
	public void RefreshVisuals()
	{
		...
		if (animController != null)
		{
			...
			for (int i = 0; i < symbolAnimControllers.Length; i++)
			{
				...
				if (i < num)
				{
					StarmapHexCellInventory.SerializedItem serializedItem = inventory.Items[i];
