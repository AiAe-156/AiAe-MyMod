...
public class WorldGenSpawner : KMonoBehaviour
{
	public class Spawnable
	{
		...
		private void Spawn()
		{
			...
			if (gameObject != null && (bool)gameObject)
			{
				...
				gameObject.Trigger(1119167081, (object)spawnInfo);
			}
			FreeResources();
		}
	...
	public List<Spawnable> GeInfoOfUnspawnedWithType<T>(int worldID) where T : KMonoBehaviour
	{
	...
	public Spawnable GetSpawnableInCellWithTag(int cell, Tag tag)
	{
		return spawnables.Find((Spawnable s) => s.cell == cell && s.spawnInfo.id == tag);
	}

	public Spawnable GetSpawnableInCell(int cell)
	{
		return spawnables.Find((Spawnable s) => s.cell == cell);
	}
	...
	public List<Spawnable> GetSpawnablesWithTag(Tag tag, int worldID, bool includeSpawned = false)
	{
		List<Spawnable> list = new List<Spawnable>();
		foreach (Spawnable item in spawnables.FindAll((Spawnable match) => (includeSpawned || !match.isSpawned) && Grid.WorldIdx[match.cell] == worldID && match.spawnInfo.id == tag))
		{
	...
	private void PlaceTemplates(Cluster clusterLayout)
	{
		...
		foreach (WorldGen world in clusterLayout.worlds)
		{
			...
			foreach (Prefab backwallEntity in world.SpawnData.backwallEntities)
			{
				backwallEntity.location_x += world.data.world.offset.x;
				backwallEntity.location_y += world.data.world.offset.y;
				backwallEntity.type = Prefab.Type.Other;
				AddSpawnable(backwallEntity);
			}
			...
			world.SpawnData.backwallEntities.Clear();
