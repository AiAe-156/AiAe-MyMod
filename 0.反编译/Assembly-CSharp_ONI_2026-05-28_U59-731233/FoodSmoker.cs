...
public class FoodSmoker : GameStateMachine<FoodSmoker, FoodSmoker.StatesInstance, IStateMachineTarget, FoodSmoker.Def>
{
	...
	{
		Chore chore = new WorkChore<FoodSmokerWorkableEmpty>(Db.Get().ChoreTypes.Cook, smi.master.GetComponent<FoodSmokerWorkableEmpty>(), null, run_until_complete: true, smi.OnEmptyComplete, null, null, allow_in_red_alert: true, null, ignore_schedule_block: false, only_when_operational: false);
		chore.AddPrecondition(ChorePreconditions.instance.IsNotARobot);
		chore.AddPrecondition(ChorePreconditions.instance.HasSkillPerk, Db.Get().SkillPerks.CanGasRange.Id);
		return chore;
	}
}
