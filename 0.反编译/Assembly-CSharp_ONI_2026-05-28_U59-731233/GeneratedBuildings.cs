...
public class GeneratedBuildings
{
	public static void LoadGeneratedBuildings(List<Type> types)
	{
		...
		foreach (PlanScreen.PlanInfo item2 in BUILDINGS.PLANORDER)
		{
			...
			foreach (string entry in list2)
			{
				item2.buildingAndSubcategoryData.RemoveAll((KeyValuePair<string, string> match) => match.Key == entry);
			}
			List<string> list3 = new List<string>();
			foreach (string entry2 in item2.data)
			{
				if (item2.buildingAndSubcategoryData.FindIndex((KeyValuePair<string, string> x) => x.Key == entry2) == -1 && Assets.GetBuildingDef(entry2) != null)
				{
					Debug.LogWarning("Mod: Building '" + entry2 + "' was not added properly to PlanInfo, use ModUtil.AddBuildingToPlanScreen instead.");
					list3.Add(entry2);
				}
			}
	...
	public static void InitializeLogicPorts(GameObject go, BuildingDef def)
	{
		...
		{
			LogicPorts logicPorts = go.AddOrGet<LogicPorts>();
			logicPorts.inputPortInfo = def.LogicInputPorts.ToArray();
		}
		if (def.LogicOutputPorts != null)
		{
			LogicPorts logicPorts2 = go.AddOrGet<LogicPorts>();
			logicPorts2.outputPortInfo = def.LogicOutputPorts.ToArray();
		}
	}
