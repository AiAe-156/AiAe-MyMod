...
public class ElementSplitterComponents : KGameObjectComponentManager<ElementSplitter>
{
	...
	private static bool CanFirstAbsorbSecond(HandleVector<int>.Handle first, HandleVector<int>.Handle second)
	{
		...
		return elementSplitter.primaryElement.ElementID == elementSplitter2.primaryElement.ElementID && elementSplitter.primaryElement.Units + elementSplitter2.primaryElement.Units < 25000f && !elementSplitter.kPrefabID.HasTag(GameTags.MarkedForMove) && !elementSplitter2.kPrefabID.HasTag(GameTags.MarkedForMove);
	}
