	...
	private struct RefreshModule
	{
		...
		private readonly void CollectCondemnedCavities()
		{
			...
			foreach (int dirtyCell in dirtyCells)
			{
				...
				CavityInfo data = roomProber.cavityInfos.GetData(cell.cavityID);
				foreach (int cell2 in data.cells)
				{
		...
		private readonly void BuildNewCavities()
		{
			...
			{
				foreach (int cell2 in roomProber.cavityInfos.GetData(condemnedCavity2).cells)
				{
					dirtyCells.Add(cell2);
				}
			}
			...
			{
				Cell cell = roomProber.grid[dirtyCell];
				if (cell.generation == visited.Generation)
				{
					continue;
				}
