...
public class HarvestDesignatable : KMonoBehaviour
{
	...
	{
		base.OnSpawn();
		growingStateManager = GetComponent<IManageGrowingStates>();
		if (growingStateManager == null)
		{
			growingStateManager = base.gameObject.GetSMI<IManageGrowingStates>();
		}
	...
	private void CreateOverlayIcon()
	{
		if (!(HarvestWhenReadyOverlayIcon != null) && GetComponent<AttackableBase>() == null)
		{
			...
			TransformExtensions.SetPosition(position: (!component2.HasTag(GameTags.Hanging)) ? (new Vector3((float)(extents.x + extents.width / 2) + 0.5f, extents.y) + (Vector3)iconOffset) : (new Vector3((float)(extents.x + extents.width / 2) + 0.5f, extents.y + extents.height) + (Vector3)iconOffset), transform: HarvestWhenReadyOverlayIcon.transform);
			RefreshOverlayIcon();
		}
	}
	...
	public virtual void MarkForHarvest()
	{
		if (CanBeHarvested())
		{
			...
			component2.RemoveStatusItem(Db.Get().MiscStatusItems.NotMarkedForHarvest);
		}
	}
