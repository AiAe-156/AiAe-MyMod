...
public class DevToolCavity : DevTool
{
	...
	protected override void RenderTo(DevPanel panel)
	{
		...
		if (screenRect.IsSome() && screenRect2.IsSome())
		{
			...
			DevToolEntity.DrawBoundingBox(screenRect3, cavityForCell.room.IsNullOrDestroyed() ? "<Room is null>" : cavityForCell.room.GetProperName(), ImGui.IsWindowFocused());
			Option<(Vector2, Vector2)> screenRect4 = forSimCell.GetScreenRect();
			if (screenRect4.IsSome())
			{
				DevToolEntity.DrawBoundingBox(screenRect4.Unwrap(), forSimCell.GetDebugName(), ImGui.IsWindowFocused());
			}
		}
	}
