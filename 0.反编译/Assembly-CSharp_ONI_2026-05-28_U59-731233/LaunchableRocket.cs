	...
	public class States : GameStateMachine<States, StatesInstance, LaunchableRocket>
	{
		...
		public override void InitializeStates(out BaseState default_state)
		{
			...
			}).Update(delegate(StatesInstance smi, float dt)
			{
				...
				float num2 = 0f;
				float num3 = 0.5f;
				num2 = Mathf.Clamp(num3 * num, 0f, 10f) * dt;
				...
				foreach (GameObject part9 in smi.master.parts)
				{
					if (!(part9 == null))
					{
						KBatchedAnimController component2 = part9.GetComponent<KBatchedAnimController>();
						...
						DoWorldDamage(part9, positionIncludingOffset);
					}
				}
				if (flag)
				{
			...
			not_grounded.landing_loop.Enter(delegate(StatesInstance smi)
			{
				...
				if (num != -1)
				{
					smi.master.parts[num].Trigger(-1358394196);
				}
			}).Update(delegate(StatesInstance smi, float dt)
			{
				KBatchedAnimController component = smi.master.gameObject.GetComponent<KBatchedAnimController>();
				...
				foreach (GameObject part11 in smi.master.parts)
				{
					if (!(part11 == null))
					{
						KBatchedAnimController component2 = part11.GetComponent<KBatchedAnimController>();
						component2.Offset = Vector3.up * smi.master.flightAnimOffset;
						Vector3 positionIncludingOffset = component2.PositionIncludingOffset;
