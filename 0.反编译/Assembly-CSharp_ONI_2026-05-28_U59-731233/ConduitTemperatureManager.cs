...
public class ConduitTemperatureManager
{
	...
	public HandleVector<int>.Handle Allocate(ConduitType conduit_type, int conduit_idx, HandleVector<int>.Handle conduit_structure_temperature_handle, ref ConduitFlow.ConduitContents contents)
	{
		...
		HandleVector<int>.Handle result = new HandleVector<int>.Handle
		{
			index = num
		};
	...
	{
		IntPtr intPtr = ConduitTemperatureManager_Update(dt, (IntPtr)Game.Instance.simData.buildingTemperatures);
		ConduitTemperatureUpdateData* ptr = (ConduitTemperatureUpdateData*)(void*)intPtr;
