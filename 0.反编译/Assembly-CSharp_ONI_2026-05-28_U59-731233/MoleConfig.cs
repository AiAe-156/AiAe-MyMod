...
public class MoleConfig : IEntityConfig
{
	...
	{
		GameObject prefab = CreateMole("Mole", STRINGS.CREATURES.SPECIES.MOLE.NAME, STRINGS.CREATURES.SPECIES.MOLE.DESC, "driller_kanim");
		return EntityTemplates.ExtendEntityToFertileCreature(prefab, this as IHasDlcRestrictions, "MoleEgg", STRINGS.CREATURES.SPECIES.MOLE.EGG_NAME, STRINGS.CREATURES.SPECIES.MOLE.DESC, "egg_driller_kanim", MoleTuning.EGG_MASS, "MoleBaby", 60.000004f, 20f, MoleTuning.EGG_CHANCES_BASE, EGG_SORT_ORDER);
	}
	...
	public static void SetSpawnNavType(GameObject inst)
	{
		...
		if (component != null && (component2 == null || component2.storage == null))
		{
			if (Grid.IsSolidCell(cell))
			{
				...
				component3.SetSceneLayer(Grid.SceneLayer.FXFront);
			}
			else
			{
				KBatchedAnimController component4 = inst.GetComponent<KBatchedAnimController>();
				component4.SetSceneLayer(Grid.SceneLayer.Creatures);
			}
		}
	}
}
