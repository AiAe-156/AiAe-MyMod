...
public class BuildMenuBuildingsScreen : KIconToggleMenu
{
	...
	public void Configure(HashedString category, IList<BuildMenu.BuildingInfo> building_infos)
	{
		...
		Setup(list);
		for (int num = 0; num < toggleInfo.Count; num++)
		{
			RefreshToggle(toggleInfo[num]);
		}
		int num2 = 0;
		...
		gridSizer.constraintCount = Mathf.Min(num2, 3);
		int num3 = Mathf.Min(num2, gridSizer.constraintCount);
		int num4 = (num2 + gridSizer.constraintCount - 1) / gridSizer.constraintCount;
		int num5 = num3 - 1;
		int num6 = num4 - 1;
		Vector2 vector = new Vector2((float)num3 * gridSizer.cellSize.x + (float)num5 * gridSizer.spacing.x + (float)gridSizer.padding.left + (float)gridSizer.padding.right, (float)num4 * gridSizer.cellSize.y + (float)num6 * gridSizer.spacing.y + (float)gridSizer.padding.top + (float)gridSizer.padding.bottom);
	...
	private void RefreshToggle(ToggleInfo info)
	{
		...
		else
		{
			...
			if (!flag)
			{
				Color color2 = (image.color = new Color(1f, 1f, 1f, 0.15f));
				color4 = color2;
			}
			else
			{
