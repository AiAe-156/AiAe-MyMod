...
public class RangeVisualizerEffect : MonoBehaviour
{
	...
	private int LastVisibleTileCount = 0;
	...
	private void OnPostRender()
	{
		...
		{
			for (int num5 = 0; num5 <= rangeMax.y - rangeMin.y; num5++)
			{
				int num6 = vector2I3.y + rangeMin.y + num5;
				for (int num7 = 0; num7 <= rangeMax.x - rangeMin.x; num7++)
				{
					int num8 = vector2I3.x + rangeMin.x + num7;
					int arg = Grid.XYToCell(num8, num6);
					bool flag2 = num8 > world_min.x && num8 < world_max.x && num6 > world_min.y && num6 < world_max.y && rangeVisualizer.BlockingCb(arg);
					pixelData[num5 * width + num7] = (byte)((!flag2) ? 255u : 0u);
					if (!flag2)
					{
						num++;
					}
