...
public class MinionTodoChoreEntry : KMonoBehaviour
{
	...
	public void Apply(Chore.Precondition.Context context)
	{
		...
		if (choreGroup != null)
		{
			icon.sprite = Assets.GetSprite(choreGroup.sprite);
		}
		else
		{
			icon.sprite = null;
			...
			if (component != null)
			{
				icon.sprite = Db.Get().Personalities.Get(component.personalityResourceId).GetMiniIcon();
			}
		}
		...
		componentInChildren.GetComponentInChildren<KButton>().onClick += delegate
		{
			...
			{
				Vector3 position = new Vector3(context.chore.target.gameObject.transform.position.x, context.chore.target.gameObject.transform.position.y + 1f, CameraController.Instance.transform.position.z);
				GameUtil.FocusCamera(position);
			}
		};
	}
