	...
	private void SpawnRow(ColonyDiagnostic diagnostic, GameObject container)
	{
		...
		reference.onClick = (System.Action)Delegate.Combine(reference.onClick, (System.Action)delegate
		{
			...
			else
			{
				...
				int num2 = (int)ColonyDiagnosticUtility.Instance.diagnosticDisplaySettings[activeWorldId][id2];
				int num3 = num2 - 1;
				if (num3 < 0)
				{
					num3 = 2;
				}
				ColonyDiagnosticUtility.Instance.diagnosticDisplaySettings[activeWorldId][id2] = (ColonyDiagnosticUtility.DisplaySetting)num3;
			}
			RefreshRows();
