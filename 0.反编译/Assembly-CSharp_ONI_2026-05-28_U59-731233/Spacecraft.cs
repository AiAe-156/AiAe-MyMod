...
public class Spacecraft
{
	...
	[Serialize]
	public float controlStationBuffTimeRemaining = 0f;
	...
	private float GetPilotNavigationEfficiency()
	{
		...
		if (!component.robotPilotControlled)
		{
			MinionStorage component2 = launchConditions.GetComponent<MinionStorage>();
			List<MinionStorage.Info> storedMinionInfo = component2.GetStoredMinionInfo();
			if (storedMinionInfo.Count < 1)
			{
				return 1f;
			}
			StoredMinionIdentity component3 = storedMinionInfo[0].serializedMinion.Get().GetComponent<StoredMinionIdentity>();
			string text = Db.Get().Attributes.SpaceNavigation.Id;
			foreach (KeyValuePair<string, bool> item in component3.MasteryBySkillID)
			{
				foreach (SkillPerk perk in Db.Get().Skills.Get(item.Key).perks)
				{
					if (Game.IsCorrectDlcActiveForCurrentSave(perk) && perk is SkillAttributePerk skillAttributePerk && skillAttributePerk.modifier.AttributeId == text)
					{
	...
	private void Land()
	{
		launchConditions.Trigger(-1165815793, (object)SpacecraftManager.instance.GetSpacecraftDestination(id));
		foreach (GameObject item in AttachableBuilding.GetAttachedNetwork(launchConditions.GetComponent<AttachableBuilding>()))
		{
			if (item != launchConditions.gameObject)
			{
				item.Trigger(-1165815793, (object)SpacecraftManager.instance.GetSpacecraftDestination(id));
			}
		}
	}
