...
public class SafeCellSensor : Sensor
{
	...
	public int RunAndGetSafeCellQueryResult(bool avoid_light)
	{
		...
		SwimMonitor.Instance sMI = brain.GetSMI<SwimMonitor.Instance>();
		PathFinderQuery pathFinderQuery;
		if (sMI != null && sMI.CanSwim())
		{
			float currentBreathValue = Db.Get().Amounts.Breath.Lookup(brain.gameObject)?.value ?? 100f;
			pathFinderQuery = PathFinderQueries.swimSafeCellQuery.Reset(brain, avoid_light, GetIgnoredFlags(), currentBreathValue);
		}
		else
		{
			pathFinderQuery = PathFinderQueries.safeCellQuery.Reset(brain, avoid_light, GetIgnoredFlags());
		}
