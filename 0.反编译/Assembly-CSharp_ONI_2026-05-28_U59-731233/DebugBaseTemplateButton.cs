...
public class DebugBaseTemplateButton : KScreen
{
	...
	private TemplateContainer GetSelectionAsAsset()
	{
		...
		for (int j = 0; j < Components.BuildingCompletes.Count; j++)
		{
			...
			KPrefabID component3 = buildingComplete.GetComponent<KPrefabID>();
			if (component2 != null)
			{
			...
			float num5 = 0f;
			Battery component4 = buildingComplete.GetComponent<Battery>();
			if (component4 != null)
			{
				num5 = component4.JoulesAvailable;
				list5.Add(new Prefab.template_amount_value("joulesAvailable", num5));
			}
			float num6 = 0f;
			Unsealable component5 = buildingComplete.GetComponent<Unsealable>();
			if (component5 != null)
			{
				num6 = (component5.facingRight ? 1 : 0);
				list5.Add(new Prefab.template_amount_value("sealedDoorDirection", num6));
			}
			float num7 = 0f;
			LogicSwitch component6 = buildingComplete.GetComponent<LogicSwitch>();
			if (component6 != null)
			{
				num7 = (component6.IsSwitchedOn ? 1 : 0);
				list5.Add(new Prefab.template_amount_value("switchSetting", num7));
			}
			int connections = 0;
			IHaveUtilityNetworkMgr component7 = buildingComplete.GetComponent<IHaveUtilityNetworkMgr>();
			if (component7 != null)
			{
				connections = (int)component7.GetNetworkManager().GetConnections(num3, is_physical_building: true);
			}
			string facadeIdId = null;
			BuildingFacade component8 = buildingComplete.GetComponent<BuildingFacade>();
			if (component8 != null)
			{
				facadeIdId = component8.CurrentFacade;
			}
			x3 -= rootX;
			y3 -= rootY;
			...
			LoreBearer component9 = buildingComplete.GetComponent<LoreBearer>();
			if (component9 != null && !string.IsNullOrEmpty(component9.poiOverrideLoreUnlockId))
			{
				prefab.loreUnlockId = component9.poiOverrideLoreUnlockId;
				prefab.loreDisplayText = component9.poiOverrideLoreDisplayText;
				prefab.loreNextCollectionId = component9.poiOverrideNextCollectionId;
			}
			Storage component10 = buildingComplete.gameObject.GetComponent<Storage>();
			if (component10 != null)
			{
				foreach (GameObject item2 in component10.items)
				{
					float units = 0f;
					...
					bool isOre = false;
					PrimaryElement component11 = item2.GetComponent<PrimaryElement>();
					if (component11 != null)
					{
						units = component11.Units;
						element2 = component11.ElementID;
						temp = component11.Temperature;
						disease = ((component11.DiseaseIdx != byte.MaxValue) ? Db.Get().Diseases[component11.DiseaseIdx].Id : null);
						disease_count2 = component11.DiseaseCount;
					}
					Rottable.Instance sMI = item2.gameObject.GetSMI<Rottable.Instance>();
					ElementChunk component12 = item2.GetComponent<ElementChunk>();
					if (component12 != null)
					{
						isOre = true;
					}
		...
		for (int num8 = 0; num8 < Components.Pickupables.Count; num8++)
		{
			...
			if ((SaveAllPickups || SelectedCells.Contains(num9)) && !Components.Pickupables[num8].gameObject.GetComponent<MinionBrain>())
			{
				...
				PrimaryElement component13 = pickupable.gameObject.GetComponent<PrimaryElement>();
				if (component13 != null)
				{
					element3 = component13.ElementID;
					units2 = component13.Units;
					temperature = component13.Temperature;
					disease2 = ((component13.DiseaseIdx != byte.MaxValue) ? Db.Get().Diseases[component13.DiseaseIdx].Id : null);
					disease_count3 = component13.DiseaseCount;
				}
				ElementChunk component14 = pickupable.gameObject.GetComponent<ElementChunk>();
				if (component14 != null)
				{
	...
	private void GetEntities<T>(IEnumerable<T> component_collection, int rootX, int rootY, ref List<Prefab> _primaryElementOres, ref List<Prefab> _otherEntities, ref HashSet<GameObject> _excludeEntities)
	{
		foreach (T item2 in component_collection)
		{
			...
			ElementChunk component3 = (item2 as KMonoBehaviour).gameObject.GetComponent<ElementChunk>();
			...
			LoreBearer component4 = (item2 as KMonoBehaviour).gameObject.GetComponent<LoreBearer>();
			if (component4 != null && !string.IsNullOrEmpty(component4.poiOverrideLoreUnlockId))
			{
				item.loreUnlockId = component4.poiOverrideLoreUnlockId;
				item.loreDisplayText = component4.poiOverrideLoreDisplayText;
				item.loreNextCollectionId = component4.poiOverrideNextCollectionId;
			}
	...
	private void OnClickSaveBase()
	{
		...
		{
			TemplateContainer template = TemplateCache.GetTemplate(SaveName);
			int num = CountLoreOverrides(template);
			int num2 = CountLoreOverrides(asset);
			if (num2 < num)
			{
				ConfirmDialogScreen confirmDialogScreen = Util.KInstantiateUI<ConfirmDialogScreen>(ScreenPrefabs.Instance.ConfirmDialogScreen.gameObject, GameScreenManager.Instance.ssOverlayCanvas.gameObject, force_active: true);
				confirmDialogScreen.PopupConfirmDialog(UI.DEBUG_TOOLS.SAVE_BASE_TEMPLATE.LORE_OVERRIDE_WARNING.Replace("{newCount}", num2.ToString()).Replace("{oldCount}", num.ToString()), delegate
				{
