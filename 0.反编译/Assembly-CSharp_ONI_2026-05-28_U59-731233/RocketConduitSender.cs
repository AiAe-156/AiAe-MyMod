...
public class RocketConduitSender : StateMachineComponent<RocketConduitSender.StatesInstance>, ISecondaryInput
{
	...
	public class States : GameStateMachine<States, StatesInstance, RocketConduitSender>
	{
		...
		public override void InitializeStates(out BaseState default_state)
		{
			...
			{
				AutoStorageDropper.Instance sMI = smi.gameObject.GetSMI<AutoStorageDropper.Instance>();
				sMI.SetInvertElementFilter(value: true);
			}).UpdateTransition(on.working.ground, delegate(StatesInstance smi, float f)
			{
				WorldContainer myWorld = smi.master.GetMyWorld();
				if ((bool)myWorld && myWorld.IsModuleInterior)
				{
					Clustercraft component = myWorld.GetComponent<Clustercraft>();
					PassengerRocketModule passengerModule = component.ModuleInterface.GetPassengerModule();
					return !passengerModule.HasTag(GameTags.RocketNotOnGround);
				}
				return false;
	...
	public void RemoveConduitPortFromNetwork()
	{
		...
		{
			IUtilityNetworkMgr networkManager = Conduit.GetNetworkManager(conduitPortInfo.conduitType);
			networkManager.RemoveFromNetworks(conduitPort.inputCell, conduitPort.networkItem, is_endpoint: true);
		}
	}
	...
	private void FindPartner()
	{
		...
		if (world != null && world.IsModuleInterior)
		{
			...
			PassengerRocketModule passengerModule = component.ModuleInterface.GetPassengerModule();
			RocketConduitReceiver[] components = passengerModule.GetComponents<RocketConduitReceiver>();
			RocketConduitReceiver[] array = components;
			foreach (RocketConduitReceiver rocketConduitReceiver in array)
			{
		...
		{
			ClustercraftExteriorDoor component2 = base.gameObject.GetComponent<ClustercraftExteriorDoor>();
			if (component2.HasTargetWorld())
			{
				WorldContainer targetWorld = component2.GetTargetWorld();
				List<RocketConduitReceiver> worldItems = Components.RocketConduitReceivers.GetWorldItems(targetWorld.id);
				foreach (RocketConduitReceiver item in worldItems)
				{
					if (item.conduitPortInfo.conduitType == conduitPortInfo.conduitType)
					{
						partnerReceiver = item;
						break;
					}
				}
			}
