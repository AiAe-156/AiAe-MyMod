...
public static class BaseSeaHorseConfig
{
	...
	public static GameObject CreatePrefab(string id, string base_trait_id, string name, string description, string anim_file, bool is_baby, string symbol_prefix, float warnLowTemp, float warnHighTemp, float lethalLowTemp, float lethalHighTemp)
	{
		...
		{
			new Diet.Info(hashSet, produced_element, CALORIES_PER_KG_OF_PEARL, TUNING.CREATURES.CONVERSION_EFFICIENCY.NORMAL)
		};
		Diet diet = new Diet(list.ToArray());
		CreatureCalorieMonitor.Def def3 = gameObject.AddOrGetDef<CreatureCalorieMonitor.Def>();
		def3.diet = diet;
		def3.minConsumedCaloriesBeforePooping = CALORIES_PER_KG_OF_PEARL * MIN_POOP_SIZE_IN_KG;
		...
		if (!is_baby)
		{
			FertilityShearable.Def def6 = gameObject.AddOrGetDef<FertilityShearable.Def>();
			def6.dropMass = 100f;
			def6.milkElement = SimHashes.FishMilk;
			def6.minimumFertility = 75f;
			def6.percentFertilityConsumedPerMilking = 0.5f;
			def6.requiresHappy = true;
			def6.suppressedByElderly = true;
		}
		return gameObject;
	}
