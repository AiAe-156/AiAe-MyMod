...
public static class BaseMinionConfig
{
	...
	{
		string text = $"ui_duplicant_{model.ToString().ToLower()}_selection";
	...
	public static GameObject BaseMinion(Tag model, string[] minionAttributes, string[] minionAmounts, AttributeModifier[] minionTraits)
	{
		...
		saveLoadRoot.associatedTag = MinionConfig.ID;
		...
		snapOn.snapPoints = new List<SnapOn.SnapPoint>(new SnapOn.SnapPoint[20]
		{
	...
	public static void BasePrefabInit(GameObject go, Tag duplicantModel)
	{
		...
		amountInstance2.value = 5f;
		AmountInstance amountInstance3 = Db.Get().Amounts.Temperature.Lookup(go);
		amountInstance3.value = statsFor.Temperature.Internal.IDEAL;
		AmountInstance amountInstance4 = Db.Get().Amounts.Breath.Lookup(go);
		amountInstance4.value = amountInstance4.GetMax();
	}
	...
	public static void BaseOnSpawn(GameObject go, Tag duplicantModel, Func<RationalAi.Instance, StateMachine.Instance>[] rationalAiSM)
	{
		...
		StateMachineController component3 = go.GetComponent<StateMachineController>();
		RationalAi.Instance instance = new RationalAi.Instance(component3, duplicantModel);
		instance.stateMachinesToRunWhenAlive = rationalAiSM;
		instance.StartSM();
		Navigator component4 = go.GetComponent<Navigator>();
		component4.transitionDriver.overrideLayers.Add(new BipedTransitionLayer(component4, 3.325f, 2.5f));
		component4.transitionDriver.overrideLayers.Add(new DoorTransitionLayer(component4));
		component4.transitionDriver.overrideLayers.Add(new TubeTransitionLayer(component4));
		component4.transitionDriver.overrideLayers.Add(new LadderDiseaseTransitionLayer(component4));
		component4.transitionDriver.overrideLayers.Add(new ReactableTransitionLayer(component4));
		component4.transitionDriver.overrideLayers.Add(new NavTeleportTransitionLayer(component4));
		component4.transitionDriver.overrideLayers.Add(new SplashTransitionLayer(component4));
		component4.transitionDriver.overrideLayers.Add(new BipedSwimTransitionLayer(component4));
	}
	...
	{
		int num = table.Sum((CellOffset[] row) => row.Length);
		CellOffset[] array = new CellOffset[num];
		int num2 = 0;
		foreach (CellOffset[] array2 in table)
		{
			CellOffset[] array3 = array2;
			foreach (CellOffset cellOffset in array3)
			{
				array[num2] = cellOffset;
				num2++;
			}
		}
		return array;
	...
	public static void SetupDreams(GameObject prefab)
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			kPrefabID.PrefabTag = new Tag(dream.id);
