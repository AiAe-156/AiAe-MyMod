...
public class ValveBase : KMonoBehaviour, ISaveLoadable
{
	...
	private void ConduitUpdate(float dt)
	{
		...
		if (num > 0f)
		{
			...
			if (num2 > 0f)
			{
				if (lastElementTransfered != contents.element && contents.element != SimHashes.Vacuum && conduitType == ConduitType.Liquid)
				{
					Element element = ElementLoader.FindElementByHash(contents.element);
					if (element != null)
					{
						GameUtil.TintLiquidSymbolOnBuilding("water_color", controller, element);
					}
				}
				lastElementTransfered = contents.element;
