...
public class QuestManager : KMonoBehaviour
{
	...
	public static QuestInstance InitializeQuest(Tag ownerId, Quest quest)
	{
		...
		{
			Dictionary<HashedString, QuestInstance> dictionary = instance.ownerToQuests[ownerId.GetHash()];
			QuestInstance questInstance = (dictionary[quest.IdHash] = new QuestInstance(quest));
			qInst = questInstance;
		}
	...
	public static QuestInstance InitializeQuest(HashedString ownerId, Quest quest)
	{
		...
		{
			Dictionary<HashedString, QuestInstance> dictionary = instance.ownerToQuests[ownerId.HashValue];
			QuestInstance questInstance = (dictionary[quest.IdHash] = new QuestInstance(quest));
			qInst = questInstance;
		}
		qInst.Initialize(quest);
