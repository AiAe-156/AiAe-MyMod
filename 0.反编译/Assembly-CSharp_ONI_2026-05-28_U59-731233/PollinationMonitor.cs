...
public class PollinationMonitor : GameStateMachine<PollinationMonitor, PollinationMonitor.StatesInstance, IStateMachineTarget, PollinationMonitor.Def>
{
	...
	public class StatesInstance : GameInstance, IWiltCause
	{
		...
		public string WiltStateString => IsInsideState(base.sm.not_pollinated) ? Db.Get().CreatureStatusItems.NotPollinated.GetName(this) : "";
		...
		public override void StartSM()
		{
			...
			if (base.sm.spawn_pollinated.Get(this))
			{
				...
				if (effects != null)
				{
					EffectInstance effectInstance = effects.Add(INITIALLY_POLLINATED_EFFECT, should_save: true);
					effectInstance.timeRemaining *= UnityEngine.Random.Range(0.75f, 1f);
				}
			}
		}
	}
