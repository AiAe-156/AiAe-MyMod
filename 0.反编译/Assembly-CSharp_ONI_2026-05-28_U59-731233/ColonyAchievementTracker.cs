...
public class ColonyAchievementTracker : KMonoBehaviour, ISaveLoadableDetails, IRenderEveryTick
{
	...
	[Serialize]
	public bool efficientlyGatheredData = false;

	[Serialize]
	public bool fullyBoostedBionic = false;

	[Serialize]
	public int deadDupeCounter = 0;

	[Serialize]
	private int geothermalProgress = 0;

	[Serialize]
	public LargeImpactorState largeImpactorState = LargeImpactorState.Alive;
	...
	[Serialize]
	public int minnowQuestsCompleted = 0;

	[Serialize]
	public bool allMinnowQuestsCompleted = false;
	...
	public void LogSuitChore(ChoreDriver driver)
	{
		...
		foreach (EquipmentSlotInstance slot in equipment.Slots)
		{
			...
			if ((bool)equippable)
			{
				...
				if (component.IsAnyPrefabID(SuitTags))
				{
					flag = true;
					break;
				}
	...
	public void OnNewDay(object data)
	{
		...
		foreach (WorldContainer worldContainer in ClusterManager.Instance.WorldContainers)
		{
			...
			if (component.Status != Clustercraft.CraftStatus.Grounded)
			{
				...
				{
					float totalValue = Db.Get().Attributes.QualityOfLife.Lookup(item2).GetTotalValue();
					if (totalValue < minimumMorale)
					{
						flag = false;
						break;
					}
