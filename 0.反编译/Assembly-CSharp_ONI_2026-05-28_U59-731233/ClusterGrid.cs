...
public class ClusterGrid
{
	...
	public StarmapHexCellInventory AddOrGetHexCellInventory(AxialI cell)
	{
		...
		if (!StarmapHexCellInventory.AllInventories.TryGetValue(cell, out value))
		{
			GameObject prefab = Assets.GetPrefab("StarmapHexCellInventory");
			StarmapHexCellInventoryVisuals component = Util.KInstantiate(prefab).GetComponent<StarmapHexCellInventoryVisuals>();
			...
			StarmapHexCellInventory.AllInventories.Add(cell, value);
		}
		return value;
	}
	...
	{
		return GetRandomVisibleAdjacentCellLocation(cell, AxialI.INVALID);
	}

	public AxialI GetRandomVisibleAdjacentCellLocation(AxialI cell, AxialI forbidden)
	{
		return AxialUtil.GetRing(cell, 1).FindAll((AxialI e) => IsCellVisible(e) && (forbidden == AxialI.INVALID || forbidden != e))?.GetRandom() ?? AxialI.INVALID;
	}
	...
	public bool GetVisibleUnidentifiedMeteorShowerWithinRadius(AxialI center, int radius, out ClusterMapMeteorShower.Instance result)
	{
		for (int i = 0; i <= radius; i++)
		{
			...
			foreach (AxialI item in ring)
			{
				...
				List<ClusterGridEntity> entitiesOfLayerAtCell = GetEntitiesOfLayerAtCell(item, EntityLayer.Meteor);
				foreach (ClusterGridEntity item2 in entitiesOfLayerAtCell)
				{
	...
	public Vector3 GetPosition(ClusterGridEntity entity)
	{
		...
		if (list.Count > 1 && entity.SpaceOutInSameHex())
		{
			...
			float num4 = Mathf.Pow(f, 0.5f);
			float num5 = 0.81f;
			float num6 = Mathf.Pow(num2, 0.5f) * num5;
			float f2 = MathF.PI * 2f * num6 * num4;
			float x = Mathf.Cos(f2) * num3 * num4;
			float y = Mathf.Sin(f2) * num3 * num4;
	...
	public List<AxialI> GetPath(AxialI start, AxialI end, ClusterDestinationSelector destination_selector, out string fail_reason, bool dodgeHiddenAsteroids = false)
	{
		...
		{
			ClusterGridEntity visibleEntityOfLayerAtCell2 = GetVisibleEntityOfLayerAtCell(end, destination_selector.requiredEntityLayer);
			if (visibleEntityOfLayerAtCell2 == null)
			{
	...
	{
		GetLocationDescription(location, out sprite, out label, out sublabel, out var _);
	}
	...
	public void GetLocationDescription(AxialI location, out Sprite sprite, out string label, out string sublabel, out EntityLayer locationEntity)
	{
		...
		else if (clusterGridEntity2 != null)
		{
