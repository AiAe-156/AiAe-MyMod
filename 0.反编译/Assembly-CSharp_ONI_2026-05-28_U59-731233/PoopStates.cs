...
public class PoopStates : GameStateMachine<PoopStates, PoopStates.Instance, IStateMachineTarget, PoopStates.Def>
{
	...
	public new class Instance : GameInstance
	{
		...
		public bool IsPoopStationStillValid()
		{
			...
			return poopStation.IsPoopStationOperational() && poopStation.GetCurrentPoopStationUser() == base.gameObject;
		}
		...
		public void FindPoopStation()
		{
			...
			foreach (IPoopStation item in items)
			{
				...
				int num3 = Grid.PosToCell(item.GetPoopStationObject());
				int cellDistance = Grid.GetCellDistance(cell_a, num3);
				if (cellDistance > num || !flag2)
				{
					continue;
				}
				int navigationCost = navigator.GetNavigationCost(num3);
				if (navigationCost != -1 && (num2 == -1 || navigationCost < num2))
				{
					GameObject currentPoopStationUser = item.GetCurrentPoopStationUser();
					bool flag3 = flag2 && (currentPoopStationUser == null || currentPoopStationUser == base.gameObject);
					if (poopStation == null || !flag || flag3)
					{
						num2 = navigationCost;
						poopStation = item;
						flag = flag3;
