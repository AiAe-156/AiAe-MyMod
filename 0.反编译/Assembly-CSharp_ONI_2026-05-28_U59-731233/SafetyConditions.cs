...
public class SafetyConditions
{
	...
	public SafetyConditions()
	{
		...
		IsNotCoveredInLiquid = new SafetyChecker.Condition("IsNotCoveredInLiquid", num *= 2, delegate(int cell, int cost, SafetyChecker.Context context)
		{
		...
		HasSomeOxygen = new SafetyChecker.Condition("HasSomeOxygen", num *= 2, (int cell, int cost, SafetyChecker.Context context) => context.oxygenBreather == null || context.navigator == null || GasBreatherFromWorldProvider.GetBestBreathableCellAroundSpecificCell(cell, (context.navigator.CurrentNavType == NavType.Swim) ? GasBreatherFromWorldProvider.DEFAULT_BREATHABLE_OFFSETS : Grid.DefaultOffset, context.oxygenBreather).IsBreathable);
		HasSomeOxygenAround = new SafetyChecker.Condition("HasSomeOxygenAround", num *= 2, (int cell, int cost, SafetyChecker.Context context) => context.oxygenBreather == null || GasBreatherFromWorldProvider.GetBestBreathableCellAroundSpecificCell(cell, GasBreatherFromWorldProvider.DEFAULT_BREATHABLE_OFFSETS, context.oxygenBreather).IsBreathable);
		...
		AbsorbCellCellChecker = new SafetyChecker(new List<SafetyChecker.Condition> { IsNotCoveredInLiquid, IsNotDoor, HasSomeOxygenAround }.ToArray());
		List<SafetyChecker.Condition> list = new List<SafetyChecker.Condition> { HasSomeOxygenAround, IsNotDoor };
