...
public class GlassDeerConfig : IEntityConfig, IHasDlcRestrictions
{
	...
	{
		GameObject prefab = BaseDeerConfig.BaseDeer(id, name, desc, anim_file, "GlassDeerBaseTrait", is_baby, "gla_");
		...
		list.Add(BaseDeerConfig.CreateDietInfo("HardSkinBerryPlant", SimHashes.Dirt.CreateTag(), HARD_SKIN_CALORIES_PER_KG, 8.333334f, null, 0f));
		list.Add(new Diet.Info(new HashSet<Tag> { "HardSkinBerry" }, SimHashes.Dirt.CreateTag(), CONSUMABLE_PLANT_MATURITY_LEVELS * HARD_SKIN_CALORIES_PER_KG / 1f, 25.000002f));
		list.Add(BaseDeerConfig.CreateDietInfo("PrickleFlower", SimHashes.Dirt.CreateTag(), BRISTLE_CALORIES_PER_KG / 2f, 8.333334f, null, 0f));
		list.Add(new Diet.Info(new HashSet<Tag> { PrickleFruitConfig.ID }, SimHashes.Dirt.CreateTag(), CONSUMABLE_PLANT_MATURITY_LEVELS * BRISTLE_CALORIES_PER_KG / 1f, 50.000004f));
		list.Add(new Diet.Info(new HashSet<Tag> { SimHashes.Katairite.CreateTag() }, SimHashes.Dirt.CreateTag(), 5000f, 0.5f));
		prefab = BaseDeerConfig.SetupDiet(prefab, list.ToArray(), 1f);
	...
	{
		GameObject prefab = CreateGlassDeer("GlassDeer", STRINGS.CREATURES.SPECIES.GLASSDEER.NAME, STRINGS.CREATURES.SPECIES.GLASSDEER.DESC, "ice_floof_kanim", is_baby: false);
		return EntityTemplates.ExtendEntityToFertileCreature(prefab, this, "GlassDeerEgg", STRINGS.CREATURES.SPECIES.GLASSDEER.EGG_NAME, STRINGS.CREATURES.SPECIES.GLASSDEER.DESC, "egg_ice_floof_kanim", DeerTuning.EGG_MASS, "GlassDeerBaby", 60.000004f, 20f, DeerTuning.EGG_CHANCES_GLASS, EGG_SORT_ORDER);
	}
