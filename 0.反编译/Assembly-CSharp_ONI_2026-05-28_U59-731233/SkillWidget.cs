...
public class SkillWidget : KMonoBehaviour, IPointerEnterHandler, IEventSystemHandler, IPointerExitHandler, IPointerClickHandler, IPointerDownHandler
{
	...
	public void Refresh(string skillID)
	{
		...
		if (flag2)
		{
			...
			{
				grantedIcon.sprite = skillGrantSourceIcon;
			}
		}
	...
	public static string SkillPerksString(Skill skill)
	{
		...
		foreach (SkillPerk perk in skill.perks)
		{
			...
			{
				string text2 = GameUtil.NamesOfBuildingsRequiringSkillPerk(perk.Id);
				if (!string.IsNullOrEmpty(text))
				{
					text += "\n";
				}
				text += ((text2 != null) ? text2 : perk.Name);
			}
		}
		return text;
	}
	...
	public void OnPointerClick(PointerEventData eventData)
	{
		...
		{
			if (component.HasMasteredSkill(skillID))
			{
				return;
			}
			...
			while (queue.Count > 0)
			{
				...
				list.Add(text);
				foreach (string priorSkill in Db.Get().Skills.Get(text).priorSkills)
				{
					queue.Enqueue(priorSkill);
				}
			}
			for (int num = list.Count - 1; num >= 0; num--)
			{
				string skillId = list[num];
				if (!component.HasMasteredSkill(skillId))
				{
