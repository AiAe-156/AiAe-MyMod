...
public class BionicMassOxygenAbsorbChore : Chore<BionicMassOxygenAbsorbChore.Instance>
{
	...
	public class Instance : GameStateMachine<States, Instance, BionicMassOxygenAbsorbChore, object>.GameInstance, BionicOxygenTankMonitor.IChore
	{
		...
		public float GetGiveupTimerTimeout()
		{
			...
			return BionicOxygenTankMonitor.AreOxygenLevelsCritical(oxygenTankMonitor) ? 2f : 4f;
		}
		...
		public void OnSimConsume(Sim.MassConsumedCallback mass_cb_info, float dt)
		{
			...
			bool flag = BionicOxygenTankMonitor.AreOxygenLevelsCritical(oxygenTankMonitor);
			float num = (flag ? CRITICAL_OXYGEN_MASS_GIVE_UP_TRESHOLD : 2f);
			if (GetAverageMassConsumedPerSecond() <= num)
			{
			...
			float num3 = oxygenTankMonitor.AddGas(mass_cb_info);
			if (num3 > Mathf.Epsilon)
			{
				SimMessages.EmitMass(Grid.PosToCell(go), mass_cb_info.elemIdx, num3, mass_cb_info.temperature, byte.MaxValue, 0);
			}
		...
		{
			return (oxygenTankMonitor == null) ? 0f : oxygenTankMonitor.OxygenPercentage;
		}
	}
	...
	public override string ResolveString(string str)
	{
		...
		string format = base.ResolveString(str);
		return string.Format(format, GameUtil.GetFormattedMass(mass, GameUtil.TimeSlice.PerSecond));
	}
	...
	{
		return IsCriticalChore(smi) && smi.oxygenTankMonitor.OxygenPercentage >= 0.25f;
	}
	...
	{
		float a = dt * ABSORB_RATE;
		float mass = Mathf.Min(a, smi.oxygenTankMonitor.SpaceAvailableInTank);
		AbsorbUpdateData absorbUpdateData = new AbsorbUpdateData(smi, dt);
		GameObject go = smi.sm.dupe.Get(smi);
		int num = Grid.PosToCell(go);
		int elementCell = num;
		SimHashes nearBreathableElement = GetNearBreathableElement(num, ABSORB_RANGE, out elementCell);
