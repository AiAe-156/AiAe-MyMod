...
public class PlantFiberProducer : KMonoBehaviour
{
	...
	private void OnHarvest(object obj)
	{
		...
		if (harvestable != null && harvestable.completed_by != null)
		{
			WorkerBase completed_by = harvestable.completed_by;
			MinionResume component = completed_by.GetComponent<MinionResume>();
			if (component.HasPerk(Db.Get().SkillPerks.CanSalvagePlantFiber))
			{
				SpawnPlantFiber();
			}
		}
