...
public class Storage : Workable, ISaveLoadableDetails, IStorage
{
	...
	public bool storeDropsFromButcherables = false;
	...
	public bool showSideScreenTitleBar = false;

	public bool useWideOffsets = false;

	public Action<List<GameObject>> onDestroyItemsDropped = null;
	...
	protected override void OnSpawn()
	{
		...
		foreach (GameObject item in items)
		{
			...
			{
				item.Trigger(856640610, (object)this);
			}
		}
	...
	public GameObject Store(GameObject go, bool hide_popups = false, bool block_events = false, bool do_disease_transfer = true, bool is_deserializing = false)
	{
		...
		if (!hide_popups && PopFXManager.Instance != null)
		{
			...
			PopFXManager.Instance.SpawnFX(Def.GetUISprite(go).first, (fxPrefix == FXPrefix.Delivered) ? PopFXManager.Instance.sprite_Plus : PopFXManager.Instance.sprite_Negative, text, target_transform, storageFXOffset);
		}
		...
		if (!is_deserializing)
		{
			...
			if (component2 != null)
			{
				...
				foreach (GameObject item in items)
				{
					...
					if (!block_events)
					{
						Trigger(-1697596308, (object)go);
						OnStorageChange?.Invoke(go);
						Trigger(-778359855, (object)this);
	...
	public PrimaryElement AddOre(SimHashes element, float mass, float temperature, byte disease_idx, int disease_count, bool keep_zero_mass = false, bool do_disease_transfer = true)
	{
		...
		if (primaryElement != null)
		{
			...
			Trigger(-1697596308, (object)primaryElement.gameObject);
			OnStorageChange?.Invoke(primaryElement.gameObject);
		}
		else
		{
			Element element2 = ElementLoader.FindElementByHash(element);
	...
	public float Transfer(Storage dest_storage, Tag tag, float amount, bool block_events = false, bool hide_popups = false)
	{
		...
		if (gameObject != null)
		{
			...
			if (amount < component.Units)
			{
				...
				if (!block_events)
				{
					Trigger(-1697596308, (object)component2.gameObject);
					OnStorageChange?.Invoke(component2.gameObject);
				}
			}
			else
			{
					...
					if (ventGas || dumpLiquid)
					{
						...
						if (component2 != null)
						{
							if (ventGas && pickupable.GetComponent<PrimaryElement>().Element.IsGas)
							{
								...
								Trigger(-1697596308, (object)pickupable.gameObject);
								OnStorageChange?.Invoke(pickupable.gameObject);
								result = true;
								if (showInWorldNotification)
								{
									PopFXManager.Instance.SpawnFX(Def.GetUISprite(component.gameObject).first, PopFXManager.Instance.sprite_Plus, pickupable.GetComponent<PrimaryElement>().Element.name + " " + GameUtil.GetFormattedMass(pickupable.TotalAmount), pickupable.transform, storageFXOffset);
								}
							}
	...
	public void DropUnlessMatching(FetchChore chore)
	{
		for (int i = 0; i < items.Count; i++)
		{
			if (!(items[i] == null))
			{
				...
				bool flag = (chore.criteria == FetchChore.MatchCriteria.MatchID && chore.tags.Contains(component.PrefabTag)) || (chore.criteria == FetchChore.MatchCriteria.MatchTags && component.HasTag(chore.tagsFirst));
				flag &= !chore.requiredTag.IsValid || component.HasTag(chore.requiredTag);
				if (!(flag & !component.HasAnyTags(chore.forbiddenTags)))
				{
	...
	{
		ConsumeAndGetDisease(tag, amount, out amount_consumed, out disease_info, out aggregate_temperature, out var _);
	}
	...
	public void ConsumeAndGetDisease(Tag tag, float amount, out float amount_consumed, out SimUtil.DiseaseInfo disease_info, out float aggregate_temperature, out SimHashes mostRelevantItemElement)
	{
		...
		for (int i = 0; i < items.Count; i++)
		{
			...
			Trigger(-1697596308, (object)gameObject);
			OnStorageChange?.Invoke(gameObject);
		}
		if (!flag)
		{
	...
	public void ConsumeIgnoringDisease(GameObject item_go)
	{
		if (items.Contains(item_go))
		{
			...
			if (component != null && component.KeepZeroMassObject)
			{
				...
				Trigger(-1697596308, (object)item_go);
				OnStorageChange?.Invoke(item_go);
			}
			else
			{
	...
	private void OnDeath(object data)
	{
		...
		DropAll(vent_gas: true, dump_liquid: true, default(Vector3), do_disease_transfer: true, collect_dropped_items);
		if (onDestroyItemsDropped != null)
		{
			onDestroyItemsDropped(list);
		}
	}
	...
	{
		if (fetchCategory != FetchCategory.Building)
		{
	...
	private void OnPriorityChanged(PrioritySetting priority)
	{
		foreach (GameObject item in items)
		{
			item.Trigger(-1626373771, (object)this);
		}
	}
