...
public class Generator : KMonoBehaviour, ISaveLoadable, IEnergyProducer, ICircuitConnected
{
	...
	protected override void OnSpawn()
	{
		...
		cachedPrefabId = base.gameObject.PrefabID();
	...
	{
		ReportManager.Instance.ReportValue(ReportManager.ReportType.EnergyCreated, joulesAvailable, selectable.GetProperName());
		float num = this.joulesAvailable + joulesAvailable;
		this.joulesAvailable = Mathf.Clamp(num, 0f, canOverPower ? float.MaxValue : Capacity);
		if (num > joulesAvailable)
		{
			ReportManager.Instance.ReportValue(ReportManager.ReportType.EnergyWasted, this.joulesAvailable - num, StringFormatter.Replace(BUILDINGS.PREFABS.GENERATOR.OVERPRODUCTION, "{Generator}", base.gameObject.GetProperName()));
		}
		if (!Game.Instance.savedInfo.powerCreatedbyGeneratorType.ContainsKey(cachedPrefabId))
		{
			Game.Instance.savedInfo.powerCreatedbyGeneratorType.Add(cachedPrefabId, 0f);
		}
		Game.Instance.savedInfo.powerCreatedbyGeneratorType[cachedPrefabId] += this.joulesAvailable;
	}
