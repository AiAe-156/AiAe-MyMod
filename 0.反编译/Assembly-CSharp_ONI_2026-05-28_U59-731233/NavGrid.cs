...
public class NavGrid
{
	...
	private byte[] DirtyBitFlags = null;

	private List<int> DirtyCells = null;
	...
	{
		DirtyBitFlags = new byte[(Grid.CellCount + 7) / 8];
		...
		NavTable = new NavTable(Grid.CellCount, id);
		this.transitions = transitions;
	...
	{
		DevOnly.Assert(Game.IsOnMainThread(), "Single (main) thread requirement due to use of static resource dirtyCellsSwapBuffer");
		int count = DirtyCells.Count;
		for (int i = 0; i < count; i++)
		{
			Grid.CellToXY(DirtyCells[i], out var x, out var y);
			int num = Grid.ClampX(x - updateRangeX);
			int num2 = Grid.ClampY(y - updateRangeY);
			int num3 = Grid.ClampX(x + updateRangeX);
			int num4 = Grid.ClampY(y + updateRangeY);
			for (int j = num2; j <= num4; j++)
			{
				for (int k = num; k <= num3; k++)
				{
					AddDirtyCell(Grid.XYToCell(k, j));
				}
			}
		}
		List<int> dirtyCells = dirtyCellsSwapBuffer;
		dirtyCellsSwapBuffer = DirtyCells;
		DirtyCells = dirtyCells;
		UpdateGraph(dirtyCellsSwapBuffer);
		foreach (int item in dirtyCellsSwapBuffer)
		{
			DirtyBitFlags[item / 8] = 0;
		}
		dirtyCellsSwapBuffer.Clear();
	}
	...
	{
		if (Grid.IsValidCell(cell) && (DirtyBitFlags[cell / 8] & (1 << cell % 8)) == 0)
		{
			DirtyCells.Add(cell);
			DirtyBitFlags[cell / 8] |= (byte)(1 << cell % 8);
		}
	}
