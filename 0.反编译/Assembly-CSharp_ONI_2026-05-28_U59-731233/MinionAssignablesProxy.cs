...
public class MinionAssignablesProxy : KMonoBehaviour, IAssignableIdentity
{
	...
	{
		MinionIdentity component = GetTargetGameObject().GetComponent<MinionIdentity>();
		if (component != null)
		{
			return component.model;
		}
		StoredMinionIdentity component2 = GetTargetGameObject().GetComponent<StoredMinionIdentity>();
		if (component2 != null)
		{
			return component2.model;
		}
		Debug.LogError("Could not get minion model");
		return Tag.Invalid;
	}
	...
	public static Ref<MinionAssignablesProxy> InitAssignableProxy(Ref<MinionAssignablesProxy> assignableProxyRef, IAssignableIdentity source)
	{
		...
		{
			GameObject gameObject = GameUtil.KInstantiate(Assets.GetPrefab(MinionAssignablesProxyConfig.ID), Grid.SceneLayer.NoLayer);
			minionAssignablesProxy = gameObject.GetComponent<MinionAssignablesProxy>();
