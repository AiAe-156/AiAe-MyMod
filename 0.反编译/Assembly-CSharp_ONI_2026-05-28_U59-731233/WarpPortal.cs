...
public class WarpPortal : Workable
{
	...
	private int GetTargetWorldID()
	{
		...
		WarpReceiver[] array = Object.FindObjectsByType<WarpReceiver>(FindObjectsSortMode.None);
	...
	private void Warp()
	{
		...
		{
			SaveGame.Instance.GetComponent<WorldGenSpawner>().SpawnTag(WarpReceiverConfig.ID);
			warpReceiver = Object.FindFirstObjectByType<WarpReceiver>();
		}
		if (warpReceiver != null)
		{
	...
	public IEnumerator DelayedWarp(WarpReceiver receiver)
	{
		...
		int prevID = base.worker.GetMyWorldId();
		int targetID = receiver.GetMyWorldId();
		GameUtil.FocusCameraOnWorld(targetID, Grid.CellToPos(Grid.PosToCell(receiver)));
		WorkerBase targetWorker = base.worker;
		targetWorker.StopWork();
		receiver.ReceiveWarpedDuplicant(targetWorker);
		ClusterManager.Instance.MigrateMinion(targetWorker.GetComponent<MinionIdentity>(), targetID, prevID);
