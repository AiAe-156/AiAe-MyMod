...
public static class GameUtil
{
	...
	{
		GameObject prefab = Assets.GetPrefab(preyTag);
	...
	public static string GetFormattedDirectPlantConsumptionValuePerCycle(Tag plantTag, float consumer_caloriesLossPerCaloriesPerKG, bool perCycle = true)
	{
		...
		IPlantConsumptionInstructions[] plantConsumptionInstructions = GetPlantConsumptionInstructions(prefab);
	...
	public static string GetFormattedFoodQuality(int quality)
	{
		...
		return string.Format(locString, adjectives[value], AddPositiveSign(quality.ToString(), quality > 0));
	}
	...
	public static string SafeStringFormat(string source, params object[] args)
	{
		for (int i = 0; i < args.Length; i++)
		{
			...
			{
				string notification_name = $"Format error in string: \"{source}\". Source is missing the {{{i}}} format marker for argument \"{args[i]}\" insertion.";
				KCrashReporter.ReportDevNotification(notification_name, Environment.StackTrace);
			}
			else
			{
	...
	{
		return (!Grid.IsValidCell(cell)) ? 0f : GetRadiationAbsorptionPercentage(Grid.Element[cell], Grid.Mass[cell], Grid.IsSolidCell(cell) && (Grid.Properties[cell] & 0x80) == 128);
	}
	...
	public static string GetActionString(Action action)
	{
		...
		GamepadButton mButton = bindingEntry.mButton;
		if (KInputManager.currentControllerIsGamepad)
		{
	...
	public static void PartitionBuildingDescriptors(GameObject buildingComplete, bool simpleInfoScreen, out List<Descriptor> allDescs, List<(ElementConverter converter, List<Descriptor> descriptors)> converterDescCache, List<Descriptor> nonConverterReqs, List<Descriptor> nonConverterEffects, out bool hasConverterReqs)
	{
		...
		if (defImplementingInterface != null && defImplementingInterface.ByproductRate > 0f)
		{
			foreach (var item4 in converterDescCache)
			{
				...
				{
					ElementConverter.ConsumedElement consumedElement = consumedElements[j];
					if (consumedElement.Tag == defImplementingInterface.ByproductAssociatedInputTag)
					{
						flag = true;
						break;
					}
	...
	public static void BuildPartitionedRequirements(List<Descriptor> result, List<Descriptor> nonConverterReqs, List<(ElementConverter converter, List<Descriptor> descriptors)> converterDescCache, bool hasConverterReqs)
	{
		...
		{
			Descriptor item = nonConverterReq;
			item.IncreaseIndent();
			result.Add(item);
		}
		if (hasConverterReqs)
		{
			...
			item2.SetupDescriptor(UI.BUILDINGEFFECTS.OPERATIONINPUTS, "", Descriptor.DescriptorType.Requirement);
			item2.IncreaseIndent();
			result.Add(item2);
		}
		foreach (var (elementConverter, list) in converterDescCache)
		{
			foreach (Descriptor item4 in list)
			{
				if (item4.type == Descriptor.DescriptorType.Requirement)
				{
					Descriptor item3 = item4;
	...
	public static void BuildPartitionedEffects(List<Descriptor> result, List<Descriptor> nonConverterEffects, List<(ElementConverter converter, List<Descriptor> descriptors)> converterDescCache)
	{
		...
		{
			Descriptor item = nonConverterEffect;
			item.IncreaseIndent();
			result.Add(item);
		}
		foreach (var item6 in converterDescCache)
		{
			ElementConverter item2 = item6.converter;
			List<Descriptor> item3 = item6.descriptors;
			string text = item2.consumedElements[0].Name;
			for (int i = 1; i < item2.consumedElements.Length; i++)
			{
				text = text + ", " + item2.consumedElements[i].Name;
			}
			Descriptor item4 = new Descriptor(text + ":", "");
			item4.IncreaseIndent();
			result.Add(item4);
			foreach (Descriptor item7 in item3)
			{
				if (item7.type != Descriptor.DescriptorType.Requirement)
				{
					Descriptor item5 = item7;
	...
	{
		Tag[] boostersWithSkillPerk = BionicUpgradeComponentConfig.GetBoostersWithSkillPerk(perkID);
		List<string> values = boostersWithSkillPerk.Select((Tag tag) => Strings.Get($"STRINGS.ITEMS.BIONIC_BOOSTERS.{tag.ToString().ToUpper()}.NAME").String).ToList();
		return string.Join("\n", values);
	}
