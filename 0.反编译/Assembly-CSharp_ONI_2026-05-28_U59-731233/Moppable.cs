...
public class Moppable : Workable, ISim1000ms, ISim200ms
{
	...
	public void Sim1000ms(float dt)
	{
		if (amountMopped > 0f)
		{
			PopFX popFX = PopFXManager.Instance.SpawnFX((lastElementMopped == null) ? PopFXManager.Instance.sprite_Resource : Def.GetUISprite(lastElementMopped).first, null, GameUtil.GetFormattedMass(0f - amountMopped), base.transform, Vector3.zero);
			if (popFX != null && lastElementMopped != null && lastElementMopped.substance != null)
			{
				popFX.SetIconTint(lastElementMopped.substance.colour);
			}
			amountMopped = 0f;
			lastElementMopped = null;
		}
	}
	...
	private void OnCellMopped(Sim.MassConsumedCallback mass_cb_info, object data)
	{
		if (!(this == null) && mass_cb_info.mass > 0f)
		{
			...
			Element element = ElementLoader.elements[mass_cb_info.elemIdx];
