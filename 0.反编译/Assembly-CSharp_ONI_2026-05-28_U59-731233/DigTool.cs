...
public class DigTool : FilteredDragTool
{
	...
	public static GameObject PlaceDig(int cell, int animationDelay = 0)
	{
		...
		if (flag && Grid.Solid[cell] && !Grid.Foundation[cell] && Grid.Objects[cell, 7] == null)
		{
			for (int i = 0; i < 45; i++)
			{
		...
		if (flag2 && !Grid.Solid[cell] && BackwallManager.HasBackwall(cell))
		{
			GameObject gameObject2 = Util.KInstantiate(Assets.GetPrefab(new Tag("DigPlacer")));
			gameObject2.SetActive(value: true);
			Grid.Objects[cell, 7] = gameObject2;
			Vector3 position2 = Grid.CellToPosCBC(cell, Instance.visualizerLayer);
			float num2 = -0.15f;
			position2.z += num2;
			gameObject2.transform.SetPosition(position2);
			gameObject2.GetComponentInChildren<EasingAnimations>().PlayAnimation("ScaleUp", Mathf.Max(0f, (float)animationDelay * 0.02f));
			return gameObject2;
		}
