...
public class ContactConductivePipeBridge : GameStateMachine<ContactConductivePipeBridge, ContactConductivePipeBridge.Instance, IStateMachineTarget, ContactConductivePipeBridge.Def>
{
	...
	public new class Instance : GameInstance
	{
		...
		public Tag tag => (type == ConduitType.Liquid) ? GameTags.Liquid : GameTags.Gas;
		...
		private void Flow(float dt)
		{
			...
			if (amountAllowedForMerging > 0f)
			{
				...
				ConduitFlow conduitFlow = ((base.def.type == ConduitType.Liquid) ? Game.Instance.liquidConduitFlow : Game.Instance.gasConduitFlow);
				float num2 = conduitFlow.AddElement(outputCell, contents.element, amountAllowedForMerging, temperature, contents.diseaseIdx, contents.diseaseCount);
		...
		private float ExchangeStorageTemperatureWithBuilding(ConduitFlow.ConduitContents content, float mass, float dt)
		{
			...
			if (mass > 0f)
			{
				...
				float killoJoulesTransfered = GetKilloJoulesTransfered(num2, dt, temperature, num, temperature2, content_heat_capacity);
				float finalContentTemperature = GetFinalContentTemperature(killoJoulesTransfered, temperature, num, temperature2, content_heat_capacity);
				...
				bool flag = finalBuildingTemperature >= 0f && finalBuildingTemperature <= 10000f;
				if (flag && finalContentTemperature >= 0f && finalContentTemperature <= 10000f)
				{
	...
	private static float GetKilloJoulesTransfered(float maxWattsTransfered, float dt, float building_Temperature, float building_heat_capacity, float content_temperature, float content_heat_capacity)
	{
		...
		float num2 = Mathf.Abs(value - content_temperature);
		float num3 = Mathf.Abs(value2 - building_Temperature);
		float a = num2 * content_heat_capacity;
		float b = num3 * building_heat_capacity;
