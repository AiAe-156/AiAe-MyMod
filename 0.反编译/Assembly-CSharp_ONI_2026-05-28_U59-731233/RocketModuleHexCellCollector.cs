...
public class RocketModuleHexCellCollector : GameStateMachine<RocketModuleHexCellCollector, RocketModuleHexCellCollector.Instance, IStateMachineTarget, RocketModuleHexCellCollector.Def>
{
	public class Def : BaseDef
	{
		...
		public bool formatCapacityBarAsUnits = false;
	...
	{
		[Serialize]
		public int LastCollectedIndex = 0;

		[Serialize]
		public float MassCharge = 0f;
	...
	public TargetParameter HexCellInventory = null;
	...
	public static bool CollectUpdate(Instance smi, float dt)
	{
		...
		for (int k = 0; k < count; k++)
		{
			...
			if (!itemData2.usesUnits || num6 >= itemData2.massPerUnit)
			{
				...
				{
					int num7 = Mathf.FloorToInt(num6 / itemData2.massPerUnit);
					mass = (float)num7 * itemData2.massPerUnit;
				}
		...
		return storage.RemainingCapacity() <= 0f || (!flag2 && num3 <= 0f);
	}
