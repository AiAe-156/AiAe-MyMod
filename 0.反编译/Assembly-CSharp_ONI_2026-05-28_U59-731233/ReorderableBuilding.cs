...
public class ReorderableBuilding : KMonoBehaviour
{
	...
	private static void ProcessToBeRemoved()
	{
		if (toBeRemoved.Count > 0)
		{
			...
			for (int num = 0; num < toBeRemoved.Count; num++)
			{
				toBeRemoved[num].RemoveModule();
			}
			toBeRemoved.Clear();
		}
	}
	...
	public GameObject ConvertModule(BuildingDef toModule, IList<Tag> materials)
	{
		...
		if (!DebugHandler.InstantBuildMode && !toModule.IsValidPlaceLocation(base.gameObject, cell, Orientation.Neutral, out var fail_reason))
		{
			...
			if (component2 != null && gameObject != null)
			{
				component2.points[0].attachedBuilding = gameObject.GetComponent<AttachableBuilding>();
				Components.BuildingAttachPoints.Add(component2);
			}
			return null;
		}
		if (materials == null)
		...
		if (component5 != null)
		{
			component5.SetAllowDeconstruction(allow: true);
			component5.ForceDestroyAndGetMaterials();
		}
		else
		{
	...
	public bool CanChangeModule()
	{
		...
		if (GetComponent<BuildingUnderConstruction>() != null)
		{
			text = GetComponent<BuildingUnderConstruction>().Def.PrefabID;
		}
		else
		{
			text = GetComponent<Building>().Def.PrefabID;
		}
		RocketModuleCluster component = GetComponent<RocketModuleCluster>();
		if (component != null)
		{
	...
	private static void UnmarkBuilding(GameObject go, AttachableBuilding aboveBuilding)
	{
		...
		if (components.Length != 0)
		{
			...
			foreach (RocketConduitSender rocketConduitSender in array)
			{
		...
		if (components2.Length != 0)
		{
			...
			foreach (RocketConduitReceiver rocketConduitReceiver in array2)
			{
