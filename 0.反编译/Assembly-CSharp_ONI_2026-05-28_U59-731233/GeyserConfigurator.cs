...
public class GeyserConfigurator : KMonoBehaviour
{
	...
	public class GeyserInstanceConfiguration
	{
		...
		{
			return (modifier.modifyElement && modifier.newElement != 0) ? modifier.newElement : geyserType.element;
		}
		...
		{
			float emitRate = GetEmitRate();
			float num = emitRate * GetOnDuration();
			float num2 = GetYearOnDuration() / GetIterationLength();
			float num3 = num2 * num;
			return num3 / GetYearLength();
		}
