...
public class SealConfig : IEntityConfig, IHasDlcRestrictions
{
	...
	public static GameObject CreateSeal(string id, string name, string desc, string anim_file, bool is_baby)
	{
		...
		CreaturePoopLoot.Def def = prefab.AddOrGetDef<CreaturePoopLoot.Def>();
		def.Loot = new CreaturePoopLoot.LootData[1]
		{
	...
	{
		GameObject prefab = CreateSeal("Seal", STRINGS.CREATURES.SPECIES.SEAL.NAME, STRINGS.CREATURES.SPECIES.SEAL.DESC, "seal_kanim", is_baby: false);
		return EntityTemplates.ExtendEntityToFertileCreature(prefab, this, "SealEgg", STRINGS.CREATURES.SPECIES.SEAL.EGG_NAME, STRINGS.CREATURES.SPECIES.SEAL.DESC, "egg_seal_kanim", SealTuning.EGG_MASS, "SealBaby", 60.000004f, 20f, SealTuning.EGG_CHANCES_BASE, EGG_SORT_ORDER);
	}
