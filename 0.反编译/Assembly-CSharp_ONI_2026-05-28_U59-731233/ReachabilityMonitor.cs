public class ReachabilityMonitor : GameStateMachine<ReachabilityMonitor, ReachabilityMonitor.Instance, Workable>
{
	...
	public new class Instance : GameInstance
	{
		...
		{
			Trigger(-1432940121, (object)BoxedBools.Box(base.sm.isReachable.Get(base.smi)));
		}
	...
	public override void InitializeStates(out BaseState default_state)
	{
		...
		reachable.Enter(delegate(Instance smi)
		{
			smi.Get<KPrefabID>().AddTag(GameTags.Reachable);
		}).Exit(delegate(Instance smi)
		{
			smi.Get<KPrefabID>().RemoveTag(GameTags.Reachable);
		}).Enter("TriggerEvent", delegate(Instance smi)
		{
			smi.TriggerEvent();
		})
