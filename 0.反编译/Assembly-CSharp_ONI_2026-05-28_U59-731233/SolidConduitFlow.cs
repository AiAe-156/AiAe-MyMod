...
public class SolidConduitFlow : IConduitFlow
{
	...
	public class SOAInfo
	{
		...
		{
			if (lastFlowInfo[idx].direction != FlowDirection.None)
			{
	...
	private void TryAdd(List<Conduit> new_path)
	{
		...
		{
			List<Conduit> list = pathList[num6];
			if (new_path.Count >= list.Count)
			{
				bool flag2 = false;
				int idx = list[0].idx;
				int idx2 = list[list.Count - 1].idx;
				int num7 = -1;
				int num8 = -1;
				for (int num9 = 0; num9 < new_path.Count; num9++)
				{
					if (new_path[num9].idx == idx)
					{
						num7 = num9;
						break;
					}
				}
				for (int num10 = 0; num10 < new_path.Count; num10++)
				{
					if (new_path[num10].idx == idx2)
					{
						num8 = num10;
						break;
					}
				...
				if (num7 != -1 && num8 != -1)
				{
					...
					while (num11 < num8)
					{
						if (new_path[num11].idx != list[num12].idx)
						{
							flag2 = false;
							break;
						}
		...
		{
			for (int num13 = new_path.Count - 1; num13 >= 0; num13--)
			{
				Conduit new_conduit = new_path[num13];
				int num14 = path2.FindIndex((Conduit t) => t.idx == new_conduit.idx);
				if (num14 != -1)
				{
	...
	{
		ConduitContents contents = new ConduitContents
		{
			pickupableHandle = HandleVector<int>.InvalidHandle
		};
		if (pickupable != null)
		{
			KBatchedAnimController component = pickupable.GetComponent<KBatchedAnimController>();
			StoredInfo initial_data = new StoredInfo
			{
				kbac = component,
