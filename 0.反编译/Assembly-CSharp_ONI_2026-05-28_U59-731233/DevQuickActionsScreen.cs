...
public class DevQuickActionsScreen : MonoBehaviour
{
	...
	public void Open(GameObject target)
	{
		...
		{
			List<DevQuickActionInstruction> devInstructions = item.GetDevInstructions();
			foreach (DevQuickActionInstruction item2 in devInstructions)
			{
				string[] array = item2.Address.Split('/');
				...
				for (int i = 0; i < array.Length; i++)
				{
					...
					else
					{
						...
						unsedEndNode.Setup(key, devQuickActionCategoryNode, item2.Action);
	...
	private DevQuickActionEndNode GetUnsedEndNode()
	{
		...
		{
			GameObject gameObject = Util.KInstantiateUI(originalEndNode.gameObject, originalEndNode.transform.parent.gameObject);
			result = gameObject.GetComponent<DevQuickActionEndNode>();
		}
	...
	private DevQuickActionCategoryNode GetUnsedCategoryNode()
	{
		...
		{
			GameObject gameObject = Util.KInstantiateUI(originalCategoryDevNode.gameObject, originalCategoryDevNode.transform.parent.gameObject);
			result = gameObject.GetComponent<DevQuickActionCategoryNode>();
		}
		SetupUnusedNodeForUse(result);
		return result;
	}
