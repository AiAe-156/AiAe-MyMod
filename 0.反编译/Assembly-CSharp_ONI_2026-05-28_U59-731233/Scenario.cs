...
public class Scenario : KMonoBehaviour
{
	...
	public void SetupElementTest()
	{
		...
		int bot = builder.Bot;
		List<Element> list = (from element4 in ElementLoader.elements
			where element4.IsSolid
			orderby element4.highTempTransitionTarget
			select element4).ToList();
		...
		List<Element> list2 = (from element4 in ElementLoader.elements
			where element4.IsLiquid && !elements.Contains(element4)
			orderby element4.highTempTransitionTarget
			select element4).ToList();
		...
		List<Element> list3 = ElementLoader.elements.Where((Element element4) => element4.state == Element.State.Gas && !elements.Contains(element4)).ToList();
		foreach (Element item3 in list3)
		{
			Element element3 = item3;
