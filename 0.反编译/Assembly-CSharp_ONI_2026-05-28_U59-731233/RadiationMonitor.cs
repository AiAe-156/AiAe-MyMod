...
public class RadiationMonitor : GameStateMachine<RadiationMonitor, RadiationMonitor.Instance>
{
	...
	public static readonly Parameter<float>.Callback COMPARE_GTE_NO_LONGER_DEADLY = (Instance smi, float p) => p < 900f * smi.difficultySettingMod;
	...
	public override void InitializeStates(out BaseState default_state)
	{
		...
		active.sick.minor.ToggleEffect((Instance smi) => smi.master.gameObject.HasTag(GameTags.Minions.Models.Bionic) ? bionic_minorSicknessEffect : minorSicknessEffect).ParamTransition(radiationExposure, active.sick.deadly, COMPARE_GTE_DEADLY).ParamTransition(radiationExposure, active.sick.extreme, COMPARE_GTE_EXTREME)
			.ParamTransition(radiationExposure, active.sick.major, COMPARE_GTE_MAJOR)
		...
		active.sick.major.ToggleEffect((Instance smi) => smi.master.gameObject.HasTag(GameTags.Minions.Models.Bionic) ? bionic_majorSicknessEffect : majorSicknessEffect).ParamTransition(radiationExposure, active.sick.deadly, COMPARE_GTE_DEADLY).ParamTransition(radiationExposure, active.sick.extreme, COMPARE_GTE_EXTREME)
		...
		active.sick.extreme.ParamTransition(radiationExposure, active.sick.deadly, COMPARE_GTE_DEADLY).ToggleEffect((Instance smi) => smi.master.gameObject.HasTag(GameTags.Minions.Models.Bionic) ? bionic_extremeSicknessEffect : extremeSicknessEffect).ToggleAnims("anim_loco_radiation3_kanim", 4f)
		...
		active.sick.deadly.ToggleAnims("anim_loco_radiation4_kanim", 4f).ToggleAnims("anim_idle_radiation4_kanim", 4f).ToggleExpression(Db.Get().Expressions.Radiation4)
			.ParamTransition(radiationExposure, active.sick.extreme, COMPARE_GTE_NO_LONGER_DEADLY)
	...
	private static void CheckRadiationLevel(Instance smi, float dt)
	{
		...
		if (Grid.IsValidCell(num))
		{
			...
			if (smi.sm.timeUntilNextExposureReact.Get(smi) <= 0f && !smi.HasTag(GameTags.InTransitTube) && COMPARE_REACT(smi, num4))
			{
				...
				sMI.AddSelfEmoteReactable(smi.master.gameObject, "RadiationReact", radiation_Glare, isOneShot: true, Db.Get().ChoreTypes.EmoteHighPriority);
			}
		}
		if (smi.sm.timeUntilNextSickReact.Get(smi) <= 0f && smi.sm.isSick.Get(smi) && !smi.HasTag(GameTags.InTransitTube))
		{
			...
			sMI2.AddSelfEmoteReactable(smi.master.gameObject, "RadiationReact", radiation_Itch, isOneShot: true, Db.Get().ChoreTypes.RadiationPain);
		}
