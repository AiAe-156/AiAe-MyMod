...
public class BasicForagePlantPlanted : StateMachineComponent<BasicForagePlantPlanted.StatesInstance>
{
	...
	public class States : GameStateMachine<States, StatesInstance, BasicForagePlantPlanted>
	{
		...
		public override void InitializeStates(out BaseState default_state)
		{
			...
			alive.idle.PlayAnim("idle").EventHandlerTransition(GameHashes.Harvest, alive.harvest, DoesNOTHavePreDeathAnimation).EventHandlerTransition(GameHashes.Harvest, alive.harvestDelayed, HasPreDeathAnimation)
				.Enter(delegate(StatesInstance smi)
				{
					smi.master.harvestable.SetCanBeHarvested(state: true);
				});
			alive.harvestDelayed.PlayAnim((StatesInstance smi) => smi.master.Pre_Death_Anim).OnAnimQueueComplete(alive.harvest);
			alive.harvest.Enter(DropSeed).EnterGoTo(dead);
	...
	public string Pre_Death_Anim = null;
	...
	private static bool DoesNOTHavePreDeathAnimation(StatesInstance smi, object o)
	{
		return string.IsNullOrEmpty(smi.master.Pre_Death_Anim);
	}

	private static bool HasPreDeathAnimation(StatesInstance smi, object o)
	{
		return !string.IsNullOrEmpty(smi.master.Pre_Death_Anim);
	}

	private static void DropSeed(StatesInstance smi)
	{
