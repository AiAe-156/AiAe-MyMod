...
public class GeoTunerSideScreen : SideScreenContent
{
	...
	private void SetRow(int idx, string name, Sprite icon, Geyser geyser, bool studied)
	{
		...
		toolTip.OnToolTip = delegate
		{
			...
			{
				float num6 = 600f / geyser.configuration.GetIterationLength();
				float num7 = emissionPerCycleModifier / num6;
				return num7 / geyser.configuration.GetOnDuration();
			};
			Func<float, float, float, float> func2 = delegate(float iterationLength, float massPerCycle, float eruptionDuration)
			{
				float num6 = 600f / iterationLength;
				float num7 = massPerCycle / num6;
				return num7 / eruptionDuration;
			};
			...
			float num3 = func2(geyser.configuration.scaledIterationLength, geyser.configuration.scaledRate, geyser.configuration.scaledIterationLength * geyser.configuration.scaledIterationPercent);
			float num4 = temperature + num * (float)count;
			float num5 = num3 + num2 * (float)count;
			...
			string text3 = UI.UISIDESCREENS.GEOTUNERSIDESCREEN.STUDIED_TOOLTIP;
			text3 += "\n";
			text3 = text3 + "\n" + UI.UISIDESCREENS.GEOTUNERSIDESCREEN.STUDIED_TOOLTIP_MATERIAL;
			text3 = text3.Replace("{MATERIAL}", newValue);
			text3 = text3 + "\n" + text;
			text3 = text3 + "\n" + text2;
			text3 += "\n";
		...
		if (usingStudiedTooltip && count > 0)
		{
			...
			toolTip2.OnToolTip = delegate
			{
				string text = UI.UISIDESCREENS.GEOTUNERSIDESCREEN.STUDIED_TOOLTIP_NUMBER_HOVERED.ToString();
				return text.Replace("{0}", count.ToString());
			};
		}
