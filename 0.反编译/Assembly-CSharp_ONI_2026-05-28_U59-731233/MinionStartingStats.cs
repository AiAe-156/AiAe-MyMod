...
public class MinionStartingStats : ITelepadDeliverable
{
	...
	public string overrideOutfitID = null;
	...
	private int GenerateTraits(bool is_starter_minion, List<ChoreGroup> disabled_chore_groups, string guaranteedAptitudeID = null, string guaranteedTraitID = null, bool isDebugMinion = false)
	{
		...
		Func<List<DUPLICANTSTATS.TraitVal>, bool, bool> func = delegate(List<DUPLICANTSTATS.TraitVal> traitPossibilities, bool flag2)
		{
			...
			float num7 = Mathf.Abs(Util.GaussianRandom());
			int num8 = traitPossibilities.Count;
			int num9;
			if (!flag2)
			{
				...
				num9 = DUPLICANTSTATS.rarityDeckActive[DUPLICANTSTATS.rarityDeckActive.Count - 1];
			...
			else
			{
				...
				num9 = list[0];
				num9 = Mathf.Max(DUPLICANTSTATS.RARITY_COMMON, num9);
				num9 = Mathf.Min(DUPLICANTSTATS.RARITY_LEGENDARY, num9);
			}
			List<DUPLICANTSTATS.TraitVal> list2 = new List<DUPLICANTSTATS.TraitVal>(traitPossibilities);
			for (int num10 = list2.Count - 1; num10 > -1; num10--)
			{
				if (list2[num10].rarity != num9)
				{
	...
	public GameObject Deliver(Vector3 location)
	{
		...
		new EmoteChore(component, Db.Get().ChoreTypes.EmoteHighPriority, "anim_interacts_portal_kanim", Telepad.PortalBirthAnim);
		return gameObject;
	}
