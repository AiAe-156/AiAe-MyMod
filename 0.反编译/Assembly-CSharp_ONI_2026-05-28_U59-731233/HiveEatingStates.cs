...
public class HiveEatingStates : GameStateMachine<HiveEatingStates, HiveEatingStates.Instance, IStateMachineTarget, HiveEatingStates.Def>
{
	...
	public new class Instance : GameInstance
	{
		...
		public void EatOreFromStorage(Instance smi, float dt)
		{
			...
			BeehiveCalorieMonitor.Instance sMI = smi.GetSMI<BeehiveCalorieMonitor.Instance>();
			Diet.Info dietInfo = sMI.stomach.diet.GetDietInfo(component.PrefabTag);
			if (dietInfo != null)
			{
	...
	public override void InitializeStates(out BaseState default_state)
	{
		...
		EatingStates eatingStates = eating;
		...
		eatingStates.ToggleStatusItem(text, tooltip, "", StatusItem.IconType.Info, NotificationType.Neutral, allow_multiples: false, default(HashedString), 129022, null, null, main).DefaultState(eating.pre).Enter(delegate(Instance smi)
		{
			smi.TurnOn();
		})
