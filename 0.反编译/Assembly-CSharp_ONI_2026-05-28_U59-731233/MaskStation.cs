...
public class MaskStation : StateMachineComponent<MaskStation.SMInstance>, IBasicBuilding
{
	private class OxygenMaskReactable : Reactable
	{
		...
		private void Run()
		{
			...
			Equipment equipment = gameObject.GetComponent<MinionIdentity>().GetEquipment();
			bool flag = !equipment.IsSlotOccupied(Db.Get().AssignableSlots.Suit);
			...
			if (flag)
			{
				...
				GameObject gameObject2 = Util.KInstantiate(Assets.GetPrefab("Oxygen_Mask".ToTag()));
				...
				gameObject2.GetComponent<PrimaryElement>().SetElement(elementID, addTags: false);
				SuitTank component2 = gameObject2.GetComponent<SuitTank>();
