...
public class ConsumablesTableScreen : TableScreen
{
	...
	protected override void OnActivate()
	{
		...
		for (int num = 0; num < EdiblesManager.GetAllFoodTypes().Count; num++)
		{
			list.Add(EdiblesManager.GetAllFoodTypes()[num]);
		}
		List<GameObject> prefabsWithTag = Assets.GetPrefabsWithTag(GameTags.Medicine);
		for (int num2 = 0; num2 < prefabsWithTag.Count; num2++)
		{
			MedicinalPillWorkable component = prefabsWithTag[num2].GetComponent<MedicinalPillWorkable>();
			if ((bool)component)
			{
				list.Add(component);
				continue;
			}
			DebugUtil.DevLogErrorFormat("Prefab tagged Medicine does not have MedicinalPill component: {0}", prefabsWithTag[num2]);
		}
		if (Game.IsDlcActiveForCurrentSave("DLC3_ID"))
		{
			...
			for (int num3 = 0; num3 < prefabsWithTag2.Count; num3++)
			{
				if (prefabsWithTag2[num3].HasTag(GameTags.DeprecatedContent))
				{
					continue;
				}
				if (array != null)
				{
					...
					{
						if (prefabsWithTag2[num3].HasTag(tag))
						{
							flag = true;
							break;
						}
				...
				Electrobank component2 = prefabsWithTag2[num3].GetComponent<Electrobank>();
				if ((bool)component2)
				{
					list.Add(component2);
					continue;
				}
				DebugUtil.DevLogErrorFormat("Prefab tagged ChargedPortableBattery does not have Electrobank component: {0}", prefabsWithTag2[num3]);
			}
			SymbolicConsumableItem item = new SymbolicConsumableItem(ConsumerManager.OXYGEN_TANK_ID, MISC.TAGS.OXYGENCANISTER, 1, 1, display: true, "ui_sprite_oxygen_canister", delegate
			{
		...
		{
			int num7 = a.MajorOrder.CompareTo(b.MajorOrder);
			if (num7 == 0)
			{
				num7 = a.MinorOrder.CompareTo(b.MinorOrder);
			}
			return num7;
		});
		...
		{
			if (!list[num6].Display)
			{
				continue;
			}
			if (list[num6].MajorOrder != num5 && num6 != 0)
			{
				string id = "QualityDivider_" + list[num6].MajorOrder;
				...
				DividerColumn dividerColumn = new DividerColumn(delegate
				{
					...
					ConsumableInfoTableColumn[] array3 = quality_group_columns;
					foreach (ConsumableInfoTableColumn consumableInfoTableColumn in array3)
					{
			...
			ConsumableInfoTableColumn item2 = AddConsumableInfoColumn(list[num6].ConsumableId, list[num6], on_load_consumable_info, get_value_consumable_info, on_click_consumable_info, set_value_consumable_info, compare_consumable_info, on_tooltip_consumable_info, on_tooltip_sort_consumable_info);
			list2.Add(item2);
			num5 = list[num6].MajorOrder;
			list4.Add(item2);
		}
	...
	private void on_load_consumable_info(IAssignableIdentity minion, GameObject widget_go)
	{
		...
		case TableRow.RowType.Header:
		{
			Image image3 = widget_go.GetComponent<HierarchyReferences>().GetReference("PortraitImage") as Image;
			GameObject gameObject = Assets.TryGetPrefab(consumable_info.ConsumableId.ToTag());
			if (gameObject == null)
			{
				image3.sprite = Assets.GetSprite(consumable_info.OverrideSpriteName());
				image3.material = Assets.UIPrefabs.TableScreenWidgets.DefaultUIMaterial;
			...
			if (component2.AnimFiles.Length != 0)
			{
				...
				image3.sprite = uISpriteFromMultiObjectAnim;
			}
			image3.color = Color.white;
			image3.material = ((ClusterManager.Instance.activeWorld.worldInventory.GetAmount(consumable_info.ConsumableId.ToTag(), includeRelatedWorlds: false) > 0f) ? Assets.UIPrefabs.TableScreenWidgets.DefaultUIMaterial : Assets.UIPrefabs.TableScreenWidgets.DesaturatedUIMaterial);
			break;
		}
		case TableRow.RowType.Default:
		...
		case TableRow.RowType.StoredMinon:
		{
			...
			case ResultValues.ConditionalGroup:
				component.ChangeState(2);
				break;
			case ResultValues.NotApplicable:
			...
			else if (foodInfo != null && minion as MinionIdentity != null)
			{
				...
				image2.color = new Color(0.72156864f, 0.44313726f, 0.5803922f, Mathf.Max((float)foodInfo.Quality - Db.Get().Attributes.FoodExpectation.Lookup(minion as MinionIdentity).GetTotalValue() + 1f, 0f) * 0.25f);
			}
			break;
		}
		}
	...
	private ResultValues get_value_consumable_info(IAssignableIdentity minion, GameObject widget_go)
	{
		...
		case TableRow.RowType.Minion:
			if (minion as MinionIdentity != null)
			{
				...
				result = ((!component.IsDietRestricted(consumable_info.ConsumableId)) ? (component.IsPermitted(consumable_info.ConsumableId) ? ResultValues.True : ResultValues.False) : ResultValues.NotApplicable);
			}
			else
			{
