...
public class CrewJobsScreen : CrewListScreen<CrewJobsEntry>
{
	...
	protected override void PositionColumnTitles()
	{
		...
		for (int num = 0; num < choreGroups.Count; num++)
		{
			ChoreGroup chore_group2 = choreGroups[num];
			Button b = EveryoneToggles.Keys.ElementAt(num);
			EveryoneToggles.Keys.ElementAt(num).onClick.AddListener(delegate
			{
