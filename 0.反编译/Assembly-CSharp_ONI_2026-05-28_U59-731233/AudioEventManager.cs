...
public class AudioEventManager : KMonoBehaviour
{
	...
	public List<PolluterDisplay> GetPollutersForCell(int cell)
	{
		...
		foreach (NoiseSplat item2 in spatialSplats.GetAllIntersecting(pos))
		{
			...
			if (!(loudness <= 0f))
			{
				PolluterDisplay item = new PolluterDisplay
				{
					name = item2.GetName(),
					value = LoudnessToDB(loudness),
					provider = item2.GetProvider()
				};
				polluters.Add(item);
			}
		}
	...
	private void RemoveExpiredSplats()
	{
		...
		int num = -1;
		for (int num2 = 0; num2 < removeTime.Count && !(removeTime[num2].first > Time.time); num2++)
		{
			NoiseSplat second = removeTime[num2].second;
			if (second != null)
			{
