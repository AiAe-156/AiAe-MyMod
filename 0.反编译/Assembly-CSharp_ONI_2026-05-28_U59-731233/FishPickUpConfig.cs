...
public class FishPickUpConfig : IBuildingConfig
{
	...
	public override void DoPostConfigureComplete(GameObject go)
	{
		...
		FixedCapturePoint.Def def2 = go.AddOrGetDef<FixedCapturePoint.Def>();
		def2.onAnimName = "on";
		def2.offAnimName = "off";
		def2.isAmountStoredOverCapacity = delegate(FixedCapturePoint.Instance smi, FixedCapturableMonitor.Instance capturable)
		{
		...
		def2.allowBabies = true;
		def2.captureCellOffset = new CellOffset(0, -1);
		def2.rancherInteractOffset = new CellOffset(0, 1);
		def2.postCaptureOffset = new CellOffset(0, 1);
		def2.logicPortId = "FishPickUpInput";
		def2.preCaptureAnimName = "working_pst";
		def2.getPreCaptureAnimSuffix = delegate(FixedCapturePoint.Instance smi)
		{
			BuildingPointStraw component = smi.GetComponent<BuildingPointStraw>();
			return (component != null) ? component.GetAnimSuffix() : "_1";
		};
		def2.getTargetCapturePoint = delegate(FixedCapturePoint.Instance smi)
		{
