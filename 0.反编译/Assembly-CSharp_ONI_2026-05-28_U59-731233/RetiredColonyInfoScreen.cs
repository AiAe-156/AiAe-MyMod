...
public class RetiredColonyInfoScreen : KModalScreen
{
	...
	private bool IsAchievementValidForDLCContext(IHasDlcRestrictions restrictions, string clusterTag)
	{
		if (DlcManager.IsCorrectDlcSubscribed(restrictions))
		{
			if (SaveLoader.Instance != null)
			{
				...
				return Game.IsCorrectDlcActiveForCurrentSave(restrictions);
			}
			return true;
		}
		return false;
	...
	private void PopulateAchievements()
	{
		...
		{
			if (!IsAchievementValidForDLCContext(resource, null))
			{
				continue;
			}
	...
	private IEnumerator ShowAchievementVeil()
	{
		...
		for (float i2 = 0f; i2 <= targetAlpha; i2 += Time.unscaledDeltaTime)
		{
			GameObject[] array2 = achievementVeils;
			foreach (GameObject go2 in array2)
			{
				go2.GetComponent<Image>().color = new Color(0f, 0f, 0f, i2);
			}
			yield return 0;
		}
		for (float i3 = 0f; i3 <= targetAlpha; i3 += Time.unscaledDeltaTime)
		{
	...
	private void UpdateAchievementData(RetiredColonyData data, string[] newlyAchieved = null)
	{
		...
		foreach (KeyValuePair<string, GameObject> achievementEntry in achievementEntries)
		{
			...
			if (data != null)
			{
				...
				foreach (string text in achievements)
				{
					if (text == achievementEntry.Key)
					{
						flag = true;
						break;
					}
			...
			if (colonyAchievement != null && !IsAchievementValidForDLCContext(colonyAchievement, colonyAchievement.clusterTag))
			{
			...
			if (!flag && data == null && this.retiredColonyData != null)
			{
				...
				foreach (RetiredColonyData retiredColonyData in array)
				{
					...
					foreach (string text2 in achievements2)
					{
						if (text2 == achievementEntry.Key)
						{
							flag2 = true;
						}
					}
	...
	private IEnumerator ComputeSizeStatGrid()
	{
		...
		yield return SequenceUtil.WaitForEndOfFrame;
		float targetAchievementWidth = base.gameObject.rectTransform().rect.width - explorerRoot.transform.parent.rectTransform().rect.width - 50f;
		targetAchievementWidth = Mathf.Min(830f, targetAchievementWidth);
		achievementsSection.GetComponent<LayoutElement>().preferredWidth = targetAchievementWidth;
	}
	...
	private void DisplayStatistics(RetiredColonyData data)
	{
		...
		int num = 2;
		for (int num2 = 0; num2 < data.Stats.Length; num2 += num)
		{
			...
			{
				if (num2 + num3 <= data.Stats.Length - 1)
				{
					RetiredColonyData.RetiredColonyStatistic retiredColonyStatistic = data.Stats[num2 + num3];
