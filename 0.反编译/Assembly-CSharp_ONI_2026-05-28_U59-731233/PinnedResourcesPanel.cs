...
public class PinnedResourcesPanel : KScreen, IRender1000ms
{
	public class PinnedResourceRow
	{
		...
		{
			bool flag = newResourceAmount != oldResourceAmount;
	...
	{
		Profiler.BeginSample("Sort Rows");
	...
	private void SyncRows()
	{
		...
		if (!flag)
		{
			foreach (KeyValuePair<Tag, PinnedResourceRow> row in rows)
			{
				bool flag2 = worldInventory.pinnedResources.Contains(row.Key) || worldInventory.notifyResources.Contains(row.Key) || (DiscoveredResources.Instance.newDiscoveries.ContainsKey(row.Key) && worldInventory.GetAmount(row.Key, includeRelatedWorlds: false) > 0f);
				if (flag2 != row.Value.gameObject.activeSelf)
				{
					flag = true;
					break;
				}
	...
	public void Refresh()
	{
		Profiler.BeginSample("PinnedResourcesPanel.Refresh");
		SyncRows();
		WorldInventory worldInventory = ClusterManager.Instance.GetWorld(ClusterManager.Instance.activeWorldId).worldInventory;
		bool flag = false;
		foreach (KeyValuePair<Tag, PinnedResourceRow> row in rows)
