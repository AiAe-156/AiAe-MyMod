...
public class SludgePressConfig : IBuildingConfig
{
	...
	private void AddRecipes(GameObject go)
	{
		...
		foreach (Element item in list)
		{
			...
			for (int num2 = 0; num2 < item.elementComposition.Length; num2++)
			{
				ElementComposition elementComposition = item.elementComposition[num2];
				Element element = ElementLoader.FindElementByName(elementComposition.elementID);
				bool isLiquid = element.IsLiquid;
				array2[num2] = new ComplexRecipe.RecipeElement(element.tag, num * elementComposition.percentage, ComplexRecipe.RecipeElement.TemperatureOperation.AverageTemperature, isLiquid);
			}
