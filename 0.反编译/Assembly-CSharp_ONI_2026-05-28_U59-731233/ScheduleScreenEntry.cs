...
public class ScheduleScreenEntry : KMonoBehaviour
{
	...
	public void Setup(Schedule schedule)
	{
		...
		int num = Mathf.CeilToInt(count / 24);
		for (int i = 0; i < num; i++)
		{
	...
	private void DuplicateTimetableRow(int sourceTimetableIdx)
	{
		...
		List<ScheduleBlock> range = blocks.GetRange(sourceTimetableIdx * 24, 24);
	...
	private void AddTimetableRow(int startingBlockIdx)
	{
		...
		ScheduleBlockPainter reference = component.GetReference<ScheduleBlockPainter>("BlockPainter");
		reference.SetEntry(this);
		KButton reference2 = component.GetReference<KButton>("DuplicateButton");
		reference2.onClick += delegate
		{
			DuplicateTimetableRow(timetableRows.IndexOf(row));
		};
		KButton reference3 = component.GetReference<KButton>("DeleteButton");
		reference3.onClick += delegate
		{
			RemoveTimetableRow(row);
		};
		KButton reference4 = component.GetReference<KButton>("RotateLeftButton");
		reference4.onClick += delegate
		{
			schedule.RotateBlocks(directionLeft: true, timetableRows.IndexOf(row));
		};
		KButton reference5 = component.GetReference<KButton>("RotateRightButton");
		...
		rotateUpButton.onClick += delegate
		{
			...
			bool flag = schedule.ShiftTimetable(up: true, timetableToShiftIdx);
		...
		{
			int timetableToShiftIdx = timetableRows.IndexOf(row);
			bool flag = schedule.ShiftTimetable(up: false, timetableToShiftIdx);
	...
	public bool PaintBlock(ScheduleBlockButton blockButton)
	{
		foreach (KeyValuePair<GameObject, List<ScheduleBlockButton>> item in blockButtonsByTimetableRow)
		{
			...
			{
				ScheduleBlockButton scheduleBlockButton = item.Value[i];
				if (scheduleBlockButton == blockButton)
				{
