...
public class AchievementWidget : KMonoBehaviour
{
	...
	private IEnumerator Flourish(float startDelay)
	{
		...
		float mysteryAdjustment = 1.1f;
		float scrollTarget = 1f + base.transform.localPosition.y * mysteryAdjustment / scrollRect.content.rect.height;
		...
		for (float i2 = 0f; i2 < 1.2f; i2 += Time.unscaledDeltaTime)
		{
			icon.transform.localScale = Vector3.one * flourish_iconScaleCurve.Evaluate(i2);
			sheenTransform.anchoredPosition = new Vector2(flourish_sheenPositionCurve.Evaluate(i2), sheenTransform.anchoredPosition.y);
			if (i2 > 1f && !colorChanged)
			{
				colorChanged = true;
		...
		CompleteFlourish();
		for (float i3 = 0f; i3 < 0.6f; i3 += Time.unscaledDeltaTime)
		{
			yield return SequenceUtil.WaitForNextFrame;
		}
