...
public class PlantSubSpeciesCatalog : KMonoBehaviour
{
	...
	private void RemoveInvalidMutantPlants()
	{
		...
		foreach (KeyValuePair<Tag, List<SubSpeciesInfo>> discoveredSubspeciesBySpecy in discoveredSubspeciesBySpecies)
		{
			GameObject prefab = Assets.GetPrefab(discoveredSubspeciesBySpecy.Key);
			if (prefab != null && prefab.GetComponent<MutantPlant>() == null)
			{
				list.Add(discoveredSubspeciesBySpecy.Key);
			}
		}
		foreach (Tag item in list)
		{
			List<SubSpeciesInfo> list2 = discoveredSubspeciesBySpecies[item];
			foreach (SubSpeciesInfo item2 in list2)
			{
				identifiedSubSpecies.Remove(item2.ID);
			}
			discoveredSubspeciesBySpecies.Remove(item);
		}
	}
}
