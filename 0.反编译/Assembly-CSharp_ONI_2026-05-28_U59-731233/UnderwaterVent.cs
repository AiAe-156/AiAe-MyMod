...
public class UnderwaterVent : GameStateMachine<UnderwaterVent, UnderwaterVent.Instance, IStateMachineTarget, UnderwaterVent.Def>
{
	...
	public new class Instance : GameInstance
	{
		...
		public void EruptionUpdate(float dt)
		{
			...
			if (num2 >= 1E-09f)
			{
				...
				Vector3 vector = Grid.CellToPos(cell) + base.def.data.BubbleSpawnOffset;
		...
		public void SpawnSolidDebri()
		{
			...
			{
				int cell = Grid.PosToCell(base.gameObject);
				Vector3 item2 = Grid.CellToPos(cell) + base.def.data.SolidSpawnOffset;
				item2.z = layerZ;
				list.Add(item2);
			}
		...
		{
			GameObject prefab = Assets.GetPrefab(base.def.data.SolidElement.CreateTag());
	...
	{
		return IsEntombed(smi) || !IsSubmerged(smi);
	}
