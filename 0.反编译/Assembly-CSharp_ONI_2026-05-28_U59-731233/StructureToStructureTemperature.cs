...
public class StructureToStructureTemperature : KMonoBehaviour
{
	...
	private List<int> conductiveCells = null;

	private HashSet<int> inContactBuildings = new HashSet<int>();

	private bool hasBeenRegister = false;
	...
	{
		List<InContactBuildingData> all_InContact_Buildings = GetAll_InContact_Buildings();
		foreach (InContactBuildingData item in all_InContact_Buildings)
		{
			Add(item);
		}
	}
