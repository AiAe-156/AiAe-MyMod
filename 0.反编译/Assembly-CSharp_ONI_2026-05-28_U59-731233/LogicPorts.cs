...
public class LogicPorts : KMonoBehaviour, IGameObjectEffectDescriptor, IRenderEveryTick
{
	...
	private void CreatePhysicalPorts(bool forceCreate = false)
	{
		...
		if (outputPortInfo != null)
		{
			...
			{
				for (int num2 = 0; num2 < outputPorts.Count; num2++)
				{
					LogicEventSender logicEventSender2 = outputPorts[num2] as LogicEventSender;
					logicEventSender2.SetValue(serializedOutputValues[num2]);
				}
			}
			else
			{
				for (int num3 = 0; num3 < outputPorts.Count; num3++)
				{
					LogicEventSender logicEventSender3 = outputPorts[num3] as LogicEventSender;
					logicEventSender3.SetValue(0);
				}
			}
		}
		...
		inputPorts = new List<ILogicUIElement>();
		for (int num4 = 0; num4 < inputPortInfo.Length; num4++)
		{
			Port info2 = inputPortInfo[num4];
	...
	private void OnLogicValueChanged(HashedString port_id, int new_value, int prev_value)
	{
		if (base.gameObject != null)
		{
			...
			base.gameObject.Trigger(-801688580, (object)logicValueChanged);
	...
	public int GetInputValue(HashedString port_id)
	{
		for (int i = 0; i < inputPortInfo.Length; i++)
		{
			...
			{
				if (!(inputPorts[i] is LogicEventHandler { Value: var value }))
				{
					return 0;
				}
				return value;
