...
public class Rottable : GameStateMachine<Rottable, Rottable.Instance, IStateMachineTarget, Rottable.Def>
{
	...
	public new class Instance : GameInstance, IRottable
	{
		...
		public void RefreshModifiers(float dt)
		{
			...
			if (!Grid.IsValidCell(cell) && !IsRottableInSpace())
			{
				return;
			}
	...
	public override void InitializeStates(out BaseState default_state)
	{
		...
		Fresh.ToggleStatusItem(Db.Get().CreatureStatusItems.Fresh, (Instance smi) => smi).ToggleTag(GameTags.RotModifierTags.Fresh).ParamTransition(rotParameter, Stale_Pre, (Instance smi, float p) => p <= smi.def.spoilTime - (smi.def.spoilTime - smi.def.staleTime))
