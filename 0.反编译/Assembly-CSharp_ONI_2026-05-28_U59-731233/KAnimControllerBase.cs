...
public abstract class KAnimControllerBase : MonoBehaviour, ISerializationCallbackReceiver
{
	...
	public HashedString currentAnim => (curAnim != null) ? curAnim.hash : default(HashedString);
	...
	{
		SetFGLayer(layer, hideSymbols: false);
	}

	public void SetFGLayer(Grid.SceneLayer layer, bool hideSymbols)
	{
		fgLayer = layer;
		GetLayering();
		if (layering != null)
		{
			layering.SetForegroundSceneLayer(fgLayer);
			if (hideSymbols)
			{
				layering.Refresh();
			}
	...
	public void RemoveAllAnimOverrides()
	{
		for (int i = 0; i < overrideAnimFiles.Count; i++)
		{
			KAnimFile file = overrideAnimFiles[i].file;
			if (file.GetData().build != null && file.GetData().build.symbols.Length != 0)
			{
				SymbolOverrideController component = GetComponent<SymbolOverrideController>();
				DebugUtil.Assert(component != null, "Anim overrides containing additional symbols require a symbol override controller.");
				component.TryRemoveBuildOverride(file.GetData());
			}
		}
		overrideAnimFiles.Clear();
	}
	...
	public bool HasAnimation(HashedString anim_name)
	{
		...
		if (flag)
		{
			bool flag2 = anims.ContainsKey(anim_name);
			bool flag3 = !flag2 && overrideAnims.ContainsKey(anim_name);
			flag = flag2 || flag3;
		}
		return flag;
	}
	...
	{
		if ((!layering.UsesLayers || layering.IsBaseLayer) && aem != null)
		{
