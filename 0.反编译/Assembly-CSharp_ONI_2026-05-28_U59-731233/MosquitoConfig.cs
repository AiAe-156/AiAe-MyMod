...
public class MosquitoConfig : IEntityConfig, IHasDlcRestrictions
{
	...
	{
		string text = CREATURES.SPECIES.MOSQUITO.NAME;
		GameObject prefab = CreateMosquito("Mosquito", CREATURES.SPECIES.MOSQUITO.NAME, CREATURES.SPECIES.MOSQUITO.DESC, "mosquito_kanim", is_baby: false);
		prefab = EntityTemplates.ExtendEntityToFertileCreature(prefab, this, "MosquitoEgg", CREATURES.SPECIES.MOSQUITO.EGG_NAME, CREATURES.SPECIES.MOSQUITO.DESC, "egg_mosquito_kanim", 1f, 0.5f, "MosquitoBaby", 4.5f, 2f, MosquitoTuning.EGG_CHANCES_BASE, EGG_SORT_ORDER, is_ranchable: false, add_fish_overcrowding_monitor: false, 0.75f, deprecated: false, preventEggFromDroppingProducts: true, 1f, allowEggCrackerRecipeCreation: false);
		prefab.AddTag(GameTags.OriginalCreature);
		MosquitoHungerMonitor mosquitoHungerMonitor = prefab.AddOrGet<MosquitoHungerMonitor>();
		...
		AgeMonitor.Def def = prefab.AddOrGetDef<AgeMonitor.Def>();
		def.minAgePercentOnSpawn = 0.5f;
		return prefab;
	}
