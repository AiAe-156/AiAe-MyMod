...
public class MovePickupableChore : Chore<MovePickupableChore.StatesInstance>
{
	...
	public class States : GameStateMachine<States, StatesInstance, MovePickupableChore>
	{
		...
		public override void InitializeStates(out BaseState default_state)
		{
			...
			{
				Pickupable pickupable = pickupablesource.Get<Pickupable>(smi);
			...
			fetch.approachCritter.Enter(delegate(StatesInstance smi)
			{
				...
				{
					IdleStates.Instance sMI = gameObject.GetSMI<IdleStates.Instance>();
					if (!sMI.IsNullOrStopped())
					{
						sMI.GoTo(sMI.sm.root);
					}
					FlopStates.Instance sMI2 = gameObject.GetSMI<FlopStates.Instance>();
					if (!sMI2.IsNullOrStopped())
					{
						sMI2.GoTo(sMI2.sm.root);
					}
					gameObject.GetComponent<Navigator>().Stop(arrived_at_destination: false, play_idle: false);
				}
			...
			approachstorage.deliveryStorage.InitializeStates(GetNavTactic, deliverer, deliverypoint, delivering.storing, delivering.deliverfail).Target(deliverer).EventHandler(GameHashes.OnStorageChange, delegate(StatesInstance smi, object data)
			{
				...
				if (gameObject != null)
				{
					GameObject gameObject2 = pickup.Get(smi);
		...
		private void DropPickupable(Storage storage, GameObject delivered)
		{
			if (delivered.GetComponent<Capturable>() != null)
			{
				...
				int cell = Grid.PosToCell(storage);
			...
			Movable component2 = delivered.GetComponent<Movable>();
			if (component2.onDeliveryComplete != null)
			{
				component2.onDeliveryComplete(delivered);
			}
		}
	...
	public GameObject movePlacer = null;
