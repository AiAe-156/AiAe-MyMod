...
public class RocketControlStation : StateMachineComponent<RocketControlStation.StatesInstance>, IGameObjectEffectDescriptor
{
	public class States : GameStateMachine<States, StatesInstance, RocketControlStation>
	{
		...
		{
			GameObject gameObject = clusterCraft.Get(smi);
	...
	public bool BuildingRestrictionsActive
	{
		get
		{
			...
			return RestrictWhenGrounded && gameObject != null && gameObject.gameObject.HasTag(GameTags.RocketOnGround);
		}
	}
	...
	{
		TagChangedEventData value = ((Boxed<TagChangedEventData>)obj).value;
		if (value.tag == GameTags.RocketOnGround)
		{
			Trigger(1861523068);
		}
	}
	...
	public List<Descriptor> GetDescriptors(GameObject go)
	{
		...
		list.Add(new Descriptor(UI.BUILDINGEFFECTS.ROCKETRESTRICTION_HEADER, UI.BUILDINGEFFECTS.TOOLTIPS.ROCKETRESTRICTION_HEADER));
		...
		list.Add(new Descriptor(UI.BUILDINGEFFECTS.ROCKETRESTRICTION_BUILDINGS.text.Replace("{buildinglist}", newValue), UI.BUILDINGEFFECTS.TOOLTIPS.ROCKETRESTRICTION_BUILDINGS.text.Replace("{buildinglist}", newValue)));
		return list;
	}
}
