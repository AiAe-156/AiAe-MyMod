...
{
	public LonelyMinionHouse.Instance House = null;
	...
	private void OnStoryStateChanged(StoryInstance.State state)
	{
		...
		if (state == StoryInstance.State.IN_PROGRESS)
		{
			...
			{
				string foodID = candidate.GetComponent<KPrefabID>().PrefabTag.Name;
				EdiblesManager.FoodInfo foodInfo = EdiblesManager.GetFoodInfo(foodID);
				int valueHandle = 0;
