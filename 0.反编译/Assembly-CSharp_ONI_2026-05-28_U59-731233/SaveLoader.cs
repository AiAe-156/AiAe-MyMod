...
public class SaveLoader : KMonoBehaviour
{
	...
	private void Save(BinaryWriter writer)
	{
		...
		SaveFileRoot saveFileRoot = PrepSaveFile();
	...
	private bool Load(IReader reader)
	{
		...
		string text2 = $"Mod Footprint ({saveFileRoot.active_mods.Count}):";
		...
		List<SimMessages.WorldOffsetData> worldOffsets = ClusterManager.Instance.WorldContainers.Select((WorldContainer container) => new SimMessages.WorldOffsetData
		{
	...
	public static string GetSavePrefix()
	{
		...
		return System.IO.Path.Combine(path, string.Format("{0}{1}", "save_files", System.IO.Path.DirectorySeparatorChar));
	}

	public static string GetCloudSavePrefix()
	{
		string path = Util.RootFolder();
		string path2 = System.IO.Path.Combine(path, string.Format("{0}{1}", "cloud_save_files", System.IO.Path.DirectorySeparatorChar));
			...
			foreach (string text in array)
			{
				try
				{
					if (!text.StartsWith("._"))
					{
						...
						SaveFileEntry item = new SaveFileEntry
						{
							path = text,
	...
	public static bool GetCloudSavesDefault()
	{
		...
		return !(cloudSavesDefaultPref == "Disabled");
	}
	...
	public bool LoadFromWorldGen()
	{
		...
		List<string> currentDlcMixingIds = CustomGameSettings.Instance.GetCurrentDlcMixingIds();
		foreach (string item in currentDlcMixingIds)
		{
		...
		{
			clusterDetailSave.globalNoiseSeed = item2.worldDetail.globalNoiseSeed;
			clusterDetailSave.globalTerrainSeed = item2.worldDetail.globalTerrainSeed;
			clusterDetailSave.globalWorldLayoutSeed = item2.worldDetail.globalWorldLayoutSeed;
			clusterDetailSave.globalWorldSeed = item2.worldDetail.globalWorldSeed;
			Vector2 vector = Grid.CellToPos2D(Grid.PosToCell(new Vector2I(item2.x, item2.y)));
			foreach (WorldDetailSave.OverworldCell overworldCell in item2.worldDetail.overworldCells)
			{
			...
			clusterDetailSave.overworldCells.AddRange(item2.worldDetail.overworldCells);
		}
		Sim.SIM_Initialize(Sim.DLL_MessageHandler);
		SimMessages.CreateSimElementsTable(ElementLoader.elements);
		Sim.AllocateCells(m_cluster.size.x, m_cluster.size.y);
		List<SimMessages.WorldOffsetData> worldOffsets = m_cluster.worlds.Select((WorldGen world) => new SimMessages.WorldOffsetData
		{
		...
		try
		{
			foreach (SimSaveFileStructure item3 in pooledList)
			{
				FastReader reader2 = new FastReader(item3.Sim);
	...
	private List<MinionMetricsData> GetMinionMetrics()
	{
		...
		foreach (MinionIdentity item in Components.MinionIdentities.Items)
		{
			...
			int instanceID = item.gameObject.GetComponent<KPrefabID>().InstanceID;
			bool flag = item.gameObject.HasTag(GameTags.Dead);
			...
			foreach (KeyValuePair<string, bool> item2 in component2.MasteryBySkillID)
			{
				if (item2.Value)
				{
					list3.Add(item2.Key);
	...
	private List<DailyReportMetricsData> GetDailyReportMetrics()
	{
		...
		if (dailyReport != null)
		{
			...
			{
				DailyReportMetricsData item = new DailyReportMetricsData
				{
					Name = reportEntry.reportType.ToString()
				};
	...
	{
		return IsAllDlcActiveForCurrentSave(required) && !IsAnyDlcActiveForCurrentSave(forbidden);
	}
