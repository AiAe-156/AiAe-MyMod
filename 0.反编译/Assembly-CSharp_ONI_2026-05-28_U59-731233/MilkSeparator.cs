...
public class MilkSeparator : GameStateMachine<MilkSeparator, MilkSeparator.Instance, IStateMachineTarget, MilkSeparator.Def>
{
	...
	public new class Instance : GameInstance
	{
		...
		{
			return base.def.CAVIAR_PRODUCTION_RATE > 0f && storage.GetAmountAvailable(base.def.FISHMILK_TAG) > 0f;
		}

		public void UpdateConverterStatusVisibility()
		{
			bool flag = storage.GetAmountAvailable(base.def.MILK_TAG) > 0f;
			bool flag2 = storage.GetAmountAvailable(base.def.FISHMILK_TAG) > 0f;
			for (int i = 0; i < elementConverters.Length; i++)
			{
				ElementConverter elementConverter = elementConverters[i];
				bool flag3 = false;
				for (int j = 0; j < elementConverter.consumedElements.Length; j++)
				{
					if (elementConverter.consumedElements[j].Tag == base.def.MILK_TAG)
					{
						flag3 = true;
						break;
					}
				...
				for (int k = 0; k < elementConverter.consumedElements.Length; k++)
				{
					if (elementConverter.consumedElements[k].IsActive != flag4)
					{
						elementConverter.SetConsumedElementActive(elementConverter.consumedElements[k].Tag, flag4);
					}
				}
				for (int l = 0; l < elementConverter.outputElements.Length; l++)
				{
					if (elementConverter.outputElements[l].IsActive != flag4)
					{
						elementConverter.SetOutputElementActive(elementConverter.outputElements[l].elementHash, flag4);
					}
				}
			}
		}
		...
		public void SpawnCaviar(float dt)
		{
			if (!(base.def.CAVIAR_PRODUCTION_RATE <= 0f) && !(storage.GetAmountAvailable(base.def.FISHMILK_TAG) <= 0f))
			{
				...
				{
					GameObject prefab = Assets.GetPrefab(base.def.CAVIAR_TAG);
		...
		{
			bool symbolVisible;
			Vector4 column = animController.GetSymbolTransform(new HashedString("object"), out symbolVisible).GetColumn(3);
		...
		public Color GetFatColor()
		{
			...
			return ((MilkFatStored <= 0f && CaviarStored <= 0f) ? flag2 : flag) ? ((Color)ElementLoader.FindElementByTag(base.def.MILK_FAT_TAG).substance.colour) : CaviarTuning.COLOR;
		}
	...
	public override void InitializeStates(out BaseState default_state)
	{
		...
		operational.working.work.Enter(BeginSeparation).Enter(UpdateConverterVisibility).PlayAnim("separating_loop", KAnim.PlayMode.Loop)
			.Update(SpawnCaviar)
			.ToggleStatusItem((Instance smi) => smi.IsProducingCaviar() ? Db.Get().BuildingStatusItems.MilkSeparatorProducingCaviar : null, (Instance smi) => smi)
			.EventHandler(GameHashes.OnStorageChange, UpdateConverterVisibility)
	...
	public static void UpdateConverterVisibility(Instance smi)
	{
		smi.UpdateConverterStatusVisibility();
	}
