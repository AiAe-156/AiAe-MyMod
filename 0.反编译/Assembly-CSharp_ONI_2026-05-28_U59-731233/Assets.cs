...
public class Assets : KMonoBehaviour, ISerializationCallbackReceiver
{
	...
	private void SubstanceListHookup()
	{
		...
		{
			GameTags.StartingMetalOres[num] = list[num].tag;
		}
		...
		{
			GameTags.StartingRefinedMetals[num2] = list2[num2].tag;
		}
		...
		{
			PLANTS.SAFE_ELEMENTS.AllWaters[num3] = list3[num3].id;
		}
		PLANTS.SAFE_ELEMENTS.MurkyWaters = PLANTS.SAFE_ELEMENTS.AllWaters.Append(new SimHashes[1] { SimHashes.Ink });
	}
	...
	public static string GetSimpleSoundEventName(EventReference event_ref)
	{
		...
		return GetSimpleSoundEventName(eventReferencePath);
	}
	...
	public static List<Type> GetPrefabsWithComponentAsListOfComponents<Type>()
	{
		...
		for (int i = 0; i < Prefabs.Count; i++)
		{
			Type component = Prefabs[i].GetComponent<Type>();
			if (component != null)
			{
				list.Add(component);
			}
