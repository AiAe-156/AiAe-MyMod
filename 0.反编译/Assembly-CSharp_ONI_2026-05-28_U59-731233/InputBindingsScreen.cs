...
public class InputBindingsScreen : KModalScreen
{
	...
	private string GetModifierString(Modifier modifiers)
	{
		...
		{
			if ((modifiers & value) != Modifier.None)
			{
				text = text + " + " + value;
			}
		}
	...
	private void CollectScreens()
	{
		...
		for (int i = 0; i < GameInputMapping.KeyBindings.Length; i++)
		{
			...
			if (bindingEntry.mGroup != null && bindingEntry.mRebindable && !screens.Contains(bindingEntry.mGroup) && DlcManager.IsCorrectDlcSubscribed(bindingEntry.dlcIds, null))
			{
	...
	private void BuildDisplay()
	{
		...
		for (int i = 0; i < GameInputMapping.KeyBindings.Length; i++)
		{
			...
			if (binding.mGroup == screens[activeScreen] && binding.mRebindable && DlcManager.IsCorrectDlcSubscribed(binding.dlcIds, null))
			{
	...
	private void Update()
	{
		...
		modifier = (Modifier)((int)modifier | (IsKeyDown(KeyCode.BackQuote) ? 16 : 0));
		bool flag = false;
