	...
	public class FishFeederBot
	{
		...
		public void RefreshStorage()
		{
			...
			foreach (GameObject item in botStorage.items)
			{
				if (!(item == null))
				{
					...
					item.transform.SetPosition(Grid.CellToPosCBC(cell2, Grid.SceneLayer.Ore));
				}
			}
			if (botStorage.IsEmpty())
			{
		...
		private void SetBallSymbol(GameObject stored_go)
		{
			if (!(stored_go == null))
			{
				...
				int cell = Grid.PosToCell(smi.transform.GetPosition());
				int cell2 = Grid.CellBelow(Grid.CellBelow(cell));
				stored_go.transform.SetPosition(Grid.CellToPosCBC(cell2, Grid.SceneLayer.BuildingUse));
			}
		}
	}
