...
public class TableScreen : ShowOptimizedKScreen
{
	...
	private bool scrollersDirty = false;

	private float targetScrollerPosition = 0f;
	...
	private void ZeroScrollers()
	{
		...
		foreach (string column_scroller in column_scrollers)
		{
			...
			{
				ScrollRect component = row.GetScroller(column_scroller).transform.parent.GetComponent<ScrollRect>();
	...
	public void PositionScrollers()
	{
		foreach (TableRow row in rows)
		{
			ScrollRect scroll_rect = row.scroll_rect;
			if (scroll_rect != null)
			{
				scroll_rect.horizontalNormalizedPosition = targetScrollerPosition;
			}
		}
		foreach (KeyValuePair<int, GameObject> worldDivider in worldDividers)
		{
			if (worldDivider.Value.activeInHierarchy)
			{
				...
				if (reference != null)
				{
					reference.horizontalNormalizedPosition = targetScrollerPosition;
				}
			}
		}
	...
	protected virtual void RefreshRows()
	{
		...
		foreach (KeyValuePair<int, bool> item5 in obsoleteWorldDividerStatus)
		{
			if (item5.Value)
			{
				Boxed<int> boxed = Boxed<int>.Get(item5.Key);
				RemoveWorldDivider(boxed);
				boxed.Release();
			}
		}
	...
	public void SortRows()
	{
		...
		{
			GameObject targetGameObject = item.Key.GetSoleOwner().GetComponent<MinionAssignablesProxy>().GetTargetGameObject();
			WorldContainer myWorld = targetGameObject.GetComponent<KMonoBehaviour>().GetMyWorld();
			int key = 255;
			if (myWorld != null)
			{
				key = myWorld.id;
	...
	protected void AddRow(IAssignableIdentity minion)
	{
		...
		if (!flag && obsoleteMinionRowStatus.ContainsKey(minion))
		{
			...
			TableRow tableRow = rows.Find((TableRow match) => match.GetIdentity() == minion);
			tableRow.RefreshColumns(columns);
			return;
		}
	...
	protected void AddWorldDivider(int worldId)
	{
		...
		TableRow component3 = gameObject.GetComponent<TableRow>();
		component3.ConfigureAsWorldDivider(columns, this);
	}

	protected void RemoveWorldDivider(object worldId)
	{
		int value = ((Boxed<int>)worldId).value;
		if (worldDividers.ContainsKey(value))
		{
			rows.Remove(worldDividers[value].GetComponent<TableRow>());
			Util.KDestroyGameObject(worldDividers[value]);
	...
	protected void on_load_name_label(IAssignableIdentity minion, GameObject widget_go)
	{
		...
		if (minion != null)
		{
			...
			if (locText2 != null)
			{
				...
				locText2.textWrappingMode = TextWrappingModes.Normal;
			}
			return;
		}
	...
	protected IEnumerator CascadeSetRowCheckBoxes(CheckboxTableColumn[] checkBoxToggleColumns, TableRow row, ResultValues state, GameObject ignore_widget = null)
	{
		...
		for (int i = 0; i < checkBoxToggleColumns.Length; i++)
		{
			...
			if (!(widget == ignore_widget) && checkBoxToggleColumns[i].isRevealed)
			{
				...
				{
				case ResultValues.False:
					needsSetting = ((state != ResultValues.False) ? true : false);
					break;
				case ResultValues.Partial:
				case ResultValues.ConditionalGroup:
					needsSetting = true;
					break;
				case ResultValues.True:
					needsSetting = state != ResultValues.True;
					break;
				}
				if (needsSetting)
				{
