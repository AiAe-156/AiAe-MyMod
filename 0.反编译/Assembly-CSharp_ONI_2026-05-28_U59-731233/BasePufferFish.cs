...
public class BasePufferFish
{
	...
	public static GameObject CreatePrefab(string id, string base_trait_id, string name, string description, string anim_file, bool is_baby, string symbol_prefix, float warnLowTemp, float warnHighTemp, float lethalLowTemp, float lethalHighTemp)
	{
		...
		trait.Add(new AttributeModifier(Db.Get().Amounts.Calories.deltaAttribute.Id, -1333.3334f, UI.TOOLTIPS.BASE_VALUE));
		...
		hashSet2.Add("Lettuce");
		HashSet<Tag> hashSet3 = new HashSet<Tag>();
		hashSet3.Add(SeaLettuceConfig.ID);
		string[] eat_anims = new string[3] { "eat_pre", "eat_loop", "idle_loop" };
		List<Diet.Info> list = new List<Diet.Info>
		{
			new Diet.Info(hashSet2, PufferFishTuning.POOP_ELEMENT, 400000f, 7.5f, null, 0f, produce_solid_tile: false, Diet.Info.FoodType.EatSolid, emmit_disease_on_cell: false, eat_anims),
			new Diet.Info(hashSet3, PufferFishTuning.POOP_ELEMENT, 400000f, 7.5f, null, 0f, produce_solid_tile: false, Diet.Info.FoodType.EatPlantDirectly, emmit_disease_on_cell: false, eat_anims)
		};
		Diet diet = new Diet(list.ToArray());
		CreatureCalorieMonitor.Def def3 = gameObject.AddOrGetDef<CreatureCalorieMonitor.Def>();
		def3.diet = diet;
		def3.minConsumedCaloriesBeforePooping = 400000f;
