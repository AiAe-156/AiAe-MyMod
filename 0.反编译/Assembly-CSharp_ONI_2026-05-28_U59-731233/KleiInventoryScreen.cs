...
public class KleiInventoryScreen : KModalScreen
{
	...
	private bool initConfigComplete = false;
	...
	public void PopulateCategories()
	{
		...
		foreach (KeyValuePair<string, List<string>> item in InventoryOrganization.categoryIdToSubcategoryIdsMap)
		{
			...
			GameObject gameObject = Util.KInstantiateUI(categoryRowPrefab, categoryListContent.gameObject, force_active: true);
	...
	private void AddItemToSubcategoryUIContainer(GameObject itemButton, string subcategoryId)
	{
		...
		if (kleiInventoryUISubcategory == null)
		{
			GameObject gameObject = Util.KInstantiateUI(subcategoryPrefab, galleryGridContent.gameObject, force_active: true);
			kleiInventoryUISubcategory = gameObject.GetComponent<KleiInventoryUISubcategory>();
	...
	private void RefreshGallery()
	{
		...
		foreach (KeyValuePair<PermitResource, MultiToggle> galleryGridButton in galleryGridButtons)
		{
			...
			if (dlcFilter.SelectedDLCID != null)
			{
				string dlcIdFrom = permitResource.GetDlcIdFrom();
				if (dlcIdFrom != dlcFilter.SelectedDLCID)
				{
					flag = false;
				}
			}
	...
	private void RefreshBarterPanel()
	{
		...
		Color color = new Color(1f, 59f / 85f, 59f / 85f);
		Color color2 = new Color(0.6f, 81f / 85f, 0.5019608f);
		...
		if (IS_ONLINE)
		{
			...
			case PermitPrintabilityState.Printable:
				...
				{
					GameObject gameObject = Util.KInstantiateUI(barterConfirmationScreenPrefab, LockerNavigator.Instance.gameObject);
					gameObject.rectTransform().sizeDelta = Vector2.zero;
					gameObject.GetComponent<BarterConfirmationScreen>().Present(SelectedPermit, isPurchase: true);
				};
				break;
			}
			if (sell_price == 0)
			...
			barterSellButton.onClick += delegate
			{
				GameObject gameObject = Util.KInstantiateUI(barterConfirmationScreenPrefab, LockerNavigator.Instance.gameObject);
				gameObject.rectTransform().sizeDelta = Vector2.zero;
				gameObject.GetComponent<BarterConfirmationScreen>().Present(SelectedPermit, isPurchase: false);
			};
		}
		else
		{
		...
		default:
			if (permit.Category == PermitCategory.Building)
			{
				...
				case "LuxuryBed":
				{
					string id = permit.Id;
					string text = id;
					if (!(text == "LuxuryBed_boat"))
					{
						if (text == "LuxuryBed_bouncy")
						{
			...
			if (permit.Category == PermitCategory.Artwork)
			{
				...
				if (Has<Sculpture>(buildingDef2))
				{
					...
					string text2 = prefabID;
					if (!(text2 == "IceSculpture"))
					{
						if (text2 == "WoodSculpture")
						{
