...
public class StarmapScreen : KModalScreen
{
	...
	public void UpdateDestinationStates()
	{
		...
		foreach (KeyValuePair<SpaceDestination, StarmapPlanet> KVP in planetWidgets)
		{
			...
			{
				stringBuilder.AppendFormat(UI.STARMAP.ANALYSIS_AMOUNT.text, GameUtil.GetFormattedPercent(100f * (SpacecraftManager.instance.GetDestinationAnalysisScore(KVP.Key) / (float)ROCKETRY.DESTINATION_ANALYSIS.COMPLETE)));
			}
	...
	private void OnSpaceDestinationAdded(object destinationObj)
	{
		...
		if ((float)spaceDestination.OneBasedDistance * 10000f > planetsMaxDistance)
		{
			planetsMaxDistance = (float)spaceDestination.OneBasedDistance * 10000f;
		}
		while (planetRows.Count < spaceDestination.distance + 1)
		{
		...
		GameObject gameObject2 = Util.KInstantiateUI(planetPrefab.gameObject, planetRows[spaceDestination.distance], force_active: true);
		planetWidgets.Add(spaceDestination, gameObject2.GetComponent<StarmapPlanet>());
	}
	...
	{
		return $"{distance:0} {UI.UNITSUFFIXES.DISTANCE.KILOMETER}";
	}
		...
		foreach (GameObject item in AttachableBuilding.GetAttachedNetwork(command.GetComponent<AttachableBuilding>()))
		{
			...
			if (component != null)
			{
				...
				else
				{
					stringBuilder.Append(component.gameObject.GetProperName());
					stringBuilder.Append(" ");
					stringBuilder.AppendFormat(UI.STARMAP.STORAGESTATS.STORAGECAPACITY, GameUtil.GetFormattedMass(0f, GameUtil.TimeSlice.None, GameUtil.MetricMassFormat.Kilogram), GameUtil.GetFormattedMass(component.storage.Capacity(), GameUtil.TimeSlice.None, GameUtil.MetricMassFormat.Kilogram));
					stringBuilder.Append("\n");
				}
			}
		}
	...
	private void LoadPlanets()
	{
		...
		{
			OnSpaceDestinationAdded(destination);
		}
		UpdateDestinationStates();
	}
	...
	private void FillRocketListPanel()
	{
		...
		foreach (Spacecraft rocket in spacecraft)
		{
			...
			else
			{
				...
				breakdownListRow7.ShowData(UI.STARMAP.ROCKETSTATUS.TIMEREMAINING, ((rocket.controlStationBuffTimeRemaining <= 0f) ? "" : UI.STARMAP.ROCKETSTATUS.BOOSTED_TIME_MODIFIER.text) + Util.FormatOneDecimalPlace(timeLeft / 600f) + " / " + GameUtil.GetFormattedCycles(duration));
	...
	private void UpdateRangeDisplay()
	{
		...
		if (currentCommandModule.robotPilotControlled)
		{
			RoboPilotModule component = currentCommandModule.GetComponent<RoboPilotModule>();
			...
			if (selectedDestination != null && num2 >= (float)selectedDestination.OneBasedDistance * 10000f)
			{
				dotColor = BreakdownListRow.Status.Green;
			}
			breakdownListRow5.ShowStatusData(UI.STARMAP.ROCKETSTATS.ROBO_PILOT_RANGE, GameUtil.GetFormattedDistance(num2), dotColor);
			breakdownListRow5.AddTooltip(string.Format(UI.STARMAP.ROCKETSTATS.ROBO_PILOT_EFFICIENCY, GameUtil.GetFormattedDistance(RoboPilotCommandModuleConfig.DATABANKRANGE * 1000f)));
		}
		BreakdownListRow breakdownListRow6 = rocketDetailsRange.AddRow();
		breakdownListRow6.ShowStatusData(UI.STARMAP.ROCKETSTATS.TOTAL_THRUST, GameUtil.GetFormattedDistance(currentCommandModule.rocketStats.GetTotalThrust() * 1000f), BreakdownListRow.Status.Green);
		breakdownListRow6.SetImportant(important: true);
		...
		breakdownListRow7.ShowStatusData(UI.STARMAP.ROCKETSTATUS.WEIGHTPENALTY, DisplayDistance(distance), BreakdownListRow.Status.Red);
		...
		BreakdownListRow breakdownListRow8 = rocketDetailsRange.AddRow();
		breakdownListRow8.ShowData(UI.STARMAP.ROCKETSTATS.TOTAL_RANGE, GameUtil.GetFormattedDistance(currentCommandModule.rocketStats.GetRocketMaxDistance() * 1000f));
		breakdownListRow8.SetImportant(important: true);
	}
			...
			if (SpacecraftManager.instance.GetDestinationAnalysisState(selectedDestination) == SpacecraftManager.DestinationAnalysisState.Complete)
			{
				foreach (KeyValuePair<Tag, int> recoverableEntity in selectedDestination.GetRecoverableEntities())
				{
					...
					if (currentRocketHasEntitiesContainer)
					{
						...
						breakdownListRow8.AddTooltip(string.Format(UI.STARMAP.CAN_CARRY_ELEMENT, prefab.GetProperName(), text2));
					}
					else
					{
						breakdownListRow8.SetDisabled(disabled: true);
						breakdownListRow8.AddTooltip(string.Format(UI.STARMAP.CANT_CARRY_ELEMENT, text2, prefab.GetProperName()));
					}
				}
			}
			if (SpacecraftManager.instance.GetDestinationAnalysisState(selectedDestination) == SpacecraftManager.DestinationAnalysisState.Complete)
	...
	private void UpdateMissionOverlay(LaunchConditionManager lcmToVisualize = null)
	{
		...
		if (lcmToVisualize != null && spacecraft != null)
		{
			...
			if (spacecraft.state != Spacecraft.MissionState.Grounded && spacecraftDestination.GetDestinationType().visitable)
			{
				...
				Vector2 vector5 = ((!flag) ? new Vector2(Mathf.Lerp(vector.x, vector3.x, num * 2f), Mathf.Lerp(vector.y, vector3.y, num * 2f)) : new Vector2(Mathf.Lerp(vector.x, vector3.x, 1f - num * 2f + 1f), Mathf.Lerp(vector.y, vector3.y, 1f - num * 2f + 1f)));
