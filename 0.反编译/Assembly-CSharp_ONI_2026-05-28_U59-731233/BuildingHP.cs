...
public class BuildingHP : Workable
{
	...
	public class SMInstance : GameStateMachine<States, SMInstance, BuildingHP, object>.GameInstance
	{
		...
		public void ShowDamagedEffect()
		{
			if (base.master.damageSourceInfo.takeDamageEffect != SpawnFXHashes.None)
			{
				...
				int cell2 = Grid.OffsetCell(cell, 0, buildingDef.HeightInCells - 1);
		...
		public void SetCrackOverlayValue(bool isCracked)
		{
			...
			if (!(component == null))
			{
				...
				for (int i = 0; i < kbacQueryList.Count; i++)
				{
					...
					component2.SetBlendValue(KBatchedAnimInstanceData.BlendActiveOptions.BuildingDamaged, isCracked);
				}
			}
		}
	}
	...
	public class States : GameStateMachine<States, SMInstance, BuildingHP>
	{
		...
		public override void InitializeStates(out BaseState default_state)
		{
			...
			damaged.Enter(delegate(SMInstance smi)
			{
				...
				if (component != null)
				{
					component.SetFlag(healthyFlag, value: false);
				}
				smi.ShowProgressBar(show: true);
				smi.master.Trigger(774203113, (object)smi.master);
				smi.SetCrackOverlayValue(isCracked: true);
				...
				.Exit(delegate(SMInstance smi)
				{
					...
					smi.ShowProgressBar(show: false);
					smi.SetCrackOverlayValue(isCracked: false);
				});
		}
	...
	public void DoDamage(int damage)
	{
		if (!invincible)
		{
			...
			Trigger(-1964935036, (object)this);
		}
	}
	...
	public void Repair(int repair_amount)
	{
		...
		Trigger(-1699355994, (object)this);
		if (hitpoints >= building.Def.HitPoints)
		{
			Trigger(-1735440190, (object)this);
		}
	}
	...
	private void OnDoBuildingDamage(object data)
	{
		if (!invincible)
		{
			damageSourceInfo = ((Boxed<DamageSourceInfo>)data).value;
