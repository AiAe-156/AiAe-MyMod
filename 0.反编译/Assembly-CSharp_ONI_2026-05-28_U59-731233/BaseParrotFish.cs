...
public class BaseParrotFish
{
	...
	public static GameObject CreatePrefab(string id, string base_trait_id, string name, string description, string anim_file, bool is_baby, string symbol_prefix, float warnLowTemp, float warnHighTemp, float lethalLowTemp, float lethalHighTemp)
	{
		...
		hashSet2.Add(SimHashes.Phosphorite.CreateTag());
		HashSet<Tag> hashSet3 = new HashSet<Tag>();
		hashSet3.Add("PlanktonCoral");
		List<Diet.Info> list = new List<Diet.Info>
		{
			new Diet.Info(hashSet2, produced_element, CALORIES_PER_KG_OF_CORAL_PICKUPABLE, CORAL_PICKUPABLE_TO_PRODUCT_EFFICIENCY),
			new Diet.Info(hashSet3, produced_element, CALORIES_PER_GROWTH_EATEN, GROWTH_TO_PRODUCT_EFFICIENCY, null, 0f, produce_solid_tile: false, Diet.Info.FoodType.EatPlantDirectly)
		};
		...
		SolidConsumerMonitor.Def def5 = gameObject.AddOrGetDef<SolidConsumerMonitor.Def>();
		def5.diet = diet;
		LureableMonitor.Def def6 = gameObject.AddOrGetDef<LureableMonitor.Def>();
		def6.lures = new Tag[1] { GameTags.Creatures.FishTrapLure };
