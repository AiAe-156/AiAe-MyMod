...
public class SapTree : GameStateMachine<SapTree, SapTree.StatesInstance, IStateMachineTarget, SapTree.Def>
{
	...
	public class StatesInstance : GameInstance
	{
		...
		private static Util.IterationInstruction checkForFoodVisitor(object obj, ref (StatesInstance, bool) context)
		{
			Pickupable pickupable = Unsafe.As<Pickupable>(obj);
			Edible component = pickupable.GetComponent<Edible>();
			if (component != null)
			{
				context.Item1.sm.foodItem.Set(pickupable.gameObject, context.Item1);
				context.Item2 = true;
			}
			return context.Item2 ? Util.IterationInstruction.Halt : Util.IterationInstruction.Continue;
		}

		public void CheckForFood()
		{
			(StatesInstance, bool) context = (this, false);
			GameScenePartitioner.Instance.VisitEntries<(StatesInstance, bool)>(feedExtents.x, feedExtents.y, feedExtents.width, feedExtents.height, GameScenePartitioner.Instance.pickupablesLayer, (GameScenePartitioner.VisitorRef<(StatesInstance, bool)>)checkForFoodVisitor, ref context);
			if (!context.Item2)
			{
	...
	public override void InitializeStates(out BaseState default_state)
	{
		...
		state.ToggleStatusItem(text, tooltip, "", StatusItem.IconType.Info, NotificationType.Neutral, allow_multiples: false, default(HashedString), 129022, null, null, main).ToggleTag(GameTags.PreventEmittingDisease).Enter(delegate(StatesInstance smi)
		{
		...
		State state2 = alive.normal.idle.PlayAnim("idle", KAnim.PlayMode.Loop);
		string text2 = CREATURES.STATUSITEMS.IDLE.NAME;
		string tooltip2 = CREATURES.STATUSITEMS.IDLE.TOOLTIP;
		main = Db.Get().StatusItemCategories.Main;
		state2.ToggleStatusItem(text2, tooltip2, "", StatusItem.IconType.Info, NotificationType.Neutral, allow_multiples: false, default(HashedString), 129022, null, null, main).ParamTransition(hasNearbyEnemy, alive.normal.attacking_pre, GameStateMachine<SapTree, StatesInstance, IStateMachineTarget, Def>.IsTrue).ParamTransition(storedSap, alive.normal.oozing, (StatesInstance smi, float p) => p >= smi.def.stomachSize)
