...
public class TerrainBG : KMonoBehaviour
{
	...
	public bool LargeImpactorFragmentsVisible => ClusterManager.Instance.activeWorld != null && ClusterManager.Instance.activeWorld.largeImpactorFragments == FIXEDTRAITS.LARGEIMPACTORFRAGMENTS.ALLOWED && SaveGame.Instance.ColonyAchievementTracker.largeImpactorState == ColonyAchievementTracker.LargeImpactorState.Defeated;
	...
	public Mesh CreateNorthernLightsPlane(string name)
	{
		...
		int[] array3 = new int[6];
		float num2 = 1f;
		float num3 = northernLightSkySize * 0.5f;
		array = new Vector3[4]
		{
			new Vector3(0f - num2, 0f - num3, 0f),
			new Vector3(num2, 0f - num3, 0f),
			new Vector3(0f - num2, num3, 0f),
