...
public class StorageTile : GameStateMachine<StorageTile, StorageTile.Instance, IStateMachineTarget, StorageTile.Def>
{
	...
	public new class Instance : GameInstance, IUserControlledCapacity
	{
		...
		public bool HasAnyDesiredContents => (TargetTag == INVALID_TAG) ? (!HasContents) : (AmountOfDesiredContentStored > 0f);

		public float AmountOfDesiredContentStored => (TargetTag == INVALID_TAG) ? 0f : storage.GetMassAvailable(TargetTag);
		...
		{
			return (treeFilterable.GetTags() == null || treeFilterable.GetTags().Count == 0) ? INVALID_TAG : treeFilterable.GetTags().GetRandom();
		}
		...
		public override void PostParamsInitialized()
		{
			...
			{
				GameObject prefab = Assets.GetPrefab(TargetTag);
				if (prefab == null)
				{
		...
		private void OnObjectReplaced(object data)
		{
			...
			foreach (GameObject item in list)
			{
				...
				component.Trigger(580035959, (object)value.Worker);
			}
		}
		...
		private void OnCopySettings(object sourceOBJ)
		{
			if (sourceOBJ != null)
			{
				GameObject go = (GameObject)sourceOBJ;
		...
		{
			int lastCellWorkerUsed = GetWorkable().LastCellWorkerUsed;
			Vector3 position = Grid.CellToPos(lastCellWorkerUsed) + Vector3.right * Grid.CellSizeInMeters * 0.5f + Vector3.up * Grid.CellSizeInMeters * 0.5f;
