...
public class DevAutoPlumber
{
	...
	public static void SetupSolidOreDelivery(Building building)
	{
		...
		if (component != null)
		{
			...
			GameObject gameObject = TrySpawnElementOreFromTag(requestedItemTag, Grid.PosToCell(building), component.Capacity);
			if (!(gameObject == null))
			{
			}
			return;
		...
		foreach (ComplexRecipe recipe in ComplexRecipeManager.Get().recipes)
		{
			foreach (Tag fabricator in recipe.fabricators)
			{
				...
				{
					GameObject gameObject2 = TrySpawnElementOreFromTag(recipeElement.material, Grid.PosToCell(building), recipeElement.amount * 10f);
					if (gameObject2 == null)
					{
					}
				}
	...
	private static GameObject PlaceSourceAndUtilityConduit(Building destinationBuilding, BuildingDef sourceDef, BuildingDef conduitDef, IUtilityNetworkMgr utlityNetworkManager, int[] conduitTypeLayers, PortSelection portSelection)
	{
		...
		int nearStartingCell = Grid.PosToCell(destinationBuilding);
		List<int> list2 = new List<int>(conduitTypeLayers);
		list2.Add(1);
		int cell = FindClearPlacementLocation(nearStartingCell, list2.ToArray(), list);
		bool flag = false;
		int num = 10;
		while (!flag)
		{
			...
			List<int> list3 = GenerateClearConduitPath(building, destinationBuilding, conduitTypeLayers, portSelection);
			if (list3 == null)
			{
			...
			{
				flag = true;
				BuildConduits(list3, conduitDef, utlityNetworkManager);
			}
		}
	...
	{
		int maxDepth = 20;
		FloodFill.Finder finder = new FloodFill.Finder(IsNeighborhoodClearBoundToArgs);
		FloodFill.BreadthTraverse(nearStartingCell, default(FloodFill.NoBoundary), FloodFill.GenerationGrid.Default(), new FloodFill.MaxDepth(maxDepth), finder);
		return finder.Cell;
		bool IsNeighborhoodClearBoundToArgs(int cell)
		{
			return IsNeighborhoodClear(cell, placementBlockingObjectLayers, rejectLocations);
		}
	}
