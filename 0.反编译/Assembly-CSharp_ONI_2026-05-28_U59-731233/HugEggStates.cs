...
public class HugEggStates : GameStateMachine<HugEggStates, HugEggStates.Instance, IStateMachineTarget, HugEggStates.Def>
{
	...
	public override void InitializeStates(out BaseState default_state)
	{
		...
		state.ToggleStatusItem(text, tooltip, "", StatusItem.IconType.Info, NotificationType.Neutral, allow_multiples: false, default(HashedString), 129022, null, null, main).OnTargetLost(target, behaviourcomplete);
	...
	{
		return (smi.sm.target.Get(smi).GetComponent<EggIncubator>() != null) ? smi.def.incubatorHugAnims : smi.def.hugAnims;
	}
