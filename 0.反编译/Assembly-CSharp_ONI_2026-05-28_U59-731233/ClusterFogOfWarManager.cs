...
public class ClusterFogOfWarManager : GameStateMachine<ClusterFogOfWarManager, ClusterFogOfWarManager.Instance, IStateMachineTarget, ClusterFogOfWarManager.Def>
{
	...
	public new class Instance : GameInstance
	{
		...
		{
			return ClusterGrid.Instance.IsValidCell(location) && GetRevealCompleteFraction(location) >= 1f;
		}
		...
		public void RevealLocation(AxialI location, int radius = 0, int peekRadius = 2)
		{
			...
			foreach (AxialI item in AxialUtil.GetAllPointsWithinRadius(location, radius))
			{
				flag |= RevealCellIfValid(item, peekRadius);
			}
			if (flag)
			{
				Game.Instance.BoxingTrigger(-1991583975, location);
			}
		}
		...
		public bool EarnRevealPointsForLocation(AxialI location, float points)
		{
			...
			{
				m_revealPointsByCell[location] = points;
				Game.Instance.BoxingTrigger(-1554423969, location);
			}
			if (IsLocationRevealed(location))
			{
