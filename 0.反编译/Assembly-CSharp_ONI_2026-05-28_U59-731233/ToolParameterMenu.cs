...
public class ToolParameterMenu : KMonoBehaviour
{
	...
	private int ToggleStateToMultiToggleInt(ToggleData data)
	{
		...
		{
			ToggleState.On => (!data.isToggleInclusive) ? 1 : 3, 
			ToggleState.Off => 0, 
			ToggleState.Disabled => 2, 
			_ => 0, 
		};
	}
	...
	public void PopulateMenu(ToggleData[] togglesData)
	{
		...
		ToggleGroup component = widgetContainer.GetComponent<ToggleGroup>();
		component.enabled = flag;
		foreach (ToggleData toggleData2 in togglesData)
		{
			GameObject gameObject = CreateToggleGameObject(toggleData2);
			widgets.Add(toggleData2.name, new Widget
			{
	...
	private void OnChange()
	{
		foreach (KeyValuePair<string, Widget> widget in widgets)
		{
			...
			gameObject.GetComponentInChildren<MultiToggle>().ChangeState(new_state_index);
		}
