...
public class BuildingLoader : KMonoBehaviour
{
	...
	protected override void OnPrefabInit()
	{
		...
		RegisterStandardItemMassOverrides();
	}
	...
	public GameObject CreateBuildingUnderConstruction(BuildingDef def)
	{
		...
		{
			gameObject.GetComponent<PrimaryElement>().MassPerUnit += GetEffectiveMass(def, i);
		}
	...
	public GameObject CreateBuildingComplete(GameObject go, BuildingDef def)
	{
		...
		for (int i = 0; i < def.Mass.Length; i++)
		{
			component2.MassPerUnit += GetEffectiveMass(def, i);
		}
	...
	{
		RegisterMassPerUnitOverride("BuildingGasket", 50f);
	}

	public static void RegisterMassPerUnitOverride(string materialCategory, float massPerUnit)
	{
		postConstructionMassPerUnitOverrides[materialCategory] = massPerUnit;
	}

	private static float GetEffectiveMass(BuildingDef def, int i)
	{
		if (postConstructionMassPerUnitOverrides.TryGetValue(def.MaterialCategory[i], out var value))
		{
			return def.Mass[i] * value;
		}
		return def.Mass[i];
	}
}
