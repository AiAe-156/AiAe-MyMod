...
public class SaveManager : KMonoBehaviour
{
	...
	private void SortAssociatedObjects(ref List<Tag> objectTags, List<Tag> associatedTags)
	{
		...
		foreach (Tag associatedTag in associatedTags)
		{
			if (associatedTag != tag && objectTags.Contains(associatedTag))
			{
				...
				objectTags.Insert(num + 1, associatedTag);
			}
		}
	}
	...
	public void Save(BinaryWriter writer)
	{
		...
		foreach (KeyValuePair<Tag, List<Tag>> item in dictionary)
		{
			SortAssociatedObjects(ref orderedKeys, item.Value);
		}
		...
		foreach (Tag orderedKey in orderedKeys)
		{
			...
			foreach (SaveLoadRoot item2 in list)
			{
		...
		foreach (Tag orderedKey2 in orderedKeys)
		{
			...
			foreach (SaveLoadRoot item3 in list2)
			{
				if (item3 == null || !(item3.GetComponent<SimCellOccupier>() == null))
				{
					continue;
				}
	...
	public bool Load(IReader reader)
	{
		...
		if (num != 7 || num2 > 37)
		{
			DebugUtil.LogWarningArgs($"SAVE FILE VERSION MISMATCH! Expected {7}.{37} but got {num}.{num2}");
			return false;
		}
		ClearScene();
		try
