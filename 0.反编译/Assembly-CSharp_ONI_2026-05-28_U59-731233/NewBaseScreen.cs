...
public class NewBaseScreen : KScreen
{
	...
	private void SpawnMinions(GameObject start_pad)
	{
		...
		Effect effect = Db.Get().effects.Get("AnewHope");
		...
		for (int i = 0; i < m_minionStartingStats.Length; i++)
		{
			...
			ApplyStartWorldBenefit(gameObject, start_pad, flag);
			if (component != null)
			{
	...
	public static void ApplyStartWorldBenefit(GameObject minion, GameObject start_pad, bool isBionic)
	{
		if (isBionic)
		{
			return;
		}
		WorldContainer myWorld = start_pad.GetMyWorld();
		if (myWorld != null && myWorld.worldTags != null && Array.IndexOf(myWorld.worldTags, new Tag("Aquatic")) >= 0)
		{
