...
public class FallingWater : KMonoBehaviour, ISim200ms
{
	...
	private struct ParticlePhysics
	{
		...
		public ParticlePhysics(Vector2 position, Vector2 velocity, int frame, ushort elementIdx, int worldIdx)
		{
			...
			colour = (element.IsMoltenMetal ? ((Color32)WaterCubes.MOLTEN_METAL_COLOR) : element.substance.colour);
			colour.a = (byte)(element.substance.Glows ? 255u : 0u);
	...
	private static Color MOLTEN_LIQUID_MIST_COLOR = new Color(1f, 0.8509804f, 8f / 15f, 1f);
	...
	protected override void OnPrefabInit()
	{
		...
		mistPool = new GameObjectPool(InstantiateMist, delegate
		{
		}, 16);
	}
	...
	public void AddParticle(Vector2 root_pos, ushort elementIdx, float base_mass, float temperature, byte disease_idx, int base_disease_count, bool skip_sound = false, bool skip_decor = false, bool debug_track = false, bool disable_randomness = false)
	{
		...
		if (!skip_sound)
		{
			if (!topSounds.TryGetValue(num, out var value))
			{
				value = new SoundInfo
				{
					handle = LoopingSoundManager.StartSound(liquid_top_loop, root_pos)
				};
			}
		...
		while (base_mass > 0f)
		{
			...
			if (state == Element.State.Solid || (Grid.Properties[num6] & 2) != 0)
			{
	...
	private bool SpawnLiquidTopDecor(float time, int cell, bool flip, Element element)
	{
		if (Grid.IsValidCell(cell) && Grid.Element[cell] == element)
		{
			...
			if (CameraController.Instance.IsVisiblePos(vector))
			{
				...
				{
					Color color = (element.IsMoltenMetal ? MOLTEN_LIQUID_MIST_COLOR : ((Color)element.substance.colour));
					value = default(MistInfo);
					value.fx = SpawnMist();
					value.fx.TintColour = color;
	...
	{
		for (int num = physics.Count - 1; num >= 0; num--)
		{
			int num2 = Grid.PosToCell(physics[num].position);
			if (num2 == cell)
			{
				ParticleProperties particleProperties = this.particleProperties[num];
				physics.RemoveAt(num);
				this.particleProperties.RemoveAt(num);
			}
		}
	}
	...
	{
		GameObject gameObject = mistPool.GetInstance();
	...
	private void OnSerializing()
	{
		...
		{
			SerializedParticleProperties item = new SerializedParticleProperties
			{
				elementID = elements[particleProperty.elementIdx].id,
				diseaseID = ((particleProperty.diseaseIdx != byte.MaxValue) ? diseases[particleProperty.diseaseIdx].IdHash : HashedString.Invalid),
	...
	private void Deserialize()
	{
		...
		if (serializedParticleProperties != null)
		{
			...
			{
				ParticleProperties item = new ParticleProperties
				{
					elementIdx = ElementLoader.GetElementIndex(serializedParticleProperty.elementID),
					diseaseIdx = ((serializedParticleProperty.diseaseID != HashedString.Invalid) ? diseases.GetIndex(serializedParticleProperty.diseaseID) : byte.MaxValue),
					mass = serializedParticleProperty.mass,
					temperature = serializedParticleProperty.temperature,
					diseaseCount = serializedParticleProperty.diseaseCount
				};
