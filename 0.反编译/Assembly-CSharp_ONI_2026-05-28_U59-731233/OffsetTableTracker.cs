...
public class OffsetTableTracker : OffsetTracker
{
	...
	public OffsetTableTracker(CellOffset[][] table, KMonoBehaviour cmp)
	{
		this.table = table;
		this.cmp = cmp;
		OnCellChangedClosure = OnCellChanged;
	}
	...
	protected override void UpdateCell(int previous_cell, int current_cell)
	{
		if (previous_cell != current_cell)
		{
			...
			{
				solidPartitionerEntry = GameScenePartitioner.Instance.Add("OffsetTableTracker.UpdateCell", cmp.gameObject, extents, GameScenePartitioner.Instance.solidChangedLayer, OnCellChangedClosure);
				validNavCellChangedPartitionerEntry = GameScenePartitioner.Instance.Add("OffsetTableTracker.UpdateCell", cmp.gameObject, extents, GameScenePartitioner.Instance.validNavCellChangedLayer, OnCellChangedClosure);
			}
			else
			{
