...
public class MinionStorage : KMonoBehaviour
{
	...
	private void CopyMinion(MinionIdentity src_id, StoredMinionIdentity dest_id)
	{
		...
		StoredMinionIdentity.IStoredMinionExtension[] array = components;
		foreach (StoredMinionIdentity.IStoredMinionExtension storedMinionExtension in array)
		{
			storedMinionExtension.PushTo(dest_id);
		}
	}
	...
	private static void CopyMinion(StoredMinionIdentity src_id, MinionIdentity dest_id)
	{
		...
		WearableAccessorizer component3 = dest_id.GetComponent<WearableAccessorizer>();
		component3.RestoreWearables(src_id.wearables, src_id.customClothingItems);
		ConsumableConsumer component4 = dest_id.GetComponent<ConsumableConsumer>();
		if (src_id.forbiddenTagSet != null)
		{
			component4.forbiddenTagSet = new HashSet<Tag>(src_id.forbiddenTagSet);
		}
		if (src_id.MasteryBySkillID != null)
		{
			MinionResume component5 = dest_id.GetComponent<MinionResume>();
			component5.RestoreResume(src_id.MasteryBySkillID, src_id.AptitudeBySkillGroup, src_id.grantedSkillIDs, src_id.TotalExperienceGained);
			component5.SetHats(src_id.currentHat, src_id.targetHat);
		}
		if (src_id.choreGroupPriorities != null)
		{
			ChoreConsumer component6 = dest_id.GetComponent<ChoreConsumer>();
			component6.SetChoreGroupPriorities(src_id.choreGroupPriorities);
		}
		AttributeLevels component7 = dest_id.GetComponent<AttributeLevels>();
		if (src_id.attributeLevels != null)
		{
			component7.SaveLoadLevels = src_id.attributeLevels.ToArray();
			component7.OnDeserialized();
		}
		Effects component8 = dest_id.GetComponent<Effects>();
		if (src_id.saveLoadImmunities != null)
		{
			foreach (Effects.SaveLoadImmunities saveLoadImmunity in src_id.saveLoadImmunities)
			{
				if (Db.Get().effects.Exists(saveLoadImmunity.effectID))
				{
					...
					component8.AddImmunity(effect, saveLoadImmunity.giverID, saveLoadImmunity.saved);
				}
			}
		}
		if (src_id.saveLoadEffects != null)
		{
			foreach (Effects.SaveLoadEffect saveLoadEffect in src_id.saveLoadEffects)
			{
				if (Db.Get().effects.Exists(saveLoadEffect.id))
				{
					...
					EffectInstance effectInstance = component8.Add(newEffect, saveLoadEffect.saved);
		...
		foreach (StoredMinionIdentity.IStoredMinionExtension storedMinionExtension in array)
		{
			storedMinionExtension.PullFrom(src_id);
		}
	}

	private static void OnDeserializedMinionSpawned(object deserializedMinionOBJ)
	{
		GameObject gameObject = (GameObject)deserializedMinionOBJ;
	...
	public GameObject DeserializeMinion(Guid id, Vector3 pos)
	{
		...
		GameObject sourceMinion = kPrefabID.gameObject;
		return DeserializeMinion(sourceMinion, pos);
	}
	...
	public static GameObject DeserializeMinion(GameObject sourceMinion, Vector3 pos)
	{
		...
		GameObject prefab = SaveLoader.Instance.saveManager.GetPrefab(BaseMinionConfig.GetMinionIDForModel(component.model));
