...
public class ClusterMapScreen : KScreen
{
	...
	{
		foreach (StarmapHexCellInventory value in StarmapHexCellInventory.AllInventories.Values)
		{
			StarmapHexCellInventoryVisuals component = value.GetComponent<StarmapHexCellInventoryVisuals>();
	...
	public bool TryHandleCancel()
	{
		...
		{
			ClusterDestinationSelector destinationSelector = m_destinationSelector;
			SetMode(Mode.Default);
			if (destinationSelector != null)
			{
				destinationSelector.Trigger(94158097);
			}
			return true;
		}
		return false;
	...
	protected override void OnShow(bool show)
	{
		...
		if (show)
		{
			...
			UpdateTearStatus();
			RefreshHexCellsInventoryVisuals();
			return;
		}
	...
	private void Update()
	{
		if (KInputManager.currentControllerIsGamepad && IsScreenActive())
		{
	...
	private void SetupVisGameObjects()
	{
		foreach (KeyValuePair<AxialI, List<ClusterGridEntity>> cellContent in ClusterGrid.Instance.cellContents)
		{
			foreach (ClusterGridEntity item in cellContent.Value)
			{
				...
				if (item.IsVisible && revealLevel != ClusterRevealLevel.Hidden && !m_gridEntityVis.ContainsKey(item))
				{
	...
	public void SelectHex(ClusterMapHex newSelectionHex)
	{
		...
		else if (m_mode == Mode.SelectDestination)
		{
			...
			if (path != null)
			{
				SetMode(Mode.FinishingSelectDestination);
	...
	{
		return m_cellVisByLocation[location].GetComponent<ClusterMapHex>();
	}
	...
	private void UpdatePaths()
	{
		...
		if (m_mode == Mode.SelectDestination && m_hoveredHex != null)
		{
			...
			if (m_selectedEntity != null)
			{
				IClusterRange component = m_selectedEntity.GetComponent<IClusterRange>();
				...
				{
					fail_reason = GameUtil.SafeStringFormat(UI.CLUSTERMAP.TOOLTIP_INVALID_DESTINATION_OUT_OF_RANGE, rangeInTiles, maxRangeInTiles);
				}
