	...
	private void RefreshToggles()
	{
		for (int i = 0; i < target.GetBitDepth(); i++)
		{
			...
			if (!toggles_by_int.ContainsKey(i))
			{
				...
				gameObject.GetComponent<HierarchyReferences>().GetReference<KImage>("stateIcon").color = (target.IsBitActive(i) ? activeColor : inactiveColor);
				gameObject.GetComponent<HierarchyReferences>().GetReference<LocText>("stateText").SetText(target.IsBitActive(i) ? UI.UISIDESCREENS.LOGICBITSELECTORSIDESCREEN.STATE_ACTIVE : UI.UISIDESCREENS.LOGICBITSELECTORSIDESCREEN.STATE_INACTIVE);
