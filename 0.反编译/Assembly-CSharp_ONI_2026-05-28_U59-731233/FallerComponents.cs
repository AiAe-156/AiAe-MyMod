...
public class FallerComponents : KGameObjectComponentManager<FallerComponent>
{
	...
	private static Action<Transform> OnLandedAction = OnLanded;
	...
	protected override void OnSpawn(HandleVector<int>.Handle h)
	{
		base.OnSpawn(h);
		FallerComponent new_data = GetData(h);
		new_data.cellChangedHandlerID = Singleton<CellChangeMonitor>.Instance.RegisterCellChangedHandler(new_data.transform, new_data.cellChangedCB, null, "FallerComponent.OnSpawn");
		SetData(h, new_data);
	}
	...
	private void OnCleanUpImmediate(HandleVector<int>.Handle h)
	{
		...
		{
			Singleton<CellChangeMonitor>.Instance.UnregisterCellChangedHandler(ref new_data.cellChangedHandlerID);
	...
	private static void AddGravity(Transform transform, Vector2 initial_velocity)
	{
		...
		{
			GameComps.Gravities.Add(transform.gameObject, initial_velocity, OnLandedAction);
	...
	private static void RemoveGravity(Transform transform)
	{
		...
		if (Grid.IsValidCell(cell2))
		{
			...
			new_data.partitionerEntry = GameScenePartitioner.Instance.Add("Faller", transform.gameObject, cell2, GameScenePartitioner.Instance.solidChangedLayer, new_data.solidChangedCB);
		}
		GameComps.Fallers.SetData(h, new_data);
	}
