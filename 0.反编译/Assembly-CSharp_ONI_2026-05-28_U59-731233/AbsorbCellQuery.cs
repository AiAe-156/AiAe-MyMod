...
public class AbsorbCellQuery : PathFinderQuery
{
	...
	public override bool IsMatch(int cell, int parent_cell, int cost)
	{
		...
		if (all_conditions_met)
		{
			...
			float num10 = 10f * breathableCellRatioInSample;
			float num11 = totalBreathableMassAroundCell * num10 - num8;
			bool flag = false;
			...
			bool flag5 = num9 >= (float)targetCellSafetyFlags || !flag2;
			float num12 = targetOxygenScore;
			if (criticalMode)
			{
				num12 = Mathf.Min(num2, num12);
			}
			if (num11 >= num12 && flag5)
			{
				if (criticalMode)
				{
					if (flag3 || !flag4)
