...
public class BubbleManager : KMonoBehaviour, ISim33ms, IRenderEveryTick
{
	...
	private bool isInfraredON = false;
	...
	public void OnDeserialized()
	{
		...
		foreach (Archetype.Id key in archetypes.Keys)
		{
			int num = 0;
		...
		foreach (WorldArchetype key2 in bubbles.Keys)
		{
			if (!archetypes.ContainsKey(key2.archetype))
			{
				pooledList2.Add(key2);
			}
		}
	...
	private static bool ShouldPop(Vector2 position, SimHashes bubbleElement, out int cell)
	{
		...
		return id == bubbleElement || !UnderwaterSoundEvent.IsVisiblyInLiquid(position);
	}
	...
	public void Sim33ms(float dt)
	{
		...
		foreach (var (item, instanceData2) in bubbles)
		{
			...
			foreach (InstanceData.Subscript item2 in instanceData2)
			{
				...
				{
					SimMessages.AddRemoveSubstance(Grid.Solid[cell] ? Grid.PosToCell(current.Position) : cell, value.element, CellEventLogger.Instance.FallingWaterAddToSim, current.Mass, current.Temperature, current.Disease.Idx, current.Disease.Count);
					pooledList.Add(current.Index);
				}
				if (!current.FadingOut)
				{
					Vector2 position = vector + velocity * 2f;
					if (ShouldPop(position, value.element, out var _))
					{
