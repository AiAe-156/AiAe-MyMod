...
public class FossilHuntInitializer : StoryTraitStateMachine<FossilHuntInitializer, FossilHuntInitializer.Instance, FossilHuntInitializer.Def>
{
	public class Def : TraitDef
	{
		...
		public bool IsMainDigsite = false;
	...
	public new class Instance : TraitInstance
	{
		...
		{
			base.StartSM();
			MajorFossilDigSite sMI = base.gameObject.GetSMI<MajorFossilDigSite>();
		...
		{
			List<WorldGenSpawner.Spawnable> spawnablesWithTag = SaveGame.Instance.worldGenSpawner.GetSpawnablesWithTag(includeSpawned: false, tags);
			foreach (WorldGenSpawner.Spawnable item in spawnablesWithTag)
			{
		...
		public void ShowLoreUnlockedPopup(int popupID)
		{
			...
			string lockId = FossilDigSiteConfig.FOSSIL_HUNT_LORE_UNLOCK_ID.For(popupID);
			bool flag = CodexCache.GetEntryForLock(lockId) != null;
		...
		{
			InfoDialogScreen infoDialogScreen = LoreBearer.ShowPopupDialog().SetHeader(CODEX.STORY_TRAITS.FOSSILHUNT.QUEST_AVAILABLE_POPUP.NAME).AddDefaultOK();
			infoDialogScreen.AddPlainText(((Option<string>)CODEX.STORY_TRAITS.FOSSILHUNT.QUEST_AVAILABLE_POPUP.DESCRIPTION.text).Value).AddOption(CODEX.STORY_TRAITS.FOSSILHUNT.QUEST_AVAILABLE_POPUP.CHECK_BUTTON, delegate(InfoDialogScreen dialog)
			{
				dialog.Deactivate();
