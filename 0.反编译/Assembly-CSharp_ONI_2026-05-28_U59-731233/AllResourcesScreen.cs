...
public class AllResourcesScreen : ShowOptimizedKScreen, ISim4000ms, ISim1000ms
{
	private class ScreenRowBase
	{
		...
		{
			bool flag = newAvailableResourceAmount != oldAvailableResourceAmount;
		...
		{
			bool flag = newTotalResourceAmount != oldTotalResourceAmount;
			if (flag && updateIfTrue)
			{
		...
		{
			bool flag = newReservedResourceAmount != oldReserverResourceAmount;
			if (flag && updateIfTrue)
			{
	...
	{
		Profiler.BeginSample("SetRowsActive");
		foreach (KeyValuePair<Tag, CategoryRow> categoryRow in categoryRows)
		{
