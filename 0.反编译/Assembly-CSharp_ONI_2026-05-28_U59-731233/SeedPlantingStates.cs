...
public class SeedPlantingStates : GameStateMachine<SeedPlantingStates, SeedPlantingStates.Instance, IStateMachineTarget, SeedPlantingStates.Def>
{
	...
	public override void InitializeStates(out BaseState default_state)
	{
		...
		state.ToggleStatusItem(text, tooltip, "", StatusItem.IconType.Info, NotificationType.Neutral, allow_multiples: false, default(HashedString), 129022, null, null, main).Exit(UnreserveSeed).Exit(DropAll)
	...
	private static void AddMouthOverride(Instance smi)
	{
		...
		KAnim.Build.Symbol symbol = component2.AnimFiles[0].GetData().build.GetSymbol(smi.def.prefix + "sq_mouth_cheeks");
		if (symbol != null)
		{
	...
	{
		SymbolOverrideController component = smi.GetComponent<SymbolOverrideController>();
		component.TryRemoveSymbolOverride("sq_mouth", 1);
	}
	...
	private static void FindSeed(Instance smi)
	{
		...
		{
			if ((plantableSeed.HasTag(GameTags.Seed) || plantableSeed.HasTag(GameTags.CropSeed)) && !plantableSeed.HasTag(GameTags.Creatures.ReservedByCreature) && !(Vector2.Distance((Vector2)smi.transform.position, (Vector2)plantableSeed.transform.position) > 25f))
			{
