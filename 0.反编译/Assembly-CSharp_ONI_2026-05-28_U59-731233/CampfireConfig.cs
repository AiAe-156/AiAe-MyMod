...
public class CampfireConfig : IBuildingConfig
{
	...
	{
		BuildingDef buildingDef = BuildingTemplates.CreateBuildingDef("Campfire", 1, 2, "campfire_small_kanim", 100, 10f, BUILDINGS.CONSTRUCTION_MASS_KG.TIER2, MATERIALS.RAW_METALS, 9999f, BuildLocationRule.OnFloor, noise: NOISE_POLLUTION.NONE, decor: DECOR_ON, temperature_modification_mass_scale: 0.1f);
	...
	public override void ConfigureBuildingTemplate(GameObject go, Tag prefab_tag)
	{
		...
		ColdImmunityProvider.Def def3 = go.AddOrGetDef<ColdImmunityProvider.Def>();
		def3.range = new CellOffset[2][]
		{
			new CellOffset[2]
			{
				new CellOffset(-1, 0),
	...
	private void AddVisualizer(GameObject go)
	{
		...
		EntityCellVisualizer entityCellVisualizer = go.AddOrGet<EntityCellVisualizer>();
		entityCellVisualizer.AddPort(EntityCellVisualizer.Ports.HeatSource, default(CellOffset));
	}
}
