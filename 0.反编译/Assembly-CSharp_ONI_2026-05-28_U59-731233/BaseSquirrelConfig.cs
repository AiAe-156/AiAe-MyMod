...
public static class BaseSquirrelConfig
{
	...
	{
		GameObject gameObject = EntityTemplates.CreatePlacedEntity(id, name, desc, 100f, decor: DECOR.BONUS.TIER0, anim: Assets.GetAnim(is_baby ? anim_file : "squirrel_build_kanim"), initialAnim: "idle_loop", sceneLayer: Grid.SceneLayer.Creatures, width: 1, height: 1);
		string navGridName = "SquirrelNavGrid";
		if (is_baby)
		{
			navGridName = "DreckoBabyNavGrid";
		}
		EntityTemplates.ExtendEntityToBasicCreature(isWarmBlooded: false, gameObject, anim_file, is_baby ? null : "squirrel_build_kanim", symbolOverridePrefix, FactionManager.FactionID.Pest, traitId, navGridName, NavType.Floor, 32, 2f, "Meat", 1f, drownVulnerable: true, entombVulnerable: false, 283.15f, 313.15f, 228.15f, 373.15f);
	...
	public static Diet.Info[] BasicDiet(Tag poopTag, float caloriesPerKg, float producedConversionRate, string diseaseId, float diseasePerKgProduced)
	{
		...
		{
			new Diet.Info(hashSet, poopTag, caloriesPerKg, producedConversionRate, diseaseId, diseasePerKgProduced, produce_solid_tile: false, Diet.Info.FoodType.EatPlantDirectly)
		};
	}
