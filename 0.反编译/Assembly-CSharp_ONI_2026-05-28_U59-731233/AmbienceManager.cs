...
public class AmbienceManager : KMonoBehaviour
{
	...
	public class LiquidLayer : Layer
	{
		...
		public override void Reset()
		{
			base.Reset();
			boilingTileCount = 0;
			averageBoilIntensity = 0f;
		}

		public override void UpdatePercentage(int cell_count)
		{
			base.UpdatePercentage(cell_count);
			boilTilePercentage = (float)boilingTileCount / (float)cell_count;
		}

		public override void UpdateParameters(Vector3 emitter_position)
		{
			base.UpdateParameters(emitter_position);
			soundEvent.setParameterByName("Boiling_Tile_Percentage", boilTilePercentage);
		}

		public override void UpdateAverageTemperature()
		{
			base.UpdateAverageTemperature();
			UpdateAverageBoilIntensity();
		}

		public void UpdateAverageBoilIntensity()
		{
			averageBoilIntensity = ((tileCount > 0) ? (averageBoilIntensity / (float)tileCount) : 0f);
			soundEvent.setParameterByName("Boiling_Intensity", averageBoilIntensity);
		}
	}
	...
	public class Layer : IComparable<Layer>
	{
		...
		public static Comparison<Layer> DefaultComparitor = (Layer a, Layer b) => a.CompareTo(b);
	...
	public class Quadrant
	{
		...
		public Layer[] gasLayers = new Layer[6];

		public LiquidLayer[] liquidLayers = new LiquidLayer[6];
		...
		public Layer[] solidLayers = new Layer[21];
		...
		public Quadrant(QuadrantDef def)
		{
			...
			for (int i = 0; i < gasLayers.Length; i++)
			{
				gasLayers[i] = new Layer(def.gasSounds[i]);
				liquidLayers[i] = new LiquidLayer(def.liquidSounds[i]);
		...
		public void Update(Vector2I min, Vector2I max, Vector3 emitter_position)
		{
				...
				for (int k = min.x; k < max.x; k++)
				{
					...
					if (Grid.IsVisible(num2))
					{
						...
						else
						{
							...
							if (element != null)
							{
								if (element.IsLiquid && Grid.IsSubstantialLiquid(num2))
								{
									AmbienceType ambience = element.substance.GetAmbience();
									if (ambience != AmbienceType.None)
									{
										liquidLayers[(int)ambience].tileCount++;
										liquidLayers[(int)ambience].averageTemperature += Grid.Temperature[num2];
										float num3 = Mathf.Clamp01(element.GetRelativeHeatLevel(Grid.Temperature[num2]) - BoilingTreshold) / num;
										liquidLayers[(int)ambience].boilingTileCount += ((num3 > 0f) ? 1 : 0);
										liquidLayers[(int)ambience].averageBoilIntensity += num3;
									}
								}
								else if (element.IsGas)
								{
									...
									if (ambience2 != AmbienceType.None)
									{
										...
										gasLayers[(int)ambience2].averageTemperature += Grid.Temperature[num2];
									}
								}
						...
						{
							radiationLayer.averageRadiation += Grid.Radiation[num2];
			...
			loopingLayers.Sort(Layer.DefaultComparitor);
			topLayers.Clear();
	...
	protected override void OnSpawn()
	{
		...
		BoilingTreshold = LiquidMaterial.GetFloat("_BoilingTreshold");
	...
	protected override void OnForcedCleanUp()
	{
		...
		foreach (Quadrant quadrant in array)
		{
			List<Layer> allLayers = quadrant.GetAllLayers();
			foreach (Layer item in allLayers)
			{
				item.Stop();
			}
		}
	...
	private void LateUpdate()
	{
		...
		quadrants[0].Update(new Vector2I(min.x, min.y), new Vector2I(vector2I.x, vector2I.y), new Vector3(vector2.x + vector6.x, vector2.y + vector6.y, emitterZPosition));
		quadrants[1].Update(new Vector2I(vector2I.x, min.y), new Vector2I(max.x, vector2I.y), new Vector3(vector3.x + vector6.x, vector2.y + vector6.y, emitterZPosition));
		quadrants[2].Update(new Vector2I(min.x, vector2I.y), new Vector2I(vector2I.x, max.y), new Vector3(vector2.x + vector6.x, vector3.y + vector6.y, emitterZPosition));
		quadrants[3].Update(new Vector2I(vector2I.x, vector2I.y), new Vector2I(max.x, max.y), new Vector3(vector3.x + vector6.x, vector3.y + vector6.y, emitterZPosition));
		float num = 0f;
		float num2 = 0f;
		float num3 = 0f;
		for (int i = 0; i < quadrants.Length; i++)
