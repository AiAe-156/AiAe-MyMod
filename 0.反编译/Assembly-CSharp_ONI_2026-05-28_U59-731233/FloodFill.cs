...
public static class FloodFill
{
	...
	public static void BreadthTraverse<BoundaryCondition, VisitTracker, MyMaxDepth, Visitor>(int startCell, BoundaryCondition boundaryCondition, VisitTracker visited, MyMaxDepth maxDepth, Visitor visitor) where BoundaryCondition : IBoundaryCondition where VisitTracker : IVisitTracker where MyMaxDepth : IMaxDepth where Visitor : IVisitor
	{
		...
		while (value.Count > 0)
		{
			...
			uint num2 = (uint)((num & 0x1FFFFFFF00000000L) >> 32);
			uint num3 = num2 + 1;
			DebugUtil.DevAssert(num3 <= 536870911, "nextDepth overflowed allocated bitfield");
			DebugUtil.DevAssert(num3 <= int.MaxValue, "nextDepth cannot be cast to int");
			if (maxDepth.Check((int)num3))
			{
				ulong num4 = (ulong)num3 << 32;
				value.Enqueue((ulong)Grid.CellLeft(cell) | num4);
				value.Enqueue((ulong)Grid.CellRight(cell) | num4);
				value.Enqueue((ulong)Grid.CellAbove(cell) | num4);
				value.Enqueue((ulong)Grid.CellBelow(cell) | num4);
			}
		}
	}
	...
	{
		int num = Find(fn, start_cell, max_depth, stop_at_solid, stop_at_liquid);
		return num != -1;
	}
