...
public class CraftModuleInterface : KMonoBehaviour, ISim4000ms
{
	...
	public float BurnableMassRemaining
	{
		get
		{
			...
			if (engine != null)
			{
				return engine.requireOxidizer ? Mathf.Min(FuelRemaining, OxidizerPowerRemaining) : FuelRemaining;
			}
			return 0f;
		}
	}
	...
	public float BurnableMassCapacity
	{
		get
		{
			...
			if (engine != null)
			{
				return engine.requireOxidizer ? Mathf.Min(FuelCapacity, OxidizerCapacity) : FuelCapacity;
			}
			return 0f;
		}
	}
	...
	public void RemoveModule(RocketModuleCluster module)
	{
		...
		{
			RocketModuleCluster rocketModuleCluster = clusterModule.Get();
			rocketModuleCluster.Trigger(1512695988);
		}
		SetBottomModule();
	...
	{
		return EvaluateConditionSet(ProcessCondition.ProcessConditionType.RocketPrep) != ProcessCondition.Status.Failure && EvaluateConditionSet(ProcessCondition.ProcessConditionType.RocketStorage) != ProcessCondition.Status.Failure && EvaluateConditionSet(ProcessCondition.ProcessConditionType.RocketFlight) != ProcessCondition.Status.Failure;
	}

	public bool CheckReadyToLaunch()
	{
		return EvaluateConditionSet(ProcessCondition.ProcessConditionType.RocketPrep) != ProcessCondition.Status.Failure && EvaluateConditionSet(ProcessCondition.ProcessConditionType.RocketStorage) != ProcessCondition.Status.Failure && EvaluateConditionSet(ProcessCondition.ProcessConditionType.RocketFlight) != ProcessCondition.Status.Failure && EvaluateConditionSet(ProcessCondition.ProcessConditionType.RocketBoard) != ProcessCondition.Status.Failure;
	}
	...
	public RocketModuleCluster GetPrimaryPilotModule(out bool is_robo_pilot)
	{
		...
		{
			RocketModuleCluster rocketModuleCluster = clusterModule.Get();
			PassengerRocketModule component = rocketModuleCluster.GetComponent<PassengerRocketModule>();
			if (component != null)
			{
	...
	{
		GameObject gameObject = Util.KInstantiate(position: new Vector3(-1f, -1f, 0f), original: Assets.GetPrefab("DebrisPayload"));
		...
		foreach (GameObject item3 in list2)
		{
			...
			if (component3 != null)
			{
				...
				while (component3.PrimaryElement.Mass > storage2.RemainingCapacity())
				{
					...
					storage2.Store(pickupable.gameObject);
					storage2 = SpawnRocketDebris(" from CMI", elementID);
					list4.Add(storage2);
				}
				if (component3.PrimaryElement.Mass > 0f)
				{
					storage2.Store(component3.gameObject);
				}
			}
		}
