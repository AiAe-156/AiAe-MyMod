...
public class StoredMinionIdentity : KMonoBehaviour, ISaveLoadable, IAssignableIdentity, IListableOption, IPersonalPriorityManager
{
	...
	protected override void OnSpawn()
	{
		...
		if (model == BionicMinionConfig.MODEL)
		{
			attributes = BionicMinionConfig.GetAttributes();
			amounts = BionicMinionConfig.GetAmounts();
			traits = BionicMinionConfig.GetTraits();
		}
		BaseMinionConfig.AddMinionAttributes(minionModifiers, attributes);
		BaseMinionConfig.AddMinionAmounts(minionModifiers, amounts);
		BaseMinionConfig.AddMinionTraits(BaseMinionConfig.GetMinionNameForModel(model), BaseMinionConfig.GetMinionBaseTraitIDForModel(model), minionModifiers, traits);
		ValidateProxy();
		CleanupLimboMinions();
	}
