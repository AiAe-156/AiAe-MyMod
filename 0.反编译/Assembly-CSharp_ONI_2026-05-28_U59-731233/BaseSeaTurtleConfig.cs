...
public static class BaseSeaTurtleConfig
{
	...
	public static GameObject CreatePrefab(string id, string base_trait_id, string name, string description, string anim_file, bool is_baby, string symbol_prefix, float warnLowTemp, float warnHighTemp, float lethalLowTemp, float lethalHighTemp)
	{
		...
		def2.lures = new Tag[1] { GameTags.Creatures.FishTrapLure };
		...
		{
			new Diet.Info(hashSet, SeaTurtleTuning.POOP_ELEMENT, 80000f, 4f, null, 0f, produce_solid_tile: false, Diet.Info.FoodType.EatPrey),
			new Diet.Info(hashSet2, SeaTurtleTuning.POOP_ELEMENT, 40000f, 2f)
		};
		Diet diet = new Diet(list.ToArray());
		CreatureCalorieMonitor.Def def4 = gameObject.AddOrGetDef<CreatureCalorieMonitor.Def>();
		def4.diet = diet;
		def4.minConsumedCaloriesBeforePooping = SeaTurtleTuning.STANDARD_CALORIES_PER_CYCLE;
