...
public class LonelyMinionHouse : StoryTraitStateMachine<LonelyMinionHouse, LonelyMinionHouse.Instance, LonelyMinionHouse.Def>
{
	...
	{
		private KAnimLink lightsLink = null;
		...
		private FilteredStorage storageFilter = null;

		private MeterController meter = null;

		private MeterController blinds = null;
		...
		private Notification knockNotification = null;

		private KBatchedAnimController knocker = null;
		...
		public override void OnCompleteStorySequence()
		{
			...
			questInstance.QuestProgressChanged = (Action<QuestInstance, Quest.State, float>)Delegate.Remove(questInstance.QuestProgressChanged, new Action<QuestInstance, Quest.State, float>(OnQuestProgressChanged));
			instance = QuestManager.GetInstance(questOwnerId, Db.Get().Quests.LonelyMinionPowerQuest);
			QuestInstance questInstance2 = instance;
			questInstance2.QuestProgressChanged = (Action<QuestInstance, Quest.State, float>)Delegate.Remove(questInstance2.QuestProgressChanged, new Action<QuestInstance, Quest.State, float>(OnQuestProgressChanged));
			instance = QuestManager.GetInstance(questOwnerId, Db.Get().Quests.LonelyMinionDecorQuest);
			QuestInstance questInstance3 = instance;
			questInstance3.QuestProgressChanged = (Action<QuestInstance, Quest.State, float>)Delegate.Remove(questInstance3.QuestProgressChanged, new Action<QuestInstance, Quest.State, float>(OnQuestProgressChanged));
		...
		private static Util.IterationInstruction tryFindMailboxVisitor(object obj, ref (Instance, bool) param)
		{
			...
			return param.Item2 ? Util.IterationInstruction.Halt : Util.IterationInstruction.Continue;
		}
		...
		private void OnWorkStateChanged(Workable w, Workable.WorkableEvent state)
		{
			...
			if (activatable != null)
			{
				if (state == Workable.WorkableEvent.WorkStarted)
				{
					...
					notifier.Add(knockNotification);
					KSelectable component = GetComponent<KSelectable>();
					component.AddStatusItem(Db.Get().MiscStatusItems.AttentionRequired, base.smi);
				}
				if (state == Workable.WorkableEvent.WorkStopped)
				{
					if (currentWorkState == Workable.WorkableEvent.WorkStarted)
					{
						if (knockNotification != null)
						{
							...
							component2.RemoveStatusItem(Db.Get().MiscStatusItems.AttentionRequired);
							knockNotification.Clear();
							knockNotification = null;
						}
		...
		{
			KSelectable component = GetComponent<KSelectable>();
			component.RemoveStatusItem(Db.Get().MiscStatusItems.AttentionRequired);
			int cell = Grid.PosToCell(base.gameObject);
			Vector3 target = Grid.CellToPosCCC(Grid.OffsetCell(cell, base.def.CompletionData.CameraTargetOffset), Grid.SceneLayer.Ore);
		...
		private void OnIntroSequenceComplete()
		{
			...
			QuestInstance instance = QuestManager.GetInstance(questOwnerId, Db.Get().Quests.LonelyMinionGreetingQuest);
		...
		private bool IsIntroSequenceComplete()
		{
			...
			return currentWorkState == Workable.WorkableEvent.WorkStopped && knocker == null;
		}
