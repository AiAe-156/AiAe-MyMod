...
public class GravitasCreatureManipulator : GameStateMachine<GravitasCreatureManipulator, GravitasCreatureManipulator.Instance, IStateMachineTarget, GravitasCreatureManipulator.Def>
{
	...
	public new class Instance : GameInstance
	{
		...
		[Serialize]
		private bool m_morphModeUnlocked = false;
		...
		public void DropCritter()
		{
			...
			Vector3 vector = base.smi.def.dropOffset.ToVector3();
		...
		public void SpawnMorph(Brain brain)
		{
			...
			else if (flag)
			{
				...
				List<FertilityMonitor.BreedingChance> initialBreedingWeights = def.initialBreedingWeights;
				tag = FertilityMonitor.EggBreedingRoll(initialBreedingWeights, excludeOriginalCreature: true);
			}
			if (!tag.IsValid)
			{
				return;
		...
		public void ShowIntroNotification()
		{
			...
			EventInfoData eventInfoData = EventInfoDataHelper.GenerateStoryTraitData(CODEX.STORY_TRAITS.CRITTER_MANIPULATOR.BEGIN_POPUP.NAME, CODEX.STORY_TRAITS.CRITTER_MANIPULATOR.BEGIN_POPUP.DESCRIPTION, CODEX.STORY_TRAITS.CLOSE_BUTTON, "crittermanipulatoractivate_kanim", EventInfoDataHelper.PopupType.BEGIN);
		...
		public static void ShowLoreUnlockedPopup(Tag species)
		{
			...
			string lockId = GravitasCreatureManipulatorConfig.CRITTER_LORE_UNLOCK_ID.For(species);
