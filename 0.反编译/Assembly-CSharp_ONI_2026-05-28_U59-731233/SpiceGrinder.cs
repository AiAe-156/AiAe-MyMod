...
public class SpiceGrinder : GameStateMachine<SpiceGrinder, SpiceGrinder.StatesInstance, IStateMachineTarget, SpiceGrinder.Def>
{
	public class Option : IConfigurableConsumerOption
	{
		...
		private void CreateDescription()
		{
			...
			for (int i = 0; i < Spice.Ingredients.Length; i++)
			{
				...
				Tag tag = ((ingredient.IngredientSet != null && ingredient.IngredientSet.Length != 0) ? ingredient.IngredientSet[0] : ((Tag)null));
	...
	public class StatesInstance : GameInstance
	{
		...
		[Serialize]
		private int spiceHash = 0;

		private SpiceInstance currentSpice;

		private Edible currentFood = null;

		private Storage seedStorage = null;

		private Storage foodStorage = null;

		private MeterController meter = null;

		private Tag[] foodFilter = new Tag[1];

		private FilteredStorage foodStorageFilter = null;
		...
		private FetchChore[] SpiceFetches = null;
		...
		public float AvailableFood => (foodStorage == null) ? 0f : foodStorage.MassStored();

		public Option SelectedOption => (currentSpice.Id == Tag.Invalid) ? null : SettingOptions[currentSpice.Id];
		...
		private void OnCopySettings(object data)
		{
			...
			SpiceGrinderWorkable component = gameObject.GetComponent<SpiceGrinderWorkable>();
			if (component != null)
			{
		...
		public void SetupFoodSymbol()
		{
			...
			bool symbolVisible;
			Vector4 column = kbac.GetSymbolTransform(HASH_FOOD, out symbolVisible).GetColumn(3);
		...
		public void OnOptionSelected(Option spiceOption)
		{
			...
			for (int i = 0; i < ingredients.Length; i++)
			{
				...
				float massAvailable = seedStorage.GetMassAvailable(ingredient.IngredientSet[0]);
				if (massAvailable < num)
				{
					SpiceFetches[i] = CreateFetchChore(ingredient.IngredientSet, ingredient.AmountKG * 10f);
				}
				if (CurrentFood != null)
