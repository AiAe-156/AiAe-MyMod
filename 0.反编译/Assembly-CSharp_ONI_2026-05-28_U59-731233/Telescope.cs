...
public class Telescope : Workable, OxygenBreather.IGasProvider, ISim200ms, BuildingStatusItems.ISkyVisInfo
{
	...
	public void Sim200ms(float dt)
	{
		...
		component2.ToggleStatusItem(Db.Get().BuildingStatusItems.SkyVisNone, !item, this);
		component2.ToggleStatusItem(Db.Get().BuildingStatusItems.SkyVisLimited, item && num < 1f, this);
		Operational component3 = GetComponent<Operational>();
		component3.SetFlag(visibleSkyFlag, item);
		if (!component3.IsActive && component3.IsOperational && chore == null)
		{
			chore = CreateChore();
	...
	protected void UpdateWorkingState(object _)
	{
		...
		component.ToggleStatusItem(Db.Get().BuildingStatusItems.NoApplicableAnalysisSelected, flag2);
