...
public class MegaBrainTank : StateMachineComponent<MegaBrainTank.StatesInstance>
{
	...
	public class StatesInstance : GameStateMachine<States, StatesInstance, MegaBrainTank, object>.GameInstance
	{
		...
		public short UnitsFromLastStore = 0;

		private float meterPositionValue;

		private float meterTarget = 0f;

		private float journalActivationTimer = 0f;

		private float lastRemainingTime = 0f;

		private byte activatedJournals = 0;

		private bool currentlyActivating = false;

		private short nextActiveBrain = 1;

		private string brainHum = null;

		private KBatchedAnimController[] controllers = null;

		private KAnimLink fxLink = null;

		private MeterController meter = null;

		private EventInfoData eventInfo = null;

		private Notification eventComplete = null;
		...
		public KSelectable Selectable { get; private set; } = null;

		public Operational Operational { get; private set; } = null;

		public ElementConverter ElementConverter { get; private set; } = null;

		public ManualDeliveryKG JournalDelivery { get; private set; } = null;

		public LoopingSounds BrainSounds { get; private set; } = null;
		...
		private void InitializeEffectsList()
		{
			...
			int capacity = ((liveMinionIdentities.Count > 32) ? liveMinionIdentities.Count : 32);
			minionEffects = new List<Effects>(capacity);
		...
		public void ShelveJournals(float dt)
		{
			...
			float num2 = BrainStorage.storageWorkTime / 5f;
			if (num2 - journalActivationTimer > 0.001f)
			{
			...
			{
				byte b = (byte)(1 << i);
				bool flag = (activatedJournals & b) == 0;
				if (flag && num3 == -1)
				{
					num3 = i;
				}
			...
			if (num3 != -1)
			{
				ShelfController.SetSymbolVisiblity(MegaBrainTankConfig.JOURNAL_SYMBOLS[num3], is_visible: true);
			}
			UnitsFromLastStore--;
		}
		...
		public void CleanTank(bool active)
		{
			...
			float massAvailable = BrainStorage.GetMassAvailable(DreamJournalConfig.ID);
			if (massAvailable > 0f && !ElementConverter.HasEnoughMassToStartConverting(includeInactive: true))
			{
		...
		public void ShowEventCompleteUI(object _ = null)
		{
			if (eventComplete != null)
			{
				...
				int cell = Grid.OffsetCell(Grid.PosToCell(base.master), new CellOffset(0, 3));
		...
		{
			int cell = Grid.OffsetCell(Grid.PosToCell(base.master), new CellOffset(0, 2));
