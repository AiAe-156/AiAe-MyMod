...
public class LimitValve : KMonoBehaviour, ISaveLoadable
{
	...
	public float Amount
	{
		...
		{
			m_amount = value;
			BoxingTrigger(-1722241721, m_amount);
			Refresh();
		}
	}
	...
	protected override void OnPrefabInit()
	{
		...
		foreach (KBatchedAnimController kBatchedAnimController in array)
		{
			...
			{
				fg_Controller = kBatchedAnimController;
				break;
			}
		}
	}
	...
	private void OnMassTransfer(SimHashes element, float transferredMass, float temperature, byte disease_idx, int disease_count, Pickupable pickupable)
	{
		...
		if (lastElemenet != element && conduitType == ConduitType.Liquid)
		{
			Element element2 = ElementLoader.FindElementByHash(element);
			if (element2 != null)
			{
				GameUtil.TintLiquidSymbolOnBuilding("gradient", controller, element2);
				GameUtil.TintLiquidSymbolOnBuilding("water_color_fg", fg_Controller, element2);
			}
		}
