...
public class MosquitoHungerMonitor : StateMachineComponent<MosquitoHungerMonitor.Instance>
{
	...
	{
		return victimParam != null && !IsVictimForbidden(smi, victimParam.GetComponent<KPrefabID>(), mustBeInSameCavity: true);
	}
	...
	public static void InitiatePokeBehaviour(Instance smi)
	{
		...
		OccupyArea component = smi.Victim.GetComponent<OccupyArea>();
		CellOffset[] array = component.OccupiedCellsOffsets;
		for (int i = 0; i < 1; i++)
		{
			array = array.Expand();
		}
