...
public class AggressiveChore : Chore<AggressiveChore.StatesInstance>
{
	public class StatesInstance : GameStateMachine<States, StatesInstance, AggressiveChore, object>.GameInstance
	{
			...
			: base(master)
		{
			...
			isBreakableCell = delegate(int cell)
			{
				if (Grid.Solid[cell])
				{
					return false;
		...
		public void FindBreakable()
		{
			...
			if (breakable == null)
			{
				int value = FloodFill.Find(isBreakableCell, Grid.PosToCell(navigator.gameObject), new FloodFill.MaxDepth(128), stopAtSolid: true, stopAtLiquid: true);
