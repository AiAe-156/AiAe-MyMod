...
public class GermExposureMonitor : GameStateMachine<GermExposureMonitor, GermExposureMonitor.Instance>
{
	...
	public new class Instance : GameInstance
	{
		...
		private string[] inateImmunities = null;
			...
			: base(master)
		{
			...
			base.gameObject.Subscribe(-1582839653, OnBionicTagsChanged);
			inateImmunities = DUPLICANTSTATS.GetStatsFor(base.gameObject).DiseaseImmunities.IMMUNITIES;
			OxygenBreather component = GetComponent<OxygenBreather>();
			if (component != null)
			{
				component.onBreathableGasConsumed = (Action<SimHashes, float, float, byte, int>)Delegate.Combine(component.onBreathableGasConsumed, new Action<SimHashes, float, float, byte, int>(OnAirConsumed));
			}
		}

		protected override void OnCleanUp()
		{
			base.gameObject.Unsubscribe(-1582839653, OnBionicTagsChanged);
			base.OnCleanUp();
		}
		...
		public void OnAirConsumed(SimHashes elementConsumed, float massConsumed, float temperature, byte disseaseIDX, int disseaseCount)
		{
			...
			{
				Disease disease = Db.Get().Diseases[disseaseIDX];
				InjectDisease(disease, disseaseCount, ElementLoader.FindElementByHash(elementConsumed).tag, Sickness.InfectionVector.Inhalation);
			}
		}
		...
		{
			if (inateImmunities == null)
			{
				return false;
			}
			for (int i = 0; i < inateImmunities.Length; i++)
			{
				if (sicknessID == inateImmunities[i])
				{
					return true;
				}
			}
		...
		public void InjectDisease(Disease disease, int count, Tag source, Sickness.InfectionVector vector)
		{
			...
			foreach (ExposureType exposureType in tYPES)
			{
				if (!(disease.id == exposureType.germ_id) || IsImmuneToDisease(exposureType.sickness_id) || count <= exposureType.exposure_threshold || !HasMinExposurePeriodElapsed(exposureType.germ_id) || !IsExposureValidForTraits(exposureType))
				{
					continue;
				}
		...
		{
			ReportManager.Instance.ReportValueWithGameObjectContext(ReportManager.ReportType.DiseaseStatus, primaryElement.DiseaseCount, base.master.gameObject, StringFormatter.Replace(UI.ENDOFDAYREPORT.NOTES.GERMS, "{0}", base.master.name));
		}
