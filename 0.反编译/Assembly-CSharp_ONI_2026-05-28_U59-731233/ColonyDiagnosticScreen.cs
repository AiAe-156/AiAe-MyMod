...
public class ColonyDiagnosticScreen : KScreen, ISim1000ms
{
	private class DiagnosticRow : ISim4000ms
	{
		...
		{
			if (force || ClusterManager.Instance.activeWorldId == worldID)
			{
		...
		private IEnumerator VisualNotificationRoutine()
		{
			...
			RectTransform contentRect = gameObject.GetComponent<HierarchyReferences>().GetReference<RectTransform>("Content");
			...
			for (float i = 0f; i < bounceDuration; i += Time.unscaledDeltaTime)
			{
				...
				indicator.sizeDelta = defaultIndicatorSizeDelta + Vector2.one * Mathf.RoundToInt(Mathf.Sin(6f * (MathF.PI * (i / bounceDuration))));
				yield return 0;
			}
			...
			for (float i2 = 0f; i2 < bounceDuration; i2 += Time.unscaledDeltaTime)
			{
				...
				indicator.sizeDelta = defaultIndicatorSizeDelta + Vector2.one * Mathf.RoundToInt(Mathf.Sin(6f * (MathF.PI * (i2 / bounceDuration))));
				yield return 0;
			}
			contentRect.anchoredPosition = Vector2.zero;
			for (float i3 = 0f; i3 < bounceDuration; i3 += Time.unscaledDeltaTime)
			{
				...
				indicator.sizeDelta = defaultIndicatorSizeDelta + Vector2.one * Mathf.RoundToInt(Mathf.Sin(6f * (MathF.PI * (i3 / bounceDuration))));
				yield return 0;
			}
			contentRect.anchoredPosition = Vector2.zero;
	...
	private void SpawnTrackerLines(int world)
	{
		...
		if (Game.IsDlcActiveForCurrentSave("DLC3_ID"))
		{
			...
			{
				AddDiagnostic<SelfChargingElectrobankDiagnostic>(world, contentContainer, diagnosticRows);
			}
			AddDiagnostic<BionicBatteryDiagnostic>(world, contentContainer, diagnosticRows);
		}
