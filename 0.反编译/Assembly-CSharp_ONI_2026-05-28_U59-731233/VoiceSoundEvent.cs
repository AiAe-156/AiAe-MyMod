	...
	public static EventInstance PlayVoice(string name, KBatchedAnimController controller, float interval_between_speaking, bool looping, bool objectIsSelectedAndVisible = false)
	{
		...
		if (text != null)
		{
			if (looping)
			{
				...
				{
					component3.UpdateFirstParameter(text, "isBionic", flag ? 1 : 0);
				}
			}
			else
			{
				eventInstance = SoundEvent.BeginOneShot(text, vector);
				eventInstance.setParameterByName("isBionic", flag ? 1 : 0);
				if (text.Contains("sleep_"))
				{
	...
	public override void Stop(AnimEventManager.EventPlayerData behaviour)
	{
		if (base.looping)
		{
			...
			if (component != null)
			{
				...
				string asset = GlobalAssets.GetSound(assetName, force_no_warning: true);
