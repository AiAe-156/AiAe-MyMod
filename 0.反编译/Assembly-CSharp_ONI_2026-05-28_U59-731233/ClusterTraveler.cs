...
public class ClusterTraveler : KMonoBehaviour, ISim200ms
{
	...
	public bool stopAndNotifyWhenPathChanges = false;
	...
	public AxialI CurrentLocation => m_clusterGridEntity.Location;

	public AxialI Destination
	{
		get
		{
			List<AxialI> currentPath = CurrentPath;
	...
	private void ForceRevealLocation(AxialI location)
	{
		if (!ClusterGrid.Instance.IsCellVisible(location))
		{
			...
			sMI.RevealLocation(location, 0, peekRadius);
		}
	}
	...
	{
		if (CurrentPath == null || getSpeedCB == null)
		{
			return 0f;
		}
		return TravelETA(m_cachedPathDestination);
	}
	...
	public float TravelETA(AxialI location)
	{
		...
		int num = CurrentPath.IndexOf(location);
		if (num == -1)
		{
			return 0f;
		}
		return Mathf.Max(0f, (float)(num + 1) * 600f - m_movePotential) / getSpeedCB();
	}
		...
		if (flag)
		{
			...
			if (!flag2 || CurrentPath.Count > 1 || !quickTravelToAsteroidIfInOrbit)
			{
				...
				if (m_movePotential >= 600f)
				{
					...
					{
						ClusterGridEntity visibleEntityOfLayerAtCell = ClusterGrid.Instance.GetVisibleEntityOfLayerAtCell(m_clusterGridEntity.Location, EntityLayer.Asteroid);
						Debug.Assert(visibleEntityOfLayerAtCell == null || (flag2 && CurrentPath.Count == 0), $"Somehow this clustercraft pathed through an asteroid at {m_clusterGridEntity.Location}");
						if (onTravelCB != null)
						{
							onTravelCB();
						}
