	...
	public class StatesInstance : GameStateMachine<States, StatesInstance, Telepad, object>.GameInstance
	{
		...
		public IEnumerator SpawnExtraPowerBanks()
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				GameObject spawn = Util.KInstantiate(Assets.GetPrefab("DisposableElectrobank_RawMetal"), Grid.CellToPosCBC(cellTarget, Grid.SceneLayer.Front) - Vector3.right / 2f);
				spawn.SetActive(value: true);
				Vector2 velocity = new Vector2((-2.5f + 5f * ((float)i / 5f)) / 2f, 2f);
				if (GameComps.Fallers.Has(spawn))
				{
					GameComps.Fallers.Remove(spawn);
				}
				GameComps.Fallers.Add(spawn, velocity);
			...
			KMonoBehaviour.PlaySound(GlobalAssets.GetSound("SandboxTool_Spawner"));
			GameObject spawn2 = Util.KInstantiate(Assets.GetPrefab("LubricationStick"), Grid.CellToPosCBC(cellTarget, Grid.SceneLayer.Front) - Vector3.right / 2f);
			spawn2.SetActive(value: true);
			Vector2 velocity2 = new Vector2(3.75f, 2.5f);
			if (GameComps.Fallers.Has(spawn2))
			{
				GameComps.Fallers.Remove(spawn2);
			}
			GameComps.Fallers.Add(spawn2, velocity2);
			yield return 0;
		}
	}
