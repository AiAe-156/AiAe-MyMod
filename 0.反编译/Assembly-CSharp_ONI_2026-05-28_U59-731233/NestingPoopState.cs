...
internal class NestingPoopState : GameStateMachine<NestingPoopState, NestingPoopState.Instance, IStateMachineTarget, NestingPoopState.Def>
{
	...
	public new class Instance : GameInstance
	{
		...
		private static readonly Func<int, bool> isValidNestingCell = (int cell) => Grid.IsValidCell(cell) && !Grid.Solid[cell] && Grid.IsValidCell(Grid.CellBelow(cell)) && Grid.Solid[Grid.CellBelow(cell)] && (IsValidPoopFromCell(cell, look_left: true) || IsValidPoopFromCell(cell, look_left: false));
		...
		private int GetTargetPoopCell()
		{
			...
			int num = FloodFill.Find(startCell: (!(currentlyPoopingElement == base.smi.def.nestingPoopElement) || !(base.smi.def.nestingPoopElement != Tag.Invalid) || lastPoopCell == -1) ? Grid.PosToCell(this) : lastPoopCell, criteria: isValidNestingCell, maxDepth: new FloodFill.MaxDepth(8), stopAtSolid: false, stopAtLiquid: true);
			if (num == -1)
			{
	...
	public override void InitializeStates(out BaseState default_state)
	{
		...
		state.ToggleStatusItem(text, tooltip, "", StatusItem.IconType.Info, NotificationType.Neutral, allow_multiples: false, default(HashedString), 129022, null, null, main).PlayAnim("poop").OnAnimQueueComplete(behaviourcomplete);
		behaviourcomplete.Enter(delegate(Instance smi)
		{
			smi.SetLastPoopCell();
		}).PlayAnim("idle_loop", KAnim.PlayMode.Loop).TriggerOnEnter(GameHashes.PoopStatesCompleted, (Instance smi) => (object)null)
