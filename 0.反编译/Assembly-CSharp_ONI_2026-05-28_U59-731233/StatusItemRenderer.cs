	...
	public struct Entry
	{
		...
		public void Render(StatusItemRenderer renderer, Vector3 camera_bl, Vector3 camera_tr, HashedString overlay, Camera camera)
		{
			...
			if (dirty)
			{
				...
				{
					num3++;
					if (!statusItem4.UseConditionalCallback(overlay, transform) && overlay != OverlayModes.None.ID && statusItem4.render_overlay != overlay)
					{
						continue;
					}
					Sprite sprite = statusItem4.sprite.sprite;
					if (!statusItem4.unique)
					{
				...
				{
					StatusItem statusItem2 = statusItems[statusItemsToRender_Index[j]];
					float x = (float)j * num4 * 2f - num4 * (float)(num2 - 1);
					if (statusItem2.sprite == null)
					{
						DebugUtil.DevLogError("Status Item " + statusItem2.Id + " has null sprite for icon '" + statusItem2.iconName + "', you need to run Collect Sprites or manually add the sprite to the TintedSprites list in the GameAssets prefab.");
						statusItem2.iconName = "status_item_exclamation";
						statusItem2.sprite = Assets.GetTintedSprite("status_item_exclamation");
					}
					Sprite sprite2 = statusItem2.sprite.sprite;
