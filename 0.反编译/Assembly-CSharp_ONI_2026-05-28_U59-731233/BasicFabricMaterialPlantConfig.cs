...
public class BasicFabricMaterialPlantConfig : IEntityConfig
{
	...
	public GameObject CreatePrefab()
	{
		...
		string name = STRINGS.CREATURES.SPECIES.SEEDS.BASICFABRICMATERIALPLANT.NAME;
		string desc = STRINGS.CREATURES.SPECIES.SEEDS.BASICFABRICMATERIALPLANT.DESC;
		KAnimFile anim = Assets.GetAnim("seed_swampreed_kanim");
		List<Tag> additionalTags = new List<Tag> { GameTags.WaterSeed };
		string domesticatedDescription = STRINGS.CREATURES.SPECIES.BASICFABRICMATERIALPLANT.DOMESTICATEDDESC;
		GameObject seed = EntityTemplates.CreateAndRegisterSeedForPlant(gameObject, dlcRestrictions, SeedProducer.ProductionType.Harvest, sEED_ID, name, desc, anim, "object", 1, additionalTags, SingleEntityReceptacle.ReceptacleDirection.Top, default(Tag), 20, domesticatedDescription);
		CodexEntryRedirector codexEntryRedirector = Assets.GetPrefab(SEED_ID).AddOrGet<CodexEntryRedirector>();
		codexEntryRedirector.CodexID = "BASICFABRICPLANT";
