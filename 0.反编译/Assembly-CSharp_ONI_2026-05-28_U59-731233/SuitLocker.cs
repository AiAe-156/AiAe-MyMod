...
public class SuitLocker : StateMachineComponent<SuitLocker.StatesInstance>
{
	...
	public class ReturnSuitWorkable : Workable
	{
		public static readonly Chore.Precondition DoesSuitNeedRechargingUrgent = new Chore.Precondition
		{
			...
			fn = delegate(ref Chore.Precondition.Context context, object data)
			{
				...
				return (component4 != null && component4.NeedsRecharging()) ? true : false;
			}
		};
		...
		public static readonly Chore.Precondition DoesSuitNeedRechargingIdle = new Chore.Precondition
		{
			...
			fn = delegate(ref Chore.Precondition.Context context, object data)
			{
				...
				LeadSuitTank component4 = slot.assignable.GetComponent<LeadSuitTank>();
				return (component4 != null) ? true : false;
			}
		};
		...
		public static Chore.Precondition HasSuitMarker = new Chore.Precondition
		{
			...
			fn = delegate(ref Chore.Precondition.Context context, object data)
			{
				...
				return suitLocker.suitMarkerState == SuitMarkerState.HasMarker;
			}
		};
	...
	public class States : GameStateMachine<States, StatesInstance, SuitLocker>
	{
		...
		public override void InitializeStates(out BaseState default_state)
		{
			...
			state.ToggleStatusItem(text, tooltip, "status_item_no_filter_set", StatusItem.IconType.Custom, NotificationType.BadMinor, allow_multiples: false, default(HashedString), 129022, null, null, main);
			State state2 = empty.configured.RefreshUserMenuOnEnter();
			string text2 = BUILDING.STATUSITEMS.SUIT_LOCKER.READY.NAME;
			...
			State state3 = waitingforsuit.EventTransition(GameHashes.OnStorageChange, charging, (StatesInstance smi) => smi.master.GetStoredOutfit() != null).Enter("CreateFetchChore", delegate(StatesInstance smi)
			{
			...
			string text3 = BUILDING.STATUSITEMS.SUIT_LOCKER.SUIT_REQUESTED.NAME;
			...
			charging.DefaultState(charging.pre).RefreshUserMenuOnEnter().EventTransition(GameHashes.OnStorageChange, empty, (StatesInstance smi) => smi.master.GetStoredOutfit() == null)
				...
				{
					KBatchedAnimController component = smi.master.GetStoredOutfit().GetComponent<KBatchedAnimController>();
					KAnim.Build build = component.AnimFiles[0].GetData().build;
					KAnim.Build.Symbol symbol = build.GetSymbol("suit");
					SymbolOverrideController component2 = smi.GetComponent<SymbolOverrideController>();
					component2.TryRemoveSymbolOverride("suit_swap");
					if (symbol != null)
					{
						component2.AddSymbolOverride("suit_swap", symbol);
					}
				});
			...
			State state4 = charging.operational.TagTransition(GameTags.Operational, charging.notoperational, on_remove: true).Transition(charging.nooxygen, (StatesInstance smi) => !smi.master.HasOxygen()).PlayAnim("charging_loop", KAnim.PlayMode.Loop)
			...
			string text4 = BUILDING.STATUSITEMS.SUIT_LOCKER.CHARGING.NAME;
			string tooltip4 = BUILDING.STATUSITEMS.SUIT_LOCKER.CHARGING.TOOLTIP;
			main = Db.Get().StatusItemCategories.Main;
			state4.ToggleStatusItem(text4, tooltip4, "", StatusItem.IconType.Info, NotificationType.Neutral, allow_multiples: false, default(HashedString), 129022, null, null, main);
			State state5 = charging.nooxygen.TagTransition(GameTags.Operational, charging.notoperational, on_remove: true).Transition(charging.operational, (StatesInstance smi) => smi.master.HasOxygen()).Transition(charging.pst, (StatesInstance smi) => smi.master.IsSuitFullyCharged())
				.PlayAnim("no_o2_loop", KAnim.PlayMode.Loop);
			string text5 = BUILDING.STATUSITEMS.SUIT_LOCKER.NO_OXYGEN.NAME;
			string tooltip5 = BUILDING.STATUSITEMS.SUIT_LOCKER.NO_OXYGEN.TOOLTIP;
			main = Db.Get().StatusItemCategories.Main;
			state5.ToggleStatusItem(text5, tooltip5, "status_item_suit_locker_no_oxygen", StatusItem.IconType.Custom, NotificationType.BadMinor, allow_multiples: false, default(HashedString), 129022, null, null, main);
			State state6 = charging.notoperational.TagTransition(GameTags.Operational, charging.operational).PlayAnim("not_charging_loop", KAnim.PlayMode.Loop).Transition(charging.pst, (StatesInstance smi) => smi.master.IsSuitFullyCharged());
			string text6 = BUILDING.STATUSITEMS.SUIT_LOCKER.NOT_OPERATIONAL.NAME;
			string tooltip6 = BUILDING.STATUSITEMS.SUIT_LOCKER.NOT_OPERATIONAL.TOOLTIP;
			main = Db.Get().StatusItemCategories.Main;
			state6.ToggleStatusItem(text6, tooltip6, "", StatusItem.IconType.Info, NotificationType.Neutral, allow_multiples: false, default(HashedString), 129022, null, null, main);
			charging.pst.PlayAnim("charging_pst").OnAnimQueueComplete(suitfullycharged);
			State state7 = suitfullycharged.EventTransition(GameHashes.OnStorageChange, empty, (StatesInstance smi) => smi.master.GetStoredOutfit() == null).PlayAnim("has_suit").RefreshUserMenuOnEnter()
				.ToggleStatusItem(Db.Get().MiscStatusItems.StoredItemDurability, (StatesInstance smi) => smi.master.GetStoredOutfit().gameObject);
			string text7 = BUILDING.STATUSITEMS.SUIT_LOCKER.FULLY_CHARGED.NAME;
			string tooltip7 = BUILDING.STATUSITEMS.SUIT_LOCKER.FULLY_CHARGED.TOOLTIP;
			main = Db.Get().StatusItemCategories.Main;
			state7.ToggleStatusItem(text7, tooltip7, "", StatusItem.IconType.Info, NotificationType.Neutral, allow_multiples: false, default(HashedString), 129022, null, null, main);
		}
	}
	...
	protected override void OnSpawn()
	{
		...
		DebugUtil.DevAssert(OutfitTags.Length == 1, "Suit Locker " + base.name + " requesting more than one suit type, this will break the fetch chore");
		if (OutfitTags.Length == 1)
		{
			GameObject prefab = Assets.GetPrefab(OutfitTags[0]);
			if (prefab != null)
			{
				PrimaryElement component = prefab.GetComponent<PrimaryElement>();
				OutfitMass = component.MassPerUnit;
			}
		}
		else
		{
			OutfitMass = TUNING.EQUIPMENT.SUITS.ATMOSUIT_MASS;
		}
	...
	{
		fetchChore = new FetchChore(Db.Get().ChoreTypes.EquipmentFetch, GetComponent<Storage>(), OutfitMass, new HashSet<Tag>(OutfitTags), FetchChore.MatchCriteria.MatchID, Tag.Invalid, new Tag[1] { GameTags.Assigned }, null, run_until_complete: true, null, null, null, Operational.State.None);
	...
	public void UnequipFrom(Equipment equipment)
	{
		...
		component2.Store(assignable.gameObject);
	}
