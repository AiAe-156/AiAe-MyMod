...
public class DisconnectTool : FilteredDragTool
{
	...
	{
		return (!singleDisconnectMode) ? Mode.Box : Mode.Line;
	}
	...
	{
		int cell2 = Grid.OffsetCell(cell, xoff, yoff);
		Grid.CellToXY(cell2, out var x, out var y);
	...
	{
		GameObject original = (singleDisconnectMode ? disconnectVisSingleModePrefab : disconnectVisMultiModePrefab);
		GameObject gameObject = GameUtil.KInstantiate(original, Grid.SceneLayer.FXFront);
