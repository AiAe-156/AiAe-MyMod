...
public static class BasePacuConfig
{
	...
	public static GameObject CreatePrefab(string id, string base_trait_id, string name, string description, string anim_file, bool is_baby, string symbol_prefix, float warnLowTemp, float warnHighTemp, float lethalLowTemp, float lethalHighTemp)
	{
		...
		list.Add(new Diet.Info(hashSet4, tag, PacuTuning.STANDARD_CALORIES_PER_CYCLE, TUNING.CREATURES.CONVERSION_EFFICIENCY.NORMAL));
		...
		def5.lures = new Tag[1] { GameTags.Creatures.FishTrapLure };
	...
	{
		List<Diet.Info> list = new List<Diet.Info>();
		List<GameObject> prefabsWithComponent = Assets.GetPrefabsWithComponent<PlantableSeed>();
		foreach (GameObject item in prefabsWithComponent)
		{
			PlantableSeed component = item.GetComponent<PlantableSeed>();
			GameObject prefab = Assets.GetPrefab(component.PlantID);
			KPrefabID component2 = item.GetComponent<KPrefabID>();
			if (!prefab.HasTag(GameTags.DeprecatedContent) && !component2.HasTag("KelpPlantSeed"))
			{
				SeedProducer component3 = prefab.GetComponent<SeedProducer>();
				if (component3 == null || component3.seedInfo.productionType == SeedProducer.ProductionType.Harvest || component3.seedInfo.productionType == SeedProducer.ProductionType.Crop || component3.seedInfo.productionType == SeedProducer.ProductionType.HarvestOnly)
				{
					HashSet<Tag> hashSet = new HashSet<Tag>();
					hashSet.Add(new Tag(component2.GetComponent<KPrefabID>().PrefabID()));
