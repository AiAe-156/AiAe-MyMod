	...
	public static void AddPotentials(PotentialScratchPad potential_scratch_pad, PotentialPath potential, int cost, ref PathFinderAbilities abilities, PathFinderQuery query, int max_links_per_cell, NavGrid.Link[] links, PotentialList potentials, PathGrid path_grid, int parent_cell, NavType parent_nav_type)
	{
		...
		for (int j = 0; j < num; j++)
		{
			...
			if (is_cell_in_range)
			{
				...
				if (IsSubmerged(link4) || link3.endNavType == NavType.Swim)
				{
		...
		{
			PotentialScratchPad.PathGridCellData pathGridCellData2 = linksInCellRange[l];
			NavGrid.Link link7 = pathGridCellData2.link;
			int link8 = link7.link;
			Cell cell_data = pathGridCellData2.pathGridCell;
			int num6 = cost + link7.cost;
			PotentialPath path = potential;
			path.cell = link8;
			path.navType = link7.endNavType;
			if (pathGridCellData2.isSubmerged || path.navType == NavType.Swim)
			{
				int submergedPathCostPenalty2 = abilities.GetSubmergedPathCostPenalty(path, link7);
				num6 += submergedPathCostPenalty2;
			}
			bool flag3 = cell_data.cost == -1;
			bool flag4 = num6 < cell_data.cost;
			if (flag3 || flag4)
			{
				PotentialPath.Flags flags = path.flags;
				bool flag5 = abilities.TraversePath(ref path, potential.cell, potential.navType, num6, link7.transitionId, pathGridCellData2.isSubmerged);
				if (path.flags != flags)
				{
				}
				if (flag5)
				{
					AddPotential(path, potential.cell, potential.navType, num6, link7.transitionId, potentials, path_grid, ref cell_data);
				}
			}
		}
	}
