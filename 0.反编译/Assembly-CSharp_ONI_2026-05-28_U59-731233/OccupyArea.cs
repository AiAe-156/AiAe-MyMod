...
public class OccupyArea : KMonoBehaviour
{
	...
	{
		base.OnSpawn();
		if (updateWithFacing && facing != null)
		{
	...
	private void UpdateRotatedCells()
	{
		...
		else if (updateWithFacing && facing != null && facingLeft != facing.facingLeft)
		{
			facingLeft = facing.facingLeft;
			_RotatedOccupiedCellsOffsets = new CellOffset[_UnrotatedOccupiedCellsOffsets.Length];
			for (int j = 0; j < _UnrotatedOccupiedCellsOffsets.Length; j++)
			{
				CellOffset cellOffset = _UnrotatedOccupiedCellsOffsets[j];
				cellOffset.x *= (facingLeft ? 1 : (-1));
				_RotatedOccupiedCellsOffsets[j] = cellOffset;
			}
		}
	}
