...
public class RocketConduitReceiver : StateMachineComponent<RocketConduitReceiver.StatesInstance>, ISecondaryOutput
{
	...
	public void RemoveConduitPortFromNetwork()
	{
		...
		{
			IUtilityNetworkMgr networkManager = Conduit.GetNetworkManager(conduitPortInfo.conduitType);
			networkManager.RemoveFromNetworks(conduitPort.outputCell, conduitPort.networkItem, is_endpoint: true);
		}
	}
	...
	private void FindPartner()
	{
		...
		if (world != null && world.IsModuleInterior)
		{
			...
			RocketConduitSender[] components = passengerModule.GetComponents<RocketConduitSender>();
			RocketConduitSender[] array = components;
			foreach (RocketConduitSender rocketConduitSender2 in array)
			{
		...
		else
		{
			...
			if (component2.HasTargetWorld())
			{
				...
				List<RocketConduitSender> worldItems = Components.RocketConduitSenders.GetWorldItems(targetWorld.id);
				foreach (RocketConduitSender item in worldItems)
				{
					if (item.conduitPortInfo.conduitType == conduitPortInfo.conduitType)
					{
						rocketConduitSender = item;
						break;
					}
				}
			}
