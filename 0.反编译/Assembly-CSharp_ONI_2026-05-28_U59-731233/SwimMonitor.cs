...
public class SwimMonitor : GameStateMachine<SwimMonitor, SwimMonitor.Instance, IStateMachineTarget>
{
	...
	public override void InitializeStates(out BaseState default_state)
	{
		...
		root.EventHandler(GameHashes.AssignedRoleChanged, CheckSwimSkill).EventHandler(GameHashes.PathAdvanced, delegate(Instance smi, object data)
		{
			OnPathAdvanced(smi, data);
		});
		cannotSwim.DoNothing().ParamTransition(hasSwimSkill, canSwim, GameStateMachine<SwimMonitor, Instance, IStateMachineTarget, object>.IsTrue);
	...
	public static float ComputeSwimOffsetY(int cell)
	{
		...
		return Mathf.Clamp((num / 1000f - 1f) * 0.5f, -0.6f, 0f);
	}
	...
	public void OnPathAdvanced(Instance smi, object data)
	{
		...
		if (flag)
		{
			...
			if (flag2)
			{
				flag3 = (smi.navigator.flags & PathFinder.PotentialPath.Flags.HasAtmoSuit) != PathFinder.PotentialPath.Flags.None || (smi.navigator.flags & PathFinder.PotentialPath.Flags.HasJetPack) != PathFinder.PotentialPath.Flags.None || (smi.navigator.flags & PathFinder.PotentialPath.Flags.HasLeadSuit) != 0;
			}
			...
			if (flag2 != flag4)
			{
				if (smi.resume.HasPerk(smi.swimStaminaPerk))
				{
					smi.ApplyAttributeModifier(swimmingStaminaModifier, flag2);
				}
				if (smi.resume.HasPerk(smi.swimAthleticPerk))
				{
					smi.ApplyAttributeModifier(swimmingAthleticsModifier, flag2);
				}
			}
			smi.previousNavType = smi.navigator.CurrentNavType;
			smi.selectable.ToggleStatusItem(HasSuitSwimPenalty, flag3);
		}
		bool flag5 = flag2 || Grid.IsSubstantialLiquid(smi.navigator.cachedCell);
		if (flag5 != smi.wasInLiquid)
		{
			smi.ApplyAttributeModifier(inLiquidStaminaModifier, flag5);
