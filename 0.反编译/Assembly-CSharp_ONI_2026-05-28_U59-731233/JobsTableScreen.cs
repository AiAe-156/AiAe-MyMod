...
public class JobsTableScreen : TableScreen
{
	...
	private void LoadValue(IAssignableIdentity minion, GameObject widget_go)
	{
		if (!(widget_go == null))
		{
			...
			{
				IPersonalPriorityManager priorityManager = GetPriorityManager(widgetRow);
				bool raycastTarget = priorityManager.IsChoreGroupDisabled(choreGroup);
				HierarchyReferences component = widget_go.GetComponent<HierarchyReferences>();
				KImage kImage = component.GetReference("FG") as KImage;
				kImage.raycastTarget = raycastTarget;
				ToolTip toolTip = component.GetReference("FGToolTip") as ToolTip;
				toolTip.enabled = raycastTarget;
				break;
			}
			}
	...
	private void UpdateWidget(GameObject widget_go, ChoreGroup chore_group, IPersonalPriorityManager priority_mgr)
	{
		...
		if (num > 0)
		{
			...
			fillColour = Color32.Lerp(GlobalAssets.Instance.colorSet.PrioritiesLowColor, GlobalAssets.Instance.colorSet.PrioritiesHighColor, num2);
		}
	...
	private void RefreshEffectListeners()
	{
		...
		for (int num = 0; num < Components.LiveMinionIdentities.Count; num++)
		{
			MinionIdentity id2 = Components.LiveMinionIdentities[num];
			Components.LiveMinionIdentities[num].gameObject.Subscribe(540773776, delegate
			{
	...
	private void InitializeHeader(ChoreGroup chore_group, GameObject widget_go)
	{
		...
		for (int num = 5; num >= 0; num--)
		{
			...
			{
				GameObject gameObject = Util.KInstantiateUI(original, items_root, force_active: true);
