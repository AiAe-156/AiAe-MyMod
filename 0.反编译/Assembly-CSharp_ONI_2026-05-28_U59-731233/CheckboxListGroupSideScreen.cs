...
public class CheckboxListGroupSideScreen : SideScreenContent
{
	...
	private ObjectPool<CheckboxContainer> checkboxContainerPool = null;

	private GameObjectPool checkboxPool = null;
	...
	{
		GameObject gameObject = Util.KInstantiateUI(checkboxGroupPrefab, groupParent.gameObject, force_active: true);
		return new CheckboxContainer(gameObject.GetComponent<HierarchyReferences>());
	}
	...
	public override bool IsValidForTarget(GameObject target)
	{
		...
		if (components != null)
		{
			...
			foreach (ICheckboxListGroupControl checkboxListGroupControl in array)
			{
				if (checkboxListGroupControl.SidescreenEnabled())
				{
					return true;
				}
			}
		}
		List<ICheckboxListGroupControl> allSMI = target.GetAllSMI<ICheckboxListGroupControl>();
		foreach (ICheckboxListGroupControl item in allSMI)
		{
	...
	private void Rebuild(GameObject buildTarget)
	{
		...
		foreach (ICheckboxListGroupControl target in targets)
		{
			...
			ICheckboxListGroupControl.ListGroup[] array = data;
			foreach (ICheckboxListGroupControl.ListGroup listGroup in array)
			{
	...
	private void Refresh(object data = null)
	{
		...
		foreach (ICheckboxListGroupControl target in targets)
		{
			...
			ICheckboxListGroupControl.ListGroup[] array = data2;
			for (int i = 0; i < array.Length; i++)
			{
				ICheckboxListGroupControl.ListGroup listGroup = array[i];
				...
				{
					LocText reference = checkboxContainer.container.GetReference<LocText>("Text");
					reference.SetText(listGroup.resolveTitleCallback(listGroup.title));
				}
