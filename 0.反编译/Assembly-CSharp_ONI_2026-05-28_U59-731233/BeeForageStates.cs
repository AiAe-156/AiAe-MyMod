...
public class BeeForageStates : GameStateMachine<BeeForageStates, BeeForageStates.Instance, IStateMachineTarget, BeeForageStates.Def>
{
	...
	public override void InitializeStates(out BaseState default_state)
	{
		...
		state.ToggleStatusItem(text, tooltip, "", StatusItem.IconType.Info, NotificationType.Neutral, allow_multiples: false, default(HashedString), 129022, null, null, main).Exit(UnreserveTarget).Exit(DropAll);
	...
	private static Util.IterationInstruction findOreVisitor(object obj, ref (Element, Navigator, Pickupable, int) context)
	{
	...
	private static bool FindOre(Instance smi)
	{
		...
		(Element, Navigator, Pickupable, int) context = (element, component, null, 100);
		GameScenePartitioner.Instance.VisitEntries<(Element, Navigator, Pickupable, int)>(extents.x, extents.y, extents.width, extents.height, GameScenePartitioner.Instance.pickupablesLayer, (GameScenePartitioner.VisitorRef<(Element, Navigator, Pickupable, int)>)findOreVisitor, ref context);
