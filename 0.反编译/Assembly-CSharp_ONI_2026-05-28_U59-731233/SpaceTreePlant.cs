...
public class SpaceTreePlant : GameStateMachine<SpaceTreePlant, SpaceTreePlant.Instance, IStateMachineTarget, SpaceTreePlant.Def>
{
	...
	public new class Instance : GameInstance
	{
		...
		public float OptimalProductionDuration => IsWildPlanted ? (base.def.OptimalProductionDuration * 4f) : base.def.OptimalProductionDuration;
		...
		public bool CanBeManuallyHarvested => UserAllowsHarvest && !HasPipeConnected;

		public bool UserAllowsHarvest => harvestDesignatable == null || (harvestDesignatable.HarvestWhenReady && harvestDesignatable.MarkedForHarvest);
		...
		public bool IsEntombed => entombDefenseSMI != null && entombDefenseSMI.IsEntombed;

		public bool IsPipingEnabled => base.sm.PipingEnabled.Get(this);

		public int BranchCount => (tree != null) ? tree.CurrentBranchCount : 0;
		...
		private void OnNewBranchSpawnedWhileTreeIsReadyForHarvest(object data)
		{
			if (data != null)
			{
				PlantBranch.Instance instance = (PlantBranch.Instance)data;
				instance.gameObject.AddTag(SpaceTreeReadyForHarvest);
			}
		}
		...
		{
			float num = dt / base.smi.OptimalProductionDuration * base.smi.GetProductionSpeed();
			float mass = Mathf.Min(num * storage.capacityKg, storage.RemainingCapacity());
			Element element = ElementLoader.GetElement(SimHashes.SugarWater.CreateTag());
			float lowTemp = element.lowTemp;
			float num2 = 8f;
			float temperature = Mathf.Max(pe.Temperature, lowTemp + num2);
		...
		{
			float num = growing.PercentOfCurrentHarvest();
			int num2 = Mathf.Clamp(Mathf.FloorToInt(num / (1f / 3f)), 0, 2);
			return "wilt" + (num2 + 1);
		}
