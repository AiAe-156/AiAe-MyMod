...
public class SpaceTreeBranch : GameStateMachine<SpaceTreeBranch, SpaceTreeBranch.Instance, IStateMachineTarget, SpaceTreeBranch.Def>
{
	...
	public new class Instance : GameInstance, IManageGrowingStates
	{
		...
		private float lastFillAmountRecorded = 0f;
		...
		public float Productivity => IsBranchFullyGrown ? Mathf.Clamp((float)CurrentAmountOfLux / (float)base.def.OPTIMAL_LUX_LEVELS, 0f, 1f) : 0f;

		public bool IsTrunkHealthy => trunk != null && !trunk.HasTag(GameTags.Wilting);

		public bool IsTrunkWildPlanted => trunk != null && !trunk.GetComponent<ReceptacleMonitor>().Replanted;
		...
		public void PlayFillAnimation(float fillLevel)
		{
			...
			if ((!entombDefenseSMI.IsEntombed || !entombDefenseSMI.IsActive) && animController != null)
			{
				...
				if (currentAnim != fillAnimNameForState)
				{
