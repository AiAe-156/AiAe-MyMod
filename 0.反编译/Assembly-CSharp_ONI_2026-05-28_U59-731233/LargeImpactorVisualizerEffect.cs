...
public class LargeImpactorVisualizerEffect : KMonoBehaviour
{
	...
	{
		if (rangeVisualizer == null || !rangeVisualizer.Active)
		{
			return;
		}
		if (rangeVisualizer.Folded)
		{
			float num = Time.unscaledTime - rangeVisualizer.LastTimeSetToFolded;
			if (num > rangeVisualizer.FoldEffectDuration + 1f)
			{
				return;
			}
		}
		...
		if (flag)
		{
			...
			{
				int num2 = vector2I2.y + rangeMin.y + i;
				for (int j = 0; j <= rangeMax.x - rangeMin.x; j++)
				{
					int num3 = vector2I2.x + rangeMin.x + j;
					int arg = Grid.XYToCell(num3, num2);
					bool flag2 = num3 > world_min.x && num3 < world_max.x && num2 > world_min.y && num2 < world_max.y && rangeVisualizer.BlockingCb(arg);
