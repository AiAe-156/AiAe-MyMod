...
public abstract class OverlayModes
{
		...
		public override void Update()
		{
			...
			foreach (SaveLoadRoot item in allIntersecting)
			{
				...
				AddTargetIfVisible(item, min, max, layerTargets, objectTargetLayer, delegate(SaveLoadRoot root)
				{
					Vector3 position = root.transform.GetPosition();
					float z = position.z;
					KPrefabID component4 = root.GetComponent<KPrefabID>();
					if (component4 != null)
					{
						if (component4.HasTag(GameTags.OverlayInFrontOfConduits))
						{
							z = Grid.GetLayerZ((ViewMode() == LiquidConduits.ID) ? Grid.SceneLayer.LiquidConduits : Grid.SceneLayer.GasConduits) - 0.2f;
						}
						else if (component4.HasTag(GameTags.OverlayBehindConduits))
						{
							z = Grid.GetLayerZ((ViewMode() == LiquidConduits.ID) ? Grid.SceneLayer.LiquidConduits : Grid.SceneLayer.GasConduits) + 0.2f;
						}
					}
	...
	public class Disease : Mode
	{
		...
		{
			return new ToolParameterMenu.ToggleData[3]
			{
	...
	public class Logic : Mode
	{
		...
		public override void Update()
		{
				...
				if (!(root == null))
				{
					...
					if (component7 != null)
					{
						...
						else if (prefabTag == bridge_id)
						{
							...
							bridgeControllers.RemoveWhere((BridgeInfo x) => x.controller == controller);
						}
						else if (prefabTag == ribbon_bridge_id)
						{
							KBatchedAnimController controller2 = root.GetComponent<KBatchedAnimController>();
							ResetRibbonTint(controller2);
							ribbonBridgeControllers.RemoveWhere((BridgeInfo x) => x.controller == controller2);
						}
						else
						{
							float defaultDepth = Mode.GetDefaultDepth(root);
					...
					AddTargetIfVisible(item, min, max, gameObjTargets, conduitTargetLayer, delegate(SaveLoadRoot root)
					{
						if (!(root == null))
						{
							...
							if (HighlightItemIDs.Contains(component7.PrefabTag))
							{
								...
								else if (component7.PrefabTag == ribbon_id)
								{
									ribbonControllers.Add(root.GetComponent<KBatchedAnimController>());
								}
								else if (component7.PrefabTag == bridge_id)
								{
									...
									int networkCell2 = component9.GetNetworkCell();
								...
								else if (component7.PrefabTag == ribbon_bridge_id)
								{
									KBatchedAnimController component10 = root.GetComponent<KBatchedAnimController>();
									LogicUtilityNetworkLink component11 = root.GetComponent<LogicUtilityNetworkLink>();
									int networkCell3 = component11.GetNetworkCell();
			...
			foreach (ILogicUIElement item2 in allIntersecting2)
			{
				if (item2 != null)
				{
		...
		protected UniformGrid<ILogicUIElement> CreateLogicUIPartition()
		{
			...
			LogicCircuitManager logicCircuitManager = Game.Instance.logicCircuitManager;
			List<ILogicUIElement> visElements = logicCircuitManager.GetVisElements();
	...
	public class Power : Mode
	{
		...
		private float[] previousTilePower = null;
		...
		public Power(Canvas powerLabelParent, LocText powerLabelPrefab, BatteryUI batteryUIPrefab, Vector3 powerLabelOffset, Vector3 batteryUIOffset, Vector3 batteryUITransformerOffset, Vector3 batteryUISmallTransformerOffset)
		{
			...
			previousTilePower = new float[Grid.CellCount];
			targetLayer = LayerMask.NameToLayer("MaskedOverlay");
			cameraLayerMask = LayerMask.GetMask("MaskedOverlay", "MaskedOverlayBG");
			selectionMask = cameraLayerMask;
		}
		...
		public override void Update()
		{
			...
			foreach (SaveLoadRoot layerTarget in layerTargets)
			{
				...
				if (component2 != null)
				{
					...
					if (num3 == -1f)
					{
						num3 = previousTilePower[networkCell2];
					}
					else
					{
						previousTilePower[networkCell2] = num3;
					}
					...
					Color32 tintColour = ((num3 <= 0f) ? GlobalAssets.Instance.colorSet.powerCircuitUnpowered : ((num3 > maxSafeWattageForCircuit) ? GlobalAssets.Instance.colorSet.powerCircuitOverloading : ((!(wattsNeededWhenActive > maxSafeWattageForCircuit) || !(maxSafeWattageForCircuit > 0f) || !(num3 / maxSafeWattageForCircuit >= 0.75f)) ? GlobalAssets.Instance.colorSet.powerCircuitSafe : GlobalAssets.Instance.colorSet.powerCircuitStraining)));
		...
		private void UpdatePowerLabels()
		{
			foreach (UpdatePowerInfo item2 in updatePowerInfo)
			{
				...
				if (generator != null && consumer == null)
				{
					...
					Color color = (unitLabel.color = (powerLabel.color = ((component2 != null && !component2.IsEnabled) ? GlobalAssets.Instance.colorSet.powerBuildingDisabled : GlobalAssets.Instance.colorSet.powerGenerator)));
					BuildingCellVisualizer component3 = generator.GetComponent<BuildingCellVisualizer>();
					if (component3 != null)
					{
						Image powerOutputIcon = component3.GetPowerOutputIcon();
		...
		private void AddBatteryUI(Battery bat)
		{
			...
			if (flag)
			{
				...
				vector2 = ((widthInCells == 2) ? batteryUISmallTransformerOffset : batteryUITransformerOffset);
			}
			vector2.x *= num;
	...
	public class Temperature : Mode
	{
		...
		public override ToolParameterMenu.ToggleData[] CreateDefaultFilters()
		{
			return new ToolParameterMenu.ToggleData[4]
			{
				new ToolParameterMenu.ToggleData(ToolParameterMenu.FILTERLAYERS.ABSOLUTETEMPERATURE, ToolParameterMenu.ToggleState.On),
				new ToolParameterMenu.ToggleData(ToolParameterMenu.FILTERLAYERS.RELATIVETEMPERATURE, ToolParameterMenu.ToggleState.Off),
				new ToolParameterMenu.ToggleData(ToolParameterMenu.FILTERLAYERS.HEATFLOW, ToolParameterMenu.ToggleState.Off),
				new ToolParameterMenu.ToggleData(ToolParameterMenu.FILTERLAYERS.STATECHANGE, ToolParameterMenu.ToggleState.Off)
			};
		}
	...
	public class TileMode : Mode
	{
		...
		private static Util.IterationInstruction updateVisitor(object obj, ref (TileMode, Vector2I, Vector2I) context)
		{
		...
		{
			Grid.GetVisibleExtents(out var min, out var max);
			(TileMode, Vector2I, Vector2I) context = (this, min, max);
			Mode.RemoveOffscreenTargets(layerTargets, min, max);
			int height = max.y - min.y;
			int width = max.x - min.x;
			Extents extents = new Extents(min.x, min.y, width, height);
			GameScenePartitioner.Instance.VisitEntries<(TileMode, Vector2I, Vector2I)>(extents.x, extents.y, extents.width, extents.height, GameScenePartitioner.Instance.pickupablesLayer, (GameScenePartitioner.VisitorRef<(TileMode, Vector2I, Vector2I)>)updateVisitor, ref context);
			GameScenePartitioner.Instance.VisitEntries<(TileMode, Vector2I, Vector2I)>(extents.x, extents.y, extents.width, extents.height, GameScenePartitioner.Instance.completeBuildings, (GameScenePartitioner.VisitorRef<(TileMode, Vector2I, Vector2I)>)updateVisitor, ref context);
			UpdateHighlightTypeOverlay(min, max, layerTargets, targetIDs, highlightConditions, BringToFrontLayerSetting.Conditional, targetLayer);
		}
		...
		public override ToolParameterMenu.ToggleData[] CreateDefaultFilters()
		{
			return new ToolParameterMenu.ToggleData[11]
			{
				new ToolParameterMenu.ToggleData(ToolParameterMenu.FILTERLAYERS.ALL, ToolParameterMenu.ToggleState.On),
				new ToolParameterMenu.ToggleData(ToolParameterMenu.FILTERLAYERS.METAL, ToolParameterMenu.ToggleState.Off),
				new ToolParameterMenu.ToggleData(ToolParameterMenu.FILTERLAYERS.BUILDABLE, ToolParameterMenu.ToggleState.Off),
				new ToolParameterMenu.ToggleData(ToolParameterMenu.FILTERLAYERS.FILTER, ToolParameterMenu.ToggleState.Off),
				new ToolParameterMenu.ToggleData(ToolParameterMenu.FILTERLAYERS.CONSUMABLEORE, ToolParameterMenu.ToggleState.Off),
				new ToolParameterMenu.ToggleData(ToolParameterMenu.FILTERLAYERS.ORGANICS, ToolParameterMenu.ToggleState.Off),
				new ToolParameterMenu.ToggleData(ToolParameterMenu.FILTERLAYERS.FARMABLE, ToolParameterMenu.ToggleState.Off),
				new ToolParameterMenu.ToggleData(ToolParameterMenu.FILTERLAYERS.LIQUIFIABLE, ToolParameterMenu.ToggleState.Off),
				new ToolParameterMenu.ToggleData(ToolParameterMenu.FILTERLAYERS.GAS, ToolParameterMenu.ToggleState.Off),
				new ToolParameterMenu.ToggleData(ToolParameterMenu.FILTERLAYERS.LIQUID, ToolParameterMenu.ToggleState.Off),
				new ToolParameterMenu.ToggleData(ToolParameterMenu.FILTERLAYERS.MISC, ToolParameterMenu.ToggleState.Off)
			};
		}
