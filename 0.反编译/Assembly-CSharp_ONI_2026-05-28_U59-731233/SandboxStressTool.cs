...
public class SandboxStressTool : BrushTool
{
	...
	protected override void OnPaintCell(int cell, int distFromOrigin)
	{
		...
		for (int i = 0; i < Components.LiveMinionIdentities.Count; i++)
		{
			...
			float num2 = -1f * SandboxToolParameterMenu.instance.settings.GetFloatSetting("SandbosTools.StressAdditive");
			Db.Get().Amounts.Stress.Lookup(Components.LiveMinionIdentities[i].gameObject).ApplyDelta(num2);
			if (num2 >= 0f)
			{
				PopFXManager.Instance.SpawnFX(Assets.GetSprite("crew_state_angry"), GameUtil.GetFormattedPercent(num2), gameObject.transform);
			}
			else
			{
				PopFXManager.Instance.SpawnFX(Assets.GetSprite("crew_state_happy"), GameUtil.GetFormattedPercent(num2), gameObject.transform);
			}
			PlaySound(num2, gameObject.transform.GetPosition());
