...
public class BuildingStatusItems : StatusItems
{
	...
	private void CreateStatusItems()
	{
		...
		NewDuplicantsAvailable.notificationClickCallback = delegate
		{
			...
			{
				KSelectable component = Components.Telepads[i].GetComponent<KSelectable>();
			...
			GameUtil.FocusCameraOnWorld(myWorldId, targetTelepad.transform.GetPosition(), 10f, delegate
			{
		...
		PendingRepair.resolveStringCallback = delegate(string str, object data)
		{
			...
			BuildingHP component = sMInstance.master.GetComponent<BuildingHP>();
		...
		ColonyLacksDupeWithMultiSkillPerk.resolveStringCallback = delegate(string str, object data)
		{
			...
			string text = "";
			string[] array2 = array;
			foreach (string perkID in array2)
			{
		...
		TrapHasCritter.resolveTooltipCallback = delegate(string str, object data)
		{
			...
			if (data != null)
			{
				...
				newValue = gameObject.GetComponent<KPrefabID>().GetProperName();
			}
		...
		SpacePOIHarvesting.resolveStringCallback = delegate(string str, object data)
		{
			...
			RocketModuleCluster component = statesInstance.gameObject.GetComponent<RocketModuleCluster>();
			Clustercraft component2 = component.CraftInterface.GetComponent<Clustercraft>();
			ClusterGridEntity pOIAtCurrentLocation = component2.GetPOIAtCurrentLocation();
		...
		{
			GeoTuner.Instance instance = (GeoTuner.Instance)data;
			Geyser assignedGeyser = instance.GetAssignedGeyser();
		...
		GeoTunerGeyserStatus.resolveTooltipCallback = delegate(string str, object data)
		{
			GeoTuner.Instance instance = (GeoTuner.Instance)data;
			Geyser assignedGeyser = instance.GetAssignedGeyser();
			string text = ((assignedGeyser != null) ? assignedGeyser.gameObject.GetProperName() : "None");
			bool flag = assignedGeyser != null && assignedGeyser.smi.GetCurrentState() != null && assignedGeyser.smi.GetCurrentState().parent == assignedGeyser.smi.sm.erupt;
			bool flag2 = assignedGeyser != null && assignedGeyser.smi.GetCurrentState() == assignedGeyser.smi.sm.dormant;
			bool flag3 = !flag2 && !flag;
		...
		{
			MorbRoverMaker.Instance instance = (MorbRoverMaker.Instance)data;
			string text = str.Replace("{GERM_NAME}", Db.Get().Diseases[instance.def.GERM_TYPE].Name);
			text = text.Replace("{0}", GameUtil.GetFormattedDiseaseAmount(instance.def.MAX_GERMS_TAKEN_PER_PACKAGE, GameUtil.TimeSlice.PerSecond));
			text = text.Replace("{1}", GameUtil.GetFormattedDiseaseAmount(instance.MorbDevelopment_GermsCollected));
			return text.Replace("{2}", GameUtil.GetFormattedDiseaseAmount(instance.def.GERMS_PER_ROVER));
		};
		...
		{
			if (_data is Tuple<Element, float> { first: var first } tuple)
			{
				if (first == null)
				{
