...
public class DuplicantStatusItems : StatusItems
{
	...
	private void CreateStatusItems()
	{
		...
		{
			ColdImmunityMonitor.Instance smi = (ColdImmunityMonitor.Instance)data;
			ExternalTemperatureMonitor.Instance sMI = smi.GetSMI<ExternalTemperatureMonitor.Instance>();
		...
		{
			HeatImmunityMonitor.Instance smi = (HeatImmunityMonitor.Instance)data;
			ExternalTemperatureMonitor.Instance sMI = smi.GetSMI<ExternalTemperatureMonitor.Instance>();
		...
		BionicMicrochipGeneration.resolveStringCallback = delegate(string str, object data)
		{
			...
			float percent = instance.Progress * 100f;
		...
		{
			SuffocationMonitor.Instance instance = (SuffocationMonitor.Instance)data;
			float timeUntilDeath = instance.GetTimeUntilDeath(instance);
		...
		ContactWithGerms.statusItemClickCallback = delegate(object data)
		{
			...
			GameUtil.FocusCamera(lastExposurePosition);
		...
		GasLiquidIrritation.resolveStringCallback = delegate(string str, object data)
		{
			...
			return instance.IsMajorIrritation() ? DUPLICANTS.STATUSITEMS.GASLIQUIDEXPOSURE.NAME_MAJOR : DUPLICANTS.STATUSITEMS.GASLIQUIDEXPOSURE.NAME_MINOR;
		};
