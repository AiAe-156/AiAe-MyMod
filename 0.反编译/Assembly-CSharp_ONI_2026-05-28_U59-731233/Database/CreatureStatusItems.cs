...
public class CreatureStatusItems : StatusItems
{
	...
	private void CreateStatusItems()
	{
		...
		Growing.resolveStringCallback = delegate(string str, object data)
		{
			...
			Crop cropComponent = manageGrowingStates.GetCropComponent();
		...
		{
			IPlantConsumeEntities plantConsumeEntities = (IPlantConsumeEntities)data;
			string[] formattedPossiblePreyList = plantConsumeEntities.GetFormattedPossiblePreyList();
			string text = "";
		...
		Hungry.resolveTooltipCallback = delegate(string str, object data)
		{
			...
			Diet diet = instance.stomach.diet;
		...
		{
			BeehiveCalorieMonitor.Instance instance = (BeehiveCalorieMonitor.Instance)data;
			Diet diet = instance.stomach.diet;
			if (diet.consumedTags.Count > 0)
			{
				string[] array = diet.consumedTags.Select((KeyValuePair<Tag, float> t) => t.Key.ProperName()).ToArray();
		...
		Desiccation.SetResolveStringCallback((string str, object data) => (data is DesiccationMonitor.Instance instance) ? string.Format(str, instance.GetEstimatedTimeUntilDeath()) : str);
