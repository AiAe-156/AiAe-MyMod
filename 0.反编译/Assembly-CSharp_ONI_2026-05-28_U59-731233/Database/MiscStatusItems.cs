...
public class MiscStatusItems : StatusItems
{
	...
	private void CreateStatusItems()
	{
		...
		BubbleContents.resolveStringCallback = delegate(string str, object data)
		{
			CellSelectionObject cellSelectionObject = (CellSelectionObject)data;
			string text = "";
			foreach (BubbleManager.CellBubbleInfo bubbleInfo in cellSelectionObject.bubbleInfos)
			{
		...
		SpicedFood.resolveTooltipCallback = delegate(string baseString, object data)
		{
			...
			string text2 = "\n    • ";
			List<SpiceInstance> list = (List<SpiceInstance>)data;
			foreach (SpiceInstance item in list)
			{
				Tag id = item.Id;
		...
		{
			ClusterMapMeteorShower.Instance instance = (ClusterMapMeteorShower.Instance)data;
			float seconds = instance.ArrivalTime - GameUtil.GetCurrentTimeInCycles() * 600f;
