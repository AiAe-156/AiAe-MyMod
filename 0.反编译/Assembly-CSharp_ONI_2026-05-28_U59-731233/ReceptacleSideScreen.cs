...
public class ReceptacleSideScreen : SideScreenContent, IRender1000ms
{
	...
	[SerializeField]
	private bool categoryStartExpanded = false;
	...
	private GameObject SpawnToggle(GameObject parent)
	{
		...
		{
			GameObject gameObject = recycledEntityToggles[recycledEntityToggles.Count - 1];
	...
	{
		return ValidRotationForDeposit(entity.direction) && (!RequiresAvailableAmountToDeposit() || GetAvailableAmount(entity.tag) > 0f) && (!runAdditionalCanDepositTest || AdditionalCanDepositTest());
	}
	...
	protected float GetAvailableAmount(Tag tag)
	{
		if (ALLOW_ORDER_IGNORING_WOLRD_NEED)
		{
			...
			ICollection<Pickupable> pickupables = myWorld.worldInventory.GetPickupables(tag, includeRelatedWorlds: true);
			float num = 0f;
	...
	protected virtual void ToggleClicked(ReceptacleToggle toggle)
	{
		...
		GameObject asset = depositObjectMap[toggle].asset;
		MutantPlant component = asset.GetComponent<MutantPlant>();
