...
public class GlobalAssets : KMonoBehaviour
{
	...
	{
		TMP_Style style = new TMP_Style("logic_on", $"<color=#{ColorUtility.ToHtmlStringRGB((Color)colorSet.logicOn)}>", "</color>");
		TMP_StyleSheet.instance.AddStyle(style);
		TMP_Style style2 = new TMP_Style("logic_off", $"<color=#{ColorUtility.ToHtmlStringRGB((Color)colorSet.logicOff)}>", "</color>");
