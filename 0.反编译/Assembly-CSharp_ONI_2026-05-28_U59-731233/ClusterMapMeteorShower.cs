...
public class ClusterMapMeteorShower : GameStateMachine<ClusterMapMeteorShower, ClusterMapMeteorShower.Instance, IStateMachineTarget, ClusterMapMeteorShower.Def>
{
	public class Def : BaseDef, IGameObjectEffectDescriptor
	{
		...
		public List<Descriptor> GetDescriptors(GameObject go)
		{
			...
			if (sMI != null && sMI.sm.IsIdentified.Get(sMI) && gameplayEvent is MeteorShowerEvent)
			{
				...
				List<MeteorShowerEvent.BombardmentInfo> meteorsInfo = meteorShowerEvent.GetMeteorsInfo();
				float num = 0f;
				...
				foreach (MeteorShowerEvent.BombardmentInfo item3 in meteorsInfo)
				{
					...
					float percent = Mathf.RoundToInt(item3.weight / num * 100f);
					string formattedPercent = GameUtil.GetFormattedPercent(percent);
	...
	public new class Instance : GameInstance, ISidescreenButtonControl
	{
		...
		public string SidescreenButtonText => base.smi.sm.IsIdentified.Get(base.smi) ? "Dev Hide" : "Identify";

		public string SidescreenButtonTooltip => base.smi.sm.IsIdentified.Get(base.smi) ? "Dev unidentify back" : "Identifies the meteor shower";
