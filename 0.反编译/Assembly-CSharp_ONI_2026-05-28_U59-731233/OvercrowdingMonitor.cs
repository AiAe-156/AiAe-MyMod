...
public class OvercrowdingMonitor : GameStateMachine<OvercrowdingMonitor, OvercrowdingMonitor.Instance, IStateMachineTarget, OvercrowdingMonitor.Def>
{
	...
	public struct RegionAnalysis
	{
		...
		public readonly bool IsDirty => occupancy == null || occupancyGeneration != occupancy.Generation;

		public readonly int CellCount => smi.RegionSize;

		public readonly bool IsPond => smi.IsInPond;

		public readonly Dictionary<Tag, int> CritterCounts => (occupancy != null) ? occupancy.CritterCounts : null;

		public readonly int OccupiedCellCount => (occupancy != null) ? occupancy.OccupiedCellCount : 0;

		public readonly int HatchedEggOccupiedCellCount => (occupancy != null) ? occupancy.HatchedEggOccupiedCellCount : 0;

		public readonly bool IsOvercrowded => !IsDegeneratePersonalSpace && UnoccupiedCellCount < 0;

		public readonly bool IsConfined => !ConfinementImmunity && !IsDegeneratePersonalSpace && CellCount < PersonalSpace;

		public readonly bool IsFutureOvercrowded => !IsDegeneratePersonalSpace && FutureUnoccupiedCellCount < 0 && HatchedEggOccupiedCellCount > 0;

		public readonly int OvercrowdedModifier => IsOvercrowded ? (-OverOccupiedCritterCount) : 0;
		...
		public readonly string Substitute(string s)
		{
			...
			string text = s.Replace("{explanation}", locString);
			return text.Replace("{contextCritterType}", smi.kpid.PrefabTag.ProperName()).Replace("{personalSpace}", $"{PersonalSpace}").Replace("{cellCount}", $"{CellCount}")
				.Replace("{occupiedCellCount}", $"{OccupiedCellCount}")
				.Replace("{unoccupiedCellCount}", $"{((!IsOvercrowded) ? UnoccupiedCellCount : 0)}")
				.Replace("{overOccupiedCellCount}", $"{(IsOvercrowded ? OverOccupiedCellCount : 0)}")
				.Replace("{bullets}", BuildCritterOccupancies());
		}
		...
		private readonly int ComputeOverOccupiedCritterCount(int personalSpace)
		{
			...
			int result;
			int num = Math.DivRem(OverOccupiedCellCount, personalSpace, out result);
			return num + ((result != 0) ? 1 : 0);
		}
		...
		private readonly string BuildCritterOccupancies()
		{
			...
			foreach (CritterOccupancy item in pooledList)
			{
				...
				string value = locString.Replace("{critterType}", item.critterType.ProperName()).Replace("{critterCount}", $"{num3}").Replace("{personalSpace}", $"{num4}")
					.Replace("{overOccupancy}", $"{item.overOccupancy}")
					.Replace("{cellCountWithFix}", $"{num5}");
	...
	public new class Instance : GameInstance
	{
		...
		public int RegionSize => (!IsFish) ? ((cavity != null) ? cavity.NumCells : 0) : ((pond != null) ? pond.cellCount : 0);
	...
	private static void AlignTagsAndEffects(Instance smi)
	{
		...
		if (flag6 != flag3)
		{
			smi.confined.instance = SetEffect(smi, smi.confined.Effect, flag6, smi.confined.Tooltip);
		}
		if (flag7 != flag)
		{
			smi.overcrowded.instance = SetEffect(smi, smi.overcrowded.Effect, flag7, smi.overcrowded.Tooltip);
		}
		if (flag8 != flag4)
		{
			smi.futureOvercrowded.instance = SetEffect(smi, smi.futureOvercrowded.Effect, flag8, smi.futureOvercrowded.Tooltip);
		}
	}
