...
public class SegmentedCreature : GameStateMachine<SegmentedCreature, SegmentedCreature.Instance, IStateMachineTarget, SegmentedCreature.Def>
{
	...
	public override void InitializeStates(out BaseState default_state)
	{
		...
		freeMovement.idle.Transition(freeMovement.moving, (Instance smi) => smi.navigator.IsMoving()).Enter(delegate(Instance smi)
		{
			PlayBodySegmentsAnim(smi, "idle_loop", KAnim.PlayMode.Loop, queue: true);
		});
		freeMovement.moving.Transition(freeMovement.idle, (Instance smi) => !smi.navigator.IsMoving()).Enter(delegate(Instance smi)
		{
	...
	private void UpdateRetractedPre(Instance smi, float dt)
	{
		...
		{
			Vector3 b = value.Position - smi.def.pathSpacing * (float)num2 * forward;
