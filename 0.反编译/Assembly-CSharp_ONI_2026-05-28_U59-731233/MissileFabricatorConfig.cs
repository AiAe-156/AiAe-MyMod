...
public class MissileFabricatorConfig : IBuildingConfig
{
	...
	{
		BuildingDef buildingDef = BuildingTemplates.CreateBuildingDef("MissileFabricator", 5, 4, "missile_fabricator_kanim", 250, 60f, TUNING.BUILDINGS.CONSTRUCTION_MASS_KG.TIER5, MATERIALS.REFINED_METALS, 1600f, BuildLocationRule.OnFloor, noise: NOISE_POLLUTION.NOISY.TIER6, decor: TUNING.BUILDINGS.DECOR.PENALTY.TIER2);
	...
	public override void ConfigureBuildingTemplate(GameObject go, Tag prefab_tag)
	{
		...
		string id = ComplexRecipeManager.MakeRecipeID("MissileFabricator", array, array2);
		...
		string id2 = ComplexRecipeManager.MakeRecipeID("MissileFabricator", array3, array4);
		complexRecipe = new ComplexRecipe(id2, array3, array4, DlcManager.DLC4, DlcManager.EXPANSION1);
		complexRecipe.time = 80f;
		complexRecipe.nameDisplay = ComplexRecipe.RecipeNameDisplay.Result;
		...
		complexRecipe.fabricators = new List<Tag> { TagManager.Create("MissileFabricator") };
		ComplexRecipe complexRecipe2 = complexRecipe;
		...
		string id3 = ComplexRecipeManager.MakeRecipeID("MissileFabricator", array5, array6);
		complexRecipe = new ComplexRecipe(id3, array5, array6, DlcManager.EXPANSION1, new string[0]);
		complexRecipe.time = 80f;
		complexRecipe.nameDisplay = ComplexRecipe.RecipeNameDisplay.Result;
		complexRecipe.description = GameUtil.SafeStringFormat(STRINGS.BUILDINGS.PREFABS.MISSILEFABRICATOR.RECIPE_DESCRIPTION_LONGRANGE, STRINGS.ITEMS.MISSILE_LONGRANGE.NAME, MISC.TAGS.REFINEDMETAL, ElementLoader.FindElementByHash(SimHashes.Fertilizer).name, ElementLoader.FindElementByHash(SimHashes.Petroleum).name);
		complexRecipe.fabricators = new List<Tag> { TagManager.Create("MissileFabricator") };
		ComplexRecipe complexRecipe3 = complexRecipe;
