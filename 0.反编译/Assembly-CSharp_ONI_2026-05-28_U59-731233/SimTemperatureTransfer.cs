...
public class SimTemperatureTransfer : KMonoBehaviour
{
	...
	protected bool forceDataSyncOnRegister = false;
	...
	protected override void OnSpawn()
	{
		...
		if (!Grid.IsValidCell(cell) || pe.Element.HasTag(GameTags.Special) || element.specificHeatCapacity == 0f)
		{
			base.enabled = false;
		}
		SimRegister();
	...
	private unsafe static void OnSetTemperature(PrimaryElement primary_element, float temperature)
	{
		...
		if (Sim.IsValidHandle(sttOptimizationHook.simHandle))
		{
			...
			float heat_capacity = mass * primary_element.Element.specificHeatCapacity;
	...
	{
		if (!base.isSpawned || simHandle != -1 || !base.enabled || !(pe.Mass > 0f))
		{
			return;
		}
		Element element = pe.Element;
