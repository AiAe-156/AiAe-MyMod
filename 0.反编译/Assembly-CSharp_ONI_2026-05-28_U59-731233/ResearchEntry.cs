...
public class ResearchEntry : KMonoBehaviour
{
	...
	protected override void OnSpawn()
	{
		...
		if (targetTech != null)
		{
			foreach (TechInstance item in Research.Instance.GetResearchQueue())
			{
				if (item.tech == targetTech)
				{
					QueueStateChanged(isSelected: true);
				}
			}
		}
		StartCoroutine(DelayedSetupForLines());
	}
	...
	public void SetupLines()
	{
		...
		{
			float num = researchScreenReference.GetEntry(targetTech).rectTransform().rect.width / 2f;
			float num2 = researchScreenReference.GetEntry(item).rectTransform().rect.width / 2f;
			...
			component.Points = new Vector2[4]
			{
				...
				new Vector2(0f - num3, 0f) + vector,
				new Vector2(0f - num3, item.center.y - targetTech.center.y) + vector2,
				new Vector2(0f - (targetTech.center.x - num - (item.center.x + num2)) - 4f, item.center.y - targetTech.center.y) + vector2
			};
	...
	public void SetTech(Tech newTech)
	{
		...
		else
		{
			...
			foreach (TechItem unlockedItem in targetTech.unlockedItems)
			{
				...
				reference3.gameObject.SetActive(flag);
				if (flag)
				{
					reference3.color = DlcManager.GetDlcBannerColor(unlockedItem.requiredDlcIds[unlockedItem.requiredDlcIds.Length - 1]);
				}
				string text2 = $"{unlockedItem.Name}\n{unlockedItem.description}";
				if (flag)
				{
					text2 += "\n";
					string[] requiredDlcIds = unlockedItem.requiredDlcIds;
	...
	public void SetEverythingOn()
	{
		...
		foreach (KeyValuePair<Tech, UILineRenderer> item in techLineMap)
		{
			...
			item.Value.Points[0].x = -lineThickness_inactive;
