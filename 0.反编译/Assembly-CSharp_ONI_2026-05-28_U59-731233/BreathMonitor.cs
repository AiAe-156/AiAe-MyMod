...
public class BreathMonitor : GameStateMachine<BreathMonitor, BreathMonitor.Instance>
{
	...
	private static bool IsLowBreath(Instance smi)
	{
		...
		return (myWorld == null || !myWorld.AlertManager.IsRedAlert()) ? (smi.breath.value < DUPLICANTSTATS.STANDARD.Breath.RETREAT_AMOUNT) : (smi.breath.value < DUPLICANTSTATS.STANDARD.Breath.SUFFOCATE_AMOUNT);
	}
