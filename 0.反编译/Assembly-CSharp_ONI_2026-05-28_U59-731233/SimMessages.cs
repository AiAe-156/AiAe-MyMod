...
public static class SimMessages
{
	...
	public unsafe static void CreateDiseaseTable(Diseases diseases)
	{
		...
		{
			Sim.SIM_HandleMessage(825301935, (int)memoryStream.Length, buffer);
		}
	}
	...
	public unsafe static void DefineWorldOffsets(List<WorldOffsetData> worldOffsets)
	{
		...
		fixed (byte* buffer = memoryStream.GetBuffer())
		{
			Sim.SIM_HandleMessage(-895846551, (int)memoryStream.Length, buffer);
		}
	}
	...
	public unsafe static void ConsumeMass(int gameCell, SimHashes element, float mass, byte width, byte height, int callbackIdx = -1)
	{
		...
		{
			UnityEngine.Debug.AssertFormat(false, "Invalid cell: {0} or height {1}", gameCell, height);
			return;
		}
		ushort elementIndex = ElementLoader.GetElementIndex(element);
		MassConsumptionMessage* ptr = stackalloc MassConsumptionMessage[1];
	...
	{
		SetBackwallDataMsg* ptr = stackalloc SetBackwallDataMsg[1];
		ptr->gameCell = cell;
		ptr->elementIdx = elemIdx;
		ptr->mass = mass;
		ptr->temperature = temperature;
		Sim.SIM_HandleMessage(-605421515, sizeof(SetBackwallDataMsg), (byte*)ptr);
	}
}
