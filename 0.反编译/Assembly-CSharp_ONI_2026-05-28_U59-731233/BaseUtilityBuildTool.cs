	...
	private IEnumerator VisUpdater()
	{
		while (true)
		{
			...
			while (i < path.Count)
			{
				...
				path[i] = node2;
				string vis_string = conduitMgr.GetVisualizerString(node2.cell) + "_place";
				...
				kbac.TintColour = (def.IsValidBuildLocation(null, node2.cell, Orientation.Neutral, replace_tile: false, out var reason) ? Color.white : Color.red);
