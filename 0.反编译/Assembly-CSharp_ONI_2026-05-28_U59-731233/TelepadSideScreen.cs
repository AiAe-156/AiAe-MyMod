...
public class TelepadSideScreen : SideScreenContent
{
	...
	private string TimeRemainingString => string.Format(UI.UISIDESCREENS.TELEPADSIDESCREEN.NEXTPRODUCTION, GameUtil.GetFormattedCycles(targetTelepad.GetTimeRemaining()));
	...
	protected override void OnSpawn()
	{
		...
		viewResearchBtn.onClick += delegate
		{
			ManagementMenu.Instance.ToggleResearch();
		};
		researchAttentionIcon.gameObject.SetActive(ManagementMenu.Instance.HasActiveResearch);
	...
	private void Update()
	{
		...
		{
			base.gameObject.SetActive(value: false);
			viewImmigrantsBtnText.text = UI.UISIDESCREENS.TELEPADSIDESCREEN.GAMEOVER;
		...
		else
		{
			...
			{
				viewImmigrantsBtnText.text = TimeRemainingString;
			}
			else
			{
				base.gameObject.SetActive(value: false);
	...
	{
		viewResearchBtn.isInteractable = ManagementMenu.Instance.CheckHasResearchCenter();
		viewImmigrantsBtn.isInteractable = !isLabel;
		viewImmigrantsBtnText.text = (isLabel ? TimeRemainingString : ((string)UI.VIEWDUPLICANTS));
		immigrantBtnAttentionIcon.SetActive(!isLabel);
	}
	...
	private void UpdateVictoryConditions()
	{
		foreach (ColonyAchievement resource in Db.Get().ColonyAchievements.resources)
		{
			...
			foreach (ColonyAchievementRequirement item in resource.requirementChecklist)
			{
				...
				if (item is VictoryColonyAchievementRequirement { shouldUpdateNameAndDescription: not false } victoryColonyAchievementRequirement)
				{
					entries[resource.Id][item].GetComponent<HierarchyReferences>().GetReference<LocText>("Label").SetText(victoryColonyAchievementRequirement.Name());
					entries[resource.Id][item].GetComponent<ToolTip>().SetSimpleTooltip(victoryColonyAchievementRequirement.Description());
				}
			}
		}
