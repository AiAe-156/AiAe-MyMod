...
public class FileErrorReporter : KMonoBehaviour
{
	...
	private void OnFileError()
	{
		if (FileUtil.errorType != FileUtil.ErrorType.None)
		{
			...
			{
				string format = (FileUtil.errorSubject.Contains("OneDrive") ? UI.FRONTEND.SUPPORTWARNINGS.IO_UNAUTHORIZED_ONEDRIVE : UI.FRONTEND.SUPPORTWARNINGS.IO_UNAUTHORIZED);
				text = string.Format(format, FileUtil.errorSubject);
				break;
			}
