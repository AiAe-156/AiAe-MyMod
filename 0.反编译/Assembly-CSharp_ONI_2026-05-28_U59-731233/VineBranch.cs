...
public class VineBranch : PlantBranchGrowerBase<VineBranch, VineBranch.Instance, IStateMachineTarget, VineBranch.Def>
{
	...
	{
		private bool isSpawningNextBranch = false;
		...
		private bool wasMarkedForDeadBeforeStartSM = false;

		public GameObject Mother => base.sm.Mother.Get(this);

		public GameObject Branch => base.sm.Branch.Get(this);

		public Instance BranchSMI => (Branch == null) ? null : Branch.GetSMI<Instance>();
		...
		public bool CanChangeShape => !isSpawningNextBranch && Branch == null && !MaxBranchNumberReached;
		...
		private bool IsMotherWilting => MotherSMI != null && MotherSMI.IsWilting;

		public bool IsWilting => wiltCondition.IsWilting() || IsMotherWilting;
		...
		public void SpawnHarvestedFruit()
		{
			...
			component.SpawnConfiguredFruit(null);
		}
		...
		public void SetupRootInformation(VineMother.Instance mother)
		{
			...
			HarvestDesignatable component2 = GetComponent<HarvestDesignatable>();
		...
		{
			return MotherSMI != null && MotherSMI.IsOnPlanterBox && MotherSMI.PlanterboxCell == cell;
		}

		private bool IsCellFoundation(int cell)
		{
			return VineBranch.IsCellFoundation(cell) || IsMotherCellFoundation(cell);
		}
		...
		public int GetCellToSpawnBranch()
		{
			...
			case Shape.Top:
			case Shape.Bottom:
				return (RootDirection == Direction.Left) ? Grid.OffsetCell(cell, 1, 0) : Grid.OffsetCell(cell, -1, 0);
			case Shape.Left:
			case Shape.Right:
				return (RootDirection == Direction.Up) ? Grid.OffsetCell(cell, 0, -1) : Grid.OffsetCell(cell, 0, 1);
			case Shape.InCornerTopLeft:
				return (RootDirection == Direction.Down) ? Grid.OffsetCell(cell, 1, 0) : Grid.OffsetCell(cell, 0, -1);
			case Shape.InCornerTopRight:
				return (RootDirection == Direction.Down) ? Grid.OffsetCell(cell, -1, 0) : Grid.OffsetCell(cell, 0, -1);
			case Shape.InCornerBottomLeft:
				return (RootDirection == Direction.Up) ? Grid.OffsetCell(cell, 1, 0) : Grid.OffsetCell(cell, 0, 1);
		...
		{
			return IsGrown ? FruitGrowthPercentage : GrowthPercentage;
		}
	...
	public static bool IsCellAvailable(GameObject questionerObj, int cell, Func<int, bool> foundationCheckFunction = null)
	{
		...
		return cell != Grid.InvalidCell && Grid.WorldIdx[cell] == num2 && ((foundationCheckFunction == null) ? (!IsCellFoundation(cell)) : (!foundationCheckFunction(cell))) && !Grid.IsLiquid(cell) && Grid.Objects[cell, 1] == null && Grid.Objects[cell, 5] == null;
	}
