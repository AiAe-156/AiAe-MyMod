...
public class BeachChairWorkable : Workable, IWorkerPrioritizable
{
	...
	protected override bool OnWorkTick(WorkerBase worker, float dt)
	{
		...
		bool flag = (float)num >= (float)BeachChairConfig.TAN_LUX - 1f;
		beachChair.SetLit(flag);
		if (flag)
		{
			loopingSound.SetParameter(soundPath, BEACH_CHAIR_LIT_PARAMETER, 1f);
			timeLit += dt;
		}
		else
		{
			loopingSound.SetParameter(soundPath, BEACH_CHAIR_LIT_PARAMETER, 0f);
		}
		return false;
	}
