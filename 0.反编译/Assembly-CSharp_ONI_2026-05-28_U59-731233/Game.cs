...
public class Game : KMonoBehaviour
{
	...
	private class SpawnFXParams
	{
		...
		public static ObjectPool<SpawnFXParams> Pool = new ObjectPool<SpawnFXParams>(() => new SpawnFXParams(), null, null, null, collectionCheck: false, 128, 1024);
	}
	...
	public float currentFallbackSunlightIntensity = 0f;
	...
	{
		SpawnFXParams spawnFXParams = Unsafe.As<SpawnFXParams>(obj);
		ushort num = (ushort)(1 << spawnFXParams.fx_idx);
		int num2 = Grid.PosToCell(spawnFXParams.pos);
		if ((Instance.activeFX[num2] & num) == 0)
		{
			Instance.activeFX[num2] |= num;
			GameObject instance = spawnFXParams.pool.GetInstance();
			SpawnPoolData spawnPoolData = Instance.fxSpawnData[spawnFXParams.fx_idx];
			...
			{
				int num3 = (int)(spawnFXParams.rotation / 90f);
				if (num3 < 0)
				{
					num3 += spawnPoolData.rotationData.Length;
				}
				text = spawnPoolData.rotationData[num3].animName;
			...
			spawnFXParams.pos.x += insideUnitCircle.x;
			spawnFXParams.pos.y += insideUnitCircle.y;
		...
		SpawnFXParams.Pool.Release(spawnFXParams);
	};
	...
	protected override void OnPrefabInit()
	{
		...
		changelistsPlayedOn.Add(732169u);
	...
	private IEnumerator SanityCheckBoundsNextFrame()
	{
		...
		{
			if (!world.IsDiscovered || world.IsModuleInterior)
			{
				continue;
			}
			int x = world.WorldOffset.X;
			while (x < world.WorldOffset.X + world.WorldSize.X)
			{
				int num;
				for (int y = 0; y < Grid.TopBorderHeight; y = num)
				{
					int cell = Grid.XYToCell(x, world.WorldOffset.Y + world.WorldSize.Y - y);
	...
	{
		StepTheSim(0f);
		StartCoroutine(SanityCheckBoundsNextFrame());
	}
	...
	private unsafe Sim.GameDataUpdate* StepTheSim(float dt)
	{
		...
		zero = Sim.HandleMessage(SimMessageHashes.PrepareGameData, Grid.Visible.Length, Grid.Visible);
		if (zero == IntPtr.Zero)
		{
			return null;
		}
		Sim.GameDataUpdate* ptr = (Sim.GameDataUpdate*)(void*)zero;
		...
		for (int j = 0; j < ptr->numSolidInfo; j++)
		{
			...
			int num = Grid.WorldIdx[solidInfo.cellIdx];
		...
		for (int k = 0; k < ptr->numCallbackInfo; k++)
		{
			...
			HandleVector<CallbackInfo>.Handle handle = new HandleVector<CallbackInfo>.Handle
			{
				index = callbackInfo.callbackIdx
			};
			if (!IsManuallyReleasedHandle(handle))
			{
		...
		for (int num2 = 0; num2 < numSpawnFXInfo; num2++)
		{
			Sim.SpawnFXInfo spawnFXInfo = ptr->spawnFXInfo[num2];
		...
		for (int num3 = 0; num3 < numUnstableCellInfo; num3++)
		{
			Sim.UnstableCellInfo unstableCellInfo = ptr->unstableCellInfo[num3];
		...
		for (int num4 = 0; num4 < numWorldDamageInfo; num4++)
		{
			Sim.WorldDamageInfo damage_info = ptr->worldDamageInfo[num4];
			WorldDamage.Instance.ApplyDamage(damage_info);
		}
		for (int num5 = 0; num5 < ptr->numRemovedMassEntries; num5++)
		{
			Sim.ConsumedMassInfo consumed_info = ptr->removedMassEntries[num5];
		...
		for (int num6 = 0; num6 < numMassConsumedCallbacks; num6++)
		{
			Sim.MassConsumedCallback arg = ptr->massConsumedCallbacks[num6];
		...
		for (int num7 = 0; num7 < numMassEmittedCallbacks; num7++)
		{
			Sim.MassEmittedCallback arg2 = ptr->massEmittedCallbacks[num7];
		...
		for (int num8 = 0; num8 < numDiseaseConsumptionCallbacks; num8++)
		{
			Sim.DiseaseConsumptionCallback arg3 = ptr->diseaseConsumptionCallbacks[num8];
		...
		for (int num9 = 0; num9 < numComponentStateChangedMessages; num9++)
		{
			Sim.ComponentStateChangedMessage componentStateChangedMessage = ptr->componentStateChangedMessages[num9];
		...
		HandleVector<ComplexCallbackInfo<Sim.ConsumedRadiationCallback>>.Handle handle6 = default(HandleVector<ComplexCallbackInfo<Sim.ConsumedRadiationCallback>>.Handle);
		for (int num10 = 0; num10 < numRadiationConsumedCallbacks; num10++)
		{
			Sim.ConsumedRadiationCallback arg4 = ptr->radiationConsumedCallbacks[num10];
			handle6.index = arg4.callbackIdx;
			ComplexCallbackInfo<Sim.ConsumedRadiationCallback> complexCallbackInfo3 = radiationConsumedCallbackManager.Release(handle6, "radiationConsumedCB");
		...
		for (int num11 = 0; num11 < numElementChunkMeltedInfos; num11++)
		{
			Sim.MeltedInfo meltedInfo = ptr->elementChunkMeltedInfos[num11];
			SimTemperatureTransfer.DoOreMeltTransition(meltedInfo.handle);
		}
		int numBuildingOverheatInfos = ptr->numBuildingOverheatInfos;
		for (int num12 = 0; num12 < numBuildingOverheatInfos; num12++)
		{
			Sim.MeltedInfo meltedInfo2 = ptr->buildingOverheatInfos[num12];
		...
		for (int num13 = 0; num13 < numBuildingNoLongerOverheatedInfos; num13++)
		{
			Sim.MeltedInfo meltedInfo3 = ptr->buildingNoLongerOverheatedInfos[num13];
		...
		for (int num14 = 0; num14 < numBuildingMeltedInfos; num14++)
		{
			Sim.MeltedInfo meltedInfo4 = ptr->buildingMeltedInfos[num14];
			StructureTemperatureComponents.DoStateTransition(meltedInfo4.handle);
		}
		int numCellMeltedInfos = ptr->numCellMeltedInfos;
		for (int num15 = 0; num15 < numCellMeltedInfos; num15++)
		{
			Sim.CellMeltedInfo cellMeltedInfo = ptr->cellMeltedInfos[num15];
		...
		for (int num16 = 0; num16 < numBackwallElementChangedInfos; num16++)
		{
			Sim.BackwallElementChangedInfo backwallElementChangedInfo = ptr->backwallElementChangedInfos[num16];
	...
	private void LateUpdate()
	{
		...
		{
			IsPaused = true;
			Trigger(-1788536802, (object)BoxedBools.Box(IsPaused));
		}
		...
		SuperluminalPerf.EndEvent();
		if (GenericGameSettings.instance.scriptedProfile.frameCount != 0 && IsPaused && SpeedControlScreen.Instance != null)
		{
	...
	private void InitializeFXSpawners()
	{
		for (int i = 0; i < fxSpawnData.Length; i++)
		{
			...
			GameObjectPool pool = new GameObjectPool(instantiator, delegate
			{
			}, fxSpawnData[fx_idx].initialCount);
			fxPools[(int)fxSpawnData[fx_idx].id] = pool;
			fxSpawner[(int)fxSpawnData[fx_idx].id] = delegate(Vector3 pos, float rotation)
			{
				SpawnFXParams spawnFXParams = SpawnFXParams.Pool.Get();
				...
				{
					GameInitializeFXSpawners_SpawnFX(spawnFXParams);
				}
				else
				{
					GameScheduler.Instance.Schedule("SpawnFX", 0f, GameInitializeFXSpawners_SpawnFX, spawnFXParams);
				}
			};
		}
	}
	...
	public void Save(BinaryWriter writer)
	{
		...
		gameSaveData.bubbleManager = world.GetComponent<BubbleManager>();
		...
		if (!changelistsPlayedOn.Contains(732169u))
		{
			changelistsPlayedOn.Add(732169u);
		}
	...
	public void Load(Deserializer deserializer)
	{
		...
		StartCoroutine(PerformanceCaptureMonitor.FinishedLoadingSave());
	}
	...
	private IEnumerator DelayedInitialSave()
	{
		...
		if (GenericGameSettings.instance.devAutoWorldGenActive)
		{
			...
			int cell = 0;
			while (cell != Grid.WidthInCells * Grid.HeightInCells)
			{
				Grid.Reveal(cell);
				int num = cell + 1;
				cell = num;
	...
	public static bool IsCorrectDlcActiveForCurrentSave(IHasDlcRestrictions restrictions)
	{
		...
		return (restrictions.GetAnyRequiredDlcIds() == null || IsAnyDlcActiveForCurrentSave(restrictions.GetAnyRequiredDlcIds())) && IsAllDlcActiveForCurrentSave(restrictions.GetRequiredDlcIds()) && !IsAnyDlcActiveForCurrentSave(restrictions.GetForbiddenDlcIds());
	}
	...
	private static bool IsAllDlcActiveForCurrentSave(string[] dlcIds)
	{
		...
		{
			if (!(text == "") && !IsDlcActiveForCurrentSave(text))
			{
				return false;
			}
		}
