...
public class TinkerStation : Workable, ISim1000ms
{
	...
	public bool alwaysTinker = false;
	...
	protected override void OnStartWork(WorkerBase worker)
	{
		...
		if (operational.IsOperational)
		{
			...
			component.AddStatusItem(Db.Get().BuildingStatusItems.ComplexFabricatorProducing, this);
	...
	protected override void OnStopWork(WorkerBase worker)
	{
		...
		KSelectable component = GetComponent<KSelectable>();
		component.RemoveStatusItem(Db.Get().BuildingStatusItems.ComplexFabricatorProducing, this);
	...
	protected override void OnCompleteWork(WorkerBase worker)
	{
		...
		PrimaryElement primaryElement = storage.FindFirstWithMass(inputMaterial, massPerTinker);
		if (primaryElement != null)
		{
			SimHashes elementID = primaryElement.ElementID;
			float aggregate_temperature = 1f;
			storage.ConsumeAndGetDisease(elementID.CreateTag(), massPerTinker, out var _, out var _, out aggregate_temperature);
			GameObject gameObject = GameUtil.KInstantiate(Assets.GetPrefab(outputPrefab), base.transform.GetPosition() + Vector3.up, Grid.SceneLayer.Ore);
	...
	private void UpdateChore()
	{
		if (operational.IsOperational && (ToolsRequested() || alwaysTinker) && HasMaterial())
		{
			if (chore == null)
			{
				...
				SetWorkTime(toolProductionTime);
			}
		}
		else if (chore != null)
		{
	...
	{
		return storage.FindFirstWithMass(inputMaterial, massPerTinker) != null;
	}
