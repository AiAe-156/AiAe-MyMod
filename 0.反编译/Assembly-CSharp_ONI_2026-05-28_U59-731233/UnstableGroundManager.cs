...
public class UnstableGroundManager : KMonoBehaviour
{
	...
	protected override void OnPrefabInit()
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			GameObjectPool pool = new GameObjectPool(() => InstantiateObj(prefab), delegate
			{
			}, 16);
			value.pool = pool;
	...
	private void Update()
	{
		...
		while (num < fallingObjects.Count)
		{
			...
			int num7 = cell;
			if (!Grid.IsValidCell(num4) || Grid.Element[num4].IsSolid || (Grid.Properties[num4] & 4) != 0)
			{
				...
				SimMessages.AddRemoveSubstance(num7, component.element, CellEventLogger.Instance.UnstableGround, component.mass, component.temperature, component.diseaseIdx, component.diseaseCount, do_vertical_solid_displacement: true, handle.index);
				...
				foreach (ScenePartitionerEntry item in pooledList)
				{
					...
					{
						GameObject go = (item.obj as KCollider2D).gameObject;
						go.Trigger(-975551167);
					}
				}
				pooledList.Recycle();
				...
				GameObject gameObject2 = GameUtil.KInstantiate(Assets.GetPrefab(EffectConfigs.OreAbsorbId), position + landEffectOffset, Grid.SceneLayer.Front);
