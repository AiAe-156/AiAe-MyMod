...
public class Tutorial : KMonoBehaviour, IRender1000ms
{
	...
	protected override void OnSpawn()
	{
		...
		{
			tutorialMessagesRemaining.Clear();
			tutorialMessagesSeen.Add(0);
			tutorialMessagesSeen.Add(1);
			tutorialMessagesSeen.Add(15);
			tutorialMessagesSeen.Add(11);
			if (GameUtil.GetCurrentCycle() > 1)
			{
				tutorialMessagesSeen.Add(6);
			}
			if (GameUtil.GetCurrentCycle() > 10)
			{
				tutorialMessagesSeen.Add(5);
				tutorialMessagesSeen.Add(10);
				tutorialMessagesSeen.Add(4);
				tutorialMessagesSeen.Add(13);
				tutorialMessagesSeen.Add(18);
				tutorialMessagesSeen.Add(7);
				tutorialMessagesSeen.Add(2);
			}
			if (GameUtil.GetCurrentCycle() > 30)
			{
				tutorialMessagesSeen.Add(8);
				tutorialMessagesSeen.Add(14);
				tutorialMessagesSeen.Add(19);
			}
			if (GameUtil.GetCurrentCycle() > 100)
			{
				tutorialMessagesSeen.Add(9);
				tutorialMessagesSeen.Add(16);
				tutorialMessagesSeen.Add(17);
			}
			if (BuildingInventory.Instance.BuildingCount("SuitLocker") > 0)
			{
				tutorialMessagesSeen.Add(12);
			}
		}
		if (saved_TM_COUNT < 24)
		{
			Debug.Log("Upgraded tutorial messages");
			saved_TM_COUNT = 24;
		}
	...
	public Message TutorialMessage(TutorialMessages tm, bool queueMessage = true)
	{
		...
		case TutorialMessages.TM_Radiation:
			message = new TutorialMessage(TutorialMessages.TM_Radiation, MISC.NOTIFICATIONS.RADIATION.NAME, MISC.NOTIFICATIONS.RADIATION.MESSAGEBODY, MISC.NOTIFICATIONS.RADIATION.TOOLTIP, null, null, null, "icon_category_radiation", DlcManager.EXPANSION1);
			break;
		case TutorialMessages.TM_BionicBattery:
			message = new TutorialMessage(TutorialMessages.TM_BionicBattery, MISC.NOTIFICATIONS.BIONICBATTERY.NAME, MISC.NOTIFICATIONS.BIONICBATTERY.MESSAGEBODY, MISC.NOTIFICATIONS.BIONICBATTERY.TOOLTIP, null, null, null, "electrobank_large", DlcManager.DLC3);
			break;
		case TutorialMessages.TM_GunkedToilet:
			message = new TutorialMessage(TutorialMessages.TM_GunkedToilet, MISC.NOTIFICATIONS.GUNKEDTOILET.NAME, MISC.NOTIFICATIONS.GUNKEDTOILET.MESSAGEBODY, MISC.NOTIFICATIONS.GUNKEDTOILET.TOOLTIP, null, null, null, "icon_plunger", DlcManager.DLC3);
			break;
		case TutorialMessages.TM_SlipperySurface:
			message = new TutorialMessage(TutorialMessages.TM_SlipperySurface, MISC.NOTIFICATIONS.SLIPPERYSURFACE.NAME, MISC.NOTIFICATIONS.SLIPPERYSURFACE.MESSAGEBODY, MISC.NOTIFICATIONS.SLIPPERYSURFACE.TOOLTIP, null, null, null, "icon_action_mop");
			break;
		case TutorialMessages.TM_BionicOil:
			message = new TutorialMessage(TutorialMessages.TM_BionicOil, MISC.NOTIFICATIONS.BIONICOIL.NAME, MISC.NOTIFICATIONS.BIONICOIL.MESSAGEBODY, MISC.NOTIFICATIONS.BIONICOIL.TOOLTIP, null, null, null, "icon_oil", DlcManager.DLC3);
			break;
		}
		...
		if (queueMessage)
		{
			...
			if (tutorialMessagesSeen.Contains((int)tm))
			{
				return null;
			}
			...
			tutorialMessagesSeen.Add((int)tm);
	...
	private string OnLowFoodTooltip(List<Notification> notifications, object data)
	{
		...
		float calories = WorldResourceAmountTracker<RationTracker>.Get().CountAmount(null, ((WorldContainer)data).worldInventory);
	...
	public void Render1000ms(float dt)
	{
		...
		if (GameClock.Instance.GetCycle() > 0 && !tutorialMessagesSeen.Contains(6) && !queuedPrioritiesMessage)
		{
	...
	private bool EnoughFood()
	{
		...
		float num = WorldResourceAmountTracker<RationTracker>.Get().CountAmount(null, ClusterManager.Instance.activeWorld.worldInventory);
	...
	private bool CanTreatSickDuplicant()
	{
		...
		for (int i = 0; i < Components.LiveMinionIdentities.Count; i++)
		{
			...
			{
				if (modifier.Sickness.severity >= Sickness.Severity.Major)
				{
					flag = true;
					break;
				}
	...
	{
		Components.Cmps<MinionIdentity> minionIdentitiesByModel = Components.GetMinionIdentitiesByModel(MinionConfig.MODEL);
		if (minionIdentitiesByModel.Count <= 0)
		{
			return true;
		}
