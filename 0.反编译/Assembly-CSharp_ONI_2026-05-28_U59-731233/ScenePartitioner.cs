...
public class ScenePartitioner : ISim1000ms
{
	...
	{
		DevOnly.Assert(Game.IsOnMainThread(), "ScenePartioner::Insert is not thread safe");
	...
	{
		DevOnly.Assert(Game.IsOnMainThread(), "ScenePartioner::Withdraw is not thread safe");
	...
	{
		DevOnly.Assert(Game.IsOnMainThread(), "ScenePartioner::Remove is not thread safe");
		if (!GameScenePartitioner.Instance.Lookup(handle, out var entry))
		{
			return;
		}
