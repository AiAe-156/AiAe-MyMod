...
public class BuildingChoresPanelDupeRow : KMonoBehaviour
{
	...
	{
		Vector3 position = choreConsumer.gameObject.transform.GetPosition() + Vector3.up;
	...
	private static string TooltipForDupe(Chore.Precondition.Context context, ChoreConsumer choreConsumer, int rank)
	{
		...
		if (!flag)
		{
			return text.Replace("{FailedPrecondition}", context.chore.GetPreconditions()[context.failedPreconditionId].condition.description);
		}
