...
public class Pickupable : Workable, IHasSortOrder
{
	...
	[SerializeField]
	public List<ChoreType> allowedChoreTypes = null;
	...
	{
		return allowedChoreTypes == null || allowedChoreTypes.Contains(choreType);
	}
	...
	{
		return UnreservedFetchAmount > 0f || FindReservedAmount(carrierID) > 0f;
	}
	...
	{
		return CouldBePickedUpCommon(carrierID) && (storage == null || !storage.automatable || !storage.automatable.GetAutomationOnly());
	}
	...
	{
		return CouldBePickedUpCommon(carrierID) && (fetchable_monitor == null || fetchable_monitor.IsFetchable());
	}
	...
	public void UpdateCachedCell(int cell)
	{
		...
		if (KPrefabID.HasTag(GameTags.PickupableStorage))
		{
			...
			component.UpdateStoredItemCachedCells();
		}
	}
