...
public class SteamUGCService : MonoBehaviour
{
	...
	private void Update()
	{
		...
		{
			Mod mod = FindMod(preview.m_nPublishedFileId);
			DebugUtil.DevAssert(mod != null, "expect mod with pending preview to be published");
			mod.previewImage = LoadPreviewImage(preview);
		...
		previews.RemoveWhere((SteamUGCDetails_t publish) => loaded_previews.Any((Mod mod4) => mod4.fileId == publish.m_nPublishedFileId) || cancelled_previews.Contains(publish.m_nPublishedFileId));
		...
		foreach (SteamUGCDetails_t publish in publishes)
		{
			...
			if ((itemState & (EItemState.k_EItemStateDownloading | EItemState.k_EItemStateDownloadPending)) == 0)
			{
				...
				Mod mod2 = new Mod(publish, LoadPreviewImage(publish));
				pooledList.Add(mod2);
				if (publish.m_hPreviewFile != UGCHandle_t.Invalid && mod2.previewImage == null)
				{
		...
		foreach (Mod mod3 in pooledList)
		{
			int num = mods.FindIndex((Mod candidate) => candidate.fileId == mod3.fileId);
			if (num == -1)
			{
				mods.Add(mod3);
				pooledList2.Add(mod3.fileId);
			}
			else
			{
				mods[num] = mod3;
				pooledList3.Add(mod3.fileId);
			}
		}
		pooledList.Recycle();
		...
		foreach (PublishedFileId_t download in downloads)
		{
			...
			if (((itemState2 & EItemState.k_EItemStateInstalled) == 0 || (itemState2 & EItemState.k_EItemStateNeedsUpdate) != EItemState.k_EItemStateNone) && (itemState2 & (EItemState.k_EItemStateDownloading | EItemState.k_EItemStateDownloadPending)) == 0 && SteamUGC.DownloadItem(download, bHighPriority: false))
			{
