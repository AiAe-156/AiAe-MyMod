...
public class OrbitalObject : KMonoBehaviour, IRenderEveryTick
{
	...
	private Vector3 CalculateWorldPos(float time, out bool behind)
	{
		...
		if (orbitData.periodInCycles > 0f)
		{
			...
			float num2 = (time + (float)timeoffset) / num - (float)(int)((time + (float)timeoffset) / num);
			float f = num2 * 2f * MathF.PI;
			float num3 = 0.5f * orbitData.radiusScale * (float)world.WorldSize.x;
			...
			Vector3 vector2 = quaternion * (vector * num3);
	...
	private string GetInitialAnim(OrbitalData data)
	{
		...
		{
			KAnimFile anim = Assets.GetAnim(data.animFile);
			KAnimFileData data2 = anim.GetData();
			KRandom kRandom = new KRandom();
			int index = kRandom.Next(0, data2.animCount - 1);
