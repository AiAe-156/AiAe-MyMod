	...
	public override void UpdateHoverElements(List<KSelectable> hoverObjects)
	{
		...
		if (flag2)
		{
			...
			if (!element.IsVacuum)
			{
				...
				string text15 = ((element2.specificHeatCapacity == 0f) ? "N/A" : text14);
			...
			if (BubbleManager.instance != null)
			{
				...
				{
					Element element3 = ElementLoader.FindElementByHash(item8.element);
					hoverTextDrawer.NewLine();
					hoverTextDrawer.DrawIcon(iconDash);
					hoverTextDrawer.DrawText(string.Concat(element3.name, " ", UI.TOOLS.GENERIC.BUBBLE_LABEL, ": ", GameUtil.GetFormattedMass(item8.totalMass)), Styles_BodyText.Standard);
				}
				pooledList.Recycle();
			}
			hoverTextDrawer.EndShadowBar();
