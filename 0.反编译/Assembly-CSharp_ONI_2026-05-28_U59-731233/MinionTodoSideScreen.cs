...
public class MinionTodoSideScreen : SideScreenContent
{
	...
	private void PopulateElements(object data = null)
	{
		...
		if (schedule != null)
		{
			...
			text = currentScheduleBlock.name;
			currentShiftLabel.SetText(string.Format(UI.UISIDESCREENS.MINIONTODOSIDESCREEN.CURRENT_SCHEDULE_BLOCK, text).ToUpper());
			currentShiftIcon.color = Db.Get().ScheduleGroups.Get(currentScheduleBlock.GroupId).uiColor;
		}
		choreTargets.Clear();
		bool flag = false;
