...
public class PropertyTextures : KMonoBehaviour, ISim200ms
{
	...
	private TextureProperties[] textureProperties = new TextureProperties[17]
	{
		...
		new TextureProperties
		{
			simProperty = Property.LiquidData,
			textureFormat = TextureFormat.RGBA32,
			filterMode = FilterMode.Point,
			updateEveryFrame = true,
			updatedExternally = true,
		...
		new TextureProperties
		{
			simProperty = Property.MaterialData,
			textureFormat = TextureFormat.RGBA32,
			filterMode = FilterMode.Point,
			updateEveryFrame = true,
			updatedExternally = true,
		...
		new TextureProperties
		{
			simProperty = Property.SolidLiquidGasMassForLight,
			textureFormat = TextureFormat.RGBA32,
			filterMode = FilterMode.Point,
			updateEveryFrame = true,
			updatedExternally = false,
	...
	public static void DestroyInstance()
	{
		...
		externalLiquidDataTex = IntPtr.Zero;
		externalMaterialDataTex = IntPtr.Zero;
	...
	{
		lerpers = new TextureLerper[17];
		texturePagePool = new TexturePagePool();
		textureBuffers = new TextureBuffer[17];
		externallyUpdatedTextures = new Texture2D[17];
		for (int i = 0; i < 17; i++)
		{
			TextureProperties item = new TextureProperties
			{
				textureFormat = TextureFormat.Alpha8,
				filterMode = FilterMode.Bilinear,
				blend = false,
			...
			{
				if (i == (int)textureProperties[j].simProperty)
				{
					item = textureProperties[j];
				}
			}
	...
	private void UpdateProperty(ref TextureProperties p, int x0, int y0, int x1, int y1)
	{
		...
		if (!p.updatedExternally)
		{
			...
			case Property.SolidLiquidGasMass:
				...
				break;
			case Property.SolidLiquidGasMassForLight:
				UpdateTextureThreaded(texture_region, x0, y0, x1, y1, UpdateSolidLiquidGasMassForLight);
				break;
		...
		else
		{
			...
			case Property.Liquid:
				...
				break;
			case Property.LiquidData:
				externallyUpdatedTextures[simProperty].LoadRawTextureData(externalLiquidDataTex, 4 * Grid.WidthInCells * Grid.HeightInCells);
				break;
			case Property.MaterialData:
				externallyUpdatedTextures[simProperty].LoadRawTextureData(externalMaterialDataTex, 4 * Grid.WidthInCells * Grid.HeightInCells);
				break;
	...
	public static Vector4 CalculateClusterWorldSize()
	{
		...
		return (!DlcManager.IsPureVanilla() && (!(CameraController.Instance != null) || !CameraController.Instance.ignoreClusterFX)) ? new Vector4(worldSize.x, worldSize.y - activeWorld.HiddenYOffset, worldOffset.x, worldOffset.y) : new Vector4(Grid.WidthInCells, Grid.HeightInCells, 0f, 0f);
	}
	...
	private void LateUpdate()
	{
		...
		Shader.SetGlobalVector(CameraZoomID, new Vector4(CameraController.Instance.OrthographicSize, CameraController.Instance.overlayCamera.aspect, num, (CameraController.Instance.OrthographicSize - CameraController.Instance.minOrthographicSize) / (num - CameraController.Instance.minOrthographicSize)));
	...
	private static void UpdateSolidDigAmount(TextureRegion region, int x0, int y0, int x1, int y1)
	{
		...
		for (int i = y0; i <= y1; i++)
		{
			...
			while (num3 <= num2)
			{
				...
				byte b2 = 0;
				byte b3 = 0;
				bool flag = Grid.ElementIdx[num3] == elementIndex;
				if (Grid.Solid[num3])
				{
					b = byte.MaxValue;
	...
	{
		UpdateSolidLiquidGasMass(region, x0, y0, x1, y1, diferenciateImpermeable: false);
	}

	private static void UpdateSolidLiquidGasMass(TextureRegion region, int x0, int y0, int x1, int y1)
	{
		UpdateSolidLiquidGasMass(region, x0, y0, x1, y1, diferenciateImpermeable: true);
	}
	...
	{
		Vector2 vector = instance.coldRange;
		Vector2 vector2 = instance.hotRange;
		for (int i = y0; i <= y1; i++)
		{
			for (int j = x0; j <= x1; j++)
			{
