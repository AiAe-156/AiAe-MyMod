...
public class PrinterceptorScreen : KModalScreen
{
	...
	public override void Show(bool show = true)
	{
		...
		if (show)
		{
			...
			{
				image.sprite = Def.GetUISprite(DatabankHelper.ID).first;
			}
	...
	private void SpawnOptionButton(Tag id)
	{
		...
		GameObject gameObject2 = Util.KInstantiateUI(optionButtonPrefab, optionGridContainer.gameObject, force_active: true);
	...
	{
		MultiToggle multiToggle = optionButtons[id];
		HierarchyReferences component = multiToggle.GetComponent<HierarchyReferences>();
		component.GetReference<LocText>("CostLabel").text = HijackedHeadquartersConfig.GetDataBankCost(id, GetPrintCount(id)).ToString();
	}
	...
	{
		return (target != null && target.printCounts.ContainsKey(id)) ? target.printCounts[id] : 0;
	}
}
