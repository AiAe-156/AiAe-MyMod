...
public readonly struct ClothingOutfitTarget : IEquatable<ClothingOutfitTarget>
{
	...
	{
		return Is<DatabaseAuthoredTemplate>() || Is<UserAuthoredTemplate>();
	}
	...
	public static IEnumerable<ClothingOutfitTarget> GetAllTemplates()
	{
		...
		foreach (var (outfitId, outfitData) in CustomClothingOutfits.Instance.Internal_GetOutfitData().OutfitIdToUserAuthoredTemplateOutfit)
		{
			if (Enum.TryParse<ClothingOutfitUtility.OutfitType>(outfitData.outfitType, ignoreCase: true, out var actualOutfitType))
			{
				yield return new ClothingOutfitTarget(new UserAuthoredTemplate(actualOutfitType, outfitId));
			}
		}
	}
