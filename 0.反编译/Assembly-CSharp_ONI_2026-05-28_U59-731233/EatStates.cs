...
public class EatStates : GameStateMachine<EatStates, EatStates.Instance, IStateMachineTarget, EatStates.Def>
{
	...
	{
		Effect effect = new Effect("StunnedEat", "", "", 5f, show_in_ui: false, trigger_floating_text: false, is_bad: true, "");
		effect.tag = GameTags.Creatures.StunnedBeingEaten;
		return effect;
	}
	...
	private static bool CheckHuntSuccess(Instance smi)
	{
		...
		bool flag = sMI?.IsWild() ?? false;
		bool flag2 = instance?.IsWild() ?? false;
		if (flag && flag2)
		{
