...
public class Edible : Workable, ISaveLoadable, IExtendSplitting
{
	...
	private WorkerState workerState = WorkerState.Irrelevant;

	private WorkerSnapshot workerSnapshot = default(WorkerSnapshot);
	...
	public override HashedString[] GetWorkPstAnims(WorkerBase worker, bool successfully_completed)
	{
		...
		{
			return workerSnapshot.useGarnish ? saltHatWorkPstAnim : hatWorkPstAnim;
		}
		return workerSnapshot.useGarnish ? saltWorkPstAnim : normalWorkPstAnim;
	}
	...
	protected override void OnStartWork(WorkerBase worker)
	{
		...
		KBatchedAnimController component2 = worker.GetComponent<KBatchedAnimController>();
		component2.Stop();
		StartConsuming();
	}
	...
	protected override bool OnWorkTick(WorkerBase worker, float dt)
	{
		...
		bool flag = OnTickConsume(worker, dt);
		if (!flag)
		{
	...
	protected override void OnStopWork(WorkerBase worker)
	{
		...
		component.SetSymbolVisiblity(SALT_SYMBOL, is_visible: true);
	...
	private void TickAnimation(WorkerBase worker)
	{
		...
		case WorkerState.EatLoop:
		{
			...
			if (worker.HasTag(GameTags.CommunalDining) && worker.HasTag(GameTags.WantsToTalk))
			{
				...
				component.SetSymbolVisiblity(SALT_SYMBOL, workerSnapshot.useGarnish);
	...
	public override List<Descriptor> GetDescriptors(GameObject go)
	{
		...
		foreach (string effect2 in foodInfo.Effects)
		{
			string text = "";
			...
			list.Add(new Descriptor(Strings.Get("STRINGS.DUPLICANTS.MODIFIERS." + effect2.ToUpper() + ".NAME"), string.Concat(Strings.Get("STRINGS.DUPLICANTS.MODIFIERS." + effect2.ToUpper() + ".DESCRIPTION"), text)));
		}
		return list;
	}
