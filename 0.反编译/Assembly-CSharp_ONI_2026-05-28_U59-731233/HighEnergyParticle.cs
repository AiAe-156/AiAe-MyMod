...
public class HighEnergyParticle : StateMachineComponent<HighEnergyParticle.StatesInstance>
{
	...
	public class States : GameStateMachine<States, StatesInstance, HighEnergyParticle>
	{
		...
		public override void InitializeStates(out BaseState default_state)
		{
			...
			ready.OnSignal(destroySimpleSignal, destroying.instant).OnSignal(destroySignal, destroying.explode, (StatesInstance smi, SignalParameter param) => smi.master.collision == CollisionType.Creature).OnSignal(destroySignal, destroying.explode, (StatesInstance smi, SignalParameter param) => smi.master.collision == CollisionType.Minion)
				.OnSignal(destroySignal, destroying.explode, (StatesInstance smi, SignalParameter param) => smi.master.collision == CollisionType.Solid)
				.OnSignal(destroySignal, destroying.blackhole, (StatesInstance smi, SignalParameter param) => smi.master.collision == CollisionType.HighEnergyParticle)
				.OnSignal(destroySignal, destroying.captured, (StatesInstance smi, SignalParameter param) => smi.master.collision == CollisionType.Captured)
				.OnSignal(destroySignal, catchAndRelease, (StatesInstance smi, SignalParameter param) => smi.master.collision == CollisionType.CaptureAndRelease)
	...
	public void CheckCollision()
	{
		...
		foreach (ScenePartitionerEntry item in pooledList)
		{
			...
			if (!(component4 == null) && !(component4 == this) && component4.isCollideable)
			{
				bool flag = component3.Intersects(component4.transform.position);
		...
		if (gameObject2 != null)
		{
			...
			while (objectLayerListItem != null)
			{
				...
				{
					Health component5 = pickupable.GetComponent<Health>();
					if (component5 != null && !component5.IsDefeated())
					{
		...
		if (gameObject3 != null)
		{
			Health component6 = gameObject3.GetComponent<Health>();
			if (component6 != null && !component6.IsDefeated() && !gameObject3.HasTag(GameTags.Dead) && !gameObject3.HasTag(GameTags.Dying))
			{
				...
				gameObject3.GetComponent<PrimaryElement>().AddDisease(Db.Get().Diseases.GetIndex(Db.Get().Diseases.RadiationPoisoning.Id), Mathf.FloorToInt(payload * 0.5f / 0.01f), "HEPImpact");
		...
		if (Grid.IsSolidCell(cell))
		{
			...
			if (gameObject4 == null || !gameObject4.HasTag(GameTags.HEPPassThrough) || capturedBy == null || capturedBy.gameObject != gameObject4)
			{
