...
public class MainMenu : KScreen
{
	...
	private KButton lockerButton = null;

	private bool itemDropOpenFlag = false;
	...
	private void RefreshInventoryNotification()
	{
		...
		GameObject gameObject = lockerButton.GetComponent<HierarchyReferences>().GetReference<RectTransform>("AttentionIcon").gameObject;
	...
	public void RefreshResumeButton(bool simpleCheck = false)
	{
		...
		if (flag)
		{
			try
			{
				...
				bool flag2 = true;
				if (header.buildVersion > 732169 || gameInfo.saveMajorVersion != 7 || gameInfo.saveMinorVersion > 37)
				{
					flag = false;
				}
