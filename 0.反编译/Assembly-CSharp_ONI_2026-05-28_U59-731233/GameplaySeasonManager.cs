...
public class GameplaySeasonManager : GameStateMachine<GameplaySeasonManager, GameplaySeasonManager.Instance, IStateMachineTarget, GameplaySeasonManager.Def>
{
	...
	public new class Instance : GameInstance
	{
		...
		public void Initialize()
		{
			...
			{
				if (Game.IsCorrectDlcActiveForCurrentSave(item) && item.startActive && !SeasonExists(item) && item.events.Count > 0)
				{
			...
			{
				if (!list.Contains(item2.Season) || !Game.IsCorrectDlcActiveForCurrentSave(item2.Season))
				{
