...
public class SelectedRecipeQueueScreen : KScreen
{
	...
	{
		public bool showFilterRow = false;
	...
	private int cycleRecipeVariantIdx = 0;
	...
	private void RefreshResultDescriptors()
	{
		...
		List<Descriptor> list2 = target.AdditionalEffectsForRecipe(selectedRecipes[0]);
		foreach (Descriptor item in list2)
		{
		...
		for (int num = 0; num < selectedRecipes[0].ingredients.Length; num++)
		{
			...
			for (int num2 = 0; num2 < selectedRecipes.Count; num2++)
			{
				...
				if (flag2 || DebugHandler.InstantBuildMode)
				{
					...
					bool flag3 = selectedMaterialOption[num] == selectedRecipes[num2].ingredients[num].material;
					if (flag3)
					{
					...
					component2.GetReference<RectTransform>("SelectionHover").gameObject.SetActive(flag3);
					...
					component3.ChangeState(flag3 ? 1 : 0);
			...
			if (hashSet.Count > 0)
			{
				text2 = text2 + " <color=#bf5858>(" + list2.Count + "/" + (list2.Count + hashSet.Count) + ")" + UIConstants.ColorSuffix;
			}
