...
public class BuildingComplete : Building
{
	...
	private void OnObjectReplaced(object data)
	{
		...
		replacingTileLayer = value.TileLayer;
	}
	...
	protected override void OnSpawn()
	{
		...
		{
			for (int num2 = 0; num2 < base.PlacementCells.Length; num2++)
			{
				Grid.PreventIdleTraversal[base.PlacementCells[num2]] = true;
			}
		}
		...
		if (attributes != null)
		{
			...
			if (component != null)
			{
				for (int num3 = 1; num3 < component.constructionElements.Length; num3++)
				{
					Tag tag = component.constructionElements[num3];
	...
	{
		string text = "AI_Inspect_" + GetComponent<KPrefabID>().PrefabTag.Name;
		return GlobalAssets.GetSound(text);
	}
		...
		if (!WasReplaced() && base.gameObject.GetMyWorldId() != 255)
		{
			...
			if (Def.IsFoundation)
			{
				...
				foreach (int num2 in array)
				{
					...
					Game.Instance.roomProber.SolidChangedEvent(num2, ignoreDoors: false);
				}
			}
			if (Def.PreventIdleTraversalPastBuilding)
			{
				for (int num3 = 0; num3 < base.PlacementCells.Length; num3++)
				{
					Grid.PreventIdleTraversal[base.PlacementCells[num3]] = false;
				}
			}
		}
