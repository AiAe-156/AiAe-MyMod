...
public class WorldLayout
{
	...
	public Tree GenerateOverworld(bool usePD, bool isRunningDebugGen)
	{
		...
		for (int num2 = 0; num2 < randomPoints.Count; num2++)
		{
			Cell cell2 = overworldGraph.AddNode(WorldGenTags.UnassignedNode.Name, randomPoints[num2]);
		...
		for (int num3 = 0; num3 < voronoiTree.ChildCount(); num3++)
		{
			list2.Add(voronoiTree.GetChild(num3).site);
		}
		if (usePD)
		{
	...
	private HashSet<WeightedSubWorld> GetNameFilterSet(VoronoiTree.Node vn, World.AllowedCellsFilter filter, List<WeightedSubWorld> subworlds)
	{
		...
		{
			int i4;
			for (i4 = 0; i4 < filter.subworldNames.Count; i4++)
			{
				hashSet.UnionWith(subworlds.FindAll((WeightedSubWorld f) => f.subWorld.name == filter.subworldNames[i4]));
			}
			break;
		}
		case World.AllowedCellsFilter.TagCommand.AtTag:
		{
			...
			int i3;
			for (i3 = 0; i3 < filter.subworldNames.Count; i3++)
			{
				hashSet.UnionWith(subworlds.FindAll((WeightedSubWorld f) => f.subWorld.name == filter.subworldNames[i3]));
			}
			break;
		}
		case World.AllowedCellsFilter.TagCommand.NotAtTag:
		{
			...
			int i2;
			for (i2 = 0; i2 < filter.subworldNames.Count; i2++)
			{
				hashSet.UnionWith(subworlds.FindAll((WeightedSubWorld f) => f.subWorld.name == filter.subworldNames[i2]));
			}
			break;
		}
	...
	private void AddSubworldChildren()
	{
		...
		for (int j = 0; j < voronoiTree.ChildCount(); j++)
		{
			...
			{
				Tree child2 = child as Tree;
				Node node2 = overworldGraph.FindNodeByID(child2.site.id);
				SubWorld subWorld = SerializingCloner.Copy(worldGen.Settings.GetSubWorld(node2.type));
				child2.AddTag(new Tag(node2.type));
				child2.AddTag(new Tag(subWorld.temperatureRange.ToString()));
				child2.AddTag(new Tag(subWorld.zoneType.ToString()));
				if (dictionary.ContainsKey(child.site.id))
				{
					subWorld.features.AddRange(dictionary[child.site.id]);
				}
				GenerateChildren(subWorld, child2, localGraph, mapHeight, j + myRandom.seed);
				child2.RelaxRecursive(0, 10, 1f, worldGen.Settings.world.layoutMethod == World.LayoutMethod.PowerTree);
				child2.VisitAll(delegate(VoronoiTree.Node n)
				{
					Debug.Assert(n.site.poly != null, $"Node {n.site.id}, child of {child2.site.id} had a null poly after final subworld relax!!");
				});
			}
		}
	...
	public Vector2I GetStartLocation()
	{
		...
		Node node = FindFirstNodeWithTag(WorldGenTags.StartLocation);
		if (node == null)
		{
			...
			node = localGraph.FindNode((Cell cell) => (uint)cell.NodeId == nodes[0].site.id);
			node.tags.Add(WorldGenTags.StartLocation);
		}
		if (node == null)
		{
			Debug.LogWarning("Couldnt find start node");
		...
		return new Vector2I((int)node.position.x, (int)node.position.y);
	}
	...
	internal void OnSerializingMethod()
	{
		try
		{
			...
			for (int num = 0; num < extra.internals.Count; num++)
			{
				Tree vt = extra.internals[num];
				if (vt.parent != null)
				{
					int num2 = extra.internals.FindIndex(0, (Tree n) => n == vt.parent);
					if (num2 >= 0)
					{
						extra.internalInternalParent.Add(new KeyValuePair<int, int>(num, num2));
					}
				}
			}
		}
