...
public class OfflineWorldGen : KMonoBehaviour
{
	...
	private bool UpdateProgress(StringKey stringKeyRoot, float completePercent, WorldGenProgressStages.Stages stage)
	{
		...
		for (int num5 = 0; num5 < WorldGenProgressStages.StageWeights.Length; num5++)
		{
			num3 += WorldGenProgressStages.StageWeights[num5].Value;
			if (num5 < (int)currentStage)
			{
				num2 += WorldGenProgressStages.StageWeights[num5].Value;
			}
		}
	...
	private void Update()
	{
		if (cluster.HasGenerationStopped)
		{
			cluster.Join();
		}
		...
		if (!debug && currentConvertedCurrentStage.Hash == UI.WORLDGEN.COMPLETE.key.Hash && currentPercent >= 1f && cluster.HasGenerationStopped)
		{
