...
public static class CreatureHelpers
{
	...
	private static float FleeCellRater(int cell, fleeThreatInfo threat)
	{
		return (float)Grid.GetCellDistance(cell, threat.threatCell) + (isInFavoredFleeDirection(cell, threat.threatCell, threat.selfCell) ? 2f : 0f);
	}
	...
	public static GameObject GetFleeTargetLocatorObject(GameObject self, GameObject threat)
	{
		...
		fleeThreatInfo threatInfo = default(fleeThreatInfo);
		...
		int num = FloodFill.FindBest((int cell) => FleeCellRater(cell, threatInfo), (int cell) => (!CanFleeTo(cell, threatInfo.nav)) ? FloodFill.BoundaryCheckResult.Halt : FloodFill.BoundaryCheckResult.Continue, Grid.PosToCell(self), 300);
		if (num != -1)
		{
	...
	{
		bool flag = ((Grid.CellToPos(threatCell).x < Grid.CellToPos(selfCell).x) ? true : false);
