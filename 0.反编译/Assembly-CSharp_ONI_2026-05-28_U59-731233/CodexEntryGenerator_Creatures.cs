...
public class CodexEntryGenerator_Creatures
{
	...
	public static Dictionary<string, CodexEntry> GenerateEntries()
	{
		...
		foreach (Tag speciesTag in array)
		{
		...
		void PushCritterEntry(Tag tag)
		{
			CodexEntry codexEntry = GenerateCritterEntry(tag, tag.ProperName(), brains);
			if (codexEntry != null)
			{
				critterEntries.Add((tag.ToString(), codexEntry));
			}
		}
	}
	...
	private static void GenerateCreatureDescriptionContainers(GameObject creature, List<ContentContainer> containers)
	{
		...
		CritterTemperatureMonitor.Def def3 = creature.GetDef<CritterTemperatureMonitor.Def>();
		if (def3 != null)
		{
			containers.Add(new ContentContainer(new List<ICodexWidget>
			{
				...
				new CodexText("    • " + string.Format(CODEX.CREATURE_DESCRIPTORS.TEMPERATURE.COMFORT_RANGE, GameUtil.GetFormattedTemperature(def3.temperatureColdUncomfortable), GameUtil.GetFormattedTemperature(def3.temperatureHotUncomfortable))),
				new CodexText("    • " + string.Format(CODEX.CREATURE_DESCRIPTORS.TEMPERATURE.NON_LETHAL_RANGE, GameUtil.GetFormattedTemperature(def3.temperatureColdDeadly), GameUtil.GetFormattedTemperature(def3.temperatureHotDeadly)))
			}, ContentContainer.ContentLayout.Vertical));
		}
		...
		OvercrowdingMonitor.Def def4 = creature.GetDef<OvercrowdingMonitor.Def>();
		if (def4 != null && def4.spaceRequiredPerCreature > 0)
		{
			containers.Add(new ContentContainer(new List<ICodexWidget>
			{
				new CodexSpacer(),
				new CodexText(CODEX.HEADERS.CRITTEROVERCROWDING, CodexTextStyle.Subtitle),
				new CodexText("    • " + string.Format(CODEX.CREATURE_DESCRIPTORS.OVERCROWDING, def4.spaceRequiredPerCreature)),
				new CodexText("    • " + string.Format(CODEX.CREATURE_DESCRIPTORS.CONFINED, def4.spaceRequiredPerCreature))
			}, ContentContainer.ContentLayout.Vertical));
		}
		int num = 0;
		string text2 = null;
		...
		if (!string.IsNullOrEmpty(text4) && !string.IsNullOrEmpty(text5))
		{
			...
			ContentContainer item = new ContentContainer(content, ContentContainer.ContentLayout.Vertical);
			...
			ContentContainer item2 = new ContentContainer(content2, ContentContainer.ContentLayout.Vertical);
			containers.Add(item);
		...
		float num2 = 0f;
		Trait trait = Db.Get().traits.Get(creature.GetComponent<Modifiers>().initialTraits[0]);
		...
		foreach (Diet.Info info in array)
		{
			...
			foreach (Tag consumedTag in info.consumedTags)
			{
				...
				bool flag = prefabDiet.IsConsumedTagAbleToBeEatenDirectly(consumedTag);
				...
				if (flag)
				{
					...
					{
						list2.Add(new CodexConversionPanel(consumedTag.ProperName(), consumedTag, num4, inputContinuous: true, GameUtil.GetFormattedDirectPlantConsumptionValuePerCycle, info.producedElement, outputAmount, outputContinuous: true, null, creature));
					}
					else if (info.foodType == Diet.Info.FoodType.EatPlantStorage)
					{
						list2.Add(new CodexConversionPanel(consumedTag.ProperName(), consumedTag, num4, inputContinuous: true, GameUtil.GetFormattedPlantStorageConsumptionValuePerCycle, info.producedElement, outputAmount, outputContinuous: true, null, creature));
					}
					else if (info.foodType == Diet.Info.FoodType.EatPrey || info.foodType == Diet.Info.FoodType.EatButcheredPrey)
					{
						float num5 = prefabDiet.AvailableCaloriesInPrey(consumedTag);
						num4 = (0f - num2) / num5;
						outputAmount = num4 * info.producedConversionRate * num5 / info.caloriesPerKg;
						list2.Add(new CodexConversionPanel(consumedTag.ProperName(), consumedTag, num4, inputContinuous: true, GameUtil.GetFormattedPreyConsumptionValuePerCycle, info.producedElement, outputAmount, outputContinuous: true, null, creature));
					}
				}
				else
				{
					list2.Add(new CodexConversionPanel(consumedTag.ProperName(), consumedTag, num4, inputContinuous: true, info.producedElement, outputAmount, outputContinuous: true, creature));
				}
				if (component3 != null)
				{
					list2.Add(new CodexConversionPanel(CODEX.HEADERS.CRITTER_EXTRA_DIET_PRODUCTION, consumedTag, num4, inputContinuous: true, component3.producedElement.CreateTag(), num4 * 1000f * component3.kgProducedPerKcalConsumed * 2f, outputContinuous: true, creature));
				}
			}
		}
