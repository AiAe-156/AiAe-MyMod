...
public class LockerMenuScreen : KModalScreen
{
	...
	private void CreateDLCLogos()
	{
		foreach (DlcManager.DlcInfo dlcInfo in DlcManager.DLC_PACKS.Values)
		{
			...
			component2.states[0].sprite = Assets.GetSprite(DlcManager.GetDlcSmallLogo(dlcInfo.id));
			string dlcTitle = DlcManager.GetDlcTitle(dlcInfo.id);
			dlcTitle = (DlcManager.IsContentSubscribed(dlcInfo.id) ? string.Concat(dlcTitle, "\n\n", UI.FRONTEND.MAINMENU.DLC.CONTENT_INSTALLED_LABEL, "\n\n", UI.FRONTEND.MAINMENU.DLC.COSMETIC_CONTENT_ACTIVE_TOOLTIP, "\n\n", UI.FRONTEND.MAINMENU.WISHLIST_AD_TOOLTIP) : string.Concat(dlcTitle, "\n\n", UI.FRONTEND.MAINMENU.WISHLIST_AD, "\n\n", UI.FRONTEND.MAINMENU.WISHLIST_AD_TOOLTIP));
			gameObject.GetComponent<ToolTip>().SetSimpleTooltip(dlcTitle);
