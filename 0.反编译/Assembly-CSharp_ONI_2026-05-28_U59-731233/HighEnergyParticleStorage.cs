...
public class HighEnergyParticleStorage : KMonoBehaviour, IStorage
{
	...
	private void UpdateLogicPorts()
	{
		...
		{
			bool flag = IsFull();
			_logicPorts.SendSignal(PORT_ID, flag ? 1 : 0);
		}
	}
	...
	{
		return particles < capacity && receiverOpen;
	}
	...
	private void DeltaParticles(float delta)
	{
		...
		if (particles <= 0f)
		{
			Trigger(155636535, (object)base.transform.gameObject);
		}
		Trigger(-1837862626, (object)base.transform.gameObject);
		UpdateLogicPorts();
	}
