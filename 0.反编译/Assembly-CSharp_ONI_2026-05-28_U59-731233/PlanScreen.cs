...
public class PlanScreen : KIconToggleMenu
{
	...
	private bool useSubCategoryLayout = false;
	...
	private bool forceUpdateAllCategoryToggles = false;
	...
	private bool categoryPanelSizeNeedsRefresh = false;
	...
	private void RefreshScale(object data = null)
	{
		...
		component.cellSize = (ScreenResolutionMonitor.UsingGamepadUIMode() ? new Vector2(54f, 50f) : new Vector2(45f, 45f));
		...
		LayoutElement component2 = copyBuildingButton.GetComponent<LayoutElement>();
		component2.minWidth = (ScreenResolutionMonitor.UsingGamepadUIMode() ? 58 : 54);
		component2.minHeight = (ScreenResolutionMonitor.UsingGamepadUIMode() ? 58 : 54);
	...
	private static void PopulateOrderInfo(HashedString category, object data, Dictionary<Tag, HashedString> category_map, Dictionary<Tag, int> order_map, ref int building_index)
	{
		...
		List<KeyValuePair<string, string>> list = (List<KeyValuePair<string, string>>)data;
	...
	private void RefreshBuildableStates(bool force_update)
	{
		...
		for (int i = 0; i < num; i++)
		{
			...
			{
				HashedString hashedString = (HashedString)item.userData;
	...
	private void SetCategoryButtonState()
	{
		...
		for (int i = 0; i < toggleEntries.Count; i++)
		{
			...
			else
			{
				...
				{
					RequirementsState buildableState = GetBuildableState(buildingDef);
					if (buildableState == RequirementsState.Complete)
					{
						flag = true;
						flag2 = false;
						break;
	...
	private void OpenCategoryPanel(ToggleInfo toggle_info, bool play_sound = true)
	{
		...
		Profiler.BeginSample("BuildButtonList");
		BuildButtonList();
		Profiler.EndSample();
		Profiler.BeginSample("UpdateBuildingButtonList");
		UpdateBuildingButtonList(activeCategoryInfo);
		Profiler.EndSample();
	...
	private void CacheSearchCaches()
	{
		...
		foreach (PlanInfo item in TUNING.BUILDINGS.PLANORDER)
		{
			foreach (KeyValuePair<string, string> buildingAndSubcategoryDatum in item.buildingAndSubcategoryData)
			{
				...
				if (flag && !buildingDefSearchCaches.TryGetValue(buildingDef.PrefabID, out value))
				{
	...
	private void SortButtons()
	{
		...
		{
			PlanBuildingToggle a = allBuildingToggles[item.PrefabID];
			Tuple<PlanBuildingToggle, string> tuple = new Tuple<PlanBuildingToggle, string>(a, item.PrefabID);
	...
	private void SortSubcategories()
	{
		...
		{
			GameObject a = allSubCategoryObjects[item];
			Tuple<GameObject, string> tuple = new Tuple<GameObject, string>(a, item);
		...
		pooledList.Recycle();
		int CompareScores(Tuple<GameObject, string> tuple2, Tuple<GameObject, string> b)
		{
			return subcategorySearchCaches[tuple2.second].CompareTo(subcategorySearchCaches[b.second]);
		}
	}
	...
	public override void OnKeyDown(KButtonEvent e)
	{
		...
		if (mouseOver && base.ConsumeMouseScroll)
		{
			...
			else if (!e.TryConsume(Action.ZoomIn) && !e.TryConsume(Action.ZoomOut))
			{
			}
		}
	...
	private void OnRecipeElementsFullySelected()
	{
		...
		if ((bool)buildingDef)
		{
			if (buildingDef.isKAnimTile && buildingDef.isUtility)
			{
				...
				baseUtilityBuildTool.Activate(buildingDef, getSelectedElementAsList, ProductInfoScreen.FacadeSelectionPanel.SelectedFacade);
			}
			else
			{
