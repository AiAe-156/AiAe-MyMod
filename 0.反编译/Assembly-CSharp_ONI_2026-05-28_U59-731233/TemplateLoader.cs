...
public static class TemplateLoader
{
	...
	public static GameObject PlaceBuilding(Prefab prefab, int root_cell)
	{
		...
		if (prefab.other_values != null)
		{
			...
			foreach (Prefab.template_amount_value template_amount_value2 in other_values)
			{
				...
				case "sealedDoorDirection":
				{
					...
					if ((bool)component6)
					{
						component6.facingRight = template_amount_value2.value != 0f;
					}
					break;
				}
				case "switchSetting":
		...
		{
			BuildingFacade component9 = gameObject.GetComponent<BuildingFacade>();
			if (component9 != null)
			{
				...
				{
					component9.ApplyBuildingFacade(buildingFacadeResource);
				}
			}
		}
		if (!string.IsNullOrEmpty(prefab.loreUnlockId) && gameObject.TryGetComponent<LoreBearer>(out var component10))
		{
			component10.poiOverrideLoreUnlockId = prefab.loreUnlockId;
			component10.poiOverrideLoreDisplayText = prefab.loreDisplayText;
			component10.poiOverrideNextCollectionId = prefab.loreNextCollectionId;
		}
		return gameObject;
	}
