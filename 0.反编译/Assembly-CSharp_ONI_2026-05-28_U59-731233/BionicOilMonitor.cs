...
public class BionicOilMonitor : GameStateMachine<BionicOilMonitor, BionicOilMonitor.Instance, IStateMachineTarget, BionicOilMonitor.Def>
{
	...
	public new class Instance : GameInstance
	{
		...
		public bool IsOnline => batterySMI != null && batterySMI.IsOnline;

		public bool HasOil => CurrentOilMass > 0f;

		public float CurrentOilPercentage => CurrentOilMass / oilAmount.GetMax();

		public float CurrentOilMass => (oilAmount == null) ? 0f : oilAmount.value;
		...
		{
			return resume.HasPerk(Db.Get().SkillPerks.EfficientBionicGears) ? "NoLubricationMinor" : "NoLubricationMajor";
		}
		...
		public void SetBaseDeltaModifierActiveState(bool isActive)
		{
			...
			if (isActive)
			{
				...
				{
					AttributeModifier attributeModifier = component.attributes.Get(BaseOilDeltaModifier.AttributeId).Modifiers[i];
					if (attributeModifier == BaseOilDeltaModifier)
					{
						flag = true;
						break;
					}
