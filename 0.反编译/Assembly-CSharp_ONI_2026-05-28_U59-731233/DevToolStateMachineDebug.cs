...
public class DevToolStateMachineDebug : DevTool
{
	...
	private void Update()
	{
		...
		StateMachineController[] array = null;
		GameObject gameObject = SelectTool.Instance?.selected?.gameObject;
		if (!lockSelection && selectedGameObject != gameObject && gameObject != null)
		{
			array = gameObject.GetComponentsInChildren<StateMachineController>();
			if (array.Length != 0)
			{
	...
	protected override void RenderTo(DevPanel panel)
	{
		...
		StateMachineController[] array2 = componentsInChildren;
		foreach (StateMachineController stateMachineController2 in array2)
		{
		...
		if (list4.Count == 0)
		{
			...
			StateMachineController[] array3 = componentsInChildren;
			foreach (StateMachineController controller in array3)
			{
	...
	private void ShowParameters(StateMachine.Instance state_machine_instance)
	{
		...
		{
			context.ShowDevTool(state_machine_instance);
		}
		ImGui.Unindent();
	}
	...
	private void ShowTransitions(StateMachine.Instance state_machine_instance)
	{
		...
		for (int i = 0; i < currentState.GetStateCount(); i++)
		{
			...
			{
				StateMachine.BaseTransition baseTransition = null;
				for (int j = 0; j < state.transitions.Count; j++)
				{
					baseTransition = state.transitions[j];
					ImGui.Text(baseTransition.ToString());
				}
			}
		}
		ImGui.Unindent();
	...
	{
		KAnimControllerBase componentInChildren = selectedGameObject.GetComponentInChildren<KAnimControllerBase>();
