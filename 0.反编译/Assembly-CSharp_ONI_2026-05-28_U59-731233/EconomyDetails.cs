...
public class EconomyDetails
{
	...
	public EconomyDetails()
	{
		...
		transformation.AddDelta(new Transformation.Delta(GetResource(GameTags.Oxygen), 0f - DUPLICANTSTATS.STANDARD.BaseStats.OXYGEN_USED_PER_SECOND));
		transformation.AddDelta(new Transformation.Delta(GetResource(GameTags.CarbonDioxide), DUPLICANTSTATS.STANDARD.BaseStats.OXYGEN_USED_PER_SECOND * Assets.GetPrefab(MinionConfig.ID).GetComponent<OxygenBreather>().O2toCO2conversion));
		transformation.AddDelta(new Transformation.Delta(duplicantTimeResource, 0.875f));
		transformation.AddDelta(new Transformation.Delta(caloriesResource, DUPLICANTSTATS.STANDARD.BaseStats.GUESSTIMATE_CALORIES_BURNED_PER_SECOND * 0.001f));
		transformation.AddDelta(new Transformation.Delta(CreateResource(new Tag(Db.Get().Amounts.Bladder.deltaAttribute.Id), amountResourceType), DUPLICANTSTATS.STANDARD.BaseStats.BLADDER_INCREASE_PER_SECOND));
	...
	public void DumpTransformations(Scenario scenario, StreamWriter o)
	{
		...
		for (int num3 = 0; num3 < transformations.Count; num3++)
		{
			Transformation transformation = transformations[num3];
		...
		int num4 = 1;
		for (int num5 = 0; num5 < transformations.Count; num5++)
		{
			Transformation transformation2 = transformations[num5];
			...
			if (flag)
			{
				WriteProduct(o, a, (num7 / transformation2.timeInSeconds).ToString("0.00000"), text2);
			}
		...
		{
			if (num9 >= num8 && num9 < num8 + used_resources.Count)
			{
				string text3 = ((char)(65 + num9 % 26)).ToString();
				int num10 = Mathf.FloorToInt((float)num9 / 26f);
				if (num10 > 0)
				{
					text3 = (char)(65 + num10 - 1) + text3;
				}
	...
	public Transformation CreateTransformation(KPrefabID prefab_id, Tag tag)
	{
		...
		if (transformation != null)
		{
			if (component2 != null && component2.consumedElements != null)
			{
				...
				for (int num2 = 0; num2 < consumedElements.Length; num2++)
				{
					ElementConverter.ConsumedElement consumedElement = consumedElements[num2];
				...
				if (component2.outputElements != null)
				{
					...
					for (int num3 = 0; num3 < outputElements.Length; num3++)
					{
						ElementConverter.OutputElement outputElement = outputElements[num3];
			...
			if (component7 != null)
			{
				if (component7.formula.inputs != null)
				{
					...
					for (int num4 = 0; num4 < inputs.Length; num4++)
					{
						EnergyGenerator.InputItem inputItem = inputs[num4];
				...
				if (component7.formula.outputs != null)
				{
					...
					for (int num5 = 0; num5 < outputs.Length; num5++)
					{
						EnergyGenerator.OutputItem outputItem = outputs[num5];
			...
			if (component11 != null)
			{
				...
				float num6 = component11.cropVal.numProduced;
				transformation.AddDelta(new Transformation.Delta(resource4, num6));
				...
				if (prefab != null)
				{
					...
					if (component17 != null)
					{
						transformation.AddDelta(new Transformation.Delta(caloriesResource, component17.FoodInfo.CaloriesPerUnit * num6 * 0.001f));
					}
				}
			}
			...
			if (components != null)
			{
				for (int num9 = 0; num9 < components.Length; num9++)
				{
			...
			if (list != null && list.Count > 0)
			{
				foreach (FertilizationMonitor.Def item in list)
				{
					...
					for (int num10 = 0; num10 < consumedElements2.Length; num10++)
					{
						PlantElementAbsorber.ConsumeInfo consumeInfo = consumedElements2[num10];
			...
			if (list2 != null && list2.Count > 0)
			{
				foreach (IrrigationMonitor.Def item2 in list2)
				{
					...
					for (int num11 = 0; num11 < consumedElements3.Length; num11++)
					{
						PlantElementAbsorber.ConsumeInfo consumeInfo2 = consumedElements3[num11];
			...
			if (component14 != null)
			{
				transformation.AddDelta(new Transformation.Delta(CreateResource(new Tag(Db.Get().Amounts.Bladder.deltaAttribute.Id), amountResourceType), 0f - DUPLICANTSTATS.STANDARD.BaseStats.BLADDER_INCREASE_PER_SECOND));
	...
	private static void DumpEconomyDetails()
	{
		...
		foreach (EdiblesManager.FoodInfo allFoodType in EdiblesManager.GetAllFoodTypes())
		{
			...
			Tag tag = TagManager.Create(allFoodType.Id);
			scenario12.AddEntry(new Scenario.Entry(tag, 1f));
			...
			while (list2.Count > 0)
			{
				Tag tag2 = list2[0];
				list2.RemoveAt(0);
				ComplexRecipe complexRecipe = ComplexRecipeManager.Get().recipes.Find((ComplexRecipe a) => a.FirstResult == tag2);
				if (complexRecipe != null)
				{
					ComplexRecipe.RecipeElement[] ingredients = complexRecipe.ingredients;
					foreach (ComplexRecipe.RecipeElement recipeElement in ingredients)
				...
				foreach (KPrefabID prefab in Assets.Prefabs)
				{
					...
					if (component != null && component.cropVal.cropId == tag2.Name)
					{
		...
		using (StreamWriter streamWriter = new StreamWriter(path2))
		{
			...
			foreach (KeyValuePair<Element, float> startingBiomeAmount in details.startingBiomeAmounts)
			{
				...
				{
					streamWriter.Write(",");
					float num2 = item12.Transform(startingBiomeAmount.Key, startingBiomeAmount.Value);
					if (num2 > 0f)
					{
