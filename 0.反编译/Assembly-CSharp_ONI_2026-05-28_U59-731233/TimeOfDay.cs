	...
	public static bool IsMilestoneApproaching
	{
		get
		{
			if (Instance != null && GameClock.Instance != null)
			{
				...
				return currentTimeRegion == TimeRegion.Night && MILESTONE_CYCLES != null && MILESTONE_CYCLES.Contains(cycle + 1);
			}
			return false;
		}
	}
	...
	public static bool IsMilestoneDay
	{
		get
		{
			if (Instance != null && GameClock.Instance != null)
			{
				TimeRegion currentTimeRegion = Instance.GetCurrentTimeRegion();
				int cycle = GameClock.Instance.GetCycle();
				return currentTimeRegion == TimeRegion.Day && MILESTONE_CYCLES != null && MILESTONE_CYCLES.Contains(cycle);
			}
			return false;
		}
	}

	public TimeRegion timeRegion { get; private set; } = TimeRegion.Invalid;
