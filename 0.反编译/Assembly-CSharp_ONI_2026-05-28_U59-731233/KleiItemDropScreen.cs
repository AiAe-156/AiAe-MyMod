...
public class KleiItemDropScreen : KModalScreen
{
	...
	private Coroutine activePresentationRoutine = null;

	private ServerRequestState serverRequestState;

	private bool giftAcknowledged = false;

	private bool noItemAvailableAcknowledged = false;

	public static KleiItemDropScreen Instance;

	private bool shouldDoCloseRoutine = false;
	...
	{
		float canvasScale = base.transform.parent.GetComponent<CanvasScaler>().scaleFactor;
		float OPEN_WIDTH = (float)Screen.width / canvasScale;
		float OPEN_HEIGHT = Mathf.Clamp((float)Screen.height / canvasScale, 720f, 900f);
		...
		}, shieldMaskRect.sizeDelta, new Vector2(shieldMaskRect.sizeDelta.x, OPEN_HEIGHT), 0.5f, Easing.CircInOut);
		yield return Updater.Ease(delegate(Vector2 v2)
		{
			shieldMaskRect.sizeDelta = v2;
	...
	private IEnumerator PresentNoItemAvailableRoutine(bool firstItem)
	{
		...
		CanvasGroup acceptButtonRectCanvasGroup = acceptButtonRect.FindOrAddComponent<CanvasGroup>();
		acceptButtonRectCanvasGroup.alpha = 0f;
	...
	private IEnumerator PresentItemRoutine(KleiItems.ItemData item, bool firstItem, bool lastItem)
	{
		...
		else if (serverRequestState.revealConfirmedByServer)
		{
			...
			animatedPod.PlaySpeedMultiplier = (firstItem ? 1f : (1f * speedMultiplier));
			animatedPod.Play("additional_pre");
			animatedPod.Queue("working_loop", KAnim.PlayMode.Loop);
			yield return Updater.WaitForSeconds(firstItem ? 1f : (1f / speedMultiplier));
			...
			else
			{
				...
				info.Sprite = permit.GetPermitPresentationInfo().sprite;
				info.UseEquipmentVis = permit.Category == PermitCategory.Equipment;
				if (permit is EquippableFacadeResource)
				{
					info.BuildOverride = (permit as EquippableFacadeResource).BuildOverride;
				}
				name = permit.Name;
				desc = permit.Description;
				rarity = permit.Rarity;
				switch (permit.Category)
				{
				case PermitCategory.Building:
					categoryString = Assets.GetPrefab((permit as BuildingFacadeResource).PrefabID).GetProperName();
					break;
				case PermitCategory.JoyResponse:
					categoryString = PermitCategories.GetDisplayName(permit.Category);
					if (permit is BalloonArtistFacadeResource)
					{
						categoryString = PermitCategories.GetDisplayName(permit.Category) + ": " + UI.KLEI_INVENTORY_SCREEN.CATEGORIES.JOY_RESPONSES.BALLOON_ARTIST;
					}
					break;
				case PermitCategory.Artwork:
					categoryString = PermitCategories.GetDisplayName(permit.Category);
					if (permit is ArtableStage)
					{
						categoryString = Assets.GetPrefab((permit as ArtableStage).prefabId).GetProperName();
					}
					break;
				default:
					categoryString = PermitCategories.GetDisplayName(permit.Category);
