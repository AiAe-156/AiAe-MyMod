...
public class GeoTuner : GameStateMachine<GeoTuner, GeoTuner.Instance, IStateMachineTarget, GeoTuner.Def>
{
	...
	public new class Instance : GameInstance
	{
		...
		{
			bool flag = newFutureGeyser != GetFutureGeyser();
			bool flag2 = GetAssignedGeyser() != newFutureGeyser;
			...
			else if (switchGeyserChore == null && flag2)
			{
				RecreateSwitchGeyserChore();
			}
		}
		...
		private void ShowSwitchingGeyserStatusItem(Chore chore)
		{
			...
			component.AddStatusItem(Db.Get().BuildingStatusItems.PendingSwitchToggle);
		}

		private void HideSwitchingGeyserStatusItem(Chore chore)
		{
			KSelectable component = base.gameObject.GetComponent<KSelectable>();
			component.RemoveStatusItem(Db.Get().BuildingStatusItems.PendingSwitchToggle);
		}

		private void OnSwitchGeyserChoreCompleted(Chore chore)
		{
			bool flag = GetCurrentState() != base.sm.nonOperational;
			Geyser futureGeyser = GetFutureGeyser();
			bool flag2 = GetAssignedGeyser() != futureGeyser;
			if (chore.isComplete && flag2)
			{
		...
		{
			Geyser assignedGeyser = GetAssignedGeyser();
			bool flag = GetCurrentState() != base.smi.sm.nonOperational;
			bool flag2 = assignedGeyser != null && GetCurrentState() != base.smi.sm.operational.noGeyserSelected;
			...
			logicPorts.SendSignal(base.def.OUTPUT_LOGIC_PORT_ID, flag4 ? 1 : 0);
			switchGeyserMeter.meterController.SetSymbolVisiblity("light_bloom", flag4);
		}
	...
	private static bool GeyserExitEruptionTransition(Instance smi, float dt)
	{
		...
		{
			bool flag = assignedGeyser.smi.GetCurrentState().parent == assignedGeyser.smi.sm.erupt;
			return !flag;
		}
		return false;
	}
	...
	{
		Geyser assignedGeyser = smi.GetAssignedGeyser();
		assignedGeyser.AddModification(smi.currentGeyserModification);
	}
	...
	{
		return IsInLabRoom(smi) && smi.storage.MassStored() == smi.storage.capacityKg;
	}
