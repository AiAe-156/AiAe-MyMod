	...
	public void Sim200ms(float dt)
	{
		...
		for (int i = vector2I.y; i <= vector2I2.y; i++)
		{
			for (int j = vector2I.x; j <= vector2I2.x; j++)
			{
				...
				{
					float num3 = targetTemperature - Grid.Temperature[num2];
					float num4 = num3 * Grid.Element[num2].specificHeatCapacity * Grid.Mass[num2];
					if (!Mathf.Approximately(0f, num4))
					{
						SimMessages.ModifyEnergy(num2, num4 * 0.2f, 5000f, (num4 > 0f) ? SimMessages.EnergySourceID.DebugHeat : SimMessages.EnergySourceID.DebugCool);
					}
				}
			}
		}
