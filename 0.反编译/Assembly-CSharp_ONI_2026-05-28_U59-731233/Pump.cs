...
public class Pump : KMonoBehaviour, ISim1000ms
{
	...
	protected override void OnPrefabInit()
	{
		...
		consumer.OnElementConsumedStored += OnElementConsumedStored;
	}

	private void OnElementConsumedStored(Sim.ConsumedMassInfo elementConsumedInfo)
	{
		if (dispenser.conduitType == ConduitType.Liquid)
		{
			Element element = ElementLoader.elements[elementConsumedInfo.removedElemIdx];
			if (lastElementConsumed != element.id && element.id != SimHashes.Vacuum)
			{
				GameUtil.TintLiquidSymbolOnBuilding("water", controller, element);
			}
			lastElementConsumed = element.id;
		}
	}
