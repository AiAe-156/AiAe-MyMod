...
public class RoomDetails
{
	...
	{
		float num = 0f;
		return (num == 0f) ? string.Format(ROOMS.DETAILS.AVERAGE_TEMPERATURE.NAME, UI.OVERLAYS.TEMPERATURE.EXTREMECOLD) : string.Format(ROOMS.DETAILS.AVERAGE_TEMPERATURE.NAME, GameUtil.GetFormattedTemperature(num));
	});
	...
	public static readonly Detail ORNAMENT_COUNT = new Detail(delegate(Room room)
	{
		int num = 0;
		...
		foreach (KPrefabID otherEntity in room.otherEntities)
		{
			if (!(otherEntity == null))
			{
				OrnamentReceptacle component2 = otherEntity.GetComponent<OrnamentReceptacle>();
		...
		return GameUtil.SafeStringFormat(ROOMS.DETAILS.ORNAMENT_COUNT.NAME, num);
	});
	...
	public static readonly Detail PLANT_COUNT = new Detail(delegate(Room room)
	{
		int num = 0;
