...
public class EntombedItemManager : KMonoBehaviour, ISim33ms
{
	...
	public static bool CanEntomb(Pickupable pickupable)
	{
		...
		{
			ElementChunk component = pickupable.GetComponent<ElementChunk>();
			if (component != null)
			{
				return true;
			}
	...
	private Item GetItem(int item_idx)
	{
		...
		{
			cell = cells[item_idx],
			elementId = elementIds[item_idx],
			mass = masses[item_idx],
			temperature = temperatures[item_idx],
			diseaseIdx = diseaseIndices[item_idx],
			diseaseCount = diseaseCounts[item_idx]
		};
	}
