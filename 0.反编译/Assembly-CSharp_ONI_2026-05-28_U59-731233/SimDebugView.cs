...
public class SimDebugView : KMonoBehaviour
{
	...
	private struct UpdateSimViewWorkItem : IWorkItem<UpdateSimViewSharedData>
	{
		...
		public void Run(UpdateSimViewSharedData shared_data, int threadIndex)
		{
	...
	private Dictionary<HashedString, Func<SimDebugView, int, Color>> getColourFuncs = new Dictionary<HashedString, Func<SimDebugView, int, Color>>
	{
		...
		{
			OverlayModes.MinionNavTableIsValid,
			GetMinionNavTableIsValid
		},
		{
			OverlayModes.MinionAsyncRenderDelta,
			GetMinionAsyncRenderDataColour
		},
		{
	...
	public void UpdateData(Texture2D texture, byte[] textureBytes, HashedString viewMode, byte alpha)
	{
		...
		if (selected != null)
		{
			selectedNavigator = selected.GetComponent<Navigator>();
		}
	...
	public Color NormalizedTemperature(float actualTemperature)
	{
		...
		return Color.Lerp((Color)GlobalAssets.Instance.colorSet.GetColorByName(temperatureThresholds[num4].colorName), (Color)GlobalAssets.Instance.colorSet.GetColorByName(temperatureThresholds[num4 + 1].colorName), t);
	}
	...
	public Color NormalizedHeatFlow(int cell)
	{
		int num = 0;
		int num2 = 0;
		float thermalComfort = GameUtil.GetThermalComfort(GameTags.Minions.Models.Standard, cell, 0f - DUPLICANTSTATS.STANDARD.BaseStats.DUPLICANT_BASE_GENERATION_KILOWATTS);
		...
		a = Mathf.Max(a, 0f);
		a = Mathf.Min(a, 1f);
		Color result = Color.Lerp((Color)GlobalAssets.Instance.colorSet.GetColorByName(heatFlowThresholds[num].colorName), (Color)GlobalAssets.Instance.colorSet.GetColorByName(heatFlowThresholds[num2].colorName), a);
		if (Grid.Solid[cell])
		{
			result = Color.black;
		}
	...
	private static Color GetMinionNavTableIsValid(SimDebugView instance, int cell)
	{
		...
		if (navGrid.NavTable.IsValid(cell))
		{
			return Color.green;
		}
		return Color.black;
	...
	private static Color GetPressureMapColour(SimDebugView instance, int cell)
	{
		...
		if (Grid.Pressure[cell] > 0f)
		{
			...
			float num = Mathf.Clamp(value, 0f, 1f);
			float num2 = num * 0.9f;
			color = new Color(num2, num2, num2, 1f);
		}
		return color;
	}
	...
	private static Color GetDecorColour(SimDebugView instance, int cell)
	{
		...
		if (!Grid.Solid[cell])
		{
			...
			result = ((!(num > 0f)) ? Color.Lerp((Color)GlobalAssets.Instance.colorSet.decorBaseline, (Color)GlobalAssets.Instance.colorSet.decorNegative, Mathf.Abs(num)) : Color.Lerp((Color)GlobalAssets.Instance.colorSet.decorBaseline, (Color)GlobalAssets.Instance.colorSet.decorPositive, Mathf.Abs(num)));
		}
		return result;
	}
	...
	private static Color GetDangerColour(SimDebugView instance, int cell)
	{
		...
		if (dangerAmount != DangerAmount.None)
		{
	...
	{
		return (instance.selectedNavigator != null && instance.selectedNavigator.PathGrid.GetCost(cell) != -1) ? Color.white : Color.black;
	}
	...
	{
		return GameScenePartitioner.Instance.DoDebugLayersContainItemsOnCell(cell) ? Color.white : Color.black;
	}
}
