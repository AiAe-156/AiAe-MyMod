...
public class SicknessTrigger : KMonoBehaviour, IGameObjectEffectDescriptor
{
	...
	public List<Descriptor> EffectDescriptors(GameObject go)
	{
		...
		foreach (KeyValuePair<GameHashes, HashSet<string>> item2 in dictionary)
		{
			...
			string text = Strings.Get("STRINGS.DUPLICANTS.DISEASES.TRIGGERS." + Enum.GetName(typeof(GameHashes), item2.Key).ToUpper()).String;
			string text2 = Strings.Get("STRINGS.DUPLICANTS.DISEASES.TRIGGERS.TOOLTIPS." + Enum.GetName(typeof(GameHashes), item2.Key).ToUpper()).String;
			...
			list.Add(new Descriptor(text, text2));
		}
		return list;
	}
