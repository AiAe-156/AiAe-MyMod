...
public class CargoModuleSideScreen : SideScreenContent, ISimEveryTick
{
	...
	{
		return target.GetComponent<Clustercraft>() != null && GetCollectionModules(target.GetComponent<Clustercraft>()).Length != 0;
	}
	...
	private void RefreshModulePanel(Clustercraft module)
	{
		...
		foreach (IHexCellCollector hexCellCollector in collectionModules)
		{
			...
			reference.sprite = hexCellCollector.GetUISprite();
			LocText reference2 = component.GetReference<LocText>("label");
			reference2.SetText(hexCellCollector.GetProperName());
		}
		RefreshProgressBars();
