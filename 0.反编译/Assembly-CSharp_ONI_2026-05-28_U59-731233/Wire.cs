...
public class Wire : KMonoBehaviour, IDisconnectable, IFirstFrameCallback, IWattageRating, IHaveUtilityNetworkMgr, IBridgedNetworkItem
{
	...
	public ushort NetworkID
	{
		...
		{
			int cell = Grid.PosToCell(base.transform.GetPosition());
			return (Game.Instance.electricalConduitSystem.GetNetworkForCell(cell) is ElectricalUtilityNetwork electricalUtilityNetwork) ? ((ushort)electricalUtilityNetwork.id) : ushort.MaxValue;
		}
	}
	...
	protected override void OnPrefabInit()
	{
		...
		if (WireCircuitStatus == null)
		{
			WireCircuitStatus = new StatusItem("WireCircuitStatus", "BUILDING", "", StatusItem.IconType.Info, NotificationType.Neutral, allow_multiples: false, OverlayModes.None.ID).SetResolveStringCallback(delegate(string str, object data)
			{
				...
				string text = ((wattsUsedByCircuit < 0f) ? "?" : GameUtil.GetFormattedWattage(wattsUsedByCircuit, unit));
				str = str.Replace("{CurrentLoadAndColor}", (wireLoadColor == Color.white.ToHexString()) ? text : ("<color=#" + wireLoadColor + ">" + text + "</color>"));
