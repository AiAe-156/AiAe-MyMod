...
public class ConduitFlowVisualizer
{
	...
	{
		public static RenderMeshContext EmptyContext = default(RenderMeshContext);
		...
		public override void RunItem(int item, ref RenderMeshContext shared_data, RenderMeshPerThreadData thread_context, int threadIndex)
		{
			...
			for (int i = num; i < num2; i++)
			{
				...
				if (lastFlowInfo.contents.mass > 0f)
				{
					...
					{
						element = null;
						int num3 = Grid.PosToCell(new Vector3(pos.x + GRID_OFFSET.x, pos.y + GRID_OFFSET.y, 0f));
						highlight = num3 == shared_data.outer.highlightedCell;
					}
					...
					{
						num4 = shared_data.outer.CalculateMassScale(lastFlowInfo.contents.mass);
					}
					thread_context.moving_balls.Add(new RenderMeshPerThreadData.Ball(lastFlowInfo.direction, pos, contentsColor, shared_data.outer.tuning.size * num4, foreground: true, highlight));
				...
				float num5 = shared_data.outer.CalculateMassScale(mass);
				if (shared_data.outer.showContents)
				{
					thread_context.static_balls.Add(new RenderMeshPerThreadData.Ball(ConduitFlow.FlowDirections.None, pos2, cellTintColour3, shared_data.outer.tuning.size * num5, foreground: false, highlight: false));
				...
				thread_context.static_balls.Add(new RenderMeshPerThreadData.Ball(ConduitFlow.FlowDirections.None, pos2, contentsColor2, shared_data.outer.tuning.size * num5, foreground: true, highlight2));
			}
		}
