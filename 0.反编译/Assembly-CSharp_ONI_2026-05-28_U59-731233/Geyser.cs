...
public class Geyser : StateMachineComponent<Geyser.StatesInstance>, IGameObjectEffectDescriptor
{
	...
	public struct GeyserModification
	{
		...
		{
			return modifyElement && newElement != (SimHashes)0;
		}
	}
	...
	private GeyserModification modifier = default(GeyserModification);

	[Serialize]
	private float serializedTimeShift = 0f;

	private const float PRE_PCT = 0.1f;

	private const float POST_PCT = 0.05f;

	public float timeShift { get; private set; } = 0f;
	...
	public void AlterTime(float timeOffset, bool shouldSurviveSaveLoad = false)
	{
		...
		if (num2 == 0f)
		{
			bool flag = num4 == 0f && configuration.GetYearOnDuration() - num3 < configuration.GetOnDuration() - num;
			if (flag | (num3 == 0f && configuration.GetYearOffDuration() - num4 >= configuration.GetOnDuration() - num))
			{
		...
		bool flag2 = num4 == 0f && configuration.GetYearOnDuration() - num3 < configuration.GetIterationLength() - num2;
		flag2 |= num3 == 0f && configuration.GetYearOffDuration() - num4 >= configuration.GetIterationLength() - num2;
		float num5 = RemainingEruptPreTime();
		if (flag2 && num5 <= 0f)
		{
			base.smi.GoTo(base.smi.sm.dormant);
			return;
		...
		float num6 = PreDuration() - num5;
		if ((num3 == 0f) ? (configuration.GetYearOffDuration() - num4 > num6) : (num6 > configuration.GetYearOnDuration() - num3))
		{
			base.smi.GoTo(base.smi.sm.dormant);
		}
		else
	...
	private void RefreshStudiedMeter()
	{
		...
		bool flag = GetAmountOfGeotunersPointingThisGeyser() > 0;
	...
	private void GenerateName()
	{
		...
		if (nameable.savedName == Strings.Get(key))
		{
			...
			Quadrant[] quadrantOfCell = myWorld.GetQuadrantOfCell(cell, 2);
	...
	public List<Descriptor> GetDescriptors(GameObject go)
	{
		...
		if (flag)
		{
			Func<float, float> func = delegate(float emissionPerCycleModifier)
			{
				...
				float num11 = emissionPerCycleModifier / num10;
				return num11 / configuration.GetOnDuration();
			};
			int amountOfGeotunersAffectingThisGeyser = GetAmountOfGeotunersAffectingThisGeyser();
			...
			float num3 = func((massModificationMethod == ModificationMethod.Percentages) ? (instance.currentGeyserModification.massPerCycleModifier * configuration.scaledRate) : instance.currentGeyserModification.massPerCycleModifier);
