...
public class EatChore : Chore<EatChore.StatesInstance>
{
	public class StatesInstance : GameStateMachine<States, StatesInstance, EatChore, object>.GameInstance
	{
		...
		{
			MinionIdentity component = diner.GetComponent<MinionIdentity>();
			Ownables soleOwner = component.GetSoleOwner();
			diner.TryGetComponent<Navigator>(out var component2);
			List<Assignable> preferredAssignables = Game.Instance.assignmentManager.GetPreferredAssignables(soleOwner, component2, Db.Get().AssignableSlots.MessStation);
			foreach (Assignable item in preferredAssignables)
			{
				IDiningSeat diningSeat = ResolveDiningSeat(item.gameObject);
				if (diningSeat == null)
				{
					continue;
				}
				...
				{
					Reservable component3 = item.GetComponent<Reservable>();
					if (component3.IsReservableBy(diner))
					{
						return item;
					}
				}
		...
		public bool UseGarnish()
		{
			...
			GameObject messStation = base.sm.messstation.Get(base.smi);
			return UseGarnish(messStation);
		}
		...
		public static (GameObject, int) CreateLocator(Sensors sensors, Transform transform, string locatorName)
		{
			...
			GameObject item = ChoreHelpers.CreateLocator(locatorName, pos);
			return (item, num);
		}
		...
		public static KAnimFile OnEnterMessStation(GameObject messStation, GameObject diner, GameObject food, bool dinerIsBionic, float? effectDurationOverride = null)
		{
			...
			if (activeGarnish != null)
			{
				storage.ConsumeIgnoringDisease(activeGarnish.itemTag, activeGarnish.consumeRate);
				effectInstance = component3.Add(activeGarnish.effectId, should_save: true);
				KAnim.Build.Symbol overrideSymbol = activeGarnish.GetOverrideSymbol();
				if (overrideSymbol != null && diner.TryGetComponent<SymbolOverrideController>(out var component4))
				{
					component4.AddSymbolOverride(Edible.SALT_SYMBOL, overrideSymbol);
				}
				if (activeGarnish.fxTintColor != Color.white && diner.TryGetComponent<KBatchedAnimController>(out var component5))
				{
					component5.SetSymbolTint(SALT_PARTICLE_SYMBOL, activeGarnish.fxTintColor);
				}
			}
			...
			KPrefabID component6 = messStation.GetComponent<KPrefabID>();
			if (effectDurationOverride.HasValue)
			{
				List<EffectInstance> result = null;
				roomOfGameObject?.roomType.TriggerRoomEffects(component6, component3, out result);
			...
			{
				roomOfGameObject?.roomType.TriggerRoomEffects(component6, component3);
			}
			return kAnimFile;
		}
		...
		public static void OnExitMessStation(GameObject messStation, GameObject diner, KAnimFile eatAnim)
		{
			...
			{
				component.RemoveSymbolOverride(Edible.SALT_SYMBOL);
			}
	...
	public class States : GameStateMachine<States, StatesInstance, EatChore>
	{
		...
		private HashedString GetEatOnFloorAnim(StatesInstance smi)
		{
			...
			if (component.CurrentNavType == NavType.Swim)
			{
