...
public class Crop : KMonoBehaviour, IGameObjectEffectDescriptor
{
	...
	{
		SpawnAndGetConfiguredFruit(callbackParam, wasCropPicked: true);
	}
	...
	public GameObject SpawnAndGetConfiguredFruit(object callbackParam, bool wasCropPicked)
	{
		...
		{
			result = SpawnAndGetSomeFruit(cropVal.cropId, yield.GetTotalValue());
			if (wasCropPicked)
			{
				Trigger(-1072826864, (object)this);
			}
		}
		return result;
	}
	...
	{
		SpawnAndGetSomeFruit(cropID, amount);
	}

	public GameObject SpawnAndGetSomeFruit(Tag cropID, float amount)
	{
		GameObject prefab = Assets.GetPrefab(cropID);
		GameObject gameObject = GameUtil.KInstantiate(prefab, base.transform.GetPosition() + cropSpawnOffset, Grid.SceneLayer.Ore);
		if (gameObject != null)
		{
			...
			Trigger(35625290, (object)gameObject);
			Edible component5 = gameObject.GetComponent<Edible>();
			string properName = gameObject.GetProperName();
			if ((bool)component5)
			{
				ReportManager.Instance.ReportValue(ReportManager.ReportType.CaloriesCreated, component5.Calories, StringFormatter.Replace(UI.ENDOFDAYREPORT.NOTES.HARVESTED, "{0}", properName), UI.ENDOFDAYREPORT.NOTES.HARVESTED_CONTEXT);
			}
			PopFXManager.Instance.SpawnFX(Def.GetUISprite(prefab).first, PopFXManager.Instance.sprite_Plus, properName, base.transform, Vector3.zero);
			return gameObject;
		}
	...
	public List<Descriptor> InformationDescriptors(GameObject go)
	{
		...
		if (prefab == null)
		{
			DebugUtil.LogWarningArgs("Crop", base.gameObject.name, "has an invalid crop prefab:", tag);
			return list;
		}
