...
public class BuildingDef : Def, IHasDlcRestrictions
{
	...
	public Func<CodexEntry, CodexEntry> ExtendCodexEntry = null;

	public bool POIUnlockable = false;
	...
	public KBatchedAnimInstanceData.BlendActiveOptions BlockTileBlendOptions = KBatchedAnimInstanceData.BlendActiveOptions.None;
	...
	public string RequiredSkillPerkID = null;
	...
	public bool CanReplace(GameObject go)
	{
		...
		return component.HasAnyTags(ReplacementTags);
	}
	...
	public List<Tag> DefaultElements()
	{
		...
		{
			List<Tag> validMaterials = MaterialSelector.GetValidMaterials(text);
	...
	public GameObject Build(int cell, Orientation orientation, Storage resource_storage, IList<Tag> selected_elements, float temperature, bool playsound = true, float timeBuilt = -1f)
	{
		...
		if (component2 != null)
		{
			...
			for (int num = 0; num < selected_elements.Count; num++)
			{
				component2.constructionElements[num] = selected_elements[num];
			}
		}
	...
	private bool IsAreaClear(GameObject source_go, int cell, Orientation orientation, ObjectLayer layer, ObjectLayer tile_layer, bool replace_tile, bool restrictToActiveWorld, out string fail_reason, bool permitUproots = true)
	{
		...
		for (int i = 0; i < PlacementOffsets.Length; i++)
		{
			...
			if (!flag2)
			{
				...
				if (gameObject2 != null)
				{
					...
					if (component != null)
					{
						...
						if ((uint)(buildLocationRule2 - 8) <= 2u)
						{
							flag3 = true;
						}
					}
			...
			{
				if (AttachmentSlotTag != GameTags.Rocket)
				{
					GameObject gameObject3 = Grid.Objects[num, 39];
					if (gameObject3 != null)
					{
				...
				GameObject gameObject4 = Grid.Objects[num, 38];
				if (gameObject4 != null && gameObject4 != source_go && gameObject4.GetComponent<BuildingPreview>() == null)
				{
					fail_reason = UI.TOOLTIPS.HELP_BUILDLOCATION_OCCUPIED;
					flag = false;
					break;
			...
			if (layer == ObjectLayer.Gantry)
			{
				...
				if (flag4 && !IsValidTileLocation(source_go, num, replace_tile, ref fail_reason))
				{
					flag = false;
					break;
				}
		...
		case BuildLocationRule.NotInTiles:
		{
			...
			else
			{
				...
				if (gameObject8 != null)
				{
					...
					{
						Building component4 = gameObject8.GetComponent<Building>();
	...
	private bool IsValidTileLocation(GameObject source_go, int cell, bool replacement_tile, ref string fail_reason)
	{
		...
		if (gameObject != null && gameObject != source_go)
		{
			...
			if (!replacement_tile && component3 != null && component3.Def.BuildLocationRule == BuildLocationRule.NotInTiles)
			{
	...
	public void MarkArea(int cell, Orientation orientation, ObjectLayer layer, GameObject go)
	{
		...
		if (InputConduitType != ConduitType.None)
		{
		...
		if (OutputConduitType != ConduitType.None)
		{
		...
		if (components != null)
		{
			...
			foreach (ISecondaryInput secondaryInput in array)
			{
				for (int l = 0; l < 4; l++)
				{
					ConduitType conduitType = (ConduitType)l;
		...
		foreach (ISecondaryOutput secondaryOutput in array2)
		{
			for (int n = 0; n < 4; n++)
			{
				ConduitType conduitType2 = (ConduitType)n;
	...
	public void UnmarkArea(int cell, Orientation orientation, ObjectLayer layer, GameObject go)
	{
		...
		foreach (ISecondaryOutput secondaryOutput in array2)
		{
			for (int m = 0; m < 4; m++)
			{
				ConduitType conduitType2 = (ConduitType)m;
				if (conduitType2 != ConduitType.None && secondaryOutput.HasSecondaryConduitType(conduitType2))
				{
					ObjectLayer objectLayerForConduitType4 = Grid.GetObjectLayerForConduitType(conduitType2);
	...
	private bool AreConduitPortsInValidPositions(GameObject source_go, int cell, Orientation orientation, out string fail_reason)
	{
		...
		if (flag && OutputConduitType != ConduitType.None)
		{
	...
	public static bool CheckFoundation(int cell, Orientation orientation, BuildLocationRule location_rule, int width, int height, Tag optionalFoundationRequiredTag = default(Tag))
	{
		...
		{
			BuildLocationRule.OnBackWall => CheckBackWallFoundation(cell, width, height, orientation), 
			...
			BuildLocationRule.WallFloor => CheckBaseFoundation(cell, orientation, BuildLocationRule.OnFloor, width, height, optionalFoundationRequiredTag) && CheckWallFoundation(cell, width, height, orientation != Orientation.FlipH), 
	...
	public static bool CheckBackWallFoundation(int cell, int width, int height, Orientation orientation)
	{
		...
		for (int i = num; i < height; i++)
		{
			...
			for (int j = num2; j <= num3; j++)
			{
				...
				if (!BackwallManager.HasBackwall(num4))
				{
					GameObject gameObject = (Grid.ObjectLayers[2].ContainsKey(num4) ? Grid.ObjectLayers[2][num4] : null);
	...
	{
		return CheckRequiresPowerInput() || CheckRequiresPowerOutput() || CheckRequiresGasInput() || CheckRequiresGasOutput() || CheckRequiresLiquidInput() || CheckRequiresLiquidOutput() || CheckRequiresSolidInput() || CheckRequiresSolidOutput() || CheckRequiresHighEnergyParticleInput() || CheckRequiresHighEnergyParticleOutput() || SelfHeatKilowattsWhenActive != 0f || ExhaustKilowattsWhenActive != 0f || DiseaseCellVisName != null;
	}
