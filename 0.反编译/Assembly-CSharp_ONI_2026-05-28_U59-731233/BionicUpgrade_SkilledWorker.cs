...
public class BionicUpgrade_SkilledWorker : BionicUpgrade_SM<BionicUpgrade_SkilledWorker, BionicUpgrade_SkilledWorker.Instance>
{
	...
	{
		return BionicUpgrade_SM<BionicUpgrade_SkilledWorker, Instance>.IsOnline(smi) && !BionicUpgrade_SM<BionicUpgrade_SkilledWorker, Instance>.IsInBedTimeChore(smi) && IsMinionWorkingWithAttribute(smi);
	}

	public static bool IsMinionWorkingWithAttribute(Instance smi)
	{
		Workable workable = smi.worker.GetWorkable();
		return workable != null && smi.worker.GetState() == WorkerBase.State.Working && workable.GetWorkAttribute() != null && workable.GetWorkAttribute().Id == ((Def)smi.def).AttributeId;
	}
