...
public class ToolMenu : KScreen
{
	...
	private void InstantiateCollectionsUI(IList<ToolCollection> collections)
	{
		...
		for (int i = 0; i < collections.Count; i++)
		{
			...
			for (int num2 = 0; num2 < tc.tools.Count; num2++)
			{
				ToolInfo ti = tc.tools[num2];
	...
	private void RefreshRowDisplay(IList<ToolCollection> row)
	{
		for (int i = 0; i < row.Count; i++)
		{
			...
			for (int num = 0; num < tc.tools.Count; num++)
			{
				if (tc.tools[num] == currentlySelectedTool)
				{
					SetToggleState(tc.tools[num].toggle, state: true);
				}
				else
				{
					SetToggleState(tc.tools[num].toggle, state: false);
				}
			}
		}
	}
		...
		if (!e.Consumed)
		{
			...
			foreach (List<ToolCollection> row in rows)
			{
				...
				for (int i = 0; i < row.Count; i++)
				{
					...
					for (int num = 0; num < row[i].tools.Count; num++)
					{
						...
						Action hotkey = row[i].tools[num].hotkey;
						if (e.IsAction(hotkey) && e.TryConsume(hotkey))
						{
							...
							else if (currentlySelectedTool != row[i].tools[num])
							{
								ChooseTool(row[i].tools[num]);
							}
						}
	...
	protected void BuildRowToggles(IList<ToolCollection> row)
	{
		for (int i = 0; i < row.Count; i++)
		{
			...
			if (row[i].tools.Count == 1)
			{
				...
				component3.AddMultiStringTooltip(row[i].tools[0].text, TooltipHeader);
