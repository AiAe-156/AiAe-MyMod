...
public class Comet : KMonoBehaviour, ISim33ms
{
	...
	public string[] lootOnDestroyedByMissile = null;
	...
	protected virtual void DepositTiles(int cell, Element element, int world, int prev_cell, float temperature)
	{
		...
		{
			return (!Grid.IsValidCellInWorld(num4, world) || Grid.Solid[num4]) ? FloodFill.BoundaryCheckResult.Halt : FloodFill.BoundaryCheckResult.Continue;
		}
	}
	...
	private void DamageThings(Vector3 pos, int cell, int damage, GameObject ignoreObject = null)
	{
		...
		foreach (ScenePartitionerEntry item in pooledList)
		{
			...
			if (component4 != null && !damagedEntities.Contains(pickupable.gameObject))
			{
				...
				float amount = (kPrefabID.HasTag(GameTags.Bunker) ? ((float)damage * bunkerDamageMultiplier) : ((float)damage));
	...
	private void PlayBuildingDamageSound(BuildingDef def, Vector3 pos, GameObject building_go)
	{
		if (def != null)
		{
			...
			if (sound == null)
			{
				...
				sound = GlobalAssets.GetSound(text);
			}
