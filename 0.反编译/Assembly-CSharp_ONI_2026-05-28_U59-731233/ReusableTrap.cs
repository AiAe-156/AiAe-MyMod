...
public class ReusableTrap : GameStateMachine<ReusableTrap, ReusableTrap.Instance, IStateMachineTarget, ReusableTrap.Def>
{
	public class Def : BaseDef
	{
		...
		public Tag[] lures = null;

		public CellOffset releaseCellOffset = CellOffset.none;

		public bool usingSymbolChaseCapturingAnimations = false;

		public Func<string> getTrappedAnimationNameCallback;

		public bool usingLure => lures != null && lures.Length != 0;
	}
	...
	public new class Instance : GameInstance, TrappedStates.ITrapStateAnimationInstructions
	{
		...
		public bool WasLastCritterLarge = false;

		public KBatchedAnimController lastCritterCapturedAnimController;

		private Chore chore;

		public bool IsCapturingLargeCritter => HasCritter && CapturedCritter.HasTag(GameTags.LargeCreature);

		public bool HasCritter => !storage.IsEmpty();

		public GameObject CapturedCritter => HasCritter ? storage.items[0] : null;
		...
		public void Release()
		{
			...
			WasLastCritterLarge = IsCapturingLargeCritter;
			int cell = Grid.OffsetCell(Grid.PosToCell(base.smi.transform.GetPosition()), base.def.releaseCellOffset);
		...
		{
			return (base.def.getTrappedAnimationNameCallback == null) ? null : base.def.getTrappedAnimationNameCallback();
		}
	}
	...
	{
		return smi.IsCapturingLargeCritter ? "capture_idle_large" : "capture_idle";
	}

	public static string GetCaptureAnimationName(Instance smi)
	{
		return smi.IsCapturingLargeCritter ? "capture_large" : "capture";
	}

	public static string GetReleaseAnimationName(Instance smi)
	{
		return smi.WasLastCritterLarge ? "release_large" : "release";
	}
	...
	public static void OptionalCapturingAnimationUpdate(Instance smi, float dt)
	{
		if (smi.def.usingSymbolChaseCapturingAnimations && smi.lastCritterCapturedAnimController != null)
		{
			...
			bool symbolVisible;
			Vector4 column = smi.animController.GetSymbolTransform(smi.CAPTURING_SYMBOL_NAME, out symbolVisible).GetColumn(3);
