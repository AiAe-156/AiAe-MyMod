...
public class ComplexFabricator : RemoteDockWorkTargetComponent, ISim200ms, ISim1000ms
{
	...
	public Tag[] ForbiddenTags => forbidMutantSeeds ? forbiddenMutantTags : null;
		...
		for (int l = 0; l < recipe_list.Length; l++)
		{
			...
			foreach (ComplexRecipe.RecipeElement recipeElement2 in array2)
			{
				...
				if (num5 > 0f)
				{
					...
					num5 *= FetchChore.GetMinimumFetchAmount(recipeElement2.material, 1f);
	...
	protected virtual List<GameObject> SpawnOrderProduct(ComplexRecipe recipe)
	{
		...
		foreach (ComplexRecipe.RecipeElement recipeElement3 in ingredients2)
		{
			...
			if (recipe.ProductHasFacade && text.IsNullOrWhiteSpace())
			{
				...
				RepairableEquipment component = gameObject.GetComponent<RepairableEquipment>();
				if (component != null)
				{
		...
		foreach (ComplexRecipe.RecipeElement recipeElement4 in results)
		{
			GameObject gameObject2 = buildStorage.FindFirst(recipeElement4.material);
			if (gameObject2 != null)
			{
				Edible component2 = gameObject2.GetComponent<Edible>();
				if ((bool)component2)
				{
			...
			case ComplexRecipe.RecipeElement.TemperatureOperation.Heated:
			{
				...
				if (recipe.ProductHasFacade && !text.IsNullOrWhiteSpace())
				{
					Equippable component4 = gameObject3.GetComponent<Equippable>();
					if (component4 != null)
					{
				...
				{
					Equippable component5 = gameObject3.GetComponent<Equippable>();
					if (component5 != null)
					{
				...
				PopFXManager.Instance.SpawnFX(Def.GetUISprite(prefab).first, PopFXManager.Instance.sprite_Plus, prefab.GetProperName(), gameObject3.transform, Vector3.zero);
				break;
			}
			case ComplexRecipe.RecipeElement.TemperatureOperation.Dehydrated:
			{
				for (int l = 0; l < (int)recipeElement4.amount; l++)
				{
					GameObject prefab2 = Assets.GetPrefab(recipeElement4.material);
					GameObject gameObject4 = GameUtil.KInstantiate(prefab2, Grid.SceneLayer.Ore);
					int cell2 = Grid.PosToCell(this);
					gameObject4.transform.SetPosition(Grid.CellToPosCCC(cell2, Grid.SceneLayer.Ore) + outputOffset);
					float amount = recipeElement2.amount / recipeElement4.amount;
					PrimaryElement component6 = gameObject4.GetComponent<PrimaryElement>();
					component6.Temperature = ((recipeElement4.temperatureOperation == ComplexRecipe.RecipeElement.TemperatureOperation.AverageTemperature) ? num : heatedTemperature);
					DehydratedFoodPackage component7 = gameObject4.GetComponent<DehydratedFoodPackage>();
					if (component7 != null)
					{
						Storage component8 = component7.GetComponent<Storage>();
						outStorage.TransferMass(component8, recipeElement2.material, amount, flatten: true);
					}
					gameObject4.SetActive(value: true);
					gameObject4.GetComponent<KMonoBehaviour>().Trigger(748399584);
					list.Add(gameObject4);
					if (storeProduced || recipeElement4.storeElement)
					{
						outStorage.Store(gameObject4);
					}
					PopFXManager.Instance.SpawnFX(Def.GetUISprite(prefab2).first, PopFXManager.Instance.sprite_Plus, prefab2.GetProperName(), gameObject4.transform, Vector3.zero);
				}
				break;
			}
			...
			SymbolOverrideController component9 = GetComponent<SymbolOverrideController>();
			if (component9 != null)
			{
				KBatchedAnimController component10 = list[0].GetComponent<KBatchedAnimController>();
				KAnim.Build build = component10.AnimFiles[0].GetData().build;
				KAnim.Build.Symbol symbol = build.GetSymbol(build.name);
				if (symbol != null)
				{
					component9.TryRemoveSymbolOverride("output_tracker");
					component9.AddSymbolOverride("output_tracker", symbol);
				}
				else
				{
					Debug.LogWarning(component9.name + " is missing symbol " + build.name);
				}
			}
		}
