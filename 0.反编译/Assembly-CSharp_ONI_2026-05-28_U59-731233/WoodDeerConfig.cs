...
public class WoodDeerConfig : IEntityConfig, IHasDlcRestrictions
{
	...
	{
		GameObject prefab = BaseDeerConfig.BaseDeer(id, name, desc, anim_file, "WoodDeerBaseTrait", is_baby);
		prefab = EntityTemplates.ExtendEntityToWildCreature(prefab, DeerTuning.PEN_SIZE_PER_CREATURE);
		...
		list.Add(BaseDeerConfig.CreateDietInfo("HardSkinBerryPlant", SimHashes.Dirt.CreateTag(), HARD_SKIN_CALORIES_PER_KG, POOP_MASS_CONVERSION_MULTIPLIER, null, 0f));
		list.Add(new Diet.Info(new HashSet<Tag> { "HardSkinBerry" }, SimHashes.Dirt.CreateTag(), CONSUMABLE_PLANT_MATURITY_LEVELS * HARD_SKIN_CALORIES_PER_KG / 1f, POOP_MASS_CONVERSION_MULTIPLIER * 3f));
		list.Add(BaseDeerConfig.CreateDietInfo("PrickleFlower", SimHashes.Dirt.CreateTag(), BRISTLE_CALORIES_PER_KG / 2f, POOP_MASS_CONVERSION_MULTIPLIER, null, 0f));
		list.Add(new Diet.Info(new HashSet<Tag> { PrickleFruitConfig.ID }, SimHashes.Dirt.CreateTag(), CONSUMABLE_PLANT_MATURITY_LEVELS * BRISTLE_CALORIES_PER_KG / 1f, POOP_MASS_CONVERSION_MULTIPLIER * 6f));
		prefab = BaseDeerConfig.SetupDiet(prefab, list.ToArray(), MIN_KG_CONSUMED_BEFORE_POOPING);
	...
	{
		GameObject prefab = CreateWoodDeer("WoodDeer", STRINGS.CREATURES.SPECIES.WOODDEER.NAME, STRINGS.CREATURES.SPECIES.WOODDEER.DESC, "ice_floof_kanim", is_baby: false);
		return EntityTemplates.ExtendEntityToFertileCreature(prefab, this, "WoodDeerEgg", STRINGS.CREATURES.SPECIES.WOODDEER.EGG_NAME, STRINGS.CREATURES.SPECIES.WOODDEER.DESC, "egg_ice_floof_kanim", DeerTuning.EGG_MASS, "WoodDeerBaby", 60.000004f, 20f, DeerTuning.EGG_CHANCES_BASE, EGG_SORT_ORDER);
	}
