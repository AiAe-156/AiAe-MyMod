...
public class SoundEvent : AnimEvent
{
	...
	{
		Vector3 sound_pos = behaviour.controller.transform.GetPosition();
		sound_pos.z = 0f;
		GameObject gameObject = behaviour.controller.gameObject;
		if (ObjectIsSelectedAndVisible(gameObject))
		{
			sound_pos = AudioHighlightListenerPosition(sound_pos);
		}
		...
		{
			Debug.Log(behaviour.name + ", " + sound + ", " + base.frame + ", " + sound_pos);
		}
		try
		{
			if (looping)
			{
				...
				else if (!component.StartSound(sound, behaviour, noiseValues, ignorePause, shouldCameraScalePosition))
				{
	...
	{
		EventReference event_ref = RuntimeManager.PathToEventReference(ev);
		return BeginOneShot(event_ref, pos, volume);
	}
	...
	{
		EventReference event_ref = RuntimeManager.PathToEventReference(sound);
		return PlayOneShot(event_ref, sound_pos, volume);
	}
	...
	{
		if (sound != null && Game.Instance != null && Game.MainCamera.orthographicSize > AudioMixer.LOW_PRIORITY_CUTOFF_DISTANCE && !AudioMixer.instance.activeNIS && GlobalAssets.IsLowPriority(sound))
		{
			return true;
		}
		return false;
