...
public class KBatchedAnimController : KAnimControllerBase, KAnimConverter.IAnimConverter
{
	...
	private ulong movingStateChangedHandlerID = 0uL;
	...
	public Func<Vector4> getPositionDataFunctionInUse = null;
	...
	public void UpdateAnim(float dt)
	{
		if (batch != null && base.transform.hasChanged)
		{
			...
			if (visibilityType != VisibilityType.Always)
			{
				...
				if (vector2I != lastChunkXY && lastChunkXY != KBatchedAnimUpdater.INVALID_CHUNK_ID)
				{
					DeRegister();
					Register();
				}
		...
		if (layering != null && layering.UsesLayers && !layering.IsBaseLayer)
		{
			return;
		}
	...
	public override Matrix2x3 GetTransformMatrix()
	{
		...
		if (rotation != 0f)
		{
			...
			Matrix2x3 matrix2x5 = Matrix2x3.Translate(pivot);
			...
			return matrix2x7 * matrix2x6 * matrix2x * navMatrix * matrix2x2;
		}
		Matrix2x3 matrix2x8 = Matrix2x3.TRS(vector, base.transform.rotation, base.transform.localScale);
		return matrix2x8 * matrix2x * navMatrix * matrix2x2;
	}
	...
	public override Matrix4x4 GetSymbolTransform(HashedString symbol, out bool symbolVisible)
	{
		if (curAnimFrameIdx != -1 && batch != null)
		{
			...
			if (symbolVisible)
			{
				Matrix4x4 matrix4x = GetTransformMatrix();
