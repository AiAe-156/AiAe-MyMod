	...
	private static IEnumerator Sequence()
	{
		...
		if (cameraTaget3 != null)
		{
			...
			foreach (MinionIdentity minion3 in Components.LiveMinionIdentities)
			{
				if (minion3 != null)
				{
					minion3.GetComponent<Facing>().Face(cameraTaget3.transform.position.x);
					Db db3 = Db.Get();
					ChoreProvider provider3 = minion3.GetComponent<ChoreProvider>();
					new EmoteChore(provider3, db3.ChoreTypes.EmoteHighPriority, db3.Emotes.Minion.Cheer, 2);
				}
			}
			yield return SequenceUtil.WaitForSecondsRealtime(0.5f);
