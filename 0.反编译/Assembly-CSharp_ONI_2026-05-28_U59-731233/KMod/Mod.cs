...
public class Mod : IHasDlcRestrictions
{
	...
	private ArchivedVersion GetMostSuitableArchive()
	{
		...
		if (!file_source.Exists("archived_versions"))
		{
			...
			return DlcManager.IsCorrectDlcSubscribed(packagedModInfo) ? archivedVersion : null;
		}
		...
		IOrderedEnumerable<ArchivedVersion> source = from v in list2
			where (long)v.info.minimumSupportedBuild <= 732169L
	...
	private PackagedModInfo GetModInfoForFolder(string relative_root)
	{
		...
		string text2 = (string.IsNullOrEmpty(relative_root) ? "root" : relative_root);
		if (!flag)
		{
			ModDevLogWarning("\t" + title + ": has no mod_info.yaml in folder '" + text2 + "'");
			return null;
		}
		string text3 = file_source.Read(Path.Combine(relative_root, "mod_info.yaml"));
		if (string.IsNullOrEmpty(text3))
		{
			ModDevLogError(string.Format("\t{0}: Failed to read {1} in folder '{2}', skipping", label, "mod_info.yaml", text2));
			return null;
		}
		...
		PackagedModInfo packagedModInfo = YamlIO.Parse<PackagedModInfo>(text3, default(FileHandle), handle_error);
		if (packagedModInfo == null)
		{
			ModDevLogError(string.Format("\t{0}: Failed to parse {1} in folder '{2}', text is {3}", label, "mod_info.yaml", text2, text3));
			return null;
		}
		if (packagedModInfo.supportedContent != null && packagedModInfo.requiredDlcIds == null && packagedModInfo.forbiddenDlcIds == null)
		{
			...
			ModDevLogWarning(string.Format("\t{0}: {1} in folder '{2}' is using supportedContent which has been deprecated. See stickied post on the Klei forums.", label, "mod_info.yaml", text2));
			...
			else
			{
				...
				{
					if (!(item2.Value == "VANILLA_ID") && !(item2.Value == "EXPANSION1_ID" && flag3))
					{
						if (item2.Value != "EXPANSION1_ID")
						{
							ModDevLogWarning(string.Format("\t{0}: {1} in folder '{2}' found a DLC '{3}' it didn't recognize, ignoring.", label, "mod_info.yaml", text2, item2.Value));
						}
						else
						{
		...
		{
			for (int num = 0; num < packagedModInfo.requiredDlcIds.Length; num++)
			{
				packagedModInfo.requiredDlcIds[num] = packagedModInfo.requiredDlcIds[num].ToUpperInvariant();
				if (!DlcManager.IsDlcId(packagedModInfo.requiredDlcIds[num]))
				{
					ModDevLogWarning(string.Format("\t{0}: {1} in folder '{2}' is using an unrecognized DLC in requiredDlcIds '{3}'", label, "mod_info.yaml", text2, packagedModInfo.requiredDlcIds[num]));
				}
			}
		}
		if (packagedModInfo.forbiddenDlcIds != null)
		{
			for (int num2 = 0; num2 < packagedModInfo.forbiddenDlcIds.Length; num2++)
			{
				packagedModInfo.forbiddenDlcIds[num2] = packagedModInfo.forbiddenDlcIds[num2].ToUpperInvariant();
				if (!DlcManager.IsDlcId(packagedModInfo.forbiddenDlcIds[num2]))
				{
					ModDevLogWarning(string.Format("\t{0}: {1} in folder '{2}' is using an unrecognized DLC in forbiddenDlcIds '{3}'", label, "mod_info.yaml", text2, packagedModInfo.forbiddenDlcIds[num2]));
				}
			}
		}
		if (packagedModInfo.lastWorkingBuild != 0)
		{
			ModDevLogError(string.Format("\t{0}: {1} in folder '{2}' is using `{3}`, please upgrade this to `{4}`", label, "mod_info.yaml", text2, "lastWorkingBuild", "minimumSupportedBuild"));
			if (packagedModInfo.minimumSupportedBuild == 0)
			{
				packagedModInfo.minimumSupportedBuild = packagedModInfo.lastWorkingBuild;
			}
		}
		ModDevLog($"\t{label}: Found valid mod_info.yaml in folder '{text2}': requiredDlcIds='{packagedModInfo.requiredDlcIds.DebugToCommaSeparatedList()}', forbiddenDlcIds='{packagedModInfo.forbiddenDlcIds.DebugToCommaSeparatedList()}' at {packagedModInfo.minimumSupportedBuild}");
	...
	public void Install()
	{
		Assert(status.ShouldTryInstall(), $"not in a state such that it can be installed: {status.ToString()}");
	...
	public bool Uninstall()
	{
		...
		else
		{
			...
			case Status.NotInstalled:
				Assert(condition: false, "already asserted above");
				break;
			case Status.Installed:
				status = Status.UninstallPending;
				break;
			default:
				Assert(condition: false, "unhandled Mod.Status");
				break;
			case Status.UninstallPending:
			case Status.ReinstallPending:
			case Status.CannotInstall:
				break;
			}
		}
		return flag;
	...
	public void Unload(Content content)
	{
		...
		Assert((content & (Content.LayerableFiles | Content.Strings | Content.DLL | Content.Translation | Content.Animation)) == 0, $"attempting to unload boot content: {(content & (Content.LayerableFiles | Content.Strings | Content.DLL | Content.Translation | Content.Animation)).ToString()}");
		if ((content & Content.LayerableFiles) != 0)
		{
