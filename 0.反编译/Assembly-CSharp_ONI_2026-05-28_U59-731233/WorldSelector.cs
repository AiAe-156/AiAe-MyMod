...
public class WorldSelector : KScreen, ISim4000ms
{
	...
	private void AddWorld(object data)
	{
		...
		GameObject gameObject = Util.KInstantiateUI(worldRowPrefab, worldRowContainer);
		MultiToggle component = gameObject.GetComponent<MultiToggle>();
		worldRows.Add(value, component);
		previousWorldDiagnosticStatus.Add(value, ColonyDiagnostic.DiagnosticResult.Opinion.Normal);
		int id = value;
		component.onClick = (System.Action)Delegate.Combine(component.onClick, (System.Action)delegate
		{
			OnWorldRowClicked(id);
	...
	{
		int value = ((Boxed<int>)data).value;
		if (worldRows.TryGetValue(value, out var value2))
		{
			value2.DeleteObject();
		}
		worldRows.Remove(value);
		previousWorldDiagnosticStatus.Remove(value);
		RefreshToggles();
	}
	...
	private void SortRows()
	{
		...
		{
			float num3 = (ClusterManager.Instance.GetWorld(x.Key).IsModuleInterior ? float.PositiveInfinity : ClusterManager.Instance.GetWorld(x.Key).DiscoveryTimestamp);
		...
		{
			list[num].Value.transform.SetSiblingIndex(num);
		}
		foreach (KeyValuePair<int, MultiToggle> item in list)
		{
			...
			foreach (KeyValuePair<int, MultiToggle> item2 in list)
			{
				...
				{
					num2 = item2.Value.gameObject.transform.GetSiblingIndex();
					item.Value.gameObject.transform.SetSiblingIndex(num2 + 1);
	...
	private IEnumerator VisualNotificationRoutine(GameObject contentGameObject, RectTransform indicator, GameObject spacer)
	{
		...
		{
			float pos = Mathf.Sin(i * MathF.PI) * 50f;
			contentRect.anchoredPosition = Vector2.left * pos;
			indicator.sizeDelta = defaultIndicatorSize + Vector2.one * Mathf.RoundToInt(Mathf.Sin(6f * (MathF.PI * (i / bounceDuration))));
			yield return 0;
		}
		for (float i2 = 0f; i2 < bounceDuration; i2 += Time.unscaledDeltaTime)
		{
			float pos2 = Mathf.Sin(i2 * MathF.PI) * 25f;
			contentRect.anchoredPosition = Vector2.left * pos2;
			indicator.sizeDelta = defaultIndicatorSize + Vector2.one * Mathf.RoundToInt(Mathf.Sin(6f * (MathF.PI * (i2 / bounceDuration))));
			yield return 0;
		}
		for (float i3 = 0f; i3 < bounceDuration; i3 += Time.unscaledDeltaTime)
		{
			float pos3 = Mathf.Sin(i3 * MathF.PI) * 12f;
			contentRect.anchoredPosition = Vector2.left * pos3;
			indicator.sizeDelta = defaultIndicatorSize + Vector2.one * Mathf.RoundToInt(Mathf.Sin(6f * (MathF.PI * (i3 / bounceDuration))));
			yield return 0;
		}
