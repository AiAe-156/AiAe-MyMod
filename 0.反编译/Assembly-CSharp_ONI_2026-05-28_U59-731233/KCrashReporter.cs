...
public class KCrashReporter : MonoBehaviour
{
	...
	private class Error
	{
		...
		public Error()
		{
			...
			{
				string pchName;
				bool flag = !SteamApps.GetCurrentBetaName(out pchName, 100);
				branch = pchName;
				if (pchName == "public_playtest")
				{
					branch = "public_testing";
				}
				if (flag || (pchName == "public_testing" && !UnityEngine.Debug.isDebugBuild))
				{
					branch = "default";
				}
			}
	...
	public static void ReportError(string msg, string stack_trace, ConfirmDialogScreen confirm_prefab, GameObject confirm_parent, string userMessage = "", bool includeSaveFile = true, string[] extraCategories = null, string[] extraFiles = null)
	{
		...
		System.Action successCallback = delegate
		{
			if (confirm_prefab != null && confirm_parent != null)
			{
				...
				confirmDialogScreen.PopupConfirmDialog(UI.CRASHSCREEN.REPORTEDERROR_SUCCESS, null, null);
			}
		};
		Action<long> failureCallback = delegate(long errorCode)
		{
			if (confirm_prefab != null && confirm_parent != null)
			{
				...
				ConfirmDialogScreen confirmDialogScreen = (ConfirmDialogScreen)KScreenManager.Instance.StartScreen(confirm_prefab.gameObject, confirm_parent);
				confirmDialogScreen.PopupConfirmDialog(text4, null, null);
			}
		};
	...
	private static IEnumerator SubmitCrashAsync(string jsonString, byte[] archiveData, System.Action successCallback, Action<long> failureCallback)
	{
		...
		{
			formData.Add(new MultipartFormDataSection("loginToken", KleiAccount.KleiToken));
		}
		using (UnityWebRequest w = UnityWebRequest.Post(uri, formData))
		{
			w.SendWebRequest();
			while (!w.isDone)
			{
			...
			if (files != null)
			{
				foreach (string file in files)
				{
					try
					{
						...
						ZipArchiveEntry zipArchiveEntry = zipArchive.CreateEntry(Path.GetFileName(file), System.IO.Compression.CompressionLevel.Fastest);
				...
				ZipArchiveEntry zipArchiveEntry2 = zipArchive.CreateEntry("Player.log", System.IO.Compression.CompressionLevel.Fastest);
