...
public class DestinationSelectPanel : KMonoBehaviour
{
	...
	public void RePlaceAsteroids()
	{
		...
		for (int i = 0; i < clusterKeys.Count; i++)
		{
			...
			ColonyDestinationAsteroidBeltData colonyDestinationAsteroidBeltData = asteroidData[key];
			float iconScale = colonyDestinationAsteroidBeltData.GetStartWorld.iconScale;
			DestinationAsteroid2 asteroid = GetAsteroid(key, (i == selectedIndex) ? (asteroidFocusScale * iconScale) : iconScale);
