...
public class BionicUpgradeComponent : Assignable, IGameObjectEffectDescriptor
{
	...
	public float CurrentWattage => HasWattageController ? WattageController.GetCurrentWattageCost() : 0f;

	public string CurrentWattageName => HasWattageController ? WattageController.GetCurrentWattageCostName() : string.Format(DUPLICANTS.MODIFIERS.BIONIC_WATTS.TOOLTIP.STANDARD_INACTIVE_TEMPLATE, this.GetProperName(), GameUtil.GetFormattedWattage(PotentialWattage));
	...
	public string GetTooltipForMinionAssigment(Assignables assignables)
	{
		...
		string text3 = "";
		List<AttributeInstance> list = new List<AttributeInstance>(targetGameObject.GetAttributes().AttributeTable);
		...
		for (int num2 = 0; num2 < list2.Count; num2++)
		{
			...
			if (list2[num2].GetTotalValue() > 0f)
			{
				text4 = UIConstants.ColorPrefixGreen;
			}
			else if (list2[num2].GetTotalValue() < 0f)
			{
				text4 = UIConstants.ColorPrefixRed;
			}
			text3 += $"{list2[num2].Name}: {text4 + list2[num2].GetFormattedValue() + UIConstants.ColorSuffix}";
