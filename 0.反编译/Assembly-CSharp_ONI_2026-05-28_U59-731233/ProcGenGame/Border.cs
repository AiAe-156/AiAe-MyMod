...
public class Border : Path
{
	...
	public Border(Neighbors neighbors, Vector2 e0, Vector2 e1)
	{
		...
		Vector2 vector2 = e0 + vector / 2f;
		Vector2 point = vector2 + normalized;
	...
	public void ConvertToMap(Chunk world, TerrainCell.ISimDataSetter simDataSetter, float neighbour0Temperature, float neighbour1Temperature, float midTemp, SeededRandom rnd, int snapLastCells)
	{
		for (int i = 0; i < pathElements.Count; i++)
		{
			...
			for (int j = 0; j < line.Count; j++)
			{
				...
				{
					SetCell(num, midTemp, simDataSetter, rnd);
				}
				for (float num2 = 0.5f; num2 <= width; num2 += 1f)
				{
					...
					if (Grid.IsValidCell(num))
					{
						SetCell(num, defaultTemperature, simDataSetter, rnd);
					}
					...
					if (Grid.IsValidCell(num))
					{
						SetCell(num, defaultTemperature2, simDataSetter, rnd);
					}
				}
			}
		}
