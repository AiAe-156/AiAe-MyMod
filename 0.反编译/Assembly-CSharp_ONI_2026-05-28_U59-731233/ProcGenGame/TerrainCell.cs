...
public class TerrainCell
{
	...
	private class SimDataSetter : ISimDataSetter
	{
		...
		{
			simData.backwallCells[cellIdx].SetValues(element.idx, pd.mass, pd.temperature);
		}
		...
		public void SetSimCell(int index, Element elem, Sim.PhysicsData pd, Sim.DiseaseCell dc)
		{
			...
			{
				if (!worldGen.HighPriorityClaimedCells.Contains(index))
				{
					simData.cells[index].SetValues(elem, pd, ElementLoader.elements);
					simData.diseaseCells[index] = dc;
				}
			}
			else
			{
				Debug.LogError("Process::SetValuesFunction Index [" + index + "] is not valid. cells.Length [" + simData.cells.Length + "]");
			}
		}
	}

	private class SimUpdater : ISimDataSetter
	{
		public void SetSimBackwall(int cellIdx, Element element, Sim.PhysicsData pd)
		{
		...
		public void SetSimCell(int index, Element elem, Sim.PhysicsData pd, Sim.DiseaseCell dc)
		{
			SimMessages.ModifyCell(index, elem.idx, pd.temperature, pd.mass, dc.diseaseIdx, dc.elementCount, SimMessages.ReplaceType.Replace);
		}
	}
	...
	public bool spawnBackwall = false;
	...
	private void HandleSprinkleOfElement(WorldGenSettings settings, Tag targetTag, Chunk world, ISimDataSetter simDataSetter, float temperatureMin, float temperatureRange, SeededRandom rnd)
	{
		...
		for (int i = 0; i < terrainPositions.Count; i++)
		{
			...
			for (int j = 0; j < filledCircle.Count; j++)
			{
				...
				if (Grid.IsValidCell(num))
				{
					...
					simDataSetter.SetSimCell(num, element2, defaultValues, invalid);
				}
			}
		}
	}
	...
	public static ElementOverride GetElementOverride(string element, SampleDescriber.Override overrides)
	{
		...
		ElementOverride result = new ElementOverride
		{
	...
	private void ApplyPlaceElementForRoom(FeatureSettings feature, string group, List<Vector2I> cells, WorldGen worldGen, Chunk world, ISimDataSetter simSetter, float temperatureMin, float temperatureRange, SeededRandom rnd, HashSet<int> highPriorityClaims)
	{
		...
		for (int l = 0; l < cells.Count; l++)
		{
			...
			if (Grid.IsValidCell(num6) && !highPriorityClaims.Contains(num6) && IsFeaturePointContainedInBorder(cells[l], worldGen))
			{
				...
				simSetter.SetSimCell(num6, elementOverride3.element, elementOverride3.pdelement, elementOverride3.dc);
			}
		}
	}
	...
	private void ApplyForeground(WorldGen worldGen, Chunk world, ISimDataSetter simDataSetter, float temperatureMin, float temperatureRange, SeededRandom rnd)
	{
		...
		ApplyPlaceElementForRoom(featureSettings, "RoomCenterElements", featureCenterPoints, worldGen, world, simDataSetter, temperatureMin, temperatureRange, rnd, worldGen.HighPriorityClaimedCells);
		if (featureBorders != null)
		{
			for (int i = 0; i < featureBorders.Count; i++)
			{
				ApplyPlaceElementForRoom(featureSettings, "RoomBorderChoices" + i, featureBorders[i], worldGen, world, simDataSetter, temperatureMin, temperatureRange, rnd, worldGen.HighPriorityClaimedCells);
			}
		}
		...
		featureSpawnPoints.RemoveWhere(delegate(int cellIdx)
		{
			if (!Grid.IsValidCell(cellIdx))
			{
				return true;
			}
			Vector2I vector2I = Grid.CellToXY(cellIdx);
			return !IsFeaturePointContainedInBorder(new Vector2(vector2I.x, vector2I.y), worldGen);
		});
		worldGen.AddHighPriorityCells(featureSpawnPoints);
	}

	private void ApplyBackground(WorldGen worldGen, Chunk world, ISimDataSetter simDataSetter, float temperatureMin, float temperatureRange, SeededRandom rnd)
	{
		...
		ElementBandConfiguration elementBands = worldGen.Settings.GetElementBandForBiome(node.GetBiome() + "_backwall") ?? elementBandConfiguration;
		...
		foreach (int availableTerrainPoint in availableTerrainPoints)
		{
			...
			if (!flag && num2 >= 100f)
			{
				if (num2 >= 300f)
				{
					simDataSetter.SetSimCell(availableTerrainPoint, WorldGen.voidElement, WorldGen.voidElement.defaultValues, Sim.DiseaseCell.Invalid);
				}
				else if (num2 >= 200f)
				{
					simDataSetter.SetSimCell(availableTerrainPoint, WorldGen.unobtaniumElement, WorldGen.unobtaniumElement.defaultValues, Sim.DiseaseCell.Invalid);
				}
				else
				{
					simDataSetter.SetSimCell(availableTerrainPoint, WorldGen.katairiteElement, WorldGen.katairiteElement.defaultValues, Sim.DiseaseCell.Invalid);
				}
				continue;
			}
			float num3 = 1f;
			...
			simDataSetter.SetSimCell(availableTerrainPoint, element, pd, dc);
			if (spawnBackwall)
			{
				worldGen.GetElementForBackwallBiomePoint(world, elementBands, pos, out element, out pd, out dc);
				pd.mass += pd.mass * 0.2f * (world.density[pos.x + world.size.x * pos.y] - 0.5f);
				float num17 = temperatureMin;
				if (element.lowTempTransition != null && temperatureMin < element.lowTemp)
				{
					num17 = element.lowTemp;
				}
				pd.temperature = num17 + world.heatOffset[availableTerrainPoint] * temperatureRange;
				if (element.IsSolid)
				{
					simDataSetter.SetSimBackwall(availableTerrainPoint, element, pd);
				}
			}
		}
		if (node.tags.Contains(WorldGenTags.SprinkleOfOxyRock))
		{
			HandleSprinkleOfElement(worldGen.Settings, WorldGenTags.SprinkleOfOxyRock, world, simDataSetter, temperatureMin, temperatureRange, rnd);
		}
		if (node.tags.Contains(WorldGenTags.SprinkleOfMetal))
		{
			HandleSprinkleOfElement(worldGen.Settings, WorldGenTags.SprinkleOfMetal, world, simDataSetter, temperatureMin, temperatureRange, rnd);
		}
	}
