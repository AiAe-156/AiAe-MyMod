...
public class MaterialSelectionPanel : KScreen, IRender200ms
{
	...
	public void RefreshSelectors()
	{
		...
		ToggleResearchRequired(state: false);
		for (int num = 0; num < activeRecipe.Ingredients.Count; num++)
		{
			materialSelectors[num].gameObject.SetActive(value: true);
			materialSelectors[num].ConfigureScreen(activeRecipe.Ingredients[num], activeRecipe);
		}
		if (priorityScreen != null)
		{
