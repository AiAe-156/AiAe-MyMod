...
public class SolidTransferArm : StateMachineComponent<SolidTransferArm.SMInstance>, ISim1000ms, IRenderEveryTick
{
	...
	private static Func<object, SolidTransferArm, Util.IterationInstruction> AsyncUpdateVisitor = delegate(object obj, SolidTransferArm arm)
	{
		...
		int cellRange = Grid.GetCellRange(arm.gameCell, pickupable.cachedCell);
		if (cellRange <= arm.pickupRange && arm.IsPickupableRelevantToMyInterests(pickupable.KPrefabID, pickupable.cachedCell) && pickupable.CouldBePickedUpByTransferArm(arm.kPrefabID.InstanceID))
		{
