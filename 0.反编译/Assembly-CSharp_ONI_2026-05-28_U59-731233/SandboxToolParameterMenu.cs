...
public class SandboxToolParameterMenu : KScreen
{
	...
	private void ConfigureSettings()
	{
		...
		{
			int intSetting = settings.GetIntSetting("SandboxTools.SelectedTargetLayer");
			string text = ((intSetting == 1) ? UI.SANDBOXTOOLS.SETTINGS.TARGET_LAYER.BACKGROUND : UI.SANDBOXTOOLS.SETTINGS.TARGET_LAYER.FOREGROUND);
	...
	private void ConfigureElementSelector()
	{
		...
		foreach (Element element in ElementLoader.elements)
		{
			...
			{
				if (tag == GameTags.HideFromSpawnTool)
				{
					flag = true;
					break;
				}
	...
	private void ConfigureEntitySelector()
	{
		...
		foreach (GameObject item12 in Assets.GetPrefabsWithTag(GameTags.CreatureBrain))
		{
			...
			if (!list2.Contains(brain.species) && Game.IsCorrectDlcActiveForCurrentSave(brain.GetComponent<KPrefabID>()))
			{
				...
				SelectorValue.SearchFilter item5 = new SelectorValue.SearchFilter(Strings.Get(key), delegate(object entity)
				{
		...
		SelectorValue.SearchFilter item8 = new SelectorValue.SearchFilter(UI.SANDBOXTOOLS.FILTERS.ENTITIES.INDUSTRIAL_PRODUCTS, delegate(object entity)
		{
			...
			return Game.IsCorrectDlcActiveForCurrentSave(kPrefabID) && (kPrefabID.HasTag(GameTags.IndustrialIngredient) || kPrefabID.HasTag(GameTags.TechComponents) || kPrefabID.HasTag(GameTags.IndustrialProduct) || kPrefabID.HasTag(GameTags.Medicine) || kPrefabID.HasTag(GameTags.MedicalSupplies) || kPrefabID.HasTag(GameTags.ChargedPortableBattery) || kPrefabID.HasTag(TableSaltConfig.TAG) || kPrefabID.HasTag(GameTags.MoltShell));
