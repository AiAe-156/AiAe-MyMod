...
public class IceKettle : GameStateMachine<IceKettle, IceKettle.Instance, IStateMachineTarget, IceKettle.Def>
{
	...
	public new class Instance : GameInstance
	{
		...
		public float CurrentTemperatureOfSolidsStored => (kettleStorage.MassStored() > 0f) ? kettleStorage.items[0].GetComponent<PrimaryElement>().Temperature : 0f;
		...
		public bool HasEnoughFuelUnitsToMeltNextBatch => !(kettleStorage.MassStored() > 0f) || FuelUnitsAvailable >= FuelRequiredForNextBratch;
		...
		public void MeltNextBatch()
		{
			if (HasAtLeastOneBatchOfSolidsWaitingToMelt)
			{
				...
				int gameCell = Grid.PosToCell(base.gameObject);
				SimMessages.AddRemoveSubstance(gameCell, element.id, null, mass, temperature, byte.MaxValue, 0);
			}
		}
		...
		public float GetUnitsOfFuelRequiredToMelt(Element elementToMelt, float massToMelt_KG, float elementToMelt_initialTemperature)
		{
			...
			float num2 = massToMelt_KG * elementToMelt.specificHeatCapacity * targetTemperature - num;
			return num2 / base.def.EnergyPerUnitOfLumber;
		}
	}
	...
	{
		return smi.HasAtLeastOneBatchOfSolidsWaitingToMelt && LiquidTankHasCapacityForNextBatch(smi) && HasEnoughFuelForNextBacth(smi);
	}
