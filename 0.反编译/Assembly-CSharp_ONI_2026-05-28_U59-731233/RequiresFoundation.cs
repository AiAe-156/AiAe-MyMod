...
public class RequiresFoundation : KGameObjectComponentManager<RequiresFoundation.Data>, IKComponentManager
{
	...
	public HandleVector<int>.Handle Add(GameObject go)
	{
		...
		BuildLocationRule buildLocationRule = buildRule;
		if (buildLocationRule == BuildLocationRule.OnBackWall)
		{
		...
		if (def.ContinuouslyCheckFoundation)
		{
			...
			BuildLocationRule buildRule2 = data.buildRule;
			BuildLocationRule buildLocationRule2 = buildRule2;
			if (buildLocationRule2 == BuildLocationRule.OnBackWall)
			{
	...
	private void OnFoundationChanged(HandleVector<int>.Handle h)
	{
		...
		if (!flag && (new_data.buildRule == BuildLocationRule.BuildingAttachPoint || new_data.buildRule == BuildLocationRule.OnFloorOrBuildingAttachPoint))
		{
			...
			AttachableBuilding component3 = new_data.go.GetComponent<AttachableBuilding>();
			AttachableBuilding.GetAttachedBelow(component3, ref buildings);
			if (buildings.Count > 0)
			{
				Operational component4 = buildings.Last().GetComponent<Operational>();
				if (component4 != null && component4.GetFlag(new_data.operationalFlag))
				{
					flag = true;
				}
			}
