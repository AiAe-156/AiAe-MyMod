...
public class Assets
{
	...
	public static void LoadSprites(Assets assets, string path = null)
	{
		...
		path = (Util.IsNullOrWhiteSpace(path) ? Path.Combine(Utils.ModPath, "assets", "sprites") : path);
		...
		foreach (string path2 in files)
		{
			...
			Sprite val = LoadSprite(path2, fileNameWithoutExtension);
			assets.SpriteAssets.Add(val);
			string path3 = Path.Combine(Path.GetDirectoryName(path), fileNameWithoutExtension + ".meta.json");
			...
			if (text != null)
			{
				AssetMetaData assetMetaData = JsonConvert.DeserializeObject<AssetMetaData>(text);
				if (assetMetaData.TintedSprite)
				{
					assets.TintedSpriteAssets.Add(new TintedSprite
					{
						sprite = val,
						name = fileNameWithoutExtension,
						color = Util.ColorFromHex(assetMetaData.ColorHex)
					});
				}
			}
		}
	...
	private static Sprite LoadSprite(string path, string spriteName)
	{
		...
		Sprite obj = Sprite.Create(val, new Rect(0f, 0f, (float)((Texture)val).width, (float)((Texture)val).height), Vector2.op_Implicit(Vector3.zero));
